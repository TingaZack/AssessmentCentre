...
//   fetchCohorts: (force?: boolean) => Promise<void>;
//   addCohort: (cohort: Omit<Cohort, "id" | "createdAt">) => Promise<void>;
//   updateCohort: (id: string, updates: Partial<Cohort>) => Promise<void>; // Added update
//   deleteCohort: (id: string) => Promise<void>;
// }
...
//     // 2. 🚀 SYNC LEARNERS (The Handshake)
//     if (newCohort.learnerIds && newCohort.learnerIds.length > 0) {
//       newCohort.learnerIds.forEach((learnerId) => {
//         const learnerRef = doc(db, "learners", learnerId);
//         batch.update(learnerRef, { cohortId: cohortId, updatedAt: timestamp });
//       });
//     }
...
//         if (state.learners) {
//           state.learners.forEach((l: any) => {
//             if (newCohort.learnerIds.includes(l.id)) l.cohortId = cohortId;
//           });
//         }
//       });
//     } catch (error) {
//       console.error("Failed to add cohort and sync:", error);
//       throw error;
//     }
//   },
...
//   ) => {
//     const { cohorts } = get();
//     const currentCohort = cohorts.find((c: Cohort) => c.id === id);
...
//     const batch = writeBatch(db);
//     const timestamp = new Date().toISOString();
//     const adminId = "admin-user-id"; // Use actual auth UID in production

//     // --- 1. STAFF HISTORY LOGIC (from useStore) ---
//     let newHistory = [...(currentCohort.staffHistory || [])];
...
//       if (newId && newId !== oldId) {
//         // Close active entry
//         const activeIdx = newHistory.findIndex(
//           (h) => h.role === role && h.staffId === oldId && h.removedAt === null,
//         );
//         if (activeIdx !== -1) {
//           newHistory[activeIdx].removedAt = timestamp;
//         }
//         // Add new audit entry
//         newHistory.push({
//           staffId: newId,
...
//       // --- 4. LOCAL STATE UPDATE ---
//       set((state: any) => {
//         const idx = state.cohorts.findIndex((c: Cohort) => c.id === id);
//         if (idx !== -1) state.cohorts[idx] = finalPayload;

//         // Also sync learners in local state if they exist
//         if (state.learners && updates.learnerIds) {
//           state.learners.forEach((l: any) => {
//             if (updates.learnerIds?.includes(l.id)) l.cohortId = id;
//             else if (currentCohort.learnerIds.includes(l.id))
//               l.cohortId = "Unassigned";
//           });
//         }
//       });
//       console.log("Cohort & Learners synced with audit trail.");
//     } catch (error) {
//       console.error("Critical Sync Failure:", error);
//       throw error;
//     }
...
//   // updateCohort: async (id: string, updates: Partial<Cohort>) => {
//   //   const { cohorts } = get();
//   //   const current = cohorts.find((c: Cohort) => c.id === id);
//   //   if (!current) return;
...
//   //   const cohortRef = doc(db, "cohorts", id);
//   //   batch.update(cohortRef, { ...updates, updatedAt: timestamp });

//   //   // 2. 🚀 SYNC LEARNERS (The "Handshake")
//   //   if (updates.learnerIds) {
//   //     const oldIds = current.learnerIds || [];
//   //     const newIds = updates.learnerIds;

//   //     // Filter who was added and who was removed
//   //     const added = newIds.filter((x: string) => !oldIds.includes(x));
//   //     const removed = oldIds.filter((x: string) => !newIds.includes(x));

//   //     // ✅ FIX: Added explicit string types to lid parameters
//   //     added.forEach((lid: string) => {
//   //       batch.update(doc(db, "learners", lid), {
//   //         cohortId: id,
//   //         updatedAt: timestamp,
...
//   //     const idx = state.cohorts.findIndex((c: Cohort) => c.id === id);
//   //     if (idx !== -1) {
//   //       state.cohorts[idx] = { ...state.cohorts[idx], ...updates };
//   //     }

//   //     // Sync local learners slice if it exists in this store
//   //     if (state.learners && updates.learnerIds) {
//   //       state.learners.forEach((l: any) => {
//   //         if (updates.learnerIds?.includes(l.id)) {
//   //           l.cohortId = id;
//   //         } else if (current.learnerIds.includes(l.id)) {
//   //           l.cohortId = "Unassigned";
//   //         }
//   //       });
//   //     }
