...
import {
    ...
    Timer, Lock, ChevronDown, ChevronUp, CheckCircle2, ClipboardList, Sparkles, RefreshCw, Send,
    Play, Info
} from 'lucide-react';
import {
...
// ─── IMPORT EXTERNAL PRINT & SURVEY COMPONENTS ───
...
export type SECAMLevel = 1 | 2 | 3 | 4;
export type CompetencyStatus = 'HC' | 'C' | 'DEV' | 'NYC' | '1' | '2' | '3' | '4' | null;
...
interface LearnerSubmission {
    ...
    title: string;
    type: string;
    status: 'not_started' | 'in_progress' | 'submitted' | 'awaiting_learner_signoff' | 'facilitator_reviewed' | 'returned' | 'graded' | 'moderated' | 'appealed' | 'missed' | 'violation' | 'upcoming' | 'scheduled';
    assignedAt: string;
    startedAt?: string;
    marks: number;
    totalMarks: number;
    ...
    surveyId?: string;
    isBootcamp?: boolean;
    framework?: 'secam' | 'qcto';
    remediationNotes?: string;
    latestCoachingLog?: any;
}
...
const cleanRichText = (html?: any) => {
    if (typeof html !== 'string' || !html) return '';
    return html.replace(/&nbsp;/g, ' ');
};

const extractPlainText = (htmlString?: any) => {
...
const getTypeBadge = (type: any) => {
    ...
    return <span className="mlab-badge" style={{ ...baseStyle, background: '#f8fafc', color: '#475569', border: '1px solid #cbd5e1' }}>{type ? String(type) : 'Task'}</span>;
};
...
const ScheduleSessionModal: React.FC<{
    learner: any;
    user: any;
    context?: LearnerSubmission | null;
    staffList: any[];
    cohortsList: any[];
    onClose: () => void;
    toast: any;
}> = ({ learner, user, context, staffList, cohortsList, onClose, toast }) => {
    const isLearner = user?.role === 'learner';
    const isMissedContext = context?.status === 'missed';
    const isNYCContext = context && (context.competency === 'NYC' || context.competency === 'DEV' || context.competency === '1' || context.competency === '2');

    const defaultCategory = isMissedContext
        ? 'Missed Assessment Re-schedule Request'
        : isNYCContext
            ? 'Academic Support & Remediation'
            : 'General Check-in';

    const defaultTopic = context
        ? isMissedContext
            ? `Requesting re-schedule for missed assessment: ${context.moduleNumber || ''} - ${context.title}`
            : isNYCContext
                ? `Remediation Coaching Request for ${context.moduleNumber || 'Module'}: ${context.title}`
                : `Regarding ${context.moduleNumber || 'Assessment'}: ${context.title}`
        : '';
    ...
    const activeCohort = cohortsList?.find(c => c.id === (context?.cohortId || learner.cohortId));
    const facilitator = staffList?.find(s => s.id === activeCohort?.facilitatorId);

    const resolvedStaffId = isLearner ? (facilitator?.id || 'unassigned') : user.uid;
    const resolvedStaffEmail = isLearner ? (facilitator?.email || 'codetribe@mlab.co.za') : user.email;
    const resolvedStaffName = isLearner ? (facilitator?.fullName || context?.facilitatorName || 'Facilitator') : user.fullName;

    const resolvedLearnerId = learner.learnerId || learner.id;
    const resolvedLearnerEmail = learner.email || learner.demographics?.learnerEmailAddress || 'noreply@mlab.co.za';
    const resolvedLearnerName = learner.fullName;
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await scheduleCoachingSession({
                ...
                learnerEmail: resolvedLearnerEmail,
            ...
            if (context?.id && isLearner) {
                await updateDoc(doc(db, 'learner_submissions', context.id), {
                    coachingRequested: true,
                    coachingRequestedAt: new Date().toISOString()
                });
            }

            toast.success(isLearner ? "Re-assessment request submitted to facilitator!" : "Support session scheduled successfully! Invites sent.");
            onClose();
        } catch (error: any) {
            ...
            <div className="mlab-modal-window mlab-modal-window--md" onClick={e => e.stopPropagation()}>
                <div className="mlab-modal-header" style={{ borderBottom: '1px solid var(--mlab-border)', paddingBottom: '1rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <div>
                            <h2 className="mlab-modal-title" style={{ margin: 0 }}>
                                {isLearner ? 'Apply for Re-assessment / Support' : 'Schedule Support Session'}
                            </h2>
                            <p style={{ margin: '4px 0 0 0', fontSize: '0.8rem', color: 'var(--mlab-grey)' }}>
                                {isLearner
                                    ? 'Request a remediation session or re-assessment slot with your facilitator.'
                                    : `Set up a 1-on-1 Google Meet session with ${learner.fullName}.`}
                            </p>
                        </div>
                    </div>
                ...
                <div className="mlab-modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', paddingTop: '1.5rem' }}>
                    ...
                    {context && (
                        <div style={{ background: isMissedContext ? '#fef2f2' : '#f0fdf4', border: `1px solid ${isMissedContext ? '#fca5a5' : '#bbf7d0'}`, padding: '10px', borderRadius: '6px', display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                            {isMissedContext ? <AlertTriangle size={16} color="#dc2626" style={{ marginTop: '2px', flexShrink: 0 }} /> : <BookOpen size={16} color="#166534" style={{ marginTop: '2px', flexShrink: 0 }} />}
                            <div>
                                <strong style={{ color: isMissedContext ? '#991b1b' : '#166534', fontSize: '0.8rem', display: 'block' }}>
                                    {isMissedContext ? 'Missed Assessment Linked' : 'Module Context Linked'}: {context.moduleNumber || 'Assessment'}
                                </strong>
                                <span style={{ color: isMissedContext ? '#b91c1c' : '#15803d', fontSize: '0.75rem', lineHeight: 1.4, display: 'inline-block', marginTop: '2px' }}>
                                    {context.title}
                                </span>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <select className="lfm-input" value={category} onChange={e => setCategory(e.target.value)} style={{ width: '100%', padding: '10px', marginTop: '6px', fontSize: '0.85rem', border: '1px solid #cbd5e1', borderRadius: '6px' }}>
                            ...
                            <option value="Missed Assessment Re-schedule Request">⚠️ Missed Assessment Re-schedule Request</option>
                    ...
                    <div>
                        <label style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--mlab-grey)', textTransform: 'uppercase' }}>Preferred Date & Time</label>
                    ...
                    <div>
                        <label style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--mlab-grey)', textTransform: 'uppercase' }}>Reason / Notes</label>
                        <textarea className="lfm-input" rows={3} placeholder={isLearner ? "Explain why you missed the assessment or what areas you need coaching on..." : "What will this session cover?"} value={topic} onChange={e => setTopic(e.target.value)} style={{ width: '100%', padding: '10px', marginTop: '6px', fontSize: '0.85rem', border: '1px solid #cbd5e1', borderRadius: '6px' }} />
                    </div>
                </div>
                ...
                <div className="mlab-modal-footer">
                    ...
                    <button onClick={handleSubmit} disabled={isSubmitting} className="mlab-btn mlab-btn--primary">
                        {isSubmitting ? <Loader2 size={16} className="vp-spin" /> : <Send size={16} />} {isLearner ? 'Submit Request' : 'Schedule & Invite'}
                    </button>
                </div>
            </div>
        </div>,
    ...
    return (
        <div className="vp-poe-card">
            <div className="vp-poe-header">
                <div className="vp-poe-title-group">
                    ...
                    <div>
                        ...
                        <p className="vp-poe-desc">Generate a complete, QCTO / SECAM-compliant PDF PoE for this learner</p>
                    </div>
                </div>
            ...
            <div className="vp-poe-notice">
                ...
                <span><strong>Compliance Note:</strong> Ensure all modules are Graded or Moderated before final export for auditors.</span>
            </div>
        </div>
    );
};
                            ...
                            >
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    <div style={{ fontSize: '0.8rem', color: 'var(--mlab-grey)', display: 'flex', gap: '8px', alignItems: 'center', marginTop: '4px' }}>
                                        ...
                                        <span>• {String(sub.status || '').replace(/_/g, ' ')}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
...
export const ViewPortfolio: React.FC = () => {
    ...
    // READ COHORT ID FROM URL QUERY STRING FIRST, THEN LOCATION.STATE
    const targetCohortId = searchParams.get('cohortId') || (location.state as any)?.cohortId || undefined;
    ...
    // 🚀 SURVEY LINKAGE & COMPLETION STATES
    const [assessmentSurveyMap, setAssessmentSurveyMap] = useState<Map<string, string>>(new Map());
    const [completedSurveyIds, setCompletedSurveyIds] = useState<Set<string>>(new Set());
    const [showSurveyPlayerModal, setShowSurveyPlayerModal] = useState<{ surveyId: string; assessmentId: string; cohortId?: string; submissionId?: string } | null>(null);
    ...
    const [scheduleContext, setScheduleContext] = useState<LearnerSubmission | null>(null);
    ...
    useEffect(() => {
        ...
        if (!staff || !staff.length) fetchStaff();
    ...
    const enrollment = useMemo(() => {
        ...
        const records = learners.filter((l: any) => l.enrollmentId === routeId || l.id === routeId || l.learnerId === routeId || l.idNumber === routeId);

        if (!records.length) return undefined;

        const base = targetCohortId ? (records.find((l: any) => l.cohortId === targetCohortId) || records[0]) : records[0];
    ...
    const activeCohort = useMemo(() => {
        return cohorts.find((c: any) => c.id === (targetCohortId || enrollment?.cohortId));
    }, [cohorts, targetCohortId, enrollment]);

    const activeCohortName = activeCohort?.name || (enrollment as any)?.cohortName || "Dormant Profile (Unassigned)";
    const effectiveStartDate = activeCohort?.startDate || enrollment?.trainingStartDate;
    const effectiveEndDate = activeCohort?.endDate || (enrollment as any)?.trainingEndDate;
    ...
    // 🚀 FETCH SURVEY RESPONSES FOR LEARNER WITH RULE ERROR SAFETY
    useEffect(() => {
        ...
        const targetIds = Array.from(new Set([currentUid, enrollment.learnerId, enrollment.authUid].filter(Boolean)));

        const fetchResponses = async () => {
            try {
                let snap;
                if (user?.role === 'learner') {
                    const q = query(collection(db, 'survey_responses'), where('learnerId', '==', currentUid));
                    snap = await getDocs(q);
                } else {
                    const q = query(collection(db, 'survey_responses'), where('learnerId', 'in', targetIds));
                    snap = await getDocs(q);
                }
                ...
                snap.docs.forEach(d => {
                    const data = d.data();
                    if (data.surveyId) completedSet.add(data.surveyId);
                });
                setCompletedSurveyIds(completedSet);
            } catch (err: any) {
                console.warn("Survey responses lookup bypassed:", err.message);
            }
        };

        fetchResponses();
    }, [enrollment, user?.uid, user?.role]);

    // ─── CROSS-REFERENCE PLACEMENT ENGINE DATA ───
    ...
    // 🚀 STRICT COHORT-ISOLATED SUBMISSION LOADING ENGINE
    useEffect(() => {
        let mounted = true;
            ...
            try {
                ...
                try {
                    ...
                    subs = Array.from(mergedDocs.values())
                        ...
                        .filter(s => {
                            if (!activeCohortId) return true;
                            return s.cohortId === activeCohortId;
                        });

                    const deduplicatedMap = new Map<string, LearnerSubmission>();
                    subs.forEach(sub => {
                        const dedupKey = `${sub.cohortId || activeCohortId}_${sub.assessmentId}`;
                        const existing = deduplicatedMap.get(dedupKey);
                        if (!existing) {
                            deduplicatedMap.set(dedupKey, sub);
                        } else {
                            const statusWeights: Record<string, number> = { 'not_started': 0, 'in_progress': 1, 'missed': 2, 'violation': 2, 'returned': 3, 'submitted': 4, 'facilitator_reviewed': 5, 'graded': 6, 'moderated': 7 };
                ...
                const newSurveyMap = new Map<string, string>();
                ...
                try {
                    if (activeCohortId) {
                        ...
                        assessmentsSnap.forEach(docSnap => {
                            ...
                            if (assData.surveyId) {
                                newSurveyMap.set(docSnap.id, assData.surveyId);
                            }

                            const belongsToCohort = (assData.cohortIds && Array.isArray(assData.cohortIds) && assData.cohortIds.includes(activeCohortId)) ||
                                (assData.cohortId === activeCohortId);
                ...
                setAssessmentSurveyMap(newSurveyMap);
                ...
                subs = subs.map(sub => {
                    ...
                    if (matchingAss) {
                        return {
                            ...
                            surveyId: matchingAss.surveyId || sub.surveyId,
                            isBootcamp: matchingAss.isBootcamp || sub.isBootcamp || false,
                            framework: matchingAss.framework || sub.framework || (matchingAss.isBootcamp ? 'secam' : 'qcto')
                        };
                    }
                    return sub;
                });
                ...
                try {
                    if (activeCohortId && activeAssessments.size > 0) {
                        ...
                        for (const [astId, assData] of activeAssessments.entries()) {
                            if (!existingAssIds.has(astId)) {
                                ...
                                const newSub = {
                                    ...
                                    surveyId: assData.surveyId || null,
                                    isBootcamp: assData.isBootcamp || false,
                                    framework: assData.framework || (assData.isBootcamp ? 'secam' : 'qcto'),
    ...
    // ─── 3. PIPELINE AND FILTER MEMOS ───
    const pipelineStats = useMemo(() => {
        const total = submissions.length;
        if (total === 0) return { total: 0, submitted: 0, facReviewed: 0, graded: 0, moderated: 0 };
        return {
            total,
            submitted: submissions.filter(s => !['not_started', 'in_progress', 'missed', 'violation'].includes(s.status)).length,
    ...
    // 🚀 COMPETENCY SUMMARY DISTRIBUTION MEMO
    const competencySummary = useMemo(() => {
        let competent = 0;
        let highlyCompetent = 0;
        let developing = 0;
        let nyc = 0;
        let pendingAss = 0;
        let activeRemediation = 0;

        submissions.forEach(sub => {
            const comp = String(sub.competency || '').toUpperCase();
            const isFinal = ['graded', 'moderated', 'appealed'].includes(sub.status);

            if (isFinal) {
                if (comp === 'HC' || comp === '4') highlyCompetent++;
                else if (comp === 'C' || comp === '3') competent++;
                else if (comp === 'DEV' || comp === '2') developing++;
                else if (comp === 'NYC' || comp === '1') nyc++;
                else competent++;
            } else if (['submitted', 'facilitator_reviewed', 'awaiting_learner_signoff'].includes(sub.status)) {
                pendingAss++;
            }

            if ((sub.attemptNumber || 1) > 1 || sub.latestCoachingLog || sub.status === 'returned') {
                activeRemediation++;
            }
        });

        const totalCompetent = competent + highlyCompetent;
        const totalReassessmentDue = developing + nyc;

        return { competent, highlyCompetent, developing, nyc, totalCompetent, totalReassessmentDue, pendingAss, activeRemediation };
    }, [submissions]);

    const filteredSubmissions = useMemo(() => {
        let filtered = submissions.filter(sub => {
            if (activeTab === 'overview') return true;
            const isPracticeActivity = INFORMAL_TYPES.includes(String(sub.type || '').toLowerCase());
        ...
        if (searchTerm) {
            ...
            filtered = filtered.filter(sub =>
                String(sub.title || '').toLowerCase().includes(term) ||
                String(sub.moduleNumber || '').toLowerCase().includes(term) ||
                String(sub.type || '').toLowerCase().includes(term) ||
                String(sub.facilitatorName || '').toLowerCase().includes(term)
            );
        }
        ...
        if (statusFilter !== 'all') {
            filtered = filtered.filter(sub => {
                const comp = String(sub.competency || '').toUpperCase();
                if (statusFilter === 'competent') return ['graded', 'moderated', 'appealed'].includes(sub.status) && ['C', 'HC', '3', '4'].includes(comp);
                if (statusFilter === 'reassessment_due') return (['graded', 'moderated', 'appealed'].includes(sub.status) && ['NYC', 'DEV', '1', '2'].includes(comp)) || sub.status === 'returned';
                if (statusFilter === 'nyc') return ['graded', 'moderated', 'appealed'].includes(sub.status) && ['NYC', '1'].includes(comp);
                ...
                if (statusFilter === 'violation') return sub.status === 'violation';
        ...
        filtered.sort((a, b) => {
            ...
            switch (sortBy) {
                case 'module':
                    comparison = String(a.moduleNumber || 'ZZZ').localeCompare(String(b.moduleNumber || 'ZZZ'));
                    break;
                case 'date':
                ...
                case 'title':
                    ...
                    break;
                case 'status':
                    const statusOrder = ['not_started', 'in_progress', 'submitted', 'facilitator_reviewed', 'graded', 'moderated', 'appealed', 'missed', 'violation'];
    ...
    const executeRemediation = async (date: string, notes: string) => {
        ...
        try {
            ...
            // 1. Sanitize snapshot to prevent Firestore undefined value errors
            const historySnapshot = JSON.parse(JSON.stringify({
                ...s,
            ...
            const historyRef = doc(collection(db, 'learner_submissions', s.id, 'history'));
            await setDoc(historyRef, historySnapshot);

            // 2. Reset the main document so learner goes back to the Gate briefing screen
            await updateDoc(doc(db, 'learner_submissions', s.id), {
                ...
                coachingRequested: deleteField(),   // 🚀 Wipes the pending flag
                coachingRequestedAt: deleteField(), // 🚀 Wipes the timestamp
            ...
            setSubmissions(p => p.map(x => x.id === s.id ? {
                ...
                coachingRequested: undefined,
                coachingRequestedAt: undefined,
                attemptNumber: (x.attemptNumber || 1) + 1,
                latestCoachingLog: {
                    date: date,
                    notes: notes.trim(),
                    facilitatorId: user?.uid || 'system',
            ...
            toast.error("Failed to unlock workbook: " + (err.message || "Unknown error"));
        }
    };
    ...
    const handleTriggerCompiledModulePrint = async (moduleCode: string, logsList: any[]) => {
        try {
            ...
            if (!resolvedMentorSig && (targetLog.mentorEmail || targetLog.processedBy)) {
                const targetEmail = String(targetLog.mentorEmail || targetLog.processedBy || '').toLowerCase().trim();
                if (targetEmail) {
                    try {
            ...
            let finalEmployerDetails = activeEmployerDetails;
            ...
            setBulkPrintPayload({
                ...
                employerDetails: finalEmployerDetails
            });
    ...
    // 🚀 DYNAMIC COMPETENCY & RE-ASSESSMENT BADGE GENERATOR
    const getStatusBadge = (sub: LearnerSubmission) => {
        ...
        const comp = String(sub.competency || '').toUpperCase();
        const isAttemptedAgain = (sub.attemptNumber || 1) > 1;

        switch (sub.status) {
            case 'moderated':
            case 'graded':
                if (comp === 'HC' || comp === '4') {
                    return <span className="mlab-badge" style={{ ...baseStyle, background: '#dcfce7', color: '#15803d', border: '1px solid #86efac' }}><Sparkles size={12} /> L4: Highly Competent</span>;
                }
                if (comp === 'C' || comp === '3' || (!sub.competency && sub.status === 'moderated')) {
                    return <span className="mlab-badge" style={{ ...baseStyle, background: '#dcfce7', color: '#166534', border: '1px solid #bbf7d0' }}><Award size={12} /> Competent (C)</span>;
                }
                if (comp === 'DEV' || comp === '2') {
                    return <span className="mlab-badge" style={{ ...baseStyle, background: '#fff7ed', color: '#c2410c', border: '1px solid #fed7aa' }}><RefreshCw size={12} /> L2: Developing (Re-assessment Due)</span>;
                }
                if (comp === 'NYC' || comp === '1') {
                    return <span className="mlab-badge" style={{ ...baseStyle, background: '#fee2e2', color: '#b91c1c', border: '1px solid #fecaca' }}><AlertCircle size={12} /> NYC (Re-assessment Due)</span>;
                }
                return <span className="mlab-badge" style={{ ...baseStyle, background: '#dcfce7', color: '#166534', border: '1px solid #bbf7d0' }}><Award size={12} /> Competent (C)</span>;
            ...
            case 'returned':
                ...
                return <span className="mlab-badge" style={{ ...baseStyle, background: '#f1f5f9', color: '#475569', border: '1px solid #e2e8f0' }}><RotateCcw size={12} /> Returned for Revision</span>;
            ...
            case 'in_progress':
                return (
                    <span className="mlab-badge" style={{ ...baseStyle, background: isAttemptedAgain ? '#ffedd5' : '#dbeafe', color: isAttemptedAgain ? '#c2410c' : '#1e40af', border: isAttemptedAgain ? '1px solid #fed7aa' : '1px solid #bfdbfe' }}>
                        <Edit3 size={12} /> {isAttemptedAgain ? `Re-assessment Active (Att. #${sub.attemptNumber})` : 'In Progress'}
                    </span>
                );

            case 'not_started':
                if (isAttemptedAgain) {
                    return <span className="mlab-badge" style={{ ...baseStyle, background: '#fef3c7', color: '#b45309', border: '1px solid #fde68a' }}><RefreshCw size={12} /> Re-assessment Unlocked (Att. #{sub.attemptNumber})</span>;
                }
            ...
            case 'violation':
                return <span className="mlab-badge" style={{ ...baseStyle, background: '#fee2e2', color: '#991b1b', border: '1px solid #fecaca' }}><ShieldAlert size={12} /> Violation</span>;
    ...
    const renderTableRow = (sub: LearnerSubmission, isGrouped: boolean) => {
        const comp = String(sub.competency || '').toUpperCase();
        const isNYC = (sub.status === 'moderated' || sub.status === 'graded') && ['NYC', 'DEV', '1', '2'].includes(comp);
        const isMissed = sub.status === 'missed';
        const isViolation = sub.status === 'violation';
        ...
        // 🚀 POST-ASSESSMENT SURVEY INDICATOR EVALUATION
        const linkedSurveyId = sub.surveyId || assessmentSurveyMap.get(sub.assessmentId);
        const isSubmittedOrGraded = ['submitted', 'facilitator_reviewed', 'graded', 'moderated', 'returned', 'appealed'].includes(sub.status);
        const isSurveyPending = Boolean(linkedSurveyId && isSubmittedOrGraded && !completedSurveyIds.has(linkedSurveyId));
            ...
            <tr key={sub.id}>
                <td style={isGrouped ? { paddingLeft: '2.5rem' } : {}}>
                    <div className="vp-assessment-cell">
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '6px' }}>
                            {sub.isScheduled && sub.scheduledDate && ['not_started', 'in_progress', 'missed', 'violation', 'upcoming'].includes(sub.status) ? (
                                <span className="vp-assessment-meta" style={{ color: (sub.status === 'missed' || sub.status === 'violation') ? '#ef4444' : '#0284c7', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
                                    <Clock size={12} />
                ...
                <td style={{ textAlign: 'right' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '8px' }}>
                        ...
                        {/* 🚀 POST-ASSESSMENT SURVEY INDICATOR BUTTON */}
                        {isSurveyPending && (
                            <button
                                ...
                                title="Post-Assessment Feedback Survey Pending"
                                onClick={(e) => {
                                    e.stopPropagation();
                                    if (user?.role === 'learner') {
                                        setShowSurveyPlayerModal({
                                        ...
                                        window.open(`/survey/${linkedSurveyId}`, '_blank');
                                    }
                                }}
                            >
                                <ClipboardList size={12} color="#b45309" /> Survey Pending
                            </button>
                        )}
                        ...
                        {(() => {
                            ...
                            // 🚀 1. MISSED ASSESSMENT ACTION (LEARNER VS STAFF)
                            if (isMissed) {
                                if (user?.role === 'learner') {
                                    return (
                                        <button
                                        ...
                                        >
                                            <AlertTriangle size={14} style={{ marginRight: '4px' }} /> Request Re-schedule
                                        </button>
                                    );
                                } else {
                                    return (
                                        <button className="mlab-btn mlab-btn--outline" style={{ color: '#dc2626', borderColor: '#fecaca' }} onClick={handleViewClick}>
                                            <ShieldAlert size={14} /> Review Missed
                                        </button>
                                    );
                                }
                            }

                            // 🚀 2. RE-ASSESSMENT / REMEDIATION ACTION FOR LEARNER (GOES VIA COACHING OR RESUME)
                            if (isNYC && user?.role === 'learner' && !hasPendingAppeal) {
                                if (sub.status === 'in_progress' || sub.status === 'not_started') {
                                    return (
                                        <button className="mlab-btn mlab-btn--primary mlab-btn--sm" onClick={handleViewClick}>
                                            <Play size={14} style={{ marginRight: '4px' }} /> Resume Re-assessment (Att. #{sub.attemptNumber || 2})
                                        </button>
                                    );
                                }

                                // Check if they already requested coaching!
                                if ((sub as any).coachingRequested) {
                                    return (
                                        <>
                                            <div className="vp-pending-tooltip-wrap">
                                                <button
                                                    className="mlab-btn mlab-btn--sm"
                                                    disabled
                                                    style={{ background: '#fff7ed', color: '#c2410c', border: '1px solid #fed7aa', cursor: 'help' }}
                                                >
                                                    <Clock size={14} style={{ marginRight: '4px' }} /> Request Pending
                                                    <Info size={12} style={{ marginLeft: '4px', opacity: 0.8 }} />
                                                </button>

                                                {/* 🚀 EXPLANATORY HOVER POPOVER */}
                                                <div className="vp-pending-popover">
                                                    <strong><ShieldCheck size={13} /> Coaching Session Requested</strong>
                                                    <p>
                                                        Your request for remediation support has been received. Per QCTO compliance, your re-assessment will unlock automatically once your facilitator conducts and logs your 1-on-1 coaching session.
                                                    </p>
                                                    {(sub as any).coachingRequestedAt && (
                                                        <small>
                                                            Requested on: {moment((sub as any).coachingRequestedAt).format('DD MMM YYYY [at] HH:mm')}
                                                        </small>
                                                    )}
                                                </div>
                                            </div>
                                ...
                                return (
                                    <>
                                        ...
                                        >
                                            <Calendar size={14} style={{ marginRight: '4px' }} /> Request Re-assessment Coaching
                                        </button>
                                        <button className="mlab-btn" style={{ color: 'green' }} onClick={handleViewClick}>
                                            <Eye size={14} /> View
                                        </button>
                            ...
                            // 🚀 3. REMEDIATION ACTION FOR STAFF (FACILITATOR UNLOCKS)
                            if (isNYC && user?.role !== 'learner' && !hasPendingAppeal) {
                                return (
                                    <button className="mlab-btn mlab-btn--warning mlab-btn--sm" style={{ color: 'var(--mlab-blue)' }} disabled={isDropped} onClick={() => setRemediationTarget(sub)}>
                                        <RefreshCw size={14} style={{ marginRight: '4px' }} /> Log Coaching &amp; Unlock Att. #{(sub.attemptNumber || 1) + 1}
                                    </button>
                                );
                            }

                            if (isViolation && user?.role !== 'learner') {
                                return (
                                    <button className="mlab-btn mlab-btn--outline" style={{ color: '#dc2626', borderColor: '#fecaca' }} onClick={handleViewClick}>
                                        <ShieldAlert size={14} /> Review Issue
                                    </button>
                                );
                            }
    ...
    return (
        <div className="admin-layout">
            ...
            <style>{`
                ...
                .vp-pending-tooltip-wrap {
                    position: relative;
                    display: inline-flex;
                }

                .vp-pending-tooltip-wrap:hover .vp-pending-popover {
                    opacity: 1;
                    visibility: visible;
                ...
                .vp-pending-popover {
                    ...
                    right: 0;
                    width: 270px;
                    background: var(--mlab-blue, #073f4e);
                    color: #ffffff;
                    padding: 10px 12px;
                    border-radius: 6px;
                ...
                .vp-pending-popover::after {
                    ...
                    border-width: 6px;
                    border-style: solid;
                    border-color: var(--mlab-blue, #073f4e) transparent transparent transparent;
                }

                .vp-pending-popover strong {
                    display: flex;
                    align-items: center;
                    gap: 5px;
                    color: var(--mlab-green, #94c73d);
                    font-size: 0.78rem;
                ...
                .vp-pending-popover p {
                    margin: 0 0 6px 0;
                    color: #e2e8f0;
                }

                .vp-pending-popover small {
                    display: block;
                    color: #94a3b8;
                    font-size: 0.68rem;
            ...
            {bulkPrintPayload && createPortal(
                <WorkplaceModuleBulkPrintable
                    ...
                    employerDetails={bulkPrintPayload.employerDetails}
                />,
                document.body
            )}
                    ...
                    <div className="vp-profile-card">
                        ...
                        <div className="vp-profile-info">
                            ...
                            <div className="vp-profile-meta">
                                ...
                                <>
                                    <span className="vp-profile-divider">|</span>
                                    <span><Briefcase size={14} /> {activeCohortName}</span>
                                </>
                            </div>
                        </div>
                        <div className="vp-profile-status">
                            ...
                            <span className={`vp-status-value vp-status-value--${enrollment.status}`}>
                                {enrollment.status === 'active' ? 'IN TRAINING' : String(enrollment.status || '').toUpperCase() || 'UNKNOWN'}
                            </span>
                        </div>
                    </div>
                                    ...
                                    {tab.id !== 'overview' && tab.id !== 'compliance' && (
                                        <span className="vp-count-badge" style={{ marginLeft: '6px', fontSize: '0.65rem', padding: '1px 6px' }}>
                                            {submissions.filter(s => {
                                                if (tab.id === 'other') return INFORMAL_TYPES.includes(String(s.type || '').toLowerCase());
                                                return (s.moduleType || 'knowledge') === tab.id && !INFORMAL_TYPES.includes(String(s.type || '').toLowerCase());
                                            }).length}
                                        </span>
                                    )}
                                </button>
                    ...
                    <div className="vp-content">
                        {activeTab === 'overview' && (
                            <div className="vp-grid vp-grid--2col">
                                ...
                                {/* 🚀 COMPETENCY SUMMARY DISTRIBUTION CARD */}
                                <div className="vp-card vp-grid-span-2" style={{ background: '#f8fafc', border: '1px solid #cbd5e1' }}>
                                    <div className="vp-card-header" style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '0.75rem' }}>
                                        <Award size={18} color="var(--mlab-blue)" />
                                        <h3 style={{ margin: 0, fontSize: '0.95rem', color: '#0f172a' }}>Competency &amp; Re-assessment Status</h3>
                                    </div>
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px', padding: '1rem 0 0 0' }}>
                                        <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '12px 16px', borderRadius: '8px' }}>
                                            <span style={{ fontSize: '0.7rem', fontWeight: 800, color: '#166534', textTransform: 'uppercase', display: 'block', marginBottom: '4px' }}>Competent (C)</span>
                                            <span style={{ fontSize: '1.4rem', fontWeight: 800, color: '#14532d' }}>{competencySummary.totalCompetent}</span>
                                            <span style={{ fontSize: '0.75rem', color: '#15803d', display: 'block', marginTop: '2px' }}>Verified Modules</span>
                                        </div>

                                        <div style={{ background: competencySummary.totalReassessmentDue > 0 ? '#fff7ed' : '#ffffff', border: `1px solid ${competencySummary.totalReassessmentDue > 0 ? '#fed7aa' : '#e2e8f0'}`, padding: '12px 16px', borderRadius: '8px' }}>
                                            <span style={{ fontSize: '0.7rem', fontWeight: 800, color: competencySummary.totalReassessmentDue > 0 ? '#c2410c' : '#64748b', textTransform: 'uppercase', display: 'block', marginBottom: '4px' }}>Re-assessment Due</span>
                                            <span style={{ fontSize: '1.4rem', fontWeight: 800, color: competencySummary.totalReassessmentDue > 0 ? '#9a3412' : '#334155' }}>{competencySummary.totalReassessmentDue}</span>
                                            <span style={{ fontSize: '0.75rem', color: competencySummary.totalReassessmentDue > 0 ? '#ea580c' : '#64748b', display: 'block', marginTop: '2px' }}>NYC / DEV Assessments</span>
                                        </div>

                                        <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', padding: '12px 16px', borderRadius: '8px' }}>
                                            <span style={{ fontSize: '0.7rem', fontWeight: 800, color: '#1e40af', textTransform: 'uppercase', display: 'block', marginBottom: '4px' }}>Pending Assessment</span>
                                            <span style={{ fontSize: '1.4rem', fontWeight: 800, color: '#1e3a8a' }}>{competencySummary.pendingAss}</span>
                                            <span style={{ fontSize: '0.75rem', color: '#1d4ed8', display: 'block', marginTop: '2px' }}>Submitted / Reviewing</span>
                                        </div>

                                        <div style={{ background: '#ffffff', border: '1px solid #e2e8f0', padding: '12px 16px', borderRadius: '8px' }}>
                                            <span style={{ fontSize: '0.7rem', fontWeight: 800, color: '#475569', textTransform: 'uppercase', display: 'block', marginBottom: '4px' }}>Remediation Active</span>
                                            <span style={{ fontSize: '1.4rem', fontWeight: 800, color: '#0f172a' }}>{competencySummary.activeRemediation}</span>
                                            <span style={{ fontSize: '0.75rem', color: '#64748b', display: 'block', marginTop: '2px' }}>Re-attempts Logged</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mlab-toolbar" style={{ borderTop: 'none', padding: '1.25rem', display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center', background: '#f8fafc', borderBottom: '1px solid #cbd5e1' }}>
                                    ...
                                    <div className="mlab-select-wrap" style={{ height: '38px', border: '1px solid var(--mlab-border)', borderRadius: '6px', background: 'white', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
                                        ...
                                        >
                                            ...
                                            <option value="competent">Competent (C / HC)</option>
                                            <option value="reassessment_due">Re-assessment Due (NYC / DEV)</option>
            ...
            {/* MODAL OVERLAY FOR SURVEY PLAYER POPUP */}
            {showSurveyPlayerModal && (
                <SurveyEnginePlayer
                    surveyId={showSurveyPlayerModal.surveyId}
                    context={{
                        assessmentId: showSurveyPlayerModal.assessmentId,
                        cohortId: showSurveyPlayerModal.cohortId,
                        learnerId: enrollment.learnerId || user?.uid || '',
                        learnerName: enrollment.fullName || user?.fullName || '',
                        submissionId: showSurveyPlayerModal.submissionId
                    }}
                    onComplete={() => {
                        setShowSurveyPlayerModal(null);
                        toast.success("Survey feedback submitted! Thank you.");
                    }}
                    onSkip={() => setShowSurveyPlayerModal(null)}
                />
            )}
        </div>
    );
...
//     cohortsList: any[];
//     onClose: () => void;
//     toast: any; // 🚀 Prop passed from parent
// }> = ({ learner, user, context, staffList, cohortsList, onClose, toast }) => {
//     const isLearner = user?.role === 'learner';
//     const isMissedContext = context?.status === 'missed';
//     const isNYCContext = context && (context.competency === 'NYC' || context.competency === 'DEV' || context.competency === '1' || context.competency === '2');
...
//             // 🚀 NEW: Flag the submission so the button turns into "Request Pending"
//             if (context?.id && isLearner) {
//                 await updateDoc(doc(db, 'learner_submissions', context.id), {
//                     coachingRequested: true,
//                     coachingRequestedAt: new Date().toISOString()
...
//             await setDoc(doc(collection(db, 'learner_submissions', s.id, 'history')), {
//                 ...s,
//                 archivedAt: new Date().toISOString()
//             });

//             // 2. Reset the main document and wipe the pending request flags
//             await updateDoc(doc(db, 'learner_submissions', s.id), {
//                 status: 'in_progress',
//                 competency: deleteField(),
//                 grading: deleteField(),
...
//                 status: 'in_progress',
//                 competency: undefined,
//                 coachingRequested: undefined,       // 🚀 Clears locally so the button resets instantly
//                 coachingRequestedAt: undefined,
//                 attemptNumber: (x.attemptNumber || 1) + 1
//             } : x));
...
//                                 // Check if they already requested it!
//                                 if ((sub as any).coachingRequested) {
//                                     return (
//                                         <>
//                                             <button className="mlab-btn mlab-btn--sm" disabled style={{ background: '#f8fafc', color: '#94a3b8', border: '1px solid #cbd5e1', cursor: 'not-allowed' }}>
//                                                 <Clock size={14} style={{ marginRight: '4px' }} /> Request Pending
//                                             </button>
//                                             <button className="mlab-btn" style={{ color: 'green' }} onClick={handleViewClick}>
//                                                 <Eye size={14} /> View
...
// //                 staffId: isLearner ? (context?.facilitatorName || 'Unassigned Staff') : user.uid,
...
// // //     Filter, ChevronLeft, ChevronRight,
// // //     ArrowUpDown, CheckSquare, Square, Printer, Layers, ShieldAlert,
// // //     Timer, Lock, ChevronDown, ChevronUp, CheckCircle2, ClipboardList
// // // } from 'lucide-react';
// // // import {
// // //     collection, query, where, getDocs, doc, getDoc, limit,
// // //     setDoc, updateDoc, deleteField, onSnapshot, addDoc, writeBatch
...
// // //                 let snap;
// // //                 if (user?.role === 'learner') {
// // //                     // Learner security rules restrict to exact auth UID
// // //                     const q = query(collection(db, 'survey_responses'), where('learnerId', '==', currentUid));
// // //                     snap = await getDocs(q);
// // //                 } else {
// // //                     // Admin / Staff query by target IDs
// // //                     const q = query(collection(db, 'survey_responses'), where('learnerId', 'in', targetIds));
// // //                     snap = await getDocs(q);
// // //                 }
...
// // //                             if ((sub.status === 'missed' || sub.status === 'violation') && user?.role !== 'learner') {
// // //                                 return (
// // //                                     <button className="mlab-btn mlab-btn--outline" style={{ color: '#dc2626', borderColor: '#fecaca' }} onClick={handleViewClick}>
// // //                                         <ShieldAlert size={14} /> Review Issue
// // //                                     </button>
...
// // //                             if ((sub.status === 'missed' || sub.status === 'violation' || sub.status === 'upcoming') && user?.role === 'learner') {
// // //                                 return (
// // //                                     <button className="mlab-btn" style={{ color: (sub.status === 'missed' || sub.status === 'violation') ? '#dc2626' : '#d97706' }} onClick={handleViewClick}>
// // //                                         <Eye size={14} /> View
// // //                                     </button>
// // //                                 );
// // //                             }
