...
export const ViewPortfolio: React.FC = () => {
    ...
    const executeRemediation = async (date: string, notes: string) => {
        ...
        try {
            ...
            // 🚀 3. BIDIRECTIONAL SYNC: Find and update open coaching session in coaching_sessions collection
            try {
                const targetLearnerId = s.learnerId || s.authUid;
                if (targetLearnerId && s.assessmentId) {
                    const sessionQ = query(
                        collection(db, 'coaching_sessions'),
                        where('learnerId', '==', targetLearnerId),
                        where('assessmentId', '==', s.assessmentId)
                    );
                    let sessionSnap = await getDocs(sessionQ);

                    if (sessionSnap.empty && s.authUid) {
                        const sessionQAuth = query(
                            collection(db, 'coaching_sessions'),
                            where('learnerId', '==', s.authUid),
                            where('assessmentId', '==', s.assessmentId)
                        );
                        sessionSnap = await getDocs(sessionQAuth);
                    }

                    for (const sessionDoc of sessionSnap.docs) {
                        const sessionData = sessionDoc.data();
                        if (sessionData.status !== 'completed' && sessionData.status !== 'cancelled') {
                            await updateDoc(doc(db, 'coaching_sessions', sessionDoc.id), {
                                notes: notes.trim(),
                                status: 'completed',
                ...
                console.warn("Coaching session sync handled error:", syncErr);
            }
                ...
                <td style={{ textAlign: 'right' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '8px' }}>
                        ...
                        {(() => {
                            ...
                            // 3. REMEDIATION ACTION FOR STAFF (FACILITATOR UNLOCKS + VIEW BUTTON)
                            if (isNYC && user?.role !== 'learner' && !hasPendingAppeal) {
                                return (
                                    <>
