...
import { RotateCcw, MessageSquare, Undo2, Scale, X, Save, AlertCircle } from 'lucide-react';
...
export const RemediationModal: React.FC<{
    ...
    return createPortal(
        <div className="lfm-overlay" onClick={onClose}>
            <div className="lfm-modal animate-fade-in" onClick={e => e.stopPropagation()} style={{ maxWidth: '520px' }}>
                <div className="lfm-header" style={{ borderBottom: "5px solid orange", }}>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
                    <div className="lfm-body">
                        <div className="lfm-flags-panel" style={{ marginTop: 0, background: "#ffda942e", borderLeft: "5px solid orange" }}>
                            <span style={{ fontSize: '0.72rem', fontWeight: 'bold', color: 'orange', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                                {submissionTitle}
                            </span>
                            <p style={{ margin: '4px 0 0 0', fontSize: '0.85rem', color: 'orange', lineHeight: 1.5 }}>
                                {isFinalAttempt
                        ...
                        <div className="lfm-fg">
                            <label>Date of Coaching / Feedback Session *</label>
                            <input
                                className="lfm-input"
                                type="date"
                                required
                        ...
                        <div className="lfm-fg">
                            <label style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                <MessageSquare size={13} /> Coaching Notes / Areas Addressed *
                            </label>
                            <textarea
                                className="lfm-input"
                                required
                                rows={4}
                                placeholder={isFinalAttempt ? "Describe the rigorous intervention applied..." : "Briefly describe what was discussed..."}
                                value={notes}
                    ...
                    <div className="lfm-footer">
                        ...
                        <button
                            ...
                            disabled={!date || !notes.trim() || !confirmed}
                        >
                            <Save size={13} /> Log Coaching & Unlock
                        </button>
                    </div>
                </form>
            </div>
        ...
        <div className="lfm-overlay" onClick={onClose}>
            <div className="lfm-modal animate-fade-in" onClick={e => e.stopPropagation()} style={{ maxWidth: '520px' }}>
                <div className="lfm-header">
                    <h2 className="lfm-header__title">
                        <Undo2 size={16} /> Return Logbook to Learner
                    </h2>
                    <button className="lfm-close-btn" type="button" onClick={onClose}>
                        <X size={20} />
                    </button>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
                    <div className="lfm-body">
                        ...
                        <div className="lfm-fg">
                            <label style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                <MessageSquare size={13} /> Reason for Return (visible to learner) *
                            </label>
                            <textarea
                                className="lfm-input"
                                required
                    ...
                    <div className="lfm-footer">
                        ...
                        >
                            <Undo2 size={13} /> Return to Learner
                        </button>
                    </div>
                </form>
            </div>
    ...
    return createPortal(
        <div className="lfm-overlay" onClick={onClose}>
            <div className="lfm-modal animate-fade-in" onClick={e => e.stopPropagation()} style={{ maxWidth: '600px' }}>
                <div className="lfm-header">
                    <h2 className="lfm-header__title">
                        <Scale size={16} /> Academic Appeal Resolution
                    </h2>
                    <button className="lfm-close-btn" type="button" onClick={onClose}>
                        <X size={20} />
                    </button>
                ...
                <div className="lfm-body">
                    <div className="lfm-flags-panel" style={{ marginTop: 0 }}>
                        <span style={{ fontSize: '0.72rem', fontWeight: 'bold', color: 'var(--mlab-blue)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                            Learner's Reason for Appeal:
                        </span>
                        <p style={{ fontSize: '0.88rem', color: 'var(--mlab-blue)', margin: '4px 0 0 0', fontStyle: 'italic', lineHeight: 1.4 }}>
                            "{appealReason}"
                        </p>
                    </div>
                    ...
                    <div className="lfm-fg">
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '4px' }}>
                            <label className="lfm-checkbox-row" style={{ padding: '10px', border: decision === 'overturn' ? '2px solid var(--mlab-blue)' : '1px solid var(--mlab-border)', background: decision === 'overturn' ? 'var(--mlab-light-blue)' : 'white' }}>
                                <input type="radio" name="decision" checked={decision === 'overturn'} onChange={() => setDecision('overturn')} />
                                <div>
                                    <strong style={{ display: 'block', color: 'var(--mlab-blue)', fontSize: '0.88rem' }}>Overturn Decision (Change to Competent)</strong>
                                    <span style={{ fontSize: '0.75rem', color: 'var(--mlab-grey)' }}>The learner's appeal is valid. The NYC grade is overturned to Competent.</span>
                                </div>
                            </label>
                            <label className="lfm-checkbox-row" style={{ padding: '10px', border: decision === 'new_attempt' ? '2px solid var(--mlab-blue)' : '1px solid var(--mlab-border)', background: decision === 'new_attempt' ? 'var(--mlab-light-blue)' : 'white' }}>
                                <input type="radio" name="decision" checked={decision === 'new_attempt'} onChange={() => setDecision('new_attempt')} />
                                <div>
                                    <strong style={{ display: 'block', color: 'var(--mlab-blue)', fontSize: '0.88rem' }}>Grant Additional Attempt</strong>
                                    <span style={{ fontSize: '0.75rem', color: 'var(--mlab-grey)' }}>The NYC grade stands, but the learner is granted a fresh attempt (even if max attempts were reached).</span>
                                </div>
                            </label>
                            <label className="lfm-checkbox-row" style={{ padding: '10px', border: decision === 'reject' ? '2px solid var(--mlab-blue)' : '1px solid var(--mlab-border)', background: decision === 'reject' ? 'var(--mlab-light-blue)' : 'white' }}>
                                <input type="radio" name="decision" checked={decision === 'reject'} onChange={() => setDecision('reject')} />
                                <div>
                                    <strong style={{ display: 'block', color: 'var(--mlab-blue)', fontSize: '0.88rem' }}>Uphold Original Grade (Reject Appeal)</strong>
                                    <span style={{ fontSize: '0.75rem', color: 'var(--mlab-grey)' }}>The appeal is dismissed. The NYC grade remains locked.</span>
                                </div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div className="lfm-fg">
                        ...
                        <textarea
                            ...
                            required
                            rows={3}
                            placeholder="Provide justification for the Academic Board's decision..."
                            value={notes}
                            onChange={e => setNotes(e.target.value)}
                            style={{ resize: 'vertical' }}
                        />
                ...
                <div className="lfm-footer">
                    ...
                    >
                        <Scale size={13} /> Finalise Resolution
                    </button>
                </div>
            </div>
        </div>,
