...
const FilePreview = ({ url }: { url: string }) => {
    ...
    const getExtension = (urlStr: string) => {
        try {
            // Strip query parameters to accurately get the extension from Firebase URLs
    ...
    // Google Docs Viewer securely handles Firebase auth tokens in URLs
...
export const SubmissionReview: React.FC = () => {
            ...
            try {
                ...
                if (!aBreakdown || Object.keys(aBreakdown).length === 0) {
                    if (['facilitator_reviewed', 'returned', 'graded', 'moderated'].includes(dbStatus)) {
                        ...
                        // INHERIT CHECKLIST FROM FACILITATOR (Mentor)
                ...
                if (!mBreakdown || Object.keys(mBreakdown).length === 0) {
                    if (['graded', 'moderated', 'returned'].includes(dbStatus)) {
                        ...
                        // INHERIT CHECKLIST FROM ASSESSOR
                        assData.blocks?.forEach((b: any) => {
    ...
    if (assessment?.blocks) {
        assessment.blocks.forEach((block: any) => {
            ...
            } else if (['mcq', 'text', 'task', 'checklist', 'logbook'].includes(block.type) && currentSectionId) {
    ...
    // 🚀 UPDATED VALIDATION FOR MODERATOR QA FIX 🚀
    const validateAllMarked = (breakdown: Record<string, GradeData>, isModerating: boolean = false) => {
        ...
        const unmarkedCount = assessment.blocks.filter((block: any) => {
            ...
            if (block.type === 'checklist') {
                // Moderator does NOT do the observation, so they shouldn't be blocked by missing their own declaration.
    ...
    const triggerSubmitFacilitator = () => {
        if (!validateAllMarked(facBreakdown, false)) return setModalConfig({ isOpen: true, type: 'warning', title: 'Incomplete Marking', message: 'You must evaluate every question and fully complete all checklists (including declarations) before submitting.', confirmText: 'Got it', onConfirm: () => setModalConfig(null) });
    ...
    const triggerSubmitGrade = () => {
        if (!validateAllMarked(assBreakdown, false)) return setModalConfig({ isOpen: true, type: 'warning', title: 'Incomplete Grading', message: 'You must evaluate every question and fully complete all checklists (including declarations) before submitting.', confirmText: 'Got it', onConfirm: () => setModalConfig(null) });
    ...
    const triggerSubmitModeration = () => {
        if (!validateAllMarked(modBreakdown, true)) return setModalConfig({ isOpen: true, type: 'warning', title: 'Incomplete QA', message: 'You must verify every question and fully check all observation blocks before endorsing.', confirmText: 'Got it', onConfirm: () => setModalConfig(null) });
    ...
    const renderBlocks = (isPrintMode: boolean) => {
        ...
        return assessment.blocks?.map((block: any, idx: number) => {
            ...
            if (['mcq', 'text', 'task', 'checklist', 'logbook'].includes(block.type)) {
                ...
                const fData = facBreakdown[block.id] || { score: 0, feedback: '', isCorrect: null, criteriaResults: [] };
                const aData = assBreakdown[block.id] || { score: 0, feedback: '', isCorrect: null, criteriaResults: [] };
                const mData = modBreakdown[block.id] || { score: 0, feedback: '', isCorrect: null, criteriaResults: [] };
                ...
                const renderFacReadOnly = isFacDone && !canFacilitatorMark && (fData.score > 0 || fData.feedback || fData.isCorrect !== null || block.type === 'checklist');
                const renderAssReadOnly = isAssDone && !canGrade && (aData.score > 0 || aData.feedback || aData.isCorrect !== null || block.type === 'checklist');
                const renderModReadOnly = isModDone && !canModerate && (mData.score > 0 || mData.feedback || mData.isCorrect !== null || block.type === 'checklist');

                return (
                    <div key={(isPrintMode ? 'print-' : '') + block.id} className="sr-q-card" style={{ borderColor: '#073f4e', borderTop: '4px solid black', marginBottom: '2rem' }}>

                        {/* ── HEADER ── */}
                        <div className="sr-q-header">
                            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                                <span className="sr-q-num" style={{ background: block.type === 'checklist' ? '#ccfbf1' : block.type === 'logbook' ? '#ffedd5' : '#f1f5f9', color: block.type === 'checklist' ? '#0d9488' : block.type === 'logbook' ? '#ea580c' : '#073f4e' }}>
                                    {block.type === 'checklist' ? 'CHK' : block.type === 'logbook' ? 'LOG' : `Q${qNum}`}
                                </span>

                                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                    <span className="sr-q-text" style={{ color: '#073f4e', fontWeight: 'bold' }}>{block.question || block.title}</span>
                                    {block.type === 'checklist' && <span style={{ fontSize: '0.75rem', color: '#64748b' }}>Assessor observation checklist.</span>}
                                    {block.type === 'logbook' && <span style={{ fontSize: '0.75rem', color: '#64748b' }}>Workplace experience hours log.</span>}
                                </div>
                        ...
                        <div className="sr-q-body">
                            ...
                            {/* 🚀 TASK (MULTI-MODAL EVIDENCE VIEWER) 🚀 */}
                            {block.type === 'task' && (
                                <div className="sr-answer-box">
                                    ...
                                    {(!learnerAns || Object.keys(learnerAns).length === 0) ? (
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                            ...
                                            {/* 🚀 USING TABS TO DISPLAY MULTI-MODAL EVIDENCE CLEANLY */}
                                            {(() => {
                                                const taskTabs = [
                            ...
                            {/* 🚀 PRACTICAL CHECKLIST (ACTIVE INTERACTIVE GRADING) 🚀 */}
                            {block.type === 'checklist' && (
                                <div style={{ marginTop: '1rem' }}>
                                    {block.criteria?.map((crit: string, i: number) => {
                                        ...
                                        return (
                                            <div key={i} style={{ marginBottom: '1.5rem', padding: '1.25rem', border: '1px solid #cbd5e1', borderRadius: '8px', background: isActiveRole ? 'white' : '#f8fafc' }}>
                                                ...
                                                {/* 🚀 LEARNER EVIDENCE VIEWER (If any evidence uploaded) 🚀 */}
                                                ...
                                                {block.requirePerCriterionTiming !== false && (
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '15px', background: '#f1f5f9', padding: '8px 12px', borderRadius: '6px', border: '1px solid #e2e8f0' }}>
                                                        ...
                                                        {isActiveRole && !isPrintMode ? (
                                                            ...
                                                            // Read-only timer display (PRINT SAFE)
                                                            <div style={{ display: 'flex', gap: '15px', fontSize: '0.75rem', color: '#334155' }}>
                                    ...
                                    {/* GLOBAL CHECKLIST DECLARATION (PRINT SAFE) */}
                            ...
                            {/* ── ACTIVE OVERALL GRADING INPUTS OR PRINT VIEWS (For all graded blocks) ── */}
                            {(maxM > 0 && block.type !== 'checklist') && (
                                <div className="sr-grade-box" style={{ borderLeft: `4px solid ${activeInkColor}`, marginTop: '1rem', padding: '1rem', background: 'white', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', borderRadius: '6px' }}>
                                    ...
                                    {(!isPrintMode && isActiveRole) ? (
                                        ...
                                        /* ── READ-ONLY BLOCK FEEDBACK VIEWERS (SCREEN OR PRINT) ── */
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                            {renderFacReadOnly && (
                                                <div style={{ background: '#eff6ff', padding: '0.75rem', borderRadius: '4px' }}>
                                                    ...
                                                    <div style={{ color: '#0369a1', fontSize: '0.85rem', whiteSpace: 'pre-wrap', wordBreak: 'break-word', overflowWrap: 'anywhere' }}><span style={{ fontWeight: 'bold', marginRight: '6px' }}>[{fData.score ?? 0}/{maxM}]</span> {fData.feedback || <em style={{ opacity: 0.7 }}>No specific coaching provided.</em>}</div>
                                                </div>
                                            )}
                                            {renderAssReadOnly && (
                                                <div style={{ background: '#fef2f2', padding: '0.75rem', borderRadius: '4px' }}>
                                                    ...
                                                    <div style={{ color: '#991b1b', fontSize: '0.85rem', whiteSpace: 'pre-wrap', wordBreak: 'break-word', overflowWrap: 'anywhere' }}><span style={{ fontWeight: 'bold', marginRight: '6px' }}>[{aData.score ?? 0}/{maxM}]</span> {aData.feedback || <em style={{ opacity: 0.7 }}>No specific feedback provided.</em>}</div>
                                                </div>
                                            )}
                                            {renderModReadOnly && (
                                                <div style={{ background: '#f0fdf4', padding: '0.75rem', borderRadius: '4px' }}>
                                                    ...
                                                    <div style={{ color: '#16a34a', fontSize: '0.85rem', whiteSpace: 'pre-wrap', wordBreak: 'break-word', overflowWrap: 'anywhere' }}><span style={{ fontWeight: 'bold', marginRight: '6px' }}>[{mData.score ?? 0}/{maxM}]</span> {mData.feedback || <em style={{ opacity: 0.7 }}>No QA feedback provided.</em>}</div>
                                                </div>
                                            )}
                                        </div>
                                    )}
                ...
                <aside className="sr-sidebar no-print">
                    ...
                    {showFacilitatorPanel && (
                        <div className="sr-summary-card" style={{ marginBottom: '1.5rem', borderTop: '4px solid blue' }}>
                            ...
                            {/* 🚀 AWAITING LEARNER SIGNOFF OVERRIDE 🚀 */}
                            {isAwaitingSignoff ? (
                                ...
                                <>
                                    ...
                                    {canFacilitatorMark ? (
                                        <button className="sr-submit-btn" style={{ background: 'blue' }} onClick={triggerSubmitFacilitator} disabled={saving}>
                                            {saving ? 'Processing...' : (hasChecklists && ['not_started', 'in_progress'].includes(currentStatus)) ? 'Save & Request Sign-off' : 'Send to Assessor'}
                                        </button>
                                    ) : (
