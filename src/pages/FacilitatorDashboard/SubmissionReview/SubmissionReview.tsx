...
const ReviewStageCard = ({
    ...
    timeOverrideValue, onTimeOverrideChange, autoTimeSeconds,
    autoSummedMinutes,
}: any) => {
                        ...
                        <div style={{ marginTop: '1rem', background: bgLight, padding: '12px', borderRadius: '6px', border: `1px solid ${colorHex}40` }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                                <input
                                    type="number"
                                    min="1"
                                    style={{ width: '90px', padding: '6px', borderRadius: '4px', border: '1px solid #cbd5e1', fontSize: '0.9rem', background: autoSummedMinutes > 0 && timeOverrideValue === '' ? '#e0f2fe' : 'white' }}
                                    value={timeOverrideValue}
                                    onChange={(e) => onTimeOverrideChange(e.target.value)}
                                    placeholder={autoSummedMinutes > 0 ? autoSummedMinutes.toString() : Math.max(1, Math.floor(autoTimeSeconds / 60)).toString()}
                                />
                                <span style={{ fontSize: '0.8rem', color: '#64748b', lineHeight: '1.4', display: 'inline-block' }}>
                                    {autoSummedMinutes > 0 ? (
                                        <><span style={{ color: '#0284c7', fontWeight: 'bold' }}>Auto-calculated from executed learner tasks: {autoSummedMinutes} mins.</span><br /></>
                                    ) : (
                                        <>Active time spent grading on this screen: <strong>{Math.floor(autoTimeSeconds / 60)} mins</strong>.<br /></>
                                    )}
                                    <em style={{ fontSize: '0.7rem' }}>(Edit this box if you need to log offline review time)</em>
                                </span>
                            </div>

                            {/* ─── QCTO COMPLIANCE INFO LINK ─── */}
                            <details style={{ borderTop: '1px dashed rgba(0,0,0,0.1)', paddingTop: '8px', marginTop: '8px' }}>
                                <summary style={{ fontSize: '0.75rem', fontWeight: 600, color: colorHexDark, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', userSelect: 'none' }}>
                                    <Info size={12} style={{ color: colorHex }} /> Why is this time tracked? (QCTO Compliance)
                                </summary>
                                <div style={{ fontSize: '0.72rem', color: '#475569', lineHeight: '1.4', marginTop: '6px', paddingLeft: '16px' }}>
                                    <p style={{ margin: '0 0 6px 0' }}>
                                        <strong>Audit Trail Evidence:</strong> Under QCTO and SETA regulations, all formative and summative interventions require verifiable duration records to validate that true evaluation took place.
                                    </p>
                                    <p style={{ margin: '0 0 4px 0' }}>
                                        This log prevents "click-through" grading fraud by tracking active assessment interaction time, ensuring compliance during external verification and site audits.
                                    </p>
                                </div>
                            </details>
                        </div>
...
export const SubmissionReview: React.FC = () => {
    ...
    // Live tick to update the background timer in the UI
    ...
    // ─── 🚀 PERSISTENT GROUP OBSERVATION STATES ───
    ...
    // ─── LOCAL STORAGE CACHE (Disaster Recovery for Group Sessions) ────────
    ...
    // ─── GROUP OBSERVATION ENGINE METRICS ─────────────────────────────────
    ...
    const handleOpenGroupMode = async () => {
        ...
        try {
            ...
            // 🚀 Ensure current learner is always initialized if session is fresh
    ...
    const handleSaveGroupMatrix = async () => {
        ...
        try {
            ...
            // Safely synchronize local UI context states for the current active workspace profile
            ...
            // 🚀 SUCCESS: Clear cache and close the session cleanly
            setSelectedGroupPeers([]);
            setGroupMatrixGrades({});
            setGroupTimeMatrix({});
            setGroupRemarks('');
    ...
    // ─── STAGE FEEDBACK CHANGE HANDLERS ──────────────────────────────────────
    ...
    // ─── AUTO-CALCULATE DURATION VALUE FOR BANNER ───
    ...
    // Auto-Sum Engine for Checklists (Aggregates task time tracking arrays)
    ...
    //  Uses the task tracker summation as the structural baseline review duration
        ...
        <div className="sr-root animate-fade-in" style={{ width: '100%', position: 'absolute', top: 0, right: 0, left: 0, bottom: 0 }}>
            ...
            <div className="sr-layout no-print">
                <div className="sr-content-pane">
                    ...
                    {/* ─── Learner Execution Task Timer Banner ─── */}
                    {/* {submission?.startedAt && (
                        <div style={{ background: '#f8fafc', border: '1px solid var(--mlab-border)', padding: '1rem', borderRadius: '8px', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                <Clock className="text-gray-400" size={20} />
                                <div>
                                    <h4 style={{ margin: '0 0 2px 0', fontSize: '0.85rem', color: 'var(--mlab-blue)', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Learner Evidence Submitted</h4>
                                    <div style={{ display: 'flex', gap: '16px', fontSize: '0.8rem', color: 'var(--mlab-grey)' }}>
                                        <span><strong>Start:</strong> {moment(submission.startedAt).format('HH:mm:ss')}</span>
                                        <span><strong>End:</strong> {submission.submittedAt ? moment(submission.submittedAt).format('HH:mm:ss') : 'In Progress'}</span>
                                        <span><strong>Duration:</strong> {computedLearnerDurationText}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {canFacilitatorMark && groupCriteriaList.length > 0 && (
                        <div className="no-print" style={{ marginBottom: '1.2rem', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '15px' }}>
                            ...
                            <button
                                ...
                                onClick={handleOpenGroupMode}
                                disabled={isFetchingPeers || saving}
                ...
                <aside className="sr-sidebar no-print">
                    ...
                    <ReviewStageCard
                        ...
                        // Option B: Manual Timer Inputs for Staff Phase
                        timeOverrideValue={facTimeOverride}
                        onTimeOverrideChange={setFacTimeOverride}
                        autoTimeSeconds={getFacTime()}
                        autoSummedMinutes={autoSummedTaskMinutes}
                            ...
                            <>
                                ...
                                {((submission.status === 'in_progress' && assessment?.moduleInfo?.timeLimit > 0) || ['not_started', 'submitted', 'facilitator_reviewed', 'awaiting_learner_signoff'].includes(submission.status)) && (
                                    <div className="no-print" style={{ marginTop: '10px', background: '#eff6ff', padding: '12px', borderRadius: '8px', border: '1px solid #bfdbfe' }}>
                                        ...
                                        {/* 🚀 NEW: Learner Task Timer Override Input Field */}
                                        {submission.status !== 'not_started' && (
                                            <div style={{ marginBottom: '12px', borderBottom: '1px dashed #bfdbfe', paddingBottom: '10px' }}>
                                                <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#1e40af', display: 'block', marginBottom: '4px', textTransform: 'uppercase' }}>
                                                    Learner Task Timer Override (Mins)
                                                </label>
                                                <input
                                                    type="number"
                                                    min="1"
                                                    placeholder="Enter true duration in minutes..."
                                                    style={{ width: '100%', padding: '6px', borderRadius: '4px', border: '1px solid #cbd5e1', fontSize: '0.85rem' }}
                                                    value={learnerTimeOverride}
                                                    onChange={(e) => handleLearnerTimeOverrideChange(e.target.value)}
                                                    disabled={saving}
                                                />
                                            </div>
                                        )}
                    ...
                    <ReviewStageCard
                        ...
                        timeOverrideValue={assTimeOverride}
                        onTimeOverrideChange={setAssTimeOverride}
                        autoTimeSeconds={getAssTime()}
                    ...
                    <ReviewStageCard
                        ...
                        timeOverrideValue={modTimeOverride}
                        onTimeOverrideChange={setModTimeOverride}
                        autoTimeSeconds={getModTime()}
...
//     signatureLabel, signatureUrl, signatureName, signatureDate, signatureMeta, signatureTagline,
//     onSubmit, submitLabel, isSaving,
//     // New Props for Option B: Manual Time Override
//     timeOverrideValue, onTimeOverrideChange, autoTimeSeconds
// }: any) => {
//     const colorHex = colorTheme === 'blue' ? 'blue' : colorTheme === 'red' ? 'red' : '#22c55e';
//     const colorHexDark = colorTheme === 'blue' ? '#1e3a8a' : colorTheme === 'red' ? '#7f1d1d' : '#14532d';
...
//                                     type="number"
//                                     min="1"
//                                     style={{ width: '90px', padding: '6px', borderRadius: '4px', border: '1px solid #cbd5e1', fontSize: '0.9rem' }}
//                                     value={timeOverrideValue}
//                                     onChange={(e) => onTimeOverrideChange(e.target.value)}
//                                     placeholder={Math.max(1, Math.floor(autoTimeSeconds / 60)).toString()}
//                                 />
//                                 <span style={{ fontSize: '0.8rem', color: '#64748b' }}>
//                                     System tracked: <strong>{Math.floor(autoTimeSeconds / 60)} mins</strong>. <br />
//                                     <em style={{ fontSize: '0.7rem' }}>(Edit this box if you observed offline)</em>
//                                 </span>
//                             </div>
//                         </div>
//                     )}
...
//     useEffect(() => {
//         const interval = setInterval(() => setLiveTick(p => p + 1), 30000); // update every 30s
//         return () => clearInterval(interval);
//     }, []);
...
//     // ─── TIME TRACKING & OVERRIDES ──────────────────────────────────────────
//     // Get live background tracked time
//     const getFacTime = () => initialFacTimeRef.current + (canFacilitatorMark ? Math.floor((performance.now() - sessionStartRef.current) / 1000) : 0);
//     const getAssTime = () => initialAssTimeRef.current + (canGrade ? Math.floor((performance.now() - sessionStartRef.current) / 1000) : 0);
//     const getModTime = () => initialModTimeRef.current + (canModerate ? Math.floor((performance.now() - sessionStartRef.current) / 1000) : 0);

//     // Resolve final time to save (uses manual override if provided, otherwise background tracker)
//     const resolveFacTime = () => facTimeOverride !== '' ? Number(facTimeOverride) * 60 : getFacTime();
//     const resolveAssTime = () => assTimeOverride !== '' ? Number(assTimeOverride) * 60 : getAssTime();
//     const resolveModTime = () => modTimeOverride !== '' ? Number(modTimeOverride) * 60 : getModTime();
...
//                 // Track time using manual overrides if provided
//                 if (canFacilitatorMark) updatePayload['grading.facilitatorTimeSpent'] = resolveFacTime();
//                 if (canGrade) updatePayload['grading.assessorTimeSpent'] = resolveAssTime();
//                 if (canModerate) updatePayload['moderation.timeSpent'] = resolveModTime();
...
//                         'grading.facilitatorSignatureUrl': user?.signatureUrl,
//                         'grading.facilitatorReviewedAt': new Date().toISOString(),
//                         'grading.facilitatorTimeSpent': resolveFacTime() // Use override if supplied
//                     });
//                     if (newStatus === 'awaiting_learner_signoff') {
//                         toast.success(isWorkplaceModule ? "Verification saved! Learner has been requested to sign the logbook." : "Observation saved! Learner has been requested to sign off.");
//                     } else {
...
//                         'grading.assessorRegNumber': user?.assessorRegNumber || 'Pending Reg',
//                         'grading.gradedAt': new Date().toISOString(),
//                         'grading.assessorTimeSpent': resolveAssTime() // Use override if supplied
//                     });
//                     toast.success("Workbook graded and signed successfully!"); setTimeout(() => window.location.reload(), 500);
//                 } catch (error) { toast.error("Failed to save grades."); } finally { setSaving(false); }
//             }, onCancel: () => setModalConfig(null)
...
//                         'moderation.moderatorRegNumber': user?.assessorRegNumber || 'Pending Reg',
//                         'moderation.moderatedAt': new Date().toISOString(),
//                         'moderation.timeSpent': resolveModTime() // Use override if supplied
//                     });
//                     toast.success("Moderation saved successfully!"); setTimeout(() => navigate(-1), 1000);
//                 } catch (error) { toast.error("Failed to save moderation."); } finally { setSaving(false); }
//             }, onCancel: () => setModalConfig(null)
