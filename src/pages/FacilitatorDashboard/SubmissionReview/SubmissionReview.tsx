...
//     const hasWorkplace = assessment?.blocks?.some((b: any) => b.type === 'qcto_workplace');
//     const hasLogbook = assessment?.blocks?.some((b: any) => b.type === 'logbook');
//     // QCTO WE modules are competency-based only — no numerical scores apply
//     const hasKnowledgeBlocks = assessment?.blocks?.some((b: any) => ['mcq', 'text', 'task'].includes(b.type));
//     const isWorkplaceModule = (hasWorkplace || hasLogbook) && !hasKnowledgeBlocks;
...
//     // Mentor-specific terminology helpers
//     const facRoleLabel = isMentor ? 'Workplace Verification' : 'Pre-Marking';
//     const facPanelTitle = isMentor ? 'Workplace Mentor Verification' : `${user?.role || 'Facilitator'} Pre-Marking`;
//     const facRemarksLabel = isMentor ? 'Supervisor Comments' : `${user?.role || 'Facilitator'} Remarks`;
//     const facSubmitLabel = isMentor ? 'Verify & Send to Assessor' : 'Send to Assessor';
//     // Computed at render time from saved data so assessors see the correct label regardless of their own role
//     const savedFacRole = submission?.grading?.facilitatorRole;
//     const facReadOnlyLabel = savedFacRole === 'mentor' ? 'Workplace Mentor Verification' : 'Facilitator Pre-Mark';
...
//     // Mentor can return a submitted logbook to learner before verifying
//     const canReturnToLearner = isMentor && ['submitted', 'in_progress'].includes(currentStatus);
...
//         crits[index] = { ...crits[index], [field]: value };
//         active.criteriaResults = crits;
//         // Auto-derive isCorrect: true if all criteria are C, false if any are NYC, null if incomplete
//         const block = assessment?.blocks?.find((b: any) => b.id === blockId);
//         const total = block?.criteria?.length || 0;
//         if (total > 0 && crits.length >= total && crits.slice(0, total).every((c: any) => c?.status)) {
//             active.isCorrect = crits.slice(0, total).every((c: any) => c?.status === 'C');
...
//         activities[index].status = status;
//         active.activityResults = activities;
//         // Auto-derive isCorrect: true if all activities are C, false if any are NYC, null if incomplete
//         const block = assessment?.blocks?.find((b: any) => b.id === blockId);
//         const total = block?.workActivities?.length || 0;
//         if (total > 0 && activities.length >= total && activities.slice(0, total).every((a: any) => a?.status)) {
//             active.isCorrect = activities.slice(0, total).every((a: any) => a?.status === 'C');
...
//     // ─── VALIDATION ───────────────────────────────────────────────────────────
//     // For mentors: only require the global obsDeclaration checkbox on checklist/workplace blocks.
//     // Mentors do NOT mark scores, so we skip isCorrect checks for them.
//     // Returns null if valid, or a string describing what is still incomplete.
//     const validateMentorVerification = (): string | null => {
//         if (!assessment?.blocks) return null;
...
//                 // Every criterion must have Observed or Not Observed selected
//                 for (let i = 0; i < criteria.length; i++) {
//                     if (!results[i]?.status) {
//                         return `Please select "Observed ✓" or "Not Observed ✗" for every criterion in the checklist "${block.title}". Item ${i + 1} is still unverified.`;
//                     }
...
//                 // Declaration checkbox must be ticked
//                 if (block.requireObservationDeclaration !== false && !grade?.obsDeclaration) {
//                     return `Please tick the verification declaration at the bottom of the checklist "${block.title}" before submitting.`;
//                 }
//             }
...
//                 // Every work activity must have Verified or Not Verified selected
//                 for (let i = 0; i < activities.length; i++) {
//                     if (!results[i]?.status) {
//                         return `Please select "Verified ✓" or "Not Verified ✗" for every Work Activity in "${block.weCode} – ${block.weTitle}". Activity ${i + 1} (${activities[i]?.code || `#${i + 1}`}) is still unverified.`;
//                     }
...
//             }
//             if (block.type === 'checklist') {
//                 // Declaration: assessor is exempt when mentor already declared (mentor owns the observation)
//                 const declarationRequired = !isModerating && block.requireObservationDeclaration
//                     && !grade?.obsDeclaration
//                     && !(isAssessorGrading && savedFacRole === 'mentor');
//                 if (declarationRequired) return true;
...
//                     if (!crits[i] || !crits[i].status) return true;
//                 }
//                 // isCorrect is auto-derived from statuses — re-derive here as fallback for validation
//                 const allHaveStatus = crits.slice(0, total).every((c: any) => c?.status);
//                 if (!allHaveStatus) return true;
//                 return false;
//             }
//             if (block.type === 'qcto_workplace') {
//                 // Declaration: assessor is exempt when mentor already declared
//                 const declarationRequired = !isModerating && block.requireObservationDeclaration
//                     && !grade?.obsDeclaration
//                     && !(isAssessorGrading && savedFacRole === 'mentor');
//                 if (declarationRequired) return true;
...
//                     if (!activities[i] || !activities[i].status) return true;
//                 }
//                 // isCorrect is auto-derived from statuses — re-derive here as fallback
//                 const allHaveStatus = activities.slice(0, total).every((a: any) => a?.status);
//                 if (!allHaveStatus) return true;
//                 return false;
//             }
...
//     // ─── SUBMIT HANDLERS ──────────────────────────────────────────────────────
//     const triggerSubmitFacilitator = () => {
//         // Mentor validation: every criterion/activity must be Observed/Verified, plus declaration + comments
//         if (isMentor) {
//             const mentorValidationError = validateMentorVerification();
//             if (mentorValidationError) {
//                 return setModalConfig({
...
//         let newStatus = 'facilitator_reviewed';
//         let confirmTitle = isMentor ? 'Verify & Submit Logbook?' : 'Complete Pre-Marking?';
//         let confirmMessage = isMentor
//             ? 'This will record your verification and send the logbook to the Assessor for formal evaluation.'
//             : 'This will finalise your feedback and send the workbook to the Assessor.';
//         let confirmBtnText = isMentor ? 'Verify & Send to Assessor' : 'Send to Assessor';

//         if (['not_started', 'in_progress'].includes(currentStatus)) {
//             if (hasChecklists || hasWorkplace) {
//                 newStatus = 'awaiting_learner_signoff';
//                 confirmTitle = isMentor ? 'Request Learner Sign-off?' : 'Request Learner Sign-off?';
//                 confirmMessage = isMentor
//                     ? 'This will save your verification and prompt the learner to log in, review your supervisor comments, and apply their signature before the logbook goes to the Assessor.'
//                     : 'This will save your practical observation and prompt the learner to log in, review your feedback, and officially sign off before it goes to the Assessor.';
//                 confirmBtnText = isMentor ? 'Save & Request Learner Signature' : 'Save & Request Sign-off';
//             } else {
//                 confirmTitle = isMentor ? 'Force Verify?' : 'Force Submission?';
//                 confirmMessage = isMentor
//                     ? 'The learner has not formally submitted yet. Verifying now will lock the logbook for the learner and pass it directly to the Assessor. Proceed?'
//                     : 'The learner has not submitted this workbook yet. Finalising your marking now will lock the workbook and pass it to the Assessor. Proceed?';
//                 confirmBtnText = isMentor ? 'Verify & Force Submit' : 'Force Submit & Send';
//             }
//         }
...
//                     });
//                     if (newStatus === 'awaiting_learner_signoff') {
//                         toast.success(isMentor ? "Verification saved! Learner has been requested to sign the logbook." : "Observation saved! Learner has been requested to sign off.");
//                     } else {
//                         toast.success(isMentor ? "Logbook verified and passed to Assessor!" : "Script marked and passed to Assessor!");
//                     }
//                     setTimeout(() => navigate(-1), 2000);
//                 } catch (error) { toast.error("Failed to save."); } finally { setSaving(false); }
//             }, onCancel: () => setModalConfig(null)
...
//     const triggerSubmitModeration = () => {
//         if (!modOutcome) return setModalConfig({ isOpen: true, type: 'warning', title: 'Missing Decision', message: 'You must select Endorse or Return to Assessor before submitting.', confirmText: 'Got it', onConfirm: () => setModalConfig(null) });
//         // When returning, QA notes are mandatory so the assessor knows what to fix
//         if (modOutcome === 'Returned' && !modFeedback.trim()) return setModalConfig({ isOpen: true, type: 'warning', title: 'Green Pen Instructions Required', message: 'You must provide Green Pen instructions explaining what the Assessor needs to fix before this can be returned.', confirmText: 'Got it', onConfirm: () => setModalConfig(null) });
//         // For WE modules skip the validateAllMarked score check — moderators only leave comments
//         if (!isWorkplaceModule && !validateAllMarked(modBreakdown, true)) return setModalConfig({ isOpen: true, type: 'warning', title: 'Incomplete QA', message: 'You must verify every question and fully check all blocks before endorsing.', confirmText: 'Got it', onConfirm: () => setModalConfig(null) });
...
//                 // Determine if this is an active mentor marking a scorable block
//                 // Mentors do NOT get score inputs — they only get declaration + comment fields
//                 const mentorActiveOnScorableBlock = isMentor && canFacilitatorMark && ['mcq', 'text', 'task'].includes(block.type);

//                 // ── GRADE BOX HELPER ──────────────────────────────────────────────
//                 // Returns read-only layers for non-active roles
//                 const renderReadOnlyLayers = () => (
//                     <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
//                         {renderFacReadOnly && (
//                             <div style={{ background: '#eff6ff', padding: '0.75rem', borderRadius: '4px' }}>
...
//                                 </div>
//                                 <div style={{ color: '#0369a1', fontSize: '0.85rem', whiteSpace: 'pre-wrap', wordBreak: 'break-word', overflowWrap: 'break-word' }}>
//                                     {/* Mentors don't produce scores — hide [0/X] prefix if facilitatorRole was mentor */}
//                                     {submission.grading?.facilitatorRole !== 'mentor' && (
//                                         <span style={{ fontWeight: 'bold', marginRight: '6px' }}>[{fData.score ?? 0}/{maxM}]</span>
//                                     )}
//                                     {fData.feedback || <em style={{ opacity: 0.7 }}>No specific {submission.grading?.facilitatorRole === 'mentor' ? 'supervisor comments' : 'coaching'} provided.</em>}
...
//                 // For facilitators: full score + feedback
//                 // For mentors on scorable blocks: only a supervisor comment textarea (no score)
//                 const renderActiveGradeControls = (blockId: string) => {
//                     // Moderators on WE modules: comment only, no marks
//                     if (isModerator && isWorkplaceModule) {
//                         return (
//                             <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '6px', padding: '1rem', marginTop: '1rem' }}>
//                                 <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px', color: '#15803d', fontWeight: 'bold', fontSize: '0.8rem', textTransform: 'uppercase' }}>
...
//                     if (mentorActiveOnScorableBlock) {
//                         // Mentor on MCQ/text/task: show read-only view of learner answer, and a supervisor comments box only
//                         return (
//                             <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '6px', padding: '1rem', marginTop: '1rem' }}>
//                                 <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px', color: '#1d4ed8', fontWeight: 'bold', fontSize: '0.8rem', textTransform: 'uppercase' }}>
//                                     <ShieldCheck size={14} /> Supervisor Observation Comments
...
//                     // Standard facilitator grade controls
//                     return (
//                         <>
//                             <div className="sr-score-input-wrap" style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
//                                 <label style={{ color: activeInkColor, fontWeight: 'bold', fontSize: '0.85rem' }}>Total Marks Awarded for this block:</label>
...
//                                     <span className="sr-q-text" style={{ color: '#073f4e', fontWeight: 'bold' }}>{block.question}</span>
//                                 </div>
//                                 {/* Mentors don't get tick/cross marking buttons */}
//                                 {!isPrintMode && isActiveRole && !isMentor && !(isModerator && isWorkplaceModule) && (
//                                     <div className="sr-visual-mark">
//                                         <button onClick={() => handleVisualMark(block.id, true, maxM)} className="sr-mark-btn" style={activeData.isCorrect === true ? { color: activeInkColor, border: `1px solid ${activeInkColor}`, background: 'white' } : {}} title="Mark Correct"><Check size={20} /></button>
//                                         <button onClick={() => handleVisualMark(block.id, false, maxM)} className="sr-mark-btn" style={activeData.isCorrect === false ? { color: activeInkColor, border: `1px solid ${activeInkColor}`, background: 'white' } : {}} title="Mark Incorrect"><X size={20} /></button>
...
//                                                                 <td style={{ padding: '10px', color: '#0f172a', fontWeight: 'bold' }}>{entry.date}</td>
//                                                                 <td style={{ padding: '10px', color: '#334155' }}>{entry.task}</td>
//                                                                 <td style={{ padding: '10px', color: '#475569' }}>{entry.startTime}</td>
//                                                                 <td style={{ padding: '10px', color: '#475569' }}>{entry.endTime}</td>
//                                                                 <td style={{ padding: '10px', color: '#0f172a', fontWeight: 'bold' }}>{entry.hours}</td>
//                                                             </tr>
...
//                                             {/* Hours tally — shown to all roles, prominently for mentor */}
//                                             <LogbookHoursTally entries={entries} requiredHours={block.requiredHours} />

//                                             {/* Mentor: supervisor comment box for the logbook */}
//                                             {isMentor && canFacilitatorMark && (
//                                                 <div style={{ marginTop: '1rem', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '6px', padding: '1rem' }}>
//                                                     <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px', color: '#1d4ed8', fontWeight: 'bold', fontSize: '0.8rem', textTransform: 'uppercase' }}>
//                                                         <ShieldCheck size={14} /> Supervisor Comments on Logbook
...
//                                 <div style={{ marginTop: '1rem' }}>
//                                     {block.criteria?.map((crit: string, i: number) => {
//                                         const critResult = activeData.criteriaResults?.[i] || { status: null, comment: '', startTime: '', endTime: '' };
//                                         let durationStr = '';
//                                         if (critResult.startTime && critResult.endTime) {
//                                             const diffMs = new Date(critResult.endTime).getTime() - new Date(critResult.startTime).getTime();
//                                             const m = Math.floor(diffMs / 60000);
//                                             const s = Math.floor((diffMs % 60000) / 1000);
//                                             durationStr = `${m}m ${s}s`;
//                                         }
...
//                                                         <span style={{ fontSize: '0.8rem', fontWeight: 'bold', color: '#475569', minWidth: '80px' }}>Task Timer:</span>
//                                                         {isActiveRole && !isPrintMode ? (
//                                                             !critResult.startTime ? (
//                                                                 <button onClick={() => handleCriterionChange(block.id, i, 'startTime', new Date().toISOString())} className="ab-btn sm" style={{ background: '#10b981', color: 'white', border: 'none', padding: '4px 12px', borderRadius: '4px', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '4px' }}><Play size={12} /> Start</button>
//                                                             ) : !critResult.endTime ? (
//                                                                 <>
//                                                                     <span style={{ fontSize: '0.75rem', color: '#0ea5e9', fontWeight: 'bold', fontStyle: 'italic' }}>In progress since {new Date(critResult.startTime).toLocaleTimeString()}...</span>
//                                                                     <button onClick={() => handleCriterionChange(block.id, i, 'endTime', new Date().toISOString())} className="ab-btn sm" style={{ background: '#ef4444', color: 'white', border: 'none', padding: '4px 12px', borderRadius: '4px', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '4px', marginLeft: 'auto' }}><Square size={12} /> Stop</button>
//                                                                 </>
//                                                             ) : (
//                                                                 <div style={{ display: 'flex', gap: '15px', fontSize: '0.75rem', color: '#334155' }}>
//                                                                     <span><strong>Start:</strong> {new Date(critResult.startTime).toLocaleTimeString()}</span>
//                                                                     <span><strong>End:</strong> {new Date(critResult.endTime).toLocaleTimeString()}</span>
//                                                                     <span style={{ color: '#0ea5e9', fontWeight: 'bold', background: '#e0f2fe', padding: '2px 6px', borderRadius: '4px' }}>Duration: {durationStr}</span>
//                                                                 </div>
//                                                             )
//                                                         ) : (
//                                                             <div style={{ display: 'flex', gap: '15px', fontSize: '0.75rem', color: '#334155' }}>
//                                                                 <span><strong>Start:</strong> {critResult.startTime ? new Date(critResult.startTime).toLocaleTimeString() : '—'}</span>
//                                                                 <span><strong>End:</strong> {critResult.endTime ? new Date(critResult.endTime).toLocaleTimeString() : '—'}</span>
//                                                                 <span style={{ color: '#0ea5e9', fontWeight: 'bold', background: '#e0f2fe', padding: '2px 6px', borderRadius: '4px' }}>Duration: {durationStr || '—'}</span>
//                                                             </div>
//                                                         )}
//                                                     </div>
...
//                                                 {/* Criterion outcome — render varies by role */}
//                                                 {/* When assessor is grading, show mentor's observation read-only above, then assessor's own row */}
//                                                 {canGrade && isFacDone && savedFacRole === 'mentor' && (() => {
//                                                     const mentorCritResult = fData.criteriaResults?.[i] || { status: null, comment: '' };
//                                                     return (
//                                                         <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '6px', padding: '10px 12px', marginBottom: '10px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
//                                                             <span style={{ fontSize: '0.7rem', fontWeight: 'bold', color: '#1d4ed8', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '5px' }}>
//                                                                 <ShieldCheck size={12} /> Workplace Mentor Observation
//                                                             </span>
//                                                             <span style={{ fontSize: '0.85rem', fontWeight: 'bold', color: mentorCritResult.status === 'C' ? '#166534' : mentorCritResult.status === 'NYC' ? '#991b1b' : '#64748b' }}>
//                                                                 {mentorCritResult.status === 'C' ? 'Observed ✓' : mentorCritResult.status === 'NYC' ? 'Not Observed ✗' : 'Not Reviewed'}
//                                                             </span>
//                                                             {mentorCritResult.comment && (
//                                                                 <span style={{ fontSize: '0.82rem', color: '#1e40af', fontStyle: 'italic' }}>{mentorCritResult.comment}</span>
//                                                             )}
//                                                         </div>
//                                                     );
//                                                 })()}
...
//                                                     {isPrintMode ? (
//                                                         <div style={{ fontSize: '0.85rem' }}>
//                                                             <span style={{ fontWeight: 'bold', color: critResult.status === 'C' ? '#166534' : critResult.status === 'NYC' ? '#991b1b' : '#64748b' }}>
//                                                                 {isMentor ? (critResult.status === 'C' ? 'Observed ✓' : critResult.status === 'NYC' ? 'Not Observed ✗' : 'Not Verified') : (critResult.status === 'C' ? 'Competent (C)' : critResult.status === 'NYC' ? 'Not Yet Competent (NYC)' : 'Not Graded')}
//                                                             </span>
//                                                         </div>
//                                                     ) : (
//                                                         <div style={{ display: 'flex', gap: '10px' }}>
//                                                             <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85rem', background: critResult.status === 'C' ? '#dcfce7' : '#f8fafc', padding: '8px 12px', borderRadius: '4px', border: critResult.status === 'C' ? '2px solid #22c55e' : '1px solid #cbd5e1', color: critResult.status === 'C' ? '#166534' : '#64748b', fontWeight: 'bold', cursor: isActiveRole ? 'pointer' : 'not-allowed' }}>
//                                                                 <input type="radio" disabled={!isActiveRole} checked={critResult.status === 'C'} onChange={() => handleCriterionChange(block.id, i, 'status', 'C')} style={{ accentColor: '#22c55e' }} />
//                                                                 {isMentor ? 'Observed ✓' : 'Competent (C)'}
//                                                             </label>
//                                                             <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85rem', background: critResult.status === 'NYC' ? '#fee2e2' : '#f8fafc', padding: '8px 12px', borderRadius: '4px', border: critResult.status === 'NYC' ? '2px solid #ef4444' : '1px solid #cbd5e1', color: critResult.status === 'NYC' ? '#991b1b' : '#64748b', fontWeight: 'bold', cursor: isActiveRole ? 'pointer' : 'not-allowed' }}>
//                                                                 <input type="radio" disabled={!isActiveRole} checked={critResult.status === 'NYC'} onChange={() => handleCriterionChange(block.id, i, 'status', 'NYC')} style={{ accentColor: '#ef4444' }} />
//                                                                 {isMentor ? 'Not Observed ✗' : 'NYC'}
//                                                             </label>
//                                                         </div>
//                                                     )}
...
//                                                         {isPrintMode ? (
//                                                             <div style={{ fontSize: '0.85rem', whiteSpace: 'pre-wrap', wordBreak: 'break-word', color: '#334155', padding: '8px', background: '#f1f5f9', borderRadius: '4px', border: '1px solid #e2e8f0' }}>
//                                                                 {critResult.comment || <em style={{ opacity: 0.5 }}>No comments provided.</em>}
//                                                             </div>
//                                                         ) : (
//                                                             <textarea
//                                                                 className="ab-input"
//                                                                 disabled={!isActiveRole || (canGrade && savedFacRole === 'mentor')}
//                                                                 rows={2}
//                                                                 placeholder={isMentor ? "Supervisor observation notes..." : (canGrade && savedFacRole === 'mentor') ? "Read-only — Mentor's observation recorded above" : "Assessor comments / reasoning..."}
//                                                                 value={(canGrade && savedFacRole === 'mentor') ? '' : critResult.comment}
//                                                                 onChange={e => handleCriterionChange(block.id, i, 'comment', e.target.value)}
//                                                                 style={{ fontSize: '0.85rem', width: '100%', border: '1px solid #e2e8f0', resize: 'vertical', background: (canGrade && savedFacRole === 'mentor') ? '#f8fafc' : 'white', color: '#94a3b8' }}
//                                                             />
//                                                         )}
//                                                     </div>
//                                                 </div>
...
//                                     {/* Global sign-off — hidden entirely from moderator; Mentors see "Verification Declaration", facilitators/assessors see "Global Assessor Sign-off" */}
//                                     {block.requireObservationDeclaration !== false && !isModerator && (
//                                         <div style={{ marginTop: '2rem', padding: '1.5rem', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px', borderLeft: `4px solid ${activeInkColor}` }}>
//                                             <h4 style={{ fontSize: '0.9rem', color: '#0f172a', margin: '0 0 15px 0', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '6px' }}>
//                                                 <ShieldCheck size={16} color={activeInkColor} />
//                                                 {isMentor ? 'Mentor Verification Declaration' : (activeInkColor === 'green' ? 'Observation Log Details' : 'Global Assessor Sign-off')}
//                                             </h4>

//                                             {block.requireTimeTracking !== false && (
//                                                 <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '15px', marginBottom: '15px' }}>
//                                                     {['obsDate', 'obsStartTime', 'obsEndTime'].map((field, fi) => {
//                                                         const labels = ['Date of Observation', 'Session Start Time', 'Session End Time'];
//                                                         const types = ['date', 'time', 'time'];
//                                                         const vals = [activeData.obsDate, activeData.obsStartTime, activeData.obsEndTime];
//                                                         const fKey = field as 'obsDate' | 'obsStartTime' | 'obsEndTime';
//                                                         return (
//                                                             <div key={field}>
//                                                                 <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#475569', marginBottom: '4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
//                                                                     {labels[fi]}
//                                                                     {isActiveRole && !isPrintMode && <button className="ab-text-btn" style={{ fontSize: '0.65rem', padding: 0 }} onClick={() => handleSetToNow(block.id, fKey)}>Set Now</button>}
//                                                                 </label>
//                                                                 {isPrintMode
//                                                                     ? <div style={{ fontWeight: 'bold', fontSize: '0.9rem', color: '#0f172a' }}>{field === 'obsDate' ? (activeData.obsDate ? moment(activeData.obsDate).format('DD/MM/YYYY') : '—') : (vals[fi] || '—')}</div>
//                                                                     : <input type={types[fi]} className="ab-input" disabled={!isActiveRole} value={vals[fi] || ''} onChange={e => handleGlobalChecklistChange(block.id, fKey, e.target.value)} />
//                                                                 }
//                                                             </div>
//                                                         );
//                                                     })}
...
//                                             {!isModerator && (
//                                                 <label style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '0.85rem', color: '#0f172a', fontWeight: 'bold', background: activeData.obsDeclaration ? '#eff6ff' : 'white', padding: '10px', borderRadius: '6px', border: '1px solid #bfdbfe', cursor: isActiveRole && !isPrintMode ? 'pointer' : 'default' }}>
//                                                     <input type="checkbox" disabled={!isActiveRole || isPrintMode} checked={activeData.obsDeclaration || false} onChange={e => handleGlobalChecklistChange(block.id, 'obsDeclaration', e.target.checked)} style={{ width: '18px', height: '18px', accentColor: activeInkColor }} />
//                                                     {isMentor
//                                                         ? 'I confirm that I have directly observed this learner performing the tasks listed above at the workplace, and that the evidence submitted is authentic.'
//                                                         : 'I officially declare that I have observed the learner performing these tasks and that the evidence was submitted by the learner.'}
//                                                 </label>
//                                             )}
//                                         </div>
...
//                                     const task = learnerAns?.[taskKey] || '';
//                                     const date = learnerAns?.[dateKey] || '';
//                                     const actResult = activeData.activityResults?.[actIdx] || { status: null, comment: '' };
...
//                                                 <div style={{ marginBottom: '1.5rem' }}>
//                                                     <div style={{ display: 'flex', gap: '20px', marginBottom: '15px', flexWrap: 'wrap' }}>
//                                                         {task && <div style={{ flex: 1, minWidth: '250px' }}><strong style={{ fontSize: '0.75rem', color: '#64748b' }}>Task Performed:</strong><div className="quill-read-only-content" dangerouslySetInnerHTML={{ __html: task }} /></div>}
//                                                         {date && <div style={{ minWidth: '120px' }}><strong style={{ fontSize: '0.75rem', color: '#64748b' }}>Date:</strong><div>{new Date(date).toLocaleDateString()}</div></div>}
//                                                     </div>
//                                                     {activity.evidenceItems?.map((evItem: any) => {
//                                                         const seData = learnerAns?.[`se_${evItem.id}`] || {};
//                                                         return (
//                                                             <div key={evItem.id} style={{ marginBottom: '1.5rem' }}>
//                                                                 <span style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#e11d48' }}>{evItem.code}</span>
//                                                                 <p style={{ margin: '4px 0 8px 0', fontSize: '0.8rem', color: '#334155' }}>{evItem.description}</p>
//                                                                 {seData.uploadUrl && <FilePreview url={seData.uploadUrl} />}
//                                                                 {seData.url && <UrlPreview url={seData.url} />}
//                                                                 {seData.code && <pre style={{ background: '#1e293b', color: '#f8fafc', padding: '0.5rem', borderRadius: '4px', overflowX: 'auto' }}><code>{seData.code}</code></pre>}
//                                                                 {seData.text && <div className="quill-read-only-content" dangerouslySetInnerHTML={{ __html: seData.text }} />}
//                                                             </div>
//                                                         );
//                                                     })}
...
//                                                 {/* Activity outcome — render varies by role */}
//                                                 {/* When assessor is grading, show mentor's verification read-only above, then assessor's own row */}
//                                                 <div style={{ borderTop: '1px dashed #cbd5e1', paddingTop: '1rem' }}>
//                                                     {canGrade && isFacDone && savedFacRole === 'mentor' && (() => {
//                                                         const mentorActResult = fData.activityResults?.[actIdx] || { status: null, comment: '' };
//                                                         return (
//                                                             <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '6px', padding: '10px 12px', marginBottom: '10px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
//                                                                 <span style={{ fontSize: '0.7rem', fontWeight: 'bold', color: '#1d4ed8', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '5px' }}>
//                                                                     <ShieldCheck size={12} /> Workplace Mentor Verification
//                                                                 </span>
//                                                                 <span style={{ fontSize: '0.85rem', fontWeight: 'bold', color: mentorActResult.status === 'C' ? '#166534' : mentorActResult.status === 'NYC' ? '#991b1b' : '#64748b' }}>
//                                                                     {mentorActResult.status === 'C' ? 'Verified ✓' : mentorActResult.status === 'NYC' ? 'Not Verified ✗' : 'Not Reviewed'}
//                                                                 </span>
//                                                                 {mentorActResult.comment && (
//                                                                     <span style={{ fontSize: '0.82rem', color: '#1e40af', fontStyle: 'italic' }}>{mentorActResult.comment}</span>
//                                                                 )}
//                                                             </div>
//                                                         );
//                                                     })()}
...
//                                                         {isPrintMode ? (
//                                                             <div style={{ fontSize: '0.85rem' }}>
//                                                                 <span style={{ fontWeight: 'bold', color: actResult.status === 'C' ? '#166534' : actResult.status === 'NYC' ? '#991b1b' : '#64748b' }}>
//                                                                     {isMentor
//                                                                         ? (actResult.status === 'C' ? 'Verified ✓' : actResult.status === 'NYC' ? 'Not Verified ✗' : 'Not Reviewed')
//                                                                         : (actResult.status === 'C' ? 'Competent' : actResult.status === 'NYC' ? 'Not Yet Competent' : 'Not Graded')}
//                                                                 </span>
//                                                             </div>
//                                                         ) : (
//                                                             <div style={{ display: 'flex', gap: '10px' }}>
//                                                                 <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85rem', background: actResult.status === 'C' ? '#dcfce7' : '#f8fafc', padding: '8px 12px', borderRadius: '4px', border: actResult.status === 'C' ? '2px solid #22c55e' : '1px solid #cbd5e1', color: actResult.status === 'C' ? '#166534' : '#64748b', fontWeight: 'bold', cursor: isActiveRole ? 'pointer' : 'not-allowed' }}>
//                                                                     <input type="radio"
//                                                                         // disabled={!isActiveRole} 
//                                                                         disabled={!canGrade || isModerator}
//                                                                         checked={actResult.status === 'C'} onChange={() => handleActivityStatusChange(block.id, actIdx, 'C')} style={{ accentColor: '#22c55e' }} />
//                                                                     {isMentor ? 'Verified ✓' : 'Competent (C)'}
//                                                                 </label>
//                                                                 <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85rem', background: actResult.status === 'NYC' ? '#fee2e2' : '#f8fafc', padding: '8px 12px', borderRadius: '4px', border: actResult.status === 'NYC' ? '2px solid #ef4444' : '1px solid #cbd5e1', color: actResult.status === 'NYC' ? '#991b1b' : '#64748b', fontWeight: 'bold', cursor: isActiveRole ? 'pointer' : 'not-allowed' }}>
//                                                                     <input type="radio"
//                                                                         disabled={!canGrade || isModerator}
//                                                                         checked={actResult.status === 'NYC'}
//                                                                         onChange={() => handleActivityStatusChange(block.id, actIdx, 'NYC')} style={{ accentColor: '#ef4444' }} />
//                                                                     {isMentor ? 'Not Verified ✗' : 'NYC'}
//                                                                 </label>
//                                                             </div>
//                                                         )}
...
//                                                             {isPrintMode ? (
//                                                                 <div style={{ fontSize: '0.85rem', whiteSpace: 'pre-wrap', color: '#334155', padding: '8px', background: '#f1f5f9', borderRadius: '4px' }}>
//                                                                     {actResult.comment || <em>No comments.</em>}
//                                                                 </div>
//                                                             ) : (
//                                                                 <textarea className="ab-input" disabled={!isActiveRole || (canGrade && savedFacRole === 'mentor')} rows={2} placeholder={isMentor ? "Supervisor verification notes..." : (canGrade && savedFacRole === 'mentor') ? "Read-only — Mentor's observation recorded above" : "Assessor comments / reasoning..."} value={(canGrade && savedFacRole === 'mentor') ? '' : actResult.comment} onChange={e => handleActivityCommentChange(block.id, actIdx, e.target.value)} style={{ fontSize: '0.85rem', width: '100%', border: '1px solid #e2e8f0', resize: 'vertical', background: (canGrade && savedFacRole === 'mentor') ? '#f8fafc' : 'white', color: '#94a3b8' }} />
//                                                             )}
//                                                         </div>
//                                                     </div>
//                                                 </div>
...
//                                 {block.requireSelfAssessment && learnerAns?.selfAssessmentDone && (
//                                     <div style={{ marginTop: '1rem', background: '#f8fafc', padding: '1rem', borderRadius: '6px' }}>
//                                         <strong style={{ color: '#e11d48' }}>Learner Self-Assessment Completed</strong>
//                                         <CheckCircle size={16} color="#22c55e" style={{ marginLeft: '8px' }} />
//                                     </div>
//                                 )}
...
//                                 {/* Global sign-off — hidden entirely from moderator */}
//                                 {block.requireObservationDeclaration !== false && !isModerator && (
//                                     <div style={{ marginTop: '2rem', padding: '1.5rem', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px', borderLeft: `4px solid ${activeInkColor}` }}>
//                                         <h4 style={{ fontSize: '0.9rem', color: '#0f172a', margin: '0 0 15px 0', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '6px' }}>
//                                             <ShieldCheck size={16} color={activeInkColor} />
...
//                                                             <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#475569', marginBottom: '4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
//                                                                 {labels[fi]}
//                                                                 {isActiveRole && !isPrintMode && <button className="ab-text-btn" style={{ fontSize: '0.65rem', padding: 0 }} onClick={() => handleSetToNow(block.id, field)}>Set Now</button>}
//                                                             </label>
//                                                             {isPrintMode
//                                                                 ? <div style={{ fontWeight: 'bold', fontSize: '0.9rem', color: '#0f172a' }}>{field === 'obsDate' ? (activeData.obsDate ? moment(activeData.obsDate).format('DD/MM/YYYY') : '—') : (vals[fi] || '—')}</div>
//                                                                 : <input type={types[fi]} className="ab-input" disabled={!isActiveRole} value={vals[fi] || ''} onChange={e => handleGlobalChecklistChange(block.id, field, e.target.value)} />
//                                                             }
//                                                         </div>
//                                                     );
//                                                 })}
...
//                                 {maxM > 0 && (
//                                     <div className="sr-grade-box" style={{ borderLeft: `4px solid ${activeInkColor}`, marginTop: '1rem', padding: '1rem', background: 'white', borderRadius: '6px' }}>
//                                         {/* Mentors don't award scores — show assessor/moderator layers only */}
//                                         {(!isPrintMode && isActiveRole && !isMentor && !(isModerator && isWorkplaceModule)) ? (
//                                             <>
//                                                 <div className="sr-score-input-wrap" style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
//                                                     <label style={{ color: activeInkColor, fontWeight: 'bold', fontSize: '0.85rem' }}>Total Marks Awarded for this block:</label>
...
//                                                 <div className="sr-feedback-wrap" style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
//                                                     <Edit3 size={16} color={activeInkColor} style={{ marginTop: '6px' }} />
//                                                     <textarea className="sr-feedback-input" rows={2} style={{ width: '100%', color: activeInkColor, fontStyle: 'italic', fontWeight: 500, padding: '8px', border: '1px solid #e2e8f0', borderRadius: '4px', resize: 'vertical' }} placeholder={canModerate ? "Moderator Green Pen QA Notes..." : "Assessor Red Pen feedback..."} value={activeData.feedback || ''} onChange={e => handleFeedbackChange(block.id, e.target.value)} />
//                                                 </div>
//                                             </>
...
//                             <p style={{ margin: 0, fontSize: '0.85rem', color: '#15803d', fontStyle: 'italic', wordBreak: 'break-word', overflowWrap: 'anywhere' }}>"{submission.latestCoachingLog.notes}" — {submission.latestCoachingLog.facilitatorName}</p>
//                         </div>
//                     )}
...
//                                 </div>
//                             )}
//                             {/* C/NYC buttons: always visible but disabled for moderator — read-only context of assessor's decision */}
//                             <div className="sr-competency-section" style={{ marginTop: '1rem' }}>
//                                 <label className="sr-sidebar-label" style={{ color: 'red' }}>Final Competency</label>
//                                 <div className="sr-comp-toggles">
//                                     <button
...
//                                         onClick={() => { if (!canGrade || isModerator) return; handleCompetencySelect('C'); }}
//                                         disabled={!canGrade || isModerator}
//                                         style={{ opacity: (!canGrade || isModerator) ? 0.5 : 1, cursor: (!canGrade || isModerator) ? 'not-allowed' : 'pointer', pointerEvents: isModerator ? 'none' : undefined }}
//                                     ><Award size={16} /> Competent (C)</button>
//                                     <button
//                                         className={`sr-comp-btn ${competency === 'NYC' ? 'active-nyc-red' : ''}`}
//                                         onClick={() => { if (!canGrade || isModerator) return; handleCompetencySelect('NYC'); }}
...
//                             <h3 style={{ margin: '0.5rem 0 0.25rem', fontSize: '1rem', color: '#334155' }}>Assessor Grading</h3>
//                             <p style={{ margin: 0, fontSize: '0.8rem', color: '#64748b' }}>
//                                 {isMentor ? 'Awaiting your verification to be completed.' : 'Awaiting facilitator to complete pre-marking.'}
//                             </p>
//                         </div>
//                     )}
...
//                             {/* Assessor's outcome read‑out (original) – kept for additional context */}
//                             <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: '6px', padding: '0.75rem 1rem', marginBottom: '1rem' }}>
//                                 <p style={{ margin: '0 0 4px 0', fontSize: '0.7rem', fontWeight: 'bold', color: '#b91c1c', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '5px' }}>
//                                     <Award size={12} /> Assessor's Declared Outcome
//                                 </p>
...
//                             {/* Green Pen overall QA notes – same as before */}
//                             <div className="sr-overall-feedback">
//                                 <label className="sr-sidebar-label" style={{ color: 'green' }}>
//                                     {modOutcome === 'Returned' ? 'Green Pen Instructions for Assessor (Required)' : 'Green Pen QA Notes (Optional)'}
//                                 </label>
...
//                                             {learnerProfile?.signatureUrl ? <TintedSignature imageUrl={learnerProfile.signatureUrl} color="black" /> : <div style={{ height: '30px', display: 'flex', alignItems: 'center', color: '#b45309', fontStyle: 'italic', fontSize: '0.75rem' }}>No Canvas Signature</div>}
//                                             <p style={{ margin: '4px 0 2px 0', fontSize: '0.9rem', fontWeight: 'bold', color: '#78350f' }}>{learner?.fullName || learnerProfile?.fullName}</p>
//                                             <p style={{ margin: 0, fontSize: '0.75rem', color: '#b45309', display: 'flex', alignItems: 'center', gap: '4px' }}><Clock size={11} /> {new Date(submission.latestCoachingLog.acknowledgedAt).toLocaleDateString()}</p>
//                                         </>
//                                     ) : (
//                                         <p style={{ margin: 0, fontSize: '0.8rem', color: '#b45309', fontStyle: 'italic' }}>Pending learner acknowledgement</p>
//                                     )}
...
//                         {isFacDone && submission.grading?.facilitatorReviewedAt && (
//                             <div style={{ background: 'white', padding: '1.25rem', borderRadius: '8px', border: '1px solid #e2e8f0', boxShadow: '0 2px 4px rgba(0,0,0,0.02)', marginBottom: '1rem' }}>
//                                 <p style={{ fontFamily: 'var(--font-heading)', fontSize: '0.75rem', textTransform: 'uppercase', color: '#64748b', margin: '0 0 8px 0', fontWeight: 'bold' }}>
//                                     {submission.grading?.facilitatorRole === 'mentor' ? 'Mentor Verification' : `${submission.grading?.facilitatorRole || 'Facilitator'} Pre-Marking`}
//                                 </p>
//                                 <p style={{ margin: '0 0 4px 0', fontSize: '0.95rem', fontWeight: 'bold', color: 'blue' }}>{submission.grading?.facilitatorName || 'Facilitator'}</p>
//                                 <p style={{ margin: 0, fontSize: '0.8rem', color: '', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '4px' }}>
//                                     <Clock size={11} /> {submission.grading?.facilitatorReviewedAt ? moment(submission.grading.facilitatorReviewedAt).format('DD/MM/YYYY HH:mm') : 'Completed'}
//                                     {submission.grading?.facilitatorTimeSpent > 0 && <><span style={{ margin: '0 4px' }}>•</span><Activity size={11} /> Active: {formatTimeSpent(submission.grading?.facilitatorTimeSpent)}</>}
//                                     {submission.grading?.facilitatorStartedAt && submission.grading?.facilitatorReviewedAt && <><span style={{ margin: '0 4px' }}>•</span><Calendar size={11} /> Spread: {formatCalendarSpread(submission.grading.facilitatorStartedAt, submission.grading.facilitatorReviewedAt)}</>}
//                                 </p>
...
//                             ) : (
//                                 <p style={{ fontSize: '0.85rem', color: '#b45309', margin: 0, lineHeight: 1.5, fontStyle: 'italic' }}>
//                                     This assessment has been finalised as Not Yet Competent. The Facilitator has been notified to conduct a coaching intervention and initiate the remediation process.
//                                 </p>
//                             )}
//                         </div>
//                     )}
...
// //     isCorrect?: boolean | null;
// //     criteriaResults?: CriterionResult[];
// //     activityResults?: WorkplaceActivityResult[];   // for qcto_workplace
// //     obsDate?: string;
// //     obsStartTime?: string;
// //     obsEndTime?: string;
// //     obsDeclaration?: boolean;
...
// //     const isFacilitator = user?.role === 'facilitator' || user?.role === 'mentor';
// //     const isAssessor = user?.role === 'assessor';
// //     const isModerator = user?.role === 'moderator';

// //     const canFacilitatorMark = isFacilitator && ['not_started', 'in_progress', 'submitted'].includes(currentStatus);
// //     const canGrade = isAssessor && (currentStatus === 'facilitator_reviewed' || currentStatus === 'returned');
// //     const canModerate = isModerator && currentStatus === 'graded';
...
// //                     if (['facilitator_reviewed', 'returned', 'graded', 'moderated'].includes(dbStatus)) {
// //                         aBreakdown = generateFreshBreakdown(false);
// //                         // Inherit checklist and workplace from facilitator
// //                         assData.blocks?.forEach((b: any) => {
// //                             if (b.type === 'checklist' && fBreakdown[b.id]) {
// //                                 aBreakdown[b.id] = {
// //                                     ...JSON.parse(JSON.stringify(fBreakdown[b.id])),
// //                                     score: 0, feedback: '', isCorrect: null
// //                                 };
// //                             }
// //                             if (b.type === 'qcto_workplace' && fBreakdown[b.id]) {
// //                                 aBreakdown[b.id] = {
// //                                     ...JSON.parse(JSON.stringify(fBreakdown[b.id])),
// //                                     score: 0, feedback: '', isCorrect: null
// //                                 };
...
// //                     if (['graded', 'moderated', 'returned'].includes(dbStatus)) {
// //                         mBreakdown = generateFreshBreakdown(false);
// //                         // Inherit checklist and workplace from assessor
// //                         assData.blocks?.forEach((b: any) => {
// //                             if (b.type === 'checklist' && aBreakdown[b.id]) {
// //                                 mBreakdown[b.id] = {
// //                                     ...JSON.parse(JSON.stringify(aBreakdown[b.id])),
// //                                     score: 0, feedback: '', isCorrect: null
// //                                 };
// //                             }
// //                             if (b.type === 'qcto_workplace' && aBreakdown[b.id]) {
// //                                 mBreakdown[b.id] = {
// //                                     ...JSON.parse(JSON.stringify(aBreakdown[b.id])),
// //                                     score: 0, feedback: '', isCorrect: null
// //                                 };
...
// //     const handleSetToNow = (blockId: string, field: 'obsDate' | 'obsStartTime' | 'obsEndTime') => {
// //         if (field === 'obsDate') {
// //             handleGlobalChecklistChange(blockId, field, moment().format('YYYY-MM-DD'));
// //         } else {
// //             handleGlobalChecklistChange(blockId, field, moment().format('HH:mm'));
// //         }
// //     };
...
// //             if (block.type === 'checklist') {
// //                 if (!isModerating && block.requireObservationDeclaration && !grade?.obsDeclaration) return true;
// //                 const crits = grade?.criteriaResults || [];
// //                 if (crits.length < (block.criteria?.length || 0)) return true;
// //                 for (let i = 0; i < (block.criteria?.length || 0); i++) {
// //                     if (!crits[i] || !crits[i].status) return true;
// //                 }
// //                 if (grade.isCorrect === null || grade.isCorrect === undefined) return true;
// //                 return false;
// //             }
...
// //                 if (!isModerating && block.requireObservationDeclaration && !grade?.obsDeclaration) return true;
// //                 const activities = grade?.activityResults || [];
// //                 if (activities.length < (block.workActivities?.length || 0)) return true;
// //                 for (let i = 0; i < (block.workActivities?.length || 0); i++) {
// //                     if (!activities[i] || !activities[i].status) return true;
// //                 }
// //                 if (grade.isCorrect === null || grade.isCorrect === undefined) return true;
// //                 return false;
...
// //     const triggerSubmitFacilitator = () => {
// //         if (!validateAllMarked(facBreakdown, false)) return setModalConfig({ isOpen: true, type: 'warning', title: 'Incomplete Marking', message: 'You must evaluate every question and fully complete all checklists and workplace checkpoints (including declarations) before submitting.', confirmText: 'Got it', onConfirm: () => setModalConfig(null) });
// //         if (!facOverallFeedback.trim()) return setModalConfig({ isOpen: true, type: 'warning', title: 'Missing Remarks', message: 'You must provide Overall Facilitator Remarks before sending this script forward.', confirmText: 'Got it', onConfirm: () => setModalConfig(null) });

// //         let newStatus = 'facilitator_reviewed';
// //         let confirmTitle = 'Complete Pre-Marking?';
// //         let confirmMessage = 'This will finalize your feedback and send the workbook to the Assessor.';
// //         let confirmBtnText = 'Send to Assessor';

// //         if (['not_started', 'in_progress'].includes(currentStatus)) {
// //             if (hasChecklists || hasWorkplace) {
// //                 newStatus = 'awaiting_learner_signoff';
// //                 confirmTitle = 'Request Learner Sign-off?';
// //                 confirmMessage = 'This will save your practical observation and prompt the learner to log in, review your feedback, and officially sign off before it goes to the Assessor.';
// //                 confirmBtnText = 'Save & Request Sign-off';
// //             } else {
// //                 confirmTitle = 'Force Submission?';
// //                 confirmMessage = 'The learner has not submitted this workbook yet. Finalizing your marking now will lock the workbook for the learner and pass it directly to the Assessor. Proceed?';
// //                 confirmBtnText = 'Force Submit & Send';
// //             }
// //         }
...
// //                     });
// //                     if (newStatus === 'awaiting_learner_signoff') {
// //                         toast.success("Observation saved! Learner has been requested to sign off.");
// //                     } else {
...
// //     const triggerSubmitModeration = () => {
// //         if (!validateAllMarked(modBreakdown, true)) return setModalConfig({ isOpen: true, type: 'warning', title: 'Incomplete QA', message: 'You must verify every question and fully check all observation and workplace blocks before endorsing.', confirmText: 'Got it', onConfirm: () => setModalConfig(null) });
...
// //                 if (block.type === 'mcq') {
// //                     return (
// //                         <div key={block.id} className="sr-q-card" style={{ borderColor: '#073f4e', borderTop: '4px solid black', marginBottom: '2rem' }}>
// //                             <div className="sr-q-header">
// //                                 <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
// //                                     <span className="sr-q-num" style={{ background: '#f1f5f9', color: '#073f4e' }}>Q{qNum}</span>
// //                                     <span className="sr-q-text" style={{ color: '#073f4e', fontWeight: 'bold' }}>{block.question}</span>
...
// //                                             {renderFacReadOnly && (
// //                                                 <div style={{ background: '#eff6ff', padding: '0.75rem', borderRadius: '4px' }}>
// //                                                     <div style={{ color: '#0284c7', fontSize: '0.7rem', textTransform: 'uppercase', fontWeight: 'bold', marginBottom: '4px' }}><Info size={13} /> Facilitator Pre-Mark</div>
// //                                                     <div style={{ color: '#0369a1', fontSize: '0.85rem', whiteSpace: 'pre-wrap' }}><span style={{ fontWeight: 'bold', marginRight: '6px' }}>[{fData.score ?? 0}/{maxM}]</span> {fData.feedback || <em style={{ opacity: 0.7 }}>No specific coaching provided.</em>}</div>
// //                                                 </div>
// //                                             )}
// //                                             {renderAssReadOnly && (
// //                                                 <div style={{ background: '#fef2f2', padding: '0.75rem', borderRadius: '4px' }}>
// //                                                     <div style={{ color: '#b91c1c', fontSize: '0.7rem', textTransform: 'uppercase', fontWeight: 'bold', marginBottom: '4px' }}><Award size={13} /> Assessor Grade</div>
// //                                                     <div style={{ color: '#991b1b', fontSize: '0.85rem', whiteSpace: 'pre-wrap' }}><span style={{ fontWeight: 'bold', marginRight: '6px' }}>[{aData.score ?? 0}/{maxM}]</span> {aData.feedback || <em style={{ opacity: 0.7 }}>No specific feedback provided.</em>}</div>
// //                                                 </div>
// //                                             )}
// //                                             {renderModReadOnly && (
// //                                                 <div style={{ background: '#f0fdf4', padding: '0.75rem', borderRadius: '4px' }}>
// //                                                     <div style={{ color: '#15803d', fontSize: '0.7rem', textTransform: 'uppercase', fontWeight: 'bold', marginBottom: '4px' }}><ShieldCheck size={13} /> Moderator QA</div>
// //                                                     <div style={{ color: '#16a34a', fontSize: '0.85rem', whiteSpace: 'pre-wrap' }}><span style={{ fontWeight: 'bold', marginRight: '6px' }}>[{mData.score ?? 0}/{maxM}]</span> {mData.feedback || <em style={{ opacity: 0.7 }}>No QA feedback provided.</em>}</div>
// //                                                 </div>
// //                                             )}
// //                                         </div>
// //                                     )}
...
// //                                                 if (taskTabs.length === 0) return <span style={{ color: '#64748b', fontStyle: 'italic' }}>No evidence provided.</span>;
...
// //                                                         <div style={{ padding: '15px' }}>
// //                                                             {activeTabId === 'text' && (
// //                                                                 <div className="quill-read-only-content" dangerouslySetInnerHTML={{ __html: learnerAns.text }} />
// //                                                             )}
// //                                                             {activeTabId === 'audio' && (
...
// //                                                             )}
// //                                                             {activeTabId === 'upload' && (
// //                                                                 <FilePreview url={learnerAns.uploadUrl} />
// //                                                             )}
// //                                                             {activeTabId === 'code' && (
// //                                                                 <pre style={{ margin: 0, overflowX: 'auto', fontSize: '0.85rem', fontFamily: 'monospace', background: '#1e293b', color: '#f8fafc', padding: '15px', borderRadius: '4px' }}>
// //                                                                     <code>{learnerAns.code}</code>
// //                                                                 </pre>
// //                                                             )}
// //                                                         </div>
// //                                                     </div>
...
// //                                 <div className="sr-answer-box">
// //                                     <div className="sr-answer-label" style={{ color: 'black', display: 'flex', alignItems: 'center', gap: '6px' }}><CalendarRange size={14} /> Workplace Hours Logged:</div>
// //                                     {(!learnerAns || !Array.isArray(learnerAns) || learnerAns.length === 0) ? (
// //                                         <span style={{ color: '#64748b', fontStyle: 'italic', display: 'block', padding: '10px' }}>No entries logged by learner.</span>
// //                                     ) : (
// //                                         <div style={{ overflowX: 'auto', border: '1px solid #e2e8f0', borderRadius: '6px' }}>
// //                                             <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem', textAlign: 'left', background: 'white' }}>
...
// //                                                 </thead>
// //                                                 <tbody>
// //                                                     {learnerAns.map((entry: any, i: number) => (
// //                                                         <tr key={i} style={{ borderBottom: '1px solid #f1f5f9', background: 'white' }}>
// //                                                             <td style={{ padding: '10px', color: '#0f172a', fontWeight: 'bold' }}>{entry.date}</td>
// //                                                             <td style={{ padding: '10px', color: '#334155' }}>{entry.task}</td>
// //                                                             <td style={{ padding: '10px', color: '#475569' }}>{entry.startTime}</td>
...
// //                                                     ))}
// //                                                     <tr style={{ background: '#f1f5f9', fontWeight: 'bold' }}>
// //                                                         <td colSpan={4} style={{ padding: '10px', textAlign: 'right' }}>Total Logged Hours:</td>
// //                                                         <td style={{ padding: '10px', color: '#ea580c' }}>{learnerAns.reduce((acc, curr) => acc + (Number(curr.hours) || 0), 0)}</td>
// //                                                     </tr>
// //                                                 </tbody>
// //                                             </table>
// //                                         </div>
...
// //                                     <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
// //                                         <span className="sr-q-text" style={{ color: '#073f4e', fontWeight: 'bold' }}>{block.title}</span>
// //                                         <span style={{ fontSize: '0.75rem', color: '#64748b' }}>Assessor observation checklist.</span>
// //                                     </div>
// //                                 </div>
// //                             </div>
// //                             <div className="sr-q-body">
...
// //                                                 {block.requireEvidencePerCriterion !== false && (
// //                                                     <div style={{ background: '#f5f3ff', border: '1px solid #c4b5fd', padding: '15px', borderRadius: '6px', marginBottom: '15px' }}>
// //                                                         <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.85rem', fontWeight: 'bold', color: '#6d28d9', marginBottom: '10px', textTransform: 'uppercase' }}>
// //                                                             <Layers size={16} /> Learner Evidence Submitted
// //                                                         </label>
...
// //                                                             if (!critEvidence.uploadUrl && !critEvidence.url && !critEvidence.code && !critEvidence.text) {
// //                                                                 return <p style={{ margin: 0, fontSize: '0.8rem', color: '#7c3aed', fontStyle: 'italic' }}>No evidence provided for this criterion.</p>;
// //                                                             }
...
// //                                                                     {critEvidence.url && <UrlPreview url={critEvidence.url} />}
// //                                                                     {critEvidence.code && (
// //                                                                         <div style={{ background: '#1e293b', border: '1px solid #0f172a', padding: '10px', borderRadius: '4px' }}>
// //                                                                             <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#94a3b8', marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '4px' }}><Code size={14} /> Code / Terminal Output</label>
// //                                                                             <pre style={{ fontSize: '0.8rem', fontFamily: 'monospace', background: 'transparent', color: '#f8fafc', border: 'none', padding: 0, margin: 0, overflowX: 'auto' }}>
// //                                                                                 <code>{critEvidence.code}</code>
// //                                                                             </pre>
// //                                                                         </div>
// //                                                                     )}
// //                                                                     {critEvidence.text && (
// //                                                                         <div style={{ background: 'white', border: '1px solid #ddd6fe', padding: '10px', borderRadius: '4px' }}>
// //                                                                             <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#5b21b6', marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '4px' }}><FileText size={14} /> Reflection / Notes</label>
// //                                                                             <div className="quill-read-only-content" dangerouslySetInnerHTML={{ __html: critEvidence.text }} />
// //                                                                         </div>
// //                                                                     )}
// //                                                                 </div>
// //                                                             );
...
// //                                                             <span style={{ fontWeight: 'bold', color: critResult.status === 'C' ? '#166534' : critResult.status === 'NYC' ? '#991b1b' : '#64748b' }}>
// //                                                                 Outcome: {critResult.status === 'C' ? 'Competent (C)' : critResult.status === 'NYC' ? 'Not Yet Competent (NYC)' : 'Not Graded'}
// //                                                             </span>
// //                                                         </div>
// //                                                     ) : (
// //                                                         <div style={{ display: 'flex', gap: '10px' }}>
// //                                                             <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85rem', background: critResult.status === 'C' ? '#dcfce7' : '#f8fafc', padding: '8px 12px', borderRadius: '4px', border: critResult.status === 'C' ? '2px solid #22c55e' : '1px solid #cbd5e1', color: critResult.status === 'C' ? '#166534' : '#64748b', fontWeight: 'bold', cursor: isActiveRole ? 'pointer' : 'not-allowed' }}>
// //                                                                 <input type="radio" disabled={!isActiveRole} checked={critResult.status === 'C'} onChange={() => handleCriterionChange(block.id, i, 'status', 'C')} style={{ accentColor: '#22c55e' }} /> Competent (C)
// //                                                             </label>
// //                                                             <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85rem', background: critResult.status === 'NYC' ? '#fee2e2' : '#f8fafc', padding: '8px 12px', borderRadius: '4px', border: critResult.status === 'NYC' ? '2px solid #ef4444' : '1px solid #cbd5e1', color: critResult.status === 'NYC' ? '#991b1b' : '#64748b', fontWeight: 'bold', cursor: isActiveRole ? 'pointer' : 'not-allowed' }}>
// //                                                                 <input type="radio" disabled={!isActiveRole} checked={critResult.status === 'NYC'} onChange={() => handleCriterionChange(block.id, i, 'status', 'NYC')} style={{ accentColor: '#ef4444' }} /> NYC
// //                                                             </label>
// //                                                         </div>
// //                                                     )}
...
// //                                                             </div>
// //                                                         ) : (
// //                                                             <textarea className="ab-input" disabled={!isActiveRole} rows={2} placeholder="Assessor comments / reasoning..." value={critResult.comment} onChange={e => handleCriterionChange(block.id, i, 'comment', e.target.value)} style={{ fontSize: '0.85rem', width: '100%', border: '1px solid #cbd5e1', resize: 'vertical' }} />
// //                                                         )}
// //                                                     </div>
// //                                                 </div>
// //                                             </div>
...
// //                                             <h4 style={{ fontSize: '0.9rem', color: '#0f172a', margin: '0 0 15px 0', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '6px' }}>
// //                                                 <ShieldCheck size={16} color={activeInkColor} />
// //                                                 {activeInkColor === 'green' ? 'Observation Log Details' : 'Global Assessor Sign-off'}
// //                                             </h4>
...
// //                                                         <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#475569', marginBottom: '4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
// //                                                             Date of Observation
// //                                                             {isActiveRole && !isPrintMode && <button className="ab-text-btn" style={{ fontSize: '0.65rem', padding: 0 }} onClick={() => handleSetToNow(block.id, 'obsDate')}>Set Now</button>}
// //                                                         </label>
// //                                                         {isPrintMode ? (
// //                                                             <div style={{ fontWeight: 'bold', fontSize: '0.9rem', color: '#0f172a' }}>{activeData.obsDate ? moment(activeData.obsDate).format('DD/MM/YYYY') : '—'}</div>
// //                                                         ) : (
// //                                                             <input type="date" className="ab-input" disabled={!isActiveRole} value={activeData.obsDate || ''} onChange={e => handleGlobalChecklistChange(block.id, 'obsDate', e.target.value)} />
// //                                                         )}
// //                                                     </div>
// //                                                     <div>
// //                                                         <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#475569', marginBottom: '4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
// //                                                             Total Session Start Time
// //                                                             {isActiveRole && !isPrintMode && <button className="ab-text-btn" style={{ fontSize: '0.65rem', padding: 0 }} onClick={() => handleSetToNow(block.id, 'obsStartTime')}>Set Now</button>}
// //                                                         </label>
// //                                                         {isPrintMode ? (
// //                                                             <div style={{ fontWeight: 'bold', fontSize: '0.9rem', color: '#0f172a' }}>{activeData.obsStartTime || '—'}</div>
// //                                                         ) : (
// //                                                             <input type="time" className="ab-input" disabled={!isActiveRole} value={activeData.obsStartTime || ''} onChange={e => handleGlobalChecklistChange(block.id, 'obsStartTime', e.target.value)} />
// //                                                         )}
// //                                                     </div>
// //                                                     <div>
// //                                                         <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#475569', marginBottom: '4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
// //                                                             Total Session End Time
// //                                                             {isActiveRole && !isPrintMode && <button className="ab-text-btn" style={{ fontSize: '0.65rem', padding: 0 }} onClick={() => handleSetToNow(block.id, 'obsEndTime')}>Set Now</button>}
// //                                                         </label>
// //                                                         {isPrintMode ? (
// //                                                             <div style={{ fontWeight: 'bold', fontSize: '0.9rem', color: '#0f172a' }}>{activeData.obsEndTime || '—'}</div>
// //                                                         ) : (
// //                                                             <input type="time" className="ab-input" disabled={!isActiveRole} value={activeData.obsEndTime || ''} onChange={e => handleGlobalChecklistChange(block.id, 'obsEndTime', e.target.value)} />
// //                                                         )}
// //                                                     </div>
// //                                                 </div>
// //                                             )}

// //                                             {activeInkColor !== 'green' && (
// //                                                 <label style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '0.85rem', color: '#0f172a', fontWeight: 'bold', background: activeData.obsDeclaration ? '#eff6ff' : 'white', padding: '10px', borderRadius: '6px', border: '1px solid #bfdbfe', cursor: isActiveRole && !isPrintMode ? 'pointer' : 'default' }}>
// //                                                     <input type="checkbox" disabled={!isActiveRole || isPrintMode} checked={activeData.obsDeclaration || false} onChange={e => handleGlobalChecklistChange(block.id, 'obsDeclaration', e.target.checked)} style={{ width: '18px', height: '18px', accentColor: activeInkColor }} />
// //                                                     I officially declare that I have observed the learner performing these tasks and that the evidence was submitted by the learner.
// //                                                 </label>
...
// //                                     <span className="sr-q-num" style={{ background: '#ffe4e6', color: '#e11d48' }}>QCTO</span>
// //                                     <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
// //                                         <span className="sr-q-text" style={{ color: '#073f4e', fontWeight: 'bold' }}>
// //                                             {block.weCode} – {block.weTitle}
// //                                         </span>
// //                                         <span style={{ fontSize: '0.75rem', color: '#64748b' }}>Workplace Experience Checkpoint</span>
// //                                     </div>
// //                                 </div>
// //                             </div>

// //                             <div className="sr-q-body">
// //                                 {/* Loop through each work activity – combine evidence and grading */}
// //                                 {block.workActivities?.map((activity: any, actIdx: number) => {
// //                                     const taskKey = `wa_${activity.id}_task`;
// //                                     const dateKey = `wa_${activity.id}_date`;
// //                                     const task = learnerAns?.[taskKey] || '';
...
// //                                                         {task && (
// //                                                             <div style={{ flex: 1 }}>
// //                                                                 <strong style={{ fontSize: '0.75rem', color: '#64748b' }}>Task Performed:</strong>
// //                                                                 <div className="quill-read-only-content" dangerouslySetInnerHTML={{ __html: task }} />
// //                                                             </div>
// //                                                         )}
// //                                                         {date && (
// //                                                             <div style={{ minWidth: '120px' }}>
// //                                                                 <strong style={{ fontSize: '0.75rem', color: '#64748b' }}>Date:</strong>
...
// //                                                         return (
// //                                                             <div key={evItem.id} style={{ marginBottom: '1.5rem' }}>
// //                                                                 <span style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#e11d48' }}>
// //                                                                     {evItem.code}
// //                                                                 </span>
// //                                                                 <p style={{ margin: '4px 0 8px 0', fontSize: '0.8rem', color: '#334155' }}>{evItem.description}</p>
// //                                                                 {seData.uploadUrl && <FilePreview url={seData.uploadUrl} />}
// //                                                                 {seData.url && <UrlPreview url={seData.url} />}
// //                                                                 {seData.code && (
// //                                                                     <pre style={{ background: '#1e293b', color: '#f8fafc', padding: '0.5rem', borderRadius: '4px', overflowX: 'auto' }}>
// //                                                                         <code>{seData.code}</code>
// //                                                                     </pre>
// //                                                                 )}
// //                                                                 {seData.text && (
// //                                                                     <div className="quill-read-only-content" dangerouslySetInnerHTML={{ __html: seData.text }} />
// //                                                                 )}
// //                                                             </div>
// //                                                         );
// //                                                     })}
...
// //                                                             <div style={{ fontSize: '0.85rem' }}>
// //                                                                 <span style={{ fontWeight: 'bold', color: actResult.status === 'C' ? '#166534' : actResult.status === 'NYC' ? '#991b1b' : '#64748b' }}>
// //                                                                     Outcome: {actResult.status === 'C' ? 'Competent' : actResult.status === 'NYC' ? 'Not Yet Competent' : 'Not Graded'}
// //                                                                 </span>
// //                                                             </div>
// //                                                         ) : (
// //                                                             <div style={{ display: 'flex', gap: '10px' }}>
// //                                                                 <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85rem', background: actResult.status === 'C' ? '#dcfce7' : '#f8fafc', padding: '8px 12px', borderRadius: '4px', border: actResult.status === 'C' ? '2px solid #22c55e' : '1px solid #cbd5e1', color: actResult.status === 'C' ? '#166534' : '#64748b', fontWeight: 'bold', cursor: isActiveRole ? 'pointer' : 'not-allowed' }}>
// //                                                                     <input type="radio" disabled={!isActiveRole} checked={actResult.status === 'C'} onChange={() => handleActivityStatusChange(block.id, actIdx, 'C')} style={{ accentColor: '#22c55e' }} /> Competent (C)
// //                                                                 </label>
// //                                                                 <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85rem', background: actResult.status === 'NYC' ? '#fee2e2' : '#f8fafc', padding: '8px 12px', borderRadius: '4px', border: actResult.status === 'NYC' ? '2px solid #ef4444' : '1px solid #cbd5e1', color: actResult.status === 'NYC' ? '#991b1b' : '#64748b', fontWeight: 'bold', cursor: isActiveRole ? 'pointer' : 'not-allowed' }}>
// //                                                                     <input type="radio" disabled={!isActiveRole} checked={actResult.status === 'NYC'} onChange={() => handleActivityStatusChange(block.id, actIdx, 'NYC')} style={{ accentColor: '#ef4444' }} /> NYC
// //                                                                 </label>
// //                                                             </div>
// //                                                         )}
// //                                                         <div style={{ flex: 1, minWidth: '250px' }}>
...
// //                                                                 </div>
// //                                                             ) : (
// //                                                                 <textarea className="ab-input" disabled={!isActiveRole} rows={2} placeholder="Assessor comments / reasoning..." value={actResult.comment} onChange={(e) => handleActivityCommentChange(block.id, actIdx, e.target.value)} style={{ fontSize: '0.85rem', width: '100%', border: '1px solid #cbd5e1', resize: 'vertical' }} />
// //                                                             )}
// //                                                         </div>
// //                                                     </div>
// //                                                 </div>
...
// //                                 {/* Self-assessment and goal planning checkboxes (global) */}
// //                                 {block.requireSelfAssessment && learnerAns?.selfAssessmentDone && (
// //                                     <div style={{ marginTop: '1rem', background: '#f8fafc', padding: '1rem', borderRadius: '6px' }}>
// //                                         <strong style={{ color: '#e11d48' }}>Learner Self‑Assessment Completed</strong>
// //                                         <CheckCircle size={16} color="#22c55e" style={{ marginLeft: '8px' }} />
...
// //                                 {/* Global Observation Declaration */}
// //                                 {block.requireObservationDeclaration !== false && (
// //                                     <div style={{ marginTop: '2rem', padding: '1.5rem', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px', borderLeft: `4px solid ${activeInkColor}` }}>
// //                                         <h4 style={{ fontSize: '0.9rem', color: '#0f172a', margin: '0 0 15px 0', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '6px' }}>
// //                                             <ShieldCheck size={16} color={activeInkColor} />
// //                                             Global Assessor Sign‑off
// //                                         </h4>
...
// //                 {/* Official print overall feedback section */}
// //                 <div className="print-page" style={{ marginTop: '20px' }}>
// //                     <h3 style={{ borderBottom: '2px solid black', paddingBottom: '10px', marginBottom: '15px' }}>Overall Assessment Feedback</h3>

// //                     {facOverallFeedback && (
// //                         <div style={{ borderLeft: '4px solid blue', padding: '15px', background: '#eff6ff', marginBottom: '15px', borderRadius: '4px' }}>
// //                             <h4 style={{ color: 'blue', margin: '0 0 8px 0', textTransform: 'uppercase', fontSize: '0.85rem' }}>Facilitator Pre-Marking Remarks</h4>
// //                             <p style={{ margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-word', color: '#1e3a8a', fontStyle: 'italic', lineHeight: 1.5 }}>{facOverallFeedback}</p>
// //                         </div>
// //                     )}
...
// //                                     {canFacilitatorMark ? (
// //                                         <button className="sr-submit-btn" style={{ background: 'blue' }} onClick={triggerSubmitFacilitator} disabled={saving}>
// //                                             {saving ? 'Processing...' : (hasChecklists || hasWorkplace) && ['not_started', 'in_progress'].includes(currentStatus) ? 'Save & Request Sign-off' : 'Send to Assessor'}
// //                                         </button>
// //                                     ) : (
...
// // //                             <div className="sr-q-body">
// // //                                 {/* Learner's Submitted Evidence – Structured as per AssessmentPlayer */}
// // //                                 <div className="sr-answer-box">
// // //                                     <div className="sr-answer-label" style={{ color: 'black', display: 'flex', alignItems: 'center', gap: '6px' }}>
// // //                                         <Layers size={14} /> Learner Evidence
// // //                                     </div>

// // //                                     {block.workActivities?.map((activity: any, actIdx: number) => {
// // //                                         // For each activity, we need to display the task description and date, plus evidence items
// // //                                         const taskKey = `wa_${activity.id}_task`;
// // //                                         const dateKey = `wa_${activity.id}_date`;
// // //                                         const task = learnerAns?.[taskKey] || '';
// // //                                         const date = learnerAns?.[dateKey] || '';
...
// // //                                         return (
// // //                                             <div key={activity.id} style={{ marginBottom: '1.5rem', padding: '1rem', border: '1px solid #cbd5e1', borderRadius: '6px', background: '#fff' }}>
// // //                                                 <p style={{ fontWeight: 'bold', color: '#0f172a', marginBottom: '8px' }}>
...
// // //                                                 {/* Evidence items for this activity */}
// // //                                                 {activity.evidenceItems && activity.evidenceItems.length > 0 && (
// // //                                                     <div style={{ marginLeft: '1rem', borderLeft: '2px solid #fecdd3', paddingLeft: '1rem' }}>
// // //                                                         {activity.evidenceItems.map((evItem: any) => {
// // //                                                             const seKey = `se_${evItem.id}`;
// // //                                                             const seData = learnerAns?.[seKey] || {};
// // //                                                             return (
...
// // //                                     {/* Learner Self-Assessment (checkbox) */}
// // //                                     {block.requireSelfAssessment && learnerAns?.selfAssessmentDone && (
// // //                                         <div style={{ marginTop: '1rem', background: '#f8fafc', padding: '1rem', borderRadius: '6px' }}>
// // //                                             <strong style={{ color: '#e11d48' }}>Learner Self‑Assessment Completed</strong>
// // //                                             <CheckCircle size={16} color="#22c55e" style={{ marginLeft: '8px' }} />
...
// // //                                     {/* Learner Goal Planning (checkbox) */}
// // //                                     {block.requireGoalPlanning && learnerAns?.goalPlanningDone && (
// // //                                         <div style={{ marginTop: '1rem', background: '#f8fafc', padding: '1rem', borderRadius: '6px' }}>
// // //                                             <strong style={{ color: '#e11d48' }}>Learner Goal Planning Completed</strong>
// // //                                             <CheckCircle size={16} color="#22c55e" style={{ marginLeft: '8px' }} />
...
// // //                                 {/* Grading Area – Per Activity */}
// // //                                 {block.workActivities && block.workActivities.map((activity: any, actIdx: number) => {
// // //                                     const actResult = activeData.activityResults?.[actIdx] || { status: null, comment: '' };
// // //                                     return (
// // //                                         <div key={activity.id} style={{ marginTop: '1.5rem', padding: '1rem', border: '1px solid #cbd5e1', borderRadius: '6px', background: isActiveRole ? 'white' : '#f8fafc' }}>
// // //                                             <p style={{ fontWeight: 'bold', marginBottom: '1rem' }}>{activity.code}: {activity.description}</p>
