...
import {
    ...
    writeBatch, doc, getDocs, increment, updateDoc
...
export const LiveAttendanceBoard: React.FC = () => {
    ...
    const executeFinalize = async () => {
        ...
        try {
            ...
            const presentLearnerIds = [...new Set(liveScans.map(s => s.learnerId))];
                            ...
                            <table className="mlab-table">
                                ...
                                <tbody>
                                    {filteredRoster.map(l => {
                                        const scanDoc = liveScans.find(s => s.learnerId === l.id || s.learnerId === l.idNumber);
                                        const isPresent = !!scanDoc;
                                                ...
                                                <td>
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                                        <span style={{ fontWeight: 600, color: isPresent ? 'var(--mlab-midnight)' : 'var(--mlab-grey)' }}>
                                                            <strong style={{ opacity: 0.6, fontSize: '0.7rem' }}>IN:</strong> {isPresent ? moment(scanDoc.checkInAt).format('HH:mm') : '—'}
                                                        </span>
                                                        {isPresent && (scanDoc.lunchOutAt || scanDoc.lunchInAt) && (
                                                            <span style={{ fontSize: '0.75rem', color: 'var(--mlab-grey)', fontWeight: 500 }}>
                                                                <strong style={{ opacity: 0.8, fontSize: '0.65rem' }}>LUNCH:</strong> {scanDoc.lunchOutAt ? moment(scanDoc.lunchOutAt).format('HH:mm') : '--:--'} to {scanDoc.lunchInAt ? moment(scanDoc.lunchInAt).format('HH:mm') : '--:--'}
                                                            </span>
                                                        )}
                                                    </div>
                                                </td>
                                                ...
                                                <td className="cdp-td--right">
                                                    {isPresent ? (
                                                        <button
                                                            ...
                                                            onClick={() => openAdjustTimeModal(scanDoc, l.fullName)}
...
//     // Calculate absent learners based on full scan objects
//     const absentLearnersList = useMemo(() => {
//         const presentIds = liveScans.map(s => s.learnerId);
//         return roster.filter(l => !presentIds.includes(l.id) && !presentIds.includes(l.idNumber));
//     }, [roster, liveScans]);
...
//             {/* ── CONTENT AREA (Safely Padded) ── */}
//             <div style={{ padding: '2rem', maxWidth: '1600px', margin: '0 auto', width: '100%', flex: 1 }}>

//                 {/* ── STATS & PROGRESS ── */}
//                 <div className="cdp-stat-row" style={{ marginBottom: '1.5rem' }}>
//                     <div className="cdp-stat-card cdp-stat-card--blue" style={{ borderRadius: '12px' }}>
//                         <div className="cdp-stat-card__icon"><Users size={20} /></div>
//                         <div className="cdp-stat-card__body">
//                             <span className="cdp-stat-card__value">{totalCount}</span>
//                             <span className="cdp-stat-card__label">Total Roster</span>
...
//                     <div className="cdp-stat-card cdp-stat-card--green" style={{ borderRadius: '12px' }}>
//                         <div className="cdp-stat-card__icon"><UserCheck size={20} /></div>
//                         <div className="cdp-stat-card__body">
//                             <span className="cdp-stat-card__value">{presentCount}</span>
//                             <span className="cdp-stat-card__label">Checked In</span>
...
//                     <div className="cdp-stat-card" style={{ flex: 2, borderTopColor: 'var(--mlab-border)', borderRadius: '12px' }}>
//                         <div style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
//                             <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
//                                 <span style={{ fontSize: '0.85rem', color: 'var(--mlab-grey)', fontWeight: 700, fontFamily: 'var(--font-heading)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Arrival Progress</span>
//                                 <span style={{ fontSize: '0.85rem', fontWeight: 800, color: 'var(--mlab-blue)' }}>{progressPercent}%</span>
...
//                 {/* ── ROSTER TABLE ── */}
//                 <div className="cdp-panel animate-fade-in" style={{ border: 'none', background: 'transparent', borderRadius: '12px' }}>
//                     <div className="vp-card" style={{ marginBottom: 0, borderRadius: '12px' }}>
...
//                         <div className="mlab-table-wrap" style={{ borderRadius: '0 0 12px 12px' }}>
//                             <table className="mlab-table">
//                                 <thead>
//                                     <tr>
//                                         <th>Learner Details</th>
...
// // // // 🚀 Use the exact same CSS as Cohort Details for perfect consistency
...
// //     const [searchTerm, setSearchTerm] = useState('');
// //     // 🚀 NOW STORES FULL SCAN OBJECTS, NOT JUST IDs
// //     const [liveScans, setLiveScans] = useState<any[]>([]);
// //     const [isInitializing, setIsInitializing] = useState(true);
// //     const [isFinalizing, setIsFinalizing] = useState(false);
...
// //         const unsubscribe = onSnapshot(q, (snapshot) => {
// //             // 🚀 Save the whole document object to state
// //             const scans = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
// //             setLiveScans(scans);
// //             setIsInitializing(false);
// //         }, (error) => {
...
// //     // 🚀 5. NEW: Adjust Check-In Time
// //     const handleAdjustTime = async () => {
// //         if (!scanToEdit) return;
// //         setIsUpdatingTime(true);

// //         try {
// //             // Parse the new HH:mm time input and convert it to today's timestamp in milliseconds
// //             const [hours, minutes] = newTimeInput.split(':');
// //             const newCheckInMs = moment().hours(Number(hours)).minutes(Number(minutes)).seconds(0).valueOf();
...
// //     const openAdjustTimeModal = (scan: any, learnerName: string) => {
// //         setScanToEdit({ ...scan, learnerName });
// //         // Set the default input value to their current check-in time
// //         setNewTimeInput(moment(scan.checkInAt).format('HH:mm'));
// //     };
...
// //                 <Sidebar role={user?.role} currentNav="attendance" onLogout={() => navigate('/login')} />
...
// //                 <Sidebar role={user?.role} currentNav="attendance" onLogout={() => navigate('/login')} />
// //                 <main className="cdp-main cdp-main--centered">
// //                     <div style={{ padding: '4rem 2rem', border: '1px dashed #cbd5e1', background: 'white', textAlign: 'center', borderRadius: '12px' }}>
// //                         <ShieldAlert size={64} color="var(--mlab-red)" style={{ margin: '0 auto 1rem' }} />
// //                         <h2 style={{ fontFamily: 'var(--font-heading)', color: '#0f172a' }}>Cohort Not Found</h2>
// //                         <p style={{ maxWidth: '400px', margin: '0.5rem auto 2rem', color: '#64748b' }}>
// //                             The cohort you are trying to view no longer exists or you do not have permission to view it.
...
// //     return (
// //         <div className="cdp-layout">
// //             <Sidebar role={user?.role} currentNav="attendance" setCurrentNav={nav => navigate(isAdmin ? `/admin?tab=${nav}` : `/${user?.role}`)} onLogout={() => navigate('/login')} />

// //             {/* 🚀 EDIT TIME MODAL */}
// //             {scanToEdit && createPortal(
// //                 <div className="lfm-overlay" style={{ zIndex: 99999 }}>
// //                     <div className="lfm-modal animate-slide-up" style={{ maxWidth: '400px', width: '90%' }}>
// //                         <div className="lfm-header" style={{ background: 'var(--mlab-blue)' }}>
// //                             <h2 className="lfm-header__title" style={{ color: 'white' }}>
// //                                 <Clock size={18} color="var(--mlab-green)" /> Adjust Check-In Time
// //                             </h2>
// //                             <button className="lfm-close-btn" onClick={() => setScanToEdit(null)}>
// //                                 <X size={20} color="white" />
// //                             </button>
// //                         </div>
// //                         <div className="lfm-body" style={{ padding: '1.5rem', background: '#f8fafc' }}>
// //                             <p style={{ margin: '0 0 1rem 0', fontSize: '0.85rem', color: 'var(--mlab-grey)' }}>
// //                                 Adjusting check-in time for <strong>{scanToEdit.learnerName}</strong>.
// //                                 The system will log this as a manual override.
// //                             </p>
// //                             <div className="lfm-fg lfm-fg--full">
...
// //                             <button className="lfm-btn lfm-btn--ghost" onClick={() => setScanToEdit(null)}>Cancel</button>
// //                             <button className="lfm-btn lfm-btn--primary" onClick={handleAdjustTime} disabled={isUpdatingTime}>
// //                                 {isUpdatingTime ? <Loader2 size={16} className="lfm-spin" /> : <Save size={16} />} Save Adjustment
// //                             </button>
// //                         </div>
// //                     </div>
// //                 </div>,
...
// //                                     <tbody>
// //                                         {filteredRoster.map(l => {
// //                                             // Find the specific scan document for this learner to get their exact checkIn time
// //                                             const scanDoc = liveScans.find(s => s.learnerId === l.id || s.learnerId === l.idNumber);
// //                                             const isPresent = !!scanDoc;
...
// //                                                     <td>
// //                                                         <span style={{ fontWeight: 600, color: isPresent ? 'var(--mlab-midnight)' : 'var(--mlab-grey)' }}>
// //                                                             {isPresent ? moment(scanDoc.checkInAt).format('HH:mm') : '—'}
// //                                                         </span>
// //                                                     </td>
// //                                                     <td>
// //                                                         {isPresent ? (
