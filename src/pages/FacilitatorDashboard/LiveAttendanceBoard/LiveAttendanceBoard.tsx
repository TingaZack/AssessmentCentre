...
export const LiveAttendanceBoard: React.FC = () => {
    ...
    // ─── CAMPUS SETTINGS ───
    const [campusType, setCampusType] = useState('physical');
    ...
    // 🚀 DIAGNOSTIC TRACKER
    useEffect(() => {
        console.log(`🔥 CURRENT RENDER STATE -> campusType: "${campusType}", fallbackTime: "${checkoutFallbackTime}"`);
    }, [campusType, checkoutFallbackTime]);
        ...
        const fetchCampusSettings = async () => {
            try {
                ...
                if (snap.exists()) {
                    ...
                    console.log("🚀 --- CAMPUS RESOLUTION DIAGNOSTIC ---");
                    console.log("1. Cohort ID:", currentCohort?.id);
                    console.log("2. Cohort Name:", currentCohort?.name);
                    console.log("3. Cohort's Assigned Campus ID:", cohortCampusId);
                    console.log("4. Available Campuses in DB:", allCampuses.map((c: any) => ({ id: c.id, name: c.name, type: c.type })));

                    const myCampus = allCampuses.find((c: any) => c.id === cohortCampusId);

                    console.log("5. Found Matching Campus Object:", myCampus ? "YES" : "NO", myCampus);

                    if (myCampus) {
                        const resolvedType = myCampus.type === 'online' ? 'online' : 'physical';
                        console.log(`6A. Match found! Forcing campusType to: "${resolvedType}"`);
                        setCampusType(resolvedType);

                        if (myCampus.campusTimes?.checkoutStart) {
                            setCheckoutFallbackTime(myCampus.campusTimes.checkoutStart);
                        }
                    } else {
                        console.log("6B. No campus matched (or none assigned). Falling back to 'physical' so 4-slots show up.");
                        setCampusType('physical');

                        const defaultCampus = allCampuses.find((c: any) => c.isDefault) || allCampuses[0];
                        if (defaultCampus?.campusTimes?.checkoutStart) {
                            setCheckoutFallbackTime(defaultCampus.campusTimes.checkoutStart);
                        }
                    }
                    console.log("---------------------------------------");
                }
            } catch (e) {
                console.warn("Failed to fetch settings, falling back to physical defaults", e);
                setCampusType('physical');
            }
        };
    ...
    const roster = useMemo(() => {
        ...
        learners.forEach(l => {
            ...
            // 🚀 FIX: Also check `l.cohortId` to catch Bootcamp applicants who aren't in the array yet!
            if (currentCohort.learnerIds?.includes(l.id) || currentCohort.learnerIds?.includes(l.idNumber) || l.cohortId === cohortId) {
    ...
    }, [currentCohort, learners, cohortId]);
    ...
    const handleAdjustTime = async () => {
        ...
        try {
            ...
            let newCheckOutMs = null;
            if (checkOutInput) {
                const [outH, outM] = checkOutInput.split(':');
                newCheckOutMs = moment().hours(Number(outH)).minutes(Number(outM)).seconds(0).valueOf();
            }
    ...
    const openAdjustTimeModal = (scan: any, learnerName: string) => {
        ...
        const getSafeTime = (field: string) => {
            if (scan[field]) return moment(scan[field]).format('HH:mm');
            return '';
        };

        let initialIn = getSafeTime('checkInAt');
        if (!initialIn && scan.timestamp) {
            const ms = scan.timestamp?.toMillis ? scan.timestamp.toMillis() : scan.timestamp?.toDate?.().getTime();
            if (ms) initialIn = moment(ms).format('HH:mm');
        }

        setNewTimeInput(initialIn);
        setLunchOutInput(getSafeTime('lunchOutAt'));
        setLunchInInput(getSafeTime('lunchInAt'));
        setCheckOutInput(getSafeTime('checkOutAt'));
    };
    ...
    // Helpers to extract safe display times from timestamps
    const getTapTime = (scan: any, specificField: string) => {
        if (!scan) return null;
        if (scan[specificField]) return moment(scan[specificField]).format('HH:mm');
        return null;
    };
    const getFallbackTapTime = (scan: any) => {
        if (!scan) return null;
        let ms = scan.checkInAt;
        if (!ms && scan.timestamp) {
            ms = scan.timestamp.toMillis ? scan.timestamp.toMillis() : (scan.timestamp.toDate ? scan.timestamp.toDate().getTime() : scan.timestamp);
        }
        if (!ms) return null;
    ...
    return (
        <div className="animate-fade-in" style={{ width: '100%', display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
            ...
            {/* EDIT TIME MODAL WITH LUNCH & CHECKOUT LOGGING */}
                ...
                <div className="lfm-overlay" style={{ zIndex: 99999 }}>
                    <div className="lfm-modal animate-slide-up" style={{ maxWidth: '500px', width: '90%' }}>
                        ...
                        <div className="lfm-body" style={{ padding: '1.5rem', background: '#f8fafc' }}>
                            ...
                            <div className="lfm-grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
                            ...
                            <table className="mlab-table">
                                ...
                                <tbody>
                                    {filteredRoster.map(l => {
                                        ...
                                        const inTime = getFallbackTapTime(firstTap);
                                        const lunchOutTime = getTapTime(firstTap, 'lunchOutAt') || getFallbackTapTime(learnerTaps[1]) || '--:--';
                                        const lunchInTime = getTapTime(firstTap, 'lunchInAt') || getFallbackTapTime(learnerTaps[2]) || '--:--';
                                        const outTime = getTapTime(firstTap, 'checkOutAt') || getFallbackTapTime(learnerTaps[3]) || '--:--';
                                                ...
                                                <td>
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                                        <span style={{ fontWeight: 600, color: isPresent ? 'var(--mlab-midnight)' : 'var(--mlab-grey)' }}>
                                                            <strong style={{ opacity: 0.6, fontSize: '0.7rem' }}>IN:</strong> {isPresent ? inTime : '—'}
                                                        </span>
                                                        {/* 🚀 FIX: Removed the "isPresent &&" so the UI always shows the slots! */}
                                                        {campusType !== 'online' && (
                                                            <>
                                                                <span style={{ fontSize: '0.75rem', color: isPresent ? 'var(--mlab-grey)' : '#94a3b8', fontWeight: 500 }}>
                                                                    <strong style={{ opacity: 0.8, fontSize: '0.65rem' }}>LUNCH:</strong> {lunchOutTime} to {lunchInTime}
                                                                </span>
                                                                <span style={{ fontSize: '0.75rem', color: isPresent ? 'var(--mlab-grey)' : '#94a3b8', fontWeight: 500 }}>
                                                                    <strong style={{ opacity: 0.8, fontSize: '0.65rem' }}>OUT:</strong> {outTime}
                                                                </span>
                                                            </>
                                                        )}
                                                    </div>
