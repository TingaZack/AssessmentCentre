...
//     // 6. FINALIZE & CLOSE DAY LOGIC (UPDATED TO AUTO-IMPUTE MISSING CHECKOUTS)
//     const handleInitiateFinalize = () => {
//         setShowReviewModal(true);
//     };
...
//             // Extract fallback time variables
//             const [fallbackH, fallbackM] = checkoutFallbackTime.split(':').map(Number);

//             // MAP ALL EXACT TIMESTAMPS
//             const scansMap: Record<string, any> = {};
//             liveSnap.docs.forEach(d => {
//                 const data = d.data();
...
//                 if (!scansMap[lId]) {
//                     scansMap[lId] = { checkInAt: null, lunchOutAt: null, lunchInAt: null, checkOutAt: null };
//                 }
...
//                 if (data.lunchOutAt) scansMap[lId].lunchOutAt = getMs(data.lunchOutAt);
//                 if (data.lunchInAt) scansMap[lId].lunchInAt = getMs(data.lunchInAt);
//                 if (data.checkOutAt) scansMap[lId].checkOutAt = getMs(data.checkOutAt);
...
//             // AUTO-IMPUTE MISSING CHECKOUTS FOR ANALYTICS
//             Object.keys(scansMap).forEach(lId => {
//                 const scan = scansMap[lId];
//                 if (scan.checkInAt && !scan.checkOutAt) {
//                     const outDate = new Date(scan.checkInAt);
//                     outDate.setHours(fallbackH || 16, fallbackM || 0, 0, 0);
//                     // Ensure checkout isn't mathematically before check-in
//                     scan.checkOutAt = Math.max(outDate.getTime(), scan.checkInAt);
//                 }
//             });
