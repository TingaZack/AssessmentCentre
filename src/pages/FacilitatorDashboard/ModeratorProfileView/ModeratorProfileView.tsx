...
import {
    ...
    Camera, Award, Calendar, Briefcase, PenTool, Scale, MapPin, Plus, Trash2, CheckCircle, Upload
} from 'lucide-react';
...
export const ModeratorProfileView: React.FC<ProfileProps> = ({ user, onUpdate }) => {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    const initialDocs: DynamicDocument[] = [
                        ...
                        {
                            id: 'moderator_cert',
                            name: 'Moderator Certificate',
                            file: null,
                            url: uploadedDocsArray.find((d: any) => d.id === 'moderator_cert')?.url || legacyDocs.moderatorCertUrl || '',
                            isFixed: true,
                            isRequired: true
                        },
                        {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const legacyDocsObject: any = { ...(profileData?.complianceDocs || {}) }; // Keep backward compatibility
