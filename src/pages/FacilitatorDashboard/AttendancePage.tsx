...
import {
    ...
    FilterX, MapPin, ChevronRight, Globe,
    Loader2, X, FileText, CheckCircle2, UploadCloud as UploadIcon
} from 'lucide-react';
...
// ─── MANDATORY AUDIT & PROOF MODAL (PARTIAL & EXCUSED ABSENCES ONLY) ─────────

interface AttendanceAuditModalProps {
    isOpen: boolean;
    onClose: () => void;
    learner: any;
    targetStatus: 'Partial' | 'Excused_Absent' | null;
    onConfirm: (category: string, notes: string, proofUrl: string, proofName: string, stipendWeight: number) => Promise<void>;
}

const AttendanceAuditModal: React.FC<AttendanceAuditModalProps> = ({
    isOpen,
    onClose,
    learner,
    targetStatus,
    ...
    const isExcused = targetStatus === 'Excused_Absent';
    const [category, setCategory] = useState<string>(isExcused ? 'Medical / Sick Note' : 'Transport / Bus Delay');
    const [notes, setNotes] = useState<string>('');
    const [stipendWeight, setStipendWeight] = useState<number>(isExcused ? 1.0 : 0.5);
    const [file, setFile] = useState<File | null>(null);
    const [isSubmitting, setIsSubmitting] = useState(false);
    const toast = useToast();

    useEffect(() => {
        if (targetStatus === 'Excused_Absent') {
            setCategory('Medical / Sick Note');
            setStipendWeight(1.0);
        } else if (targetStatus === 'Partial') {
            setCategory('Transport / Bus Delay');
            setStipendWeight(0.5);
        }
    }, [targetStatus]);

    if (!isOpen || !learner || !targetStatus) return null;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!file) {
            toast.error("Supporting proof (WhatsApp screenshot, sick note, or email) is mandatory.");
            return;
        }
        ...
        try {
            const storagePath = `attendance_evidence/${targetStatus.toLowerCase()}/${learner.idNumber || learner.id}_${Date.now()}_${file.name}`;
            const storageRef = ref(storage, storagePath);
            const uploadSnap = await uploadBytes(storageRef, file);
            const proofUrl = await getDownloadURL(uploadSnap.ref);
            const proofName = file.name;

            await onConfirm(category, notes, proofUrl, proofName, stipendWeight);
            onClose();
        } catch (err: any) {
            console.error("Failed to process audit evidence:", err);
            toast.error(err.message || "Failed to log exception evidence.");
        } finally {
            setIsSubmitting(false);
        }
    };

    const headerColor = isExcused ? '#0284c7' : '#f59e0b';
    const headerBg = isExcused ? '#e0f2fe' : '#fffbeb';
    const headerText = isExcused ? '#0369a1' : '#b45309';
    ...
    return createPortal(
        <div className="wm-overlay animate-fade-in" onClick={onClose} style={{ zIndex: 999999 }}>
            <div className="wm-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '520px' }}>
                <div className="wm-modal__header" style={{ borderBottom: `3px solid ${headerColor}`, paddingBottom: '1rem' }}>
                    <div className="wm-modal__header-icon" style={{ background: headerBg, color: headerText }}>
                        <FileText size={20} />
                    </div>
                    <div>
                        <h2 className="wm-modal__title">HR Audit • Log {targetStatus.replace('_', ' ')}</h2>
                        <p className="wm-modal__subtitle">
                            {isExcused
                                ? 'Authorize paid absence. Physical absent record is retained for audit integrity.'
                                : 'Authorize short hours. Upload mandatory proof (WhatsApp, email, or doctor note).'}
                        </p>
                    </div>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="wm-modal__body" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', padding: 16 }}>
                        <div style={{ background: '#f8fafc', padding: '10px 12px', border: '1px solid #cbd5e1' }}>
                            <span style={{ fontSize: '0.72rem', color: 'var(--mlab-grey)', fontWeight: 700, textTransform: 'uppercase' }}>Candidate Name:</span>
                            <div style={{ fontSize: '0.95rem', fontWeight: 800, color: 'var(--mlab-blue)' }}>{learner.fullName} ({learner.idNumber || 'No ID'})</div>
                        </div>

                        {/* Category Selector */}
                        <div className="wm-form-group">
                            <label className="wm-form-label">Authorization Category <span style={{ color: 'red' }}>*</span></label>
                            {isExcused ? (
                                <select className="wm-form-input" value={category} onChange={e => setCategory(e.target.value)} required>
                                    <option value="Medical / Sick Note">Medical / Doctor's Sick Note</option>
                                    <option value="Court Subpoena / Legal">Court Subpoena / Legal Duty</option>
                                    <option value="Bereavement / Funeral">Bereavement / Family Funeral</option>
                                    <option value="Official Interview">Official Job / Workplace Interview</option>
                                    <option value="Transport / Force Majeure">Transport Strike / Force Majeure Waiver</option>
                                    <option value="Other Authorized Absence">Other Authorized HR Exception</option>
                                </select>
                            ) : (
                                <select className="wm-form-input" value={category} onChange={e => setCategory(e.target.value)} required>
                                    <option value="Transport / Bus Delay">Transport / Traffic Delay</option>
                                    <option value="Medical / Clinic Appointment">Medical Appointment (Short Hours)</option>
                                    <option value="Power Outage / Loadshedding">Power / Internet Outage</option>
                                    <option value="Official Early Release">Official Early Release Authorization</option>
                                    <option value="Personal / Family Emergency">Personal Emergency (Partial)</option>
                                    <option value="Other Partial Exception">Other Partial Attendance Reason</option>
                                </select>
                            )}
                        </div>

                        {/* Stipend Payout Rate Selector */}
                        <div className="wm-form-group">
                            <label className="wm-form-label">Stipend Payout Rate <span style={{ color: 'red' }}>*</span></label>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                                <button
                                    type="button"
                                    onClick={() => setStipendWeight(0.5)}
                                    style={{
                                        padding: '8px', fontSize: '0.8rem', fontWeight: 700,
                                        background: stipendWeight === 0.5 ? '#f59e0b' : '#f1f5f9',
                                        color: stipendWeight === 0.5 ? 'white' : '#64748b',
                                ...
                                <button
                                    type="button"
                                    onClick={() => setStipendWeight(1.0)}
                                    style={{
                                        padding: '8px', fontSize: '0.8rem', fontWeight: 700,
                                        background: stipendWeight === 1.0 ? '#16a34a' : '#f1f5f9',
                                        color: stipendWeight === 1.0 ? 'white' : '#64748b',
                                        border: '1px solid #16a34a', cursor: 'pointer',
                                        borderRadius: 0,
                                    }}
                                >
                                    1.0 Full Pay (Excused)
                                </button>
                            </div>
                        </div>
                        ...
                        {/* Audit Notes */}
                        <div className="wm-form-group">
                            <label className="wm-form-label">Facilitator Audit Notes</label>
                            <textarea
                                className="wm-form-input"
                                style={{ minHeight: '70px', resize: 'vertical' }}
                                placeholder={isExcused ? "E.g., Medical certificate received via WhatsApp from Dr. Mokoena..." : "E.g., Taxi breakdown screenshot received via WhatsApp..."}
                                value={notes}
                        ...
                        <div className="wm-form-group">
                            <label className="wm-form-label" style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                <UploadIcon size={14} /> Upload Supporting Evidence / Proof <span style={{ color: 'red' }}>*</span>
                            </label>
                            <input
                                type="file"
                                className="wm-form-input"
                                ...
                                required
                            />
                            <span style={{ fontSize: '0.72rem', color: 'var(--mlab-grey)', marginTop: '2px', display: 'block' }}>
                                Accepts sick notes, court orders, or screenshots (WhatsApp / Email / SMS).
                            </span>
                            {file && (
                    ...
                    <div className="wm-modal__footer">
                        <button type="button" className="wm-btn wm-btn--ghost" onClick={onClose} disabled={isSubmitting}>Cancel</button>
                        <button
                            type="submit"
                            className="mlab-btn"
                            style={{ background: headerColor, color: 'white', border: 'none' }}
                            disabled={isSubmitting || !file}
                        >
                            {isSubmitting ? <Loader2 size={16} className="spin" /> : <CheckCircle2 size={16} />} Upload & Save Authorization
                        </button>
                    </div>
                </form>
            </div>
...
export const AttendanceRingCard: React.FC<AttendanceRingCardProps> = ({
    ...
    const config = {
        k: { cls: 'mc-k', fillId: 'rK' },
        p: { cls: 'mc-p', fillId: 'rP' },
        w: { cls: 'mc-w', fillId: 'rW' },
        r: { cls: 'mc-r', fillId: 'rR' }
    };
...
export const AttendancePage: React.FC = () => {
    ...
    // ─── MANDATORY AUDIT MODAL TARGET STATE ───
    const [auditModalConfig, setAuditModalConfig] = useState<{
        learner: any | null;
        targetStatus: 'Partial' | 'Excused_Absent' | null;
    }>({ learner: null, targetStatus: null });
    ...
    const statusFilter = (searchParams.get('status') as 'all' | 'present' | 'partial' | 'excused_absent' | 'absent') || 'all';
    ...
    useEffect(() => {
        const getAttendance = async () => {
            try {
                ...
                // EXCLUDE DROPPED / WITHDRAWN / ARCHIVED LEARNERS
                const enrolledProfiles = allLearners.filter(l => {
                    ...
                    const enrolSt = String(enrol?.status || '').toLowerCase();
                    const isDropped = st === 'dropped' || st === 'withdrawn' || st === 'archived' || l.isArchived || enrolSt === 'dropped' || enrolSt === 'withdrawn';
                ...
                if (isBootcamp) setMasterLog(logData);
                ...
                if (isBootcamp) {
                    ...
                    roster = enrolledProfiles.map(learner => {
                        ...
                        let finalStatus: 'Present' | 'Partial' | 'Excused_Absent' | 'Absent' = 'Absent';
                        let stipendWeight = 0.5;

                        if (rec) {
                            actualDuration = rec.actualDuration || 0;
                            finalStatus = rec.status || 'Absent';
                            stipendWeight = typeof rec.stipendWeight === 'number' ? rec.stipendWeight : (finalStatus === 'Partial' ? 0.5 : (finalStatus === 'Absent' ? 0.0 : 1.0));
                        } else if (zoomMatch) {
                        ...
                        return {
                            ...
                            isPresent: finalStatus === 'Present' || finalStatus === 'Partial' || finalStatus === 'Excused_Absent',
                            status: finalStatus,
                            stipendWeight: stipendWeight,
                            actualDuration: actualDuration,
                            expectedDuration: expectedDuration,
                            compliancePct: compliancePct,
                            overridden: rec?.overridden || false,
                            reason: rec?.reason || '',
                            evidence: rec?.evidence || (rec?.proofUrl ? { documentUrl: rec.proofUrl, documentName: rec.proofName } : null)
                        };
                    });
                    ...
                    let qctoData: any = { presentLearners: [], absentLearners: [], partialLearners: [], excusedLearners: [], reasons: {}, proofs: {}, stipendWeights: {} };
                    ...
                    roster = enrolledProfiles.map(learner => {
                        ...
                        const isPartial = qctoData.partialLearners?.includes(id) || false;
                        const isExcused = qctoData.excusedLearners?.includes(id) || false;

                        let status: 'Present' | 'Partial' | 'Excused_Absent' | 'Absent' = 'Absent';
                        if (isPresent) status = 'Present';
                        else if (isPartial) status = 'Partial';
                        else if (isExcused) status = 'Excused_Absent';

                        const defaultWeight = status === 'Partial' ? (qctoData.stipendWeights?.[id] ?? 0.5) : (status === 'Absent' ? 0.0 : 1.0);

                        return {
                            ...learner,
                            isPresent: status !== 'Absent',
                            status: status,
                            stipendWeight: defaultWeight,
    ...
    const summaryStats = useMemo(() => {
        let present = 0, partial = 0, excused = 0, absent = 0, ghosts = 0, bounced = 0;
        let attendingMaleCount = 0, attendingFemaleCount = 0;
        let totalWatchMins = 0, expectedDuration = 120;
        ...
        attendanceList.forEach(l => {
            ...
            else if (l.status === 'Excused_Absent') excused++;
            else {
                absent++;
                if (actualMins > 0) bounced++;
                else ghosts++;
            }

            if (l.status === 'Present' || l.status === 'Partial') {
                totalWatchMins += actualMins;
                const rawGender = String(l.demographics?.genderCode || l.gender || '').trim().toLowerCase();
                if (rawGender === 'm' || rawGender.includes('male')) attendingMaleCount++;
                if (rawGender === 'f' || rawGender.includes('female')) attendingFemaleCount++;
            }
        });
        ...
        return {
            present, partial, excused, absent, ghosts, bounced,
    ...
    // 🚀 RESTRICTED STATUS ROTATION: Direct Absent -> Present is blocked.
    // Absent -> Partial (requires proof modal)
    // Partial -> Excused_Absent (requires proof modal)
    // Excused_Absent -> Absent (resets to 0.0)
    // Present -> Partial (requires proof modal)
    const handleStatusRotate = (learner: any) => {
        if (isLocked) return;

        let nextStatus: 'Present' | 'Partial' | 'Excused_Absent' | 'Absent' = 'Absent';

        if (learner.status === 'Present') {
            nextStatus = 'Partial';
        } else if (learner.status === 'Partial') {
            nextStatus = 'Excused_Absent';
        } else if (learner.status === 'Excused_Absent') {
            nextStatus = 'Absent';
        } else if (learner.status === 'Absent') {
            // 🚀 ABSENT LEARNERS CANNOT BE CHANGED TO PRESENT DIRECTLY
            // They can only move to Partial or Excused_Absent with mandatory proof
            nextStatus = 'Partial';
        }

        // 🚀 MANDATORY POPUP FOR PARTIAL OR EXCUSED ABSENCE
        if (nextStatus === 'Partial' || nextStatus === 'Excused_Absent') {
            setAuditModalConfig({ learner, targetStatus: nextStatus });
            return;
        }

        executeStatusUpdate(learner, nextStatus, 0.0);
    };

    // EXECUTE DB SAVE
    const executeStatusUpdate = async (
        learner: any,
        nextStatus: 'Present' | 'Partial' | 'Excused_Absent' | 'Absent',
        stipendWeight: number,
        reasonNote?: string,
        category?: string,
        proofUrl?: string,
        proofName?: string
        ...
        if (attendanceMode === 'bootcamp') {
            ...
            try {
                ...
                if (reasonNote) payload.reason = reasonNote;
                if (category) payload.excuseCategory = category;
                if (proofUrl) {
                    payload.proofUrl = proofUrl;
                    payload.proofName = proofName;
                    payload.evidence = { documentUrl: proofUrl, documentName: proofName, category };
                }
                ...
                setAttendanceList(prev => prev.map(l => {
                    if (l.id === learner.id || l.idNumber === learner.idNumber) {
                        return {
                            ...l,
                            isPresent: nextStatus !== 'Absent',
                            status: nextStatus,
                            stipendWeight: stipendWeight,
                            overridden: true,
                            reason: reasonNote || l.reason,
                            evidence: proofUrl ? { documentUrl: proofUrl, documentName: proofName } : l.evidence
                        };
                    }
                    return l;
                }));

                toast.success(`Status updated to ${nextStatus.replace('_', ' ')} for ${learner.fullName}`);
            } catch (error) {
            ...
            if (!recordId) {
                toast.error("No QCTO register found for this date.");
                return;
            }
            ...
            const updates: any = {
                ...
                partialLearners: arrayRemove(idNumber),
                excusedLearners: arrayRemove(idNumber),
                absentLearners: arrayRemove(idNumber),
                [`stipendWeights.${idNumber}`]: stipendWeight
            };

            if (reasonNote) updates[`reasons.${idNumber}`] = reasonNote;
            if (proofUrl) updates[`proofs.${idNumber}`] = { url: proofUrl, name: proofName };

            if (nextStatus === 'Present') updates.presentLearners = arrayUnion(idNumber);
            else if (nextStatus === 'Partial') updates.partialLearners = arrayUnion(idNumber);
            else if (nextStatus === 'Excused_Absent') updates.excusedLearners = arrayUnion(idNumber);
            else updates.absentLearners = arrayUnion(idNumber);
                ...
                setAttendanceList(prev => prev.map(l => {
                    if (l.idNumber === idNumber) {
                        return {
                            ...
                            stipendWeight: stipendWeight,
                            reason: reasonNote || l.reason,
                            proof: proofUrl ? { url: proofUrl, name: proofName } : l.proof
                        };
                    }
                    return l;
                }));
    ...
    // CONFIRM AUDIT (PARTIAL OR EXCUSED_ABSENT) FROM MANDATORY MODAL
    const handleConfirmAudit = async (category: string, notes: string, proofUrl: string, proofName: string, stipendWeight: number) => {
        if (!auditModalConfig.learner || !auditModalConfig.targetStatus) return;
        const auditReason = `[${category}] ${notes}`.trim();
        await executeStatusUpdate(
            auditModalConfig.learner,
            auditModalConfig.targetStatus,
            stipendWeight,
            auditReason,
            category,
            proofUrl,
    ...
    // STIPEND WEIGHT TOGGLE FOR PARTIAL ATTENDANCE
    const handleWeightToggle = async (learner: any, newWeight: 0.5 | 1.0) => {
        if (isLocked) return;
        ...
        if (attendanceMode === 'bootcamp') {
            ...
            try {
                ...
                setAttendanceList(prev => prev.map(l => {
                    if (l.id === learner.id || l.idNumber === learner.idNumber) {
                        return { ...l, stipendWeight: newWeight, overridden: true };
                    }
                    return l;
                }));

                toast.success(`Stipend payout weight updated to ${newWeight * 100}% for ${learner.fullName}.`);
            } catch (error) {
                toast.error("Failed to update stipend weight.");
            }
        } else {
            if (!recordId) {
                toast.error("No QCTO register found for this date.");
            ...
            try {
                await updateDoc(attRef, {
                    [`stipendWeights.${idNumber}`]: newWeight
                });

                setAttendanceList(prev => prev.map(l => {
                    if (l.idNumber === idNumber) {
                        return { ...l, stipendWeight: newWeight };
                    }
                    return l;
                }));
    ...
    const handleSaveUpdates = async () => {
        ...
        setTimeout(() => {
            setIsLocked(true);
            setIsSaving(false);
            toast.success("Attendance register locked and saved.");
        }, 600);
    };
    ...
    const filteredLearners = useMemo(() => {
        return attendanceList.filter(l => {
            ...
            if (statusFilter !== 'all') {
                matchesStatus = String(l.status).toLowerCase() === statusFilter.toLowerCase();
            }
    ...
    return (
        <div className="cdp-layout">
            ...
            {/* MANDATORY AUDIT & PROOF MODAL */}
            <AttendanceAuditModal
                isOpen={!!auditModalConfig.learner && !!auditModalConfig.targetStatus}
                onClose={() => setAuditModalConfig({ learner: null, targetStatus: null })}
                learner={auditModalConfig.learner}
                targetStatus={auditModalConfig.targetStatus}
                onConfirm={handleConfirmAudit}
            />

            {/* CSS STYLES FOR RING CARDS */}
            <style>{`
                ...
                <div className="cdp-content">
                    <div className="mc-cards-wrapper">
                        <AttendanceRingCard
                            ...
                            theme="w"
                            icon={<Users size={18} />}
                            bar1Label="Present (Full Pay)"
                            bar1Val={`${summaryStats.present}`}
                            bar1Pct={summaryStats.totalAttendees > 0 ? Math.round((summaryStats.present / summaryStats.totalAttendees) * 100) : 0}
                            bar2Label="Partial (Short Hours)"
                            bar2Val={`${summaryStats.partial}`}
                        ...
                        <AttendanceRingCard
                            ...
                            typeLabel="Verified Leave"
                            mainValue={summaryStats.excused}
                            totalValue={`${summaryStats.totalEnrolled} Enrolled`}
                            pct={summaryStats.totalEnrolled > 0 ? Math.round((summaryStats.excused / summaryStats.totalEnrolled) * 100) : 0}
                            theme="p"
                            icon={<FileText size={18} />}
                            bar1Label="Excused (Paid 1.0)"
                            bar1Val={`${summaryStats.excused}`}
                            bar1Pct={summaryStats.totalEnrolled > 0 ? Math.round((summaryStats.excused / summaryStats.totalEnrolled) * 100) : 0}
                            bar2Label="Unexcused (AWOL)"
                            bar2Val={`${summaryStats.absent}`}
                            bar2Pct={summaryStats.totalEnrolled > 0 ? Math.round((summaryStats.absent / summaryStats.totalEnrolled) * 100) : 0}
                            statusText="HR Authorized"
                        />
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1.5rem', background: 'white', padding: '1rem 1.5rem', border: '1px solid var(--mlab-border)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                            <div className="mlab-search" style={{ border: '1px solid #cbd5e1', padding: '8px 16px', display: 'flex', alignItems: 'center', gap: '8px', minWidth: '300px', flex: 1, borderRadius: 0 }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', borderRadius: 0 }}>
                                ...
                                <select value={statusFilter} onChange={(e) => updateUrlParams({ status: e.target.value })}
                                    ...
                                    <option value="partial">Partial (Short Hours)</option>
                                    <option value="excused_absent">Excused Absence (Paid)</option>
                                    <option value="absent">Unexcused Absent (AWOL)</option>
                                </select>
                            </div>

                            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                {hasActiveFilters && (
                                    <button onClick={handleClearFilters} className="mlab-btn mlab-btn--sm animate-fade-in" style={{ background: '#fef2f2', color: '#dc2626', border: '1px solid #fca5a5' }}>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center', borderTop: '1px solid #f1f5f9', paddingTop: '1rem' }}>
                            {isEcosystemEvent && (
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    ...
                                    <select value={locationFilter} onChange={(e) => updateUrlParams({ location: e.target.value })} style={{ padding: '6px 12px', fontSize: '0.85rem', border: '1px solid #cbd5e1', borderRadius: 0, outline: 'none' }}>
                        ...
                        <div className="vp-card" style={{ marginBottom: 0, border: 'none', minHeight: '650px', display: 'flex', flexDirection: 'column' }}>
                            <div className="vp-card-header" style={{ borderBottom: '1px solid var(--mlab-border)', padding: '1rem 1.5rem', background: 'var(--mlab-bg)' }}>
                                <div className="vp-card-title-group" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    ...
                                    <h3 style={{ margin: 0, fontFamily: 'var(--font-heading)', color: 'var(--mlab-blue)', textTransform: 'uppercase', fontSize: '0.9rem' }}>
                                        Showing {filteredLearners.length} Active Records
                                    </h3>
                                </div>
                            </div>
                            ...
                            <div className="mlab-table-wrap" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                                <table className="mlab-table" style={{ tableLayout: 'fixed' }}>
                                    <colgroup>
                                        <col style={{ width: '22%' }} />
                                        {isEcosystemEvent && <col style={{ width: '12%' }} />}
                                        <col style={{ width: '14%' }} />
                                        <col style={{ width: '12%' }} />
                                        <col style={{ width: '22%' }} />
                                        <col style={{ width: '18%' }} />
                                    ...
                                    <tbody>
                                        {paginatedLearners.length > 0 ? paginatedLearners.map(learner => {
                                            ...
                                            const isPartial = learner.status === 'Partial';
                                            const isExcused = learner.status === 'Excused_Absent';

                                            return (
                                                <tr key={learner.idNumber || learner.id} className="animate-fade-in" style={{ borderLeft: isPartial ? '4px solid #f59e0b' : isExcused ? '4px solid #0284c7' : learner.isPresent ? '4px solid var(--mlab-green)' : '4px solid var(--mlab-red)', transition: 'all 0.3s ease' }}>
                                                    ...
                                                    {/* Status Badge */}
                                                    <td>
                                                        <span className={`cdp-status-badge ${learner.status === 'Present' ? 'cdp-status-badge--active' : 'cdp-status-badge--dropped'}`} style={isPartial ? { background: '#fef3c7', color: '#b45309' } : isExcused ? { background: '#e0f2fe', color: '#0369a1' } : {}}>
                                                        ...
                                                        {attendanceMode === 'bootcamp' ? (
                                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                                                ...
                                                                <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                                                                    ...
                                                                    {learner.overridden && (
                                                                        <span style={{ fontSize: '0.65rem', background: '#fff7ed', color: '#c2410c', padding: '2px 6px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                                ...
                                                                {learner.reason && (
                                                                    <span style={{ fontSize: '0.75rem', color: '#0369a1', fontStyle: 'italic', background: '#e0f2fe', padding: '2px 6px', borderRadius: '4px' }}>
                                                                        "{learner.reason}"
                                                                    </span>
                                                                )}

                                                                {learner.evidence?.documentUrl && (
                                                                    <a href={learner.evidence.documentUrl} target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '0.75rem', color: '#2563eb', fontWeight: 'bold', textDecoration: 'none', background: '#eff6ff', padding: '4px 8px', alignSelf: 'flex-start', border: '1px solid #bfdbfe' }}>
                                                            ...
                                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                                                ...
                                                                {(learner.proof || learner.evidence?.documentUrl) && (
                                                                    <a href={learner.proof?.url || learner.evidence?.documentUrl} target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '0.75rem', color: '#2563eb', fontWeight: 'bold', textDecoration: 'none', background: '#eff6ff', padding: '4px 8px', alignSelf: 'flex-start', border: '1px solid #bfdbfe' }}>
                                                                        <DownloadCloud size={12} /> View Attached Proof
                                                                    </a>
                                                                )}
                                                            </div>
                                                    ...
                                                    {/* Stipend Weight Discretion Selector */}
                                                    <td>
                                                        {isPartial ? (
                                                            <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                                                                <button
                                                                    disabled={isLocked}
                                                                    onClick={() => handleWeightToggle(learner, 0.5)}
                                                                    style={{ padding: '3px 6px', fontSize: '0.68rem', fontWeight: 700, background: learner.stipendWeight === 0.5 ? '#f59e0b' : '#f1f5f9', color: learner.stipendWeight === 0.5 ? 'white' : '#64748b', border: '1px solid #f59e0b', cursor: isLocked ? 'not-allowed' : 'pointer' }}
                                                                >
                                                                    0.5 Half
                                                                </button>
                                                                <button
                                                                    disabled={isLocked}
                                                                    onClick={() => handleWeightToggle(learner, 1.0)}
                                                                    style={{ padding: '3px 6px', fontSize: '0.68rem', fontWeight: 700, background: learner.stipendWeight === 1.0 ? '#16a34a' : '#f1f5f9', color: learner.stipendWeight === 1.0 ? 'white' : '#64748b', border: '1px solid #16a34a', cursor: isLocked ? 'not-allowed' : 'pointer' }}
                                                                >
                                                                    1.0 Full
                                                                </button>
                                                            </div>
                                                        ) : (
                                                            <span style={{ fontSize: '0.78rem', fontWeight: 700, color: learner.status === 'Absent' ? '#dc2626' : '#16a34a' }}>
                                                                {learner.status === 'Absent' ? '0.0 Rate (Unpaid)' : '1.0 Rate (Full Pay)'}
                                                            </span>
                                                        )}
                                                    </td>
                                                    ...
                                                    <td style={{ textAlign: 'right' }}>
                                                        <button
                                                            ...
                                                            onClick={() => handleStatusRotate(learner)}
                                                            style={{ cursor: isLocked ? 'not-allowed' : 'pointer', fontSize: '0.72rem', padding: '4px 10px' }}
                                                        >
                                                            Rotate Status
                                                        </button>
                                                    </td>
                                            ...
                                            <tr>
                                                <td colSpan={7} style={{ textAlign: 'center', padding: '3rem', color: 'var(--mlab-grey)' }}>
                                                    <Filter size={32} style={{ margin: '0 auto 1rem', opacity: 0.4 }} />
                                                    <p style={{ margin: 0, fontWeight: 500 }}>No active learners match your current filter settings.</p>
                                                </td>
                                            </tr>
                                        )}
                                    </tbody>
                            ...
                            {/* PAGINATION FOOTER */}
                            {totalPages > 1 && (
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 1.5rem', background: '#f8fafc', borderTop: '1px solid var(--mlab-blue)', marginTop: 'auto' }}>
                                    <div style={{ fontSize: '0.85rem', color: 'var(--mlab-grey)', fontWeight: 500 }}>
                                        Showing <strong>{(currentPage - 1) * ITEMS_PER_PAGE + 1}</strong> to <strong>{Math.min(currentPage * ITEMS_PER_PAGE, filteredLearners.length)}</strong> of <strong>{filteredLearners.length}</strong> active records
                                    </div>
                                    <div style={{ display: 'flex', gap: '8px' }}>
                                        <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="wm-btn wm-btn--ghost" style={{ padding: '6px 12px', border: '1px solid #cbd5e1', display: 'flex', alignItems: 'center', gap: '6px', opacity: currentPage === 1 ? 0.5 : 1 }}>
                                        ...
                                        <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="wm-btn wm-btn--ghost" style={{ padding: '6px 12px', border: '1px solid #cbd5e1', display: 'flex', alignItems: 'center', gap: '6px', opacity: currentPage === totalPages ? 0.5 : 1 }}>
