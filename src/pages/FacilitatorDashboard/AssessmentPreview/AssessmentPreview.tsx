                ...
                <div className="mlab-preview-doc">
                    ...
                    <header className="mlab-preview-header">
                        ...
                        <div className="mlab-preview-header__meta">
                            <span className={`mlab-meta-chip mlab-meta-chip--type-${assessment.type.replace(/\s+/g, '-').toLowerCase()}`}>
                                {assessment.type}
                            </span>
                        ...
                        {blocks.map((block: any) => {
                            ...
                            if (block.type === 'section') return (
                                <div key={block.id} id={`block-${block.id}`} className="mlab-block-section" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                    ...
                                    {block.content && (
                                        <div className="quill-read-only-content" style={{ fontSize: '0.95rem', color: '#dde4e89e' }} dangerouslySetInnerHTML={{ __html: block.content }} />
                                    )}
                            ...
                            if (block.type === 'info') return (
                                <div key={block.id} id={`block-${block.id}`} className="mlab-block-info">
                                    ...
                                    <p className="mlab-block-info__content" style={{ whiteSpace: 'pre-wrap', color: 'grey' }}>{block.content}</p>
                                    {renderBlockImage(block)}
                                </div>
                            );
