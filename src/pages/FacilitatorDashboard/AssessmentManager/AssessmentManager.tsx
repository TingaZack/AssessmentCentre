...
import {
    Plus, Calendar, FileText, Trash2, Edit, PlayCircle, AlertCircle,
    Search, Filter, Copy, Clock, Video, GraduationCap, Users, RefreshCw,
    ArrowUpDown, X, ShieldAlert
} from 'lucide-react';
                    ...
                    <tbody>
                        {filteredAssessments.length > 0 ? (
                            filteredAssessments.map((test: any) => {
                                ...
                                const isProctoredAssessment = test.requiresInvigilation || test.isProctored;
                                    ...
                                    <tr key={test.id}>
                                        ...
                                        <td style={{ textAlign: 'right' }}>
                                            <div className="mlab-icon-btn-group" style={{ justifyContent: 'flex-end' }}>
                                                ...
                                                {/* 🚀 ROLE-AWARE LIVE INVIGILATOR DASHBOARD BUTTON */}
                                                {isProctoredAssessment && test.status !== 'draft' && (
                                                    <button
                                                        className="mlab-icon-btn mlab-icon-btn--green"
                                                        onClick={() => {
                                                            const path = user?.role === 'admin'
                                                                ? `/admin/invigilate/${test.id}`
                                                                : `/facilitator/assessment/invigilate/${test.id}`;
                                                            navigate(path);
                                                        }}
                                                        title="Open Live Invigilator Dashboard"
                                                        disabled={isProcessing}
                                                    >
                                                        <ShieldAlert size={14} />
                                                    </button>
