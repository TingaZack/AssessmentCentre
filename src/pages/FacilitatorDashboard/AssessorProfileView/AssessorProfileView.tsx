...
export const AssessorProfileView: React.FC<ProfileProps> = ({ profile, user, onUpdate, hideSignaturePrompt = false }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(doc(db, 'users', targetId), (docSnap) => {
            if (docSnap.exists()) {
                ...
                // 🚀 DEBUG PRINTS: Keep an eye on incoming structure changes
                console.log("📄 [Assessor Debug] Raw Firestore Data Incoming:", data);
                console.log("📄 [Assessor Debug] documentHistory from DB:", data.documentHistory);

                setLiveProfile((prev: any) => ({
                    ...prev,
                    ...data,
                    documentHistory: data.documentHistory || [] // Ensure fallback array structure
                }));
            }
        });
    ...
    useEffect(() => {
        if (!isEditing && liveProfile) {
            ...
            if (['assessor', 'moderator'].includes(liveProfile.role)) {
                currentDocs.splice(1, 0, { id: 'assessor_cert', name: `${liveProfile.role === 'moderator' ? 'Moderator' : 'Assessor'} Certificate`, file: null, url: uploadedDocsArray.find((d: any) => d.id === 'assessor_cert')?.url || legacyDocs.assessorCertUrl || '', isFixed: true, isRequired: true });
                currentDocs.splice(2, 0, { id: 'reg_letter', name: 'SETA Reg. Letter', file: null, url: uploadedDocsArray.find((d: any) => d.id === 'reg_letter')?.url || legacyDocs.regLetterUrl || '', isFixed: true, isRequired: true });
            } else if (liveProfile.role === 'mentor') {
                currentDocs.splice(1, 0, { id: 'workplace_cert', name: 'Workplace Mentor Certificate', file: null, url: uploadedDocsArray.find((d: any) => d.id === 'workplace_cert')?.url || legacyDocs.workplaceCertUrl || '', isFixed: true, isRequired: false });
            }

            // Conditionally add Work Permit if Foreign National
            if (liveProfile.nationalityType === 'Foreign National') {
                currentDocs.push({ id: 'permit', name: 'Work Permit / Visa', file: null, url: uploadedDocsArray.find((d: any) => d.id === 'permit')?.url || legacyDocs.workPermitUrl || '', isFixed: true, isRequired: true });
            }

            // Append any additional custom documents
            const coreDocIds = ['id', 'cv', 'assessor_cert', 'reg_letter', 'permit', 'workplace_cert'];
    ...
    const executeSave = async () => {
        ...
        try {
            ...
            const newHistory = [...(liveProfile.documentHistory || [])]; // Maintain the audit trail

            for (const docItem of docsList) {
                let finalUrl = docItem.url;

                if (docItem.file) {
                    // Archive the old version into history before overwriting
                    if (docItem.url) {
                        newHistory.push({
                            id: docItem.id,
                ...
                if (finalUrl) {
                    ...
                    if (docItem.id === 'workplace_cert') legacyDocsObject.workplaceCertUrl = finalUrl;
                }
            }
            ...
            const updatedData = {
                ...
                documentHistory: newHistory, // 👈 Save audit log array
    ...
    const isMentor = liveProfile?.role === 'mentor';
    const isRegAuthority = ['assessor', 'moderator'].includes(liveProfile?.role);

    // Get Mentor Company
    const getEmployerName = (employerId?: string) => {
        if (!employerId) return 'Not Linked';
        const emp = employers.find(e => e.id === employerId);
        return emp ? emp.name : 'Unknown Company';
    };

    const renderDocumentVault = () => {
        console.log("📄 [Assessor Debug] Rendering Vault Section. liveProfile.documentHistory =", liveProfile?.documentHistory);
        ...
        return (
            <div className="lpv-vault-links" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '0.5rem' }}>
                        {docsList.map((docItem) => (
                            <div key={docItem.id} style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                ...
                                />
                                {/* 🚀 File preview blocks while actively inside Edit Mode */}
                ...
                {/* 🚀 Document History Log Interface Panel */}
    ...
    return (
        <div className="lpv-wrapper animate-fade-in">
            {/* Overwrite Confirmation Shield Modal */}
            ...
            {isRegAuthority && (
                <div className={`lpv-banner ${liveProfile.assessorRegNumber ? 'lpv-banner--verified' : 'lpv-banner--pending'}`}>
                    <ShieldCheck
                        size={22}
                        className="lpv-banner__icon"
                        color={liveProfile.assessorRegNumber ? 'var(--mlab-green-dark)' : 'var(--mlab-amber)'}
                    />
                    <div>
                        <span className="lpv-banner__title">
                            Registration Status: {liveProfile.assessorRegNumber ? 'Verified & Registered' : 'Pending SETA Registration'}
                        </span>
                        <p className="lpv-banner__desc">
                            {liveProfile.assessorRegNumber
                                ? 'Credentials meet all QCTO regulatory requirements.'
                                : 'Compliance audit in progress. Ensure SETA registration number is provided.'}
                        </p>
                    </div>
                </div>
            )}
            ...
            <div className="lpv-layout">
                <div className="lpv-main-stack">
                    ...
                    <section className="lpv-panel">
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '2rem', padding: '1rem', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                            ...
                            <div>
                                <h4 style={{ margin: '0 0 0.25rem 0', color: '#0f172a', fontSize: '1.05rem' }}>{displayData?.fullName || 'Staff Member'}</h4>
                    ...
                    <section className="lpv-panel">
                        ...
                        <div style={{ padding: '1.5rem', background: '#f8fafc', border: '1px dashed #cbd5e1', borderRadius: '8px', textAlign: 'center' }}>
                            {liveProfile?.signatureUrl ? (
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                    ...
                                    <span style={{ fontSize: '0.7rem', color: liveProfile.signatureColor || '#0f172a', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 'bold', marginTop: '10px' }}>
                                        Active Authenticated Signature
                                    </span>
                                </div>
                            ) : (
                        ...
                        <p style={{ marginTop: '1rem', fontSize: '0.75rem', color: '#64748b', fontStyle: 'italic' }}>
                            Signatures are securely recorded with timestamps for QCTO compliance auditing.
                        </p>
                    </section>

                    {/* Registration & Scope Section */}
                    <section className="lpv-panel">
                        <h3 className="lp-section-title lpv-panel__title--simple" style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '1rem', fontSize: '1rem', color: '#0f172a' }}>
                            <Award size={16} /> Professional Registration
                        </h3>

                        {isMentor ? (
                            <div className="lpv-grid-2">
                                <div className="lpv-field">
                                    <div className="lpv-field__label"><Briefcase size={13} /> Linked Host Company</div>
                                    {isEditing ? (
                                        <select className="lpv-input" value={displayData?.employerId || ''} onChange={e => update('employerId', e.target.value)}>
                                        ...
                                        <div className="lpv-field__value" style={{ fontWeight: 'bold', color: '#b45309' }}>
                                            {getEmployerName(liveProfile?.employerId)}
                                        </div>
                                    )}
                                </div>
                                <EditField label="Job Title / Position" value={displayData?.jobTitle} icon={<User size={13} />} isEditing={isEditing} onChange={val => update('jobTitle', val)} />
                            </div>
                        ) : (
                            <div className="lpv-grid-2">
                                <EditField label="SETA Reg. Number" value={displayData?.assessorRegNumber} icon={<ShieldCheck size={13} />} isEditing={isEditing} onChange={val => update('assessorRegNumber', val)} />
                        ...
                        <div className="lpv-divider" style={{ margin: '1.5rem 0' }} />
                ...
                <aside className="lpv-aside">
                    <div className="lpv-qual-card" style={{ background: isRegAuthority ? 'var(--mlab-red)' : isMentor ? '#b45309' : 'var(--mlab-blue)' }}>
                        ...
                        {isEditing ? (
                            <input
                                type="text"
                                className="lpv-input"
                                style={{ background: 'rgba(255,255,255,0.1)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', marginTop: '8px' }}
                                value={displayData?.highestQualification || ''}
                        ...
                        <div style={{ marginTop: '10px' }}>
                            <span className="lpv-qual-card__saqa" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                                Experience:
                                {isEditing ? (
                                    <input
                                        type="number"
                                        style={{ width: '50px', background: 'white', color: 'black', border: 'none', borderRadius: '4px', padding: '2px 6px', fontSize: '0.8rem' }}
                                        value={displayData?.yearsExperience || 0}
                                        onChange={e => update('yearsExperience', Number(e.target.value))}
                                    />
                                ) : (
                                    `${liveProfile?.yearsExperience || 0} Years`
                                )}
                            </span>
                        </div>
                    </div>
