...
export const MentorDashboard: React.FC = () => {
    ...
    // Accessing updated store properties
    const {
        user,
        learners,
        assessments,
    ...
    useEffect(() => {
        const loadDashboardData = async () => {
            setLoading(true);
            try {
                // Parallel fetch to ensure all workplace data is synced
                await Promise.all([
                    fetchLearners(),
                    fetchAssessments(),
                    fetchSubmissions(),
    ...
    const mentorData = useMemo(() => {
        ...
        submissions.forEach(sub => {
            // 🚀 1. STRICT CHECK: Does this submission specifically belong to THIS mentor?
            if (sub.mentorId !== user.uid) return;

            // 2. Must be a workplace module
            ...
            // 3. Routing based on submission status
...
//     // ─── 🚀 MENTOR FILTERING LOGIC (UPDATED FOR SAFETY) 🚀 ───
//     const mentorData = useMemo(() => {
//         if (!user || !submissions || !assessments) return { pending: [], completed: [] };

//         // 1. Identify only Workplace assessments
//         const workplaceAssessments = assessments.filter(a => a.moduleType === 'workplace');
//         const workplaceAssIds = new Set(workplaceAssessments.map(a => a.id));

//         // 2. Find all learners assigned specifically to this mentor (if enrollments are used)
//         const myLearnerIds = new Set(
//             (enrollments || [])
//                 .filter((e: any) => e.mentorId === user.uid)
//                 .map((e: any) => e.learnerId)
//         );

//         // Determine if we should enforce strict mapping.
//         // If the mentor has NO assigned learners via enrollments, we bypass the strict check 
//         // to prevent a completely broken dashboard, assuming they might just be reviewing cohort-wide.
//         const enforceStrictMapping = myLearnerIds.size > 0;

//         const pending: any[] = [];
//         const completed: any[] = [];

//         submissions.forEach(sub => {
//             // Must be a workplace assessment
//             if (!workplaceAssIds.has(sub.assessmentId)) return;

//             // Strict mapping check (only if they actually have assigned learners)
//             if (enforceStrictMapping && !myLearnerIds.has(sub.learnerId)) return;

//             const assInfo = workplaceAssessments.find(a => a.id === sub.assessmentId);
//             const learnerInfo = learners.find(l => l.learnerId === sub.learnerId || l.id === sub.learnerId);
...
//             // Logic: Mentors review 'submitted' or 'in_progress' workplace tasks
//             if (['submitted', 'in_progress'].includes(status)) {
//                 pending.push(subExtended);
//             }
//             // Logic: 'awaiting_learner_signoff' or beyond means the Mentor has already acted
//             else if (['awaiting_learner_signoff', 'facilitator_reviewed', 'returned', 'graded', 'moderated'].includes(status)) {
//                 // If they completed it, ensure they were the ones who actually graded it OR it belongs to their mapped learner
//                 if (sub.grading?.facilitatorId === user.uid || myLearnerIds.has(sub.learnerId)) {
//                     completed.push(subExtended);
//                 }
//             }
//         });
...
//             completed: completed.sort((a, b) => new Date(b.grading?.facilitatorReviewedAt || 0).getTime() - new Date(a.grading?.facilitatorReviewedAt || 0).getTime())
//         };
//     }, [user, submissions, assessments, enrollments, learners]);
...
//                         <div>
//                             <p style={{ margin: 0, fontSize: '0.75rem', color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Assigned Learners</p>
//                             <h3 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 800, color: '#0f172a' }}>{new Set([...mentorData.pending, ...mentorData.completed].map(s => s.learnerId)).size}</h3>
//                         </div>
//                     </div>
//                 </div>
...
// //     const mentorData = useMemo(() => {
// //         if (!user || !submissions || !assessments || !enrollments) return { pending: [], completed: [] };

// //         // 1. Find all learners assigned specifically to this mentor
// //         const myLearnerIds = new Set(
// //             enrollments
// //                 .filter((e: any) => e.mentorId === user.uid)
// //                 .map((e: any) => e.learnerId)
...
// //         // 2. Identify only Workplace assessments
// //         const workplaceAssIds = new Set(
// //             assessments.filter(a => a.moduleType === 'workplace').map(a => a.id)
// //         );
...
// //         submissions.forEach(sub => {
// //             // Only process if it belongs to an assigned learner AND is a workplace logbook
// //             if (!myLearnerIds.has(sub.learnerId) || !workplaceAssIds.has(sub.assessmentId)) return;
...
// //         return {
// //             pending: pending.sort((a, b) => new Date(b.updatedAt || 0).getTime() - new Date(a.updatedAt || 0).getTime()),
// //             completed: completed.sort((a, b) => new Date(b.grading?.facilitatorReviewedAt || 0).getTime() - new Date(a.grading?.facilitatorReviewedAt || 0).getTime())
// //         };
// //     }, [user, submissions, assessments, enrollments, learners]);
...
// //                                 </div>
// //                                 <h3 style={{ margin: '0 0 4px 0', color: '#334155', fontSize: '1rem', fontWeight: 700 }}>No results found</h3>
// //                                 <p style={{ margin: 0, color: '#64748b', fontSize: '0.85rem' }}>There are currently no workplace logbooks in this category.</p>
// //                             </div>
// //                         ) : (
// //                             <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
// //                                 <thead>
...
// //                                                 <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
// //                                                     <div style={{ width: '36px', height: '36px', background: 'var(--mlab-light-blue)', color: 'var(--mlab-blue)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '0.85rem' }}>
// //                                                         {sub.learner?.fullName?.charAt(0)}
// //                                                     </div>
// //                                                     <div>
// //                                                         <div style={{ fontWeight: 700, color: '#0f172a', fontSize: '0.9rem' }}>{sub.learner?.fullName || 'Unlinked Profile'}</div>
// //                                                         <div style={{ color: '#64748b', fontSize: '0.75rem' }}>ID: {sub.learner?.idNumber || 'N/A'}</div>
// //                                                     </div>
// //                                                 </div>
// //                                             </td>
...
// //                                                             <Clock size={12} /> Awaiting Sign-off
// //                                                         </span>
// //                                                         <span style={{ fontSize: '0.7rem', color: '#94a3b8' }}>Created: {new Date(sub.createdAt).toLocaleDateString()}</span>
// //                                                     </div>
// //                                                 ) : (
// //                                                     <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
// //                                                         <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', color: '#16a34a', background: '#f0fdf4', padding: '4px 8px', borderRadius: '4px', fontSize: '0.7rem', fontWeight: 800, width: 'fit-content' }}>
