...
export const MentorProfileSetup: React.FC = () => {
    ...
    const [smeCertDoc, setSmeCertDoc] = useState<File | null>(null); // Optional Subject Matter Expert / Mentor Cert
...
//     // 🚀 NEW: Check if Date of Birth matches the first 6 digits of SA ID
//     const doesIdMatchDoB = (id?: string, dob?: string) => {
//         if (!id || id.length < 6 || !dob) return false;
...
//             await updateDoc(doc(db, 'users', user.uid), finalData);
//             await refreshUser();
//             navigate('/mentor'); // 🚀 Route directly to the Mentor Dashboard
//         } catch (error) {
//             console.error(error);
//             alert('Compliance sync failed. Please check your connection.');
//         } finally {
...
//     // 🚀 Dynamic Error Checking for SA ID Mismatch
//     const idMismatchError = formData.nationalityType === 'South African' &&
//         formData.idNumber && formData.idNumber.length === 13 &&
//         formData.dateOfBirth &&
//         !doesIdMatchDoB(formData.idNumber, formData.dateOfBirth);
...
//                             <DocUpload label="SME / Mentor Certificate (Optional)" file={smeCertDoc} onUpload={setSmeCertDoc} isOptional={true} />
//                         </div>
...
//                         <div className="lp-actions">
//                             <button className="lp-btn-ghost" onClick={() => setStep(2)}>Back</button>
//                             <button className="lp-btn-primary" onClick={handleSubmit} disabled={loading || !formData.popiaConsent || !idDoc || !cvDoc || (formData.nationalityType === 'Foreign National' && !permitDoc)}>
//                                 {loading ? <Loader2 className="spin" size={15} /> : 'Complete Registration'} <Save size={15} />
//                             </button>
//                         </div>
//                     </div>
...
// // import {
// //     User, Upload, CheckCircle,
// //     Save, ChevronRight, ShieldCheck, Loader2, Camera, Calendar, Fingerprint, Globe, Briefcase, Award
// // } from 'lucide-react';
// // import { doc, updateDoc } from 'firebase/firestore';
// // import { ref, uploadBytes, getDownloadURL } from 'firebase/storage';
// // import { useStore } from '../../../store/useStore';
