...
const mapStatus = (status?: string, marks?: number, totalMarks?: number) => {
    ...
    if (s === "competent" || s === "pass" || s === "c" || s === "moderated") return "Competent";
    if (s === "not yet competent" || s === "not competent" || s === "fail" || s === "nyc") return "Not Yet Competent";
    if (s === "not started" || s === "upcoming") return "Not Started";
    if (s === "pending" || s === "in progress" || s === "submitted" || s === "facilitator_reviewed" || s === "awaiting_learner_signoff") return "In Progress";
...
const StatementOfResults: React.FC = () => {
    ...
    const { learners, cohorts, programmes, settings } = useStore() as any;
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            ...
            try {
                ...
                const storeLearner = learners?.find((l: any) => l.id === id || l.verificationCode === id || l.idNumber === id);
                ...
                if (data && isMounted) {
                    ...
                    const actualUserId = data.userId || data.learnerId || docId;

                    // 🚀 1. DYNAMIC PROGRAMME TEMPLATE LOOKUP
                    const activeCohortId = data.cohortId || data.cohortRunId;
                    const storeCohort = cohorts?.find((c: any) => c.id === activeCohortId);
                    const templateId = storeCohort?.programmeId || storeCohort?.qualificationId || data.programmeId || data.qualificationId;

                    let storeProgramme = programmes?.find((p: any) => p.id === templateId);
                    if (!storeProgramme && data.qualification?.saqaId) {
                        storeProgramme = programmes?.find((p: any) => String(p.saqaId || '') === String(data.qualification.saqaId));
                    }

                    // 🚀 2. FETCH LEARNER SUBMISSIONS FOR DYNAMIC MODULE STATUS
                    let learnerSubs: any[] = [];
                    try {
                        const subQ = query(collection(db, 'learner_submissions'), where('cohortId', '==', activeCohortId || ''));
                        const subSnap = await getDocs(subQ);
                        learnerSubs = subSnap.docs
                            .map(d => d.data())
                            .filter(s => s.learnerId === actualUserId || s.authUid === actualUserId || s.enrollmentId === docId);
                    } catch (e) {
                        console.warn("Submissions lookup for SoR handled:", e);
                    }

                    // 🚀 3. HYDRATE MODULE ARRAYS DYNAMICALLY FROM BLUEPRINT
                    const hydrateCategory = (storedModules: any[], blueprintModules: any[], moduleType: string) => {
                        if (Array.isArray(storedModules) && storedModules.length > 0) {
                            return storedModules.map((m: any) => ({
                                ...m,
                                status: mapStatus(m.status, m.marks, m.totalMarks)
                            }));
                        }

                        if (Array.isArray(blueprintModules) && blueprintModules.length > 0) {
                            return blueprintModules.map((bMod: any) => {
                                const modCode = bMod.code || bMod.moduleCode || '';
                                const matchedSub = learnerSubs.find(s =>
                                    (modCode && (s.moduleNumber === modCode || s.assessmentId?.includes(modCode))) ||
                                    (bMod.name && s.title?.toLowerCase().includes(bMod.name.toLowerCase()))
                                );

                                const status = matchedSub ? mapStatus(matchedSub.status, matchedSub.marks, matchedSub.totalMarks) : "Not Started";
                                const dateAssessed = matchedSub?.submittedAt ? formatDateSA(matchedSub.submittedAt) : (matchedSub?.assignedAt ? formatDateSA(matchedSub.assignedAt) : "Pending");

                                return {
                                    code: modCode,
                                    name: bMod.name || `${moduleType} Module`,
                                    credits: Number(bMod.credits) || 0,
                                    status: status,
                    ...
                    const dynamicKM = hydrateCategory(data.knowledgeModules, storeProgramme?.knowledgeModules, "Knowledge");
                    const dynamicPM = hydrateCategory(data.practicalModules, storeProgramme?.practicalModules, "Practical");
                    const dynamicWM = hydrateCategory(data.workExperienceModules, storeProgramme?.workExperienceModules, "Work Experience");

                    // 🚀 4. DYNAMIC EISA READINESS EVALUATION
                    const allKMCompetent = dynamicKM.length > 0 && dynamicKM.every(m => m.status === 'Competent');
                    const calculatedEisaAdmission = data.eisaAdmission !== undefined ? data.eisaAdmission : allKMCompetent;
                    ...
                    setLearnerData({
                        ...
                        qualification: {
                            name: storeProgramme?.name || data.qualification?.name || "Occupational Certificate",
                            saqaId: storeProgramme?.saqaId || data.qualification?.saqaId || "N/A",
                            credits: (storeProgramme?.credits || data.qualification?.credits || 0).toString(),
                            nqfLevel: `Level ${storeProgramme?.nqfLevel || data.qualification?.nqfLevel || 5}`,
                            dateAssessed: data.qualification?.dateAssessed || finalIssueDate
                        },
                        knowledgeModules: dynamicKM,
                        practicalModules: dynamicPM,
                        workExperienceModules: dynamicWM,
                        eisaAdmission: calculatedEisaAdmission,
    ...
    }, [id, learners, cohorts, programmes]);

    const displayCampus = useMemo(() => {
        if (learnerData?.cohortId && settings?.campuses) {
            const cohort = cohorts?.find((c: any) => c.id === learnerData.cohortId);
            if (cohort?.campusId) {
                const specificCampus = settings.campuses.find((c: any) => c.id === cohort.campusId);
                if (specificCampus) return specificCampus;
            }
        }
        if (settings?.campuses && settings.campuses.length > 0) {
            const defaultCampus = settings.campuses.find((c: any) => c.isDefault) || settings.campuses[0];
    ...
    // Public web3 verification stays on the frontend (Read-only)
    useEffect(() => {
    ...
    // ==========================================
    // THE FULL-STACK MINTING PROCESS
    // ==========================================
                ...
                <div className="no-print status-banner-container">
                    ...
                    {verificationStatus === 'idle' && !learnerData.ipfsHash && (
                        <div className="status-banner banner-warning">
                            <span className="banner-icon">⏳</span>
                            <div>
                                <p className="banner-title">Provisional / In-Progress SoR</p>
                                <p className="banner-sub">This record reflects live assessment progress and has not yet been minted to the blockchain.</p>
                            </div>
                        </div>
                    )}
                    {mintingStatus === 'error' && (
                        ...
                        <div className="letterhead-content">
                            <div className="provider-info">
                                ...
                                <div className="provider-details">
                                    ...
                                    <div style={{ fontWeight: '600', color: 'var(--mlab-green)' }}>
                                        {displayCampus.name}
                                    </div>
