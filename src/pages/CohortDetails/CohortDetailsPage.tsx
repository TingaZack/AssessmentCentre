...
export const CohortDetailsPage: React.FC = () => {
    ...
    // 🚀 NEW STATE: Toggle inside the Curriculum tab
    ...
    // 🚀 NEW STATE: Store expanded module accordion states in the history view
    ...
    // 🚀 NEW LOGIC: Grouping historical logs
    ...
    // 🚀 NEW TOGGLE FOR HISTORY ACCORDION
    const toggleHistoryAccordion = (moduleCode: string) => {
        setExpandedHistoryModules(prev => {
            const next = new Set(prev);
            if (next.has(moduleCode)) next.delete(moduleCode);
            else next.add(moduleCode);
            return next;
                        ...
                        <div className="animate-fade-in" style={{ paddingBottom: selectedTopicCount > 0 ? '80px' : '0' }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '1rem' }}>
                                <div style={{ background: 'white', display: 'inline-flex', padding: '4px', borderRadius: '8px', border: '1px solid var(--mlab-border)', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
                                    <button 
                                        onClick={() => setCurriculumViewMode('blueprint')}
                                        style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 600, fontSize: '0.8rem', background: curriculumViewMode === 'blueprint' ? 'var(--mlab-blue)' : 'transparent', color: curriculumViewMode === 'blueprint' ? 'white' : 'var(--mlab-grey)' }}
                                    >
                                        <CheckSquare size={14} /> Log New Topics
                                    </button>
                                    <button 
                                        onClick={() => setCurriculumViewMode('history')}
                                        style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 600, fontSize: '0.8rem', background: curriculumViewMode === 'history' ? 'var(--mlab-blue)' : 'transparent', color: curriculumViewMode === 'history' ? 'white' : 'var(--mlab-grey)' }}
                                    >
                                        <Clock size={14} /> View Past Sessions
                                    </button>
                                </div>
                            </div>
                            ...
                            <div style={{ borderRadius: '8px', overflow: 'hidden', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}>
                                <div className="lfm-header">
                                    <h2 className="lfm-header__title">
                                        <BookOpen size={18} /> 
                                        {curriculumViewMode === 'blueprint' ? 'Curriculum Blueprint Tracker' : 'Past Session Reports & Traceability'}
                                    </h2>
                                </div>
                                                            ...
                                                            return (
                                                                <div key={`hist-${modCode}`}>
                                                                    <div 
                                                                        ...
                                                                        style={{ cursor: 'pointer', borderBottomColor: 'var(--mlab-blue)', opacity: isOpen ? 1 : 0.85 }} 
                                                                        onClick={() => toggleHistoryAccordion(modCode)}
                                                                    >
                                                                        ...
                                                                        <span style={{ fontSize: '0.7rem', color: 'var(--mlab-blue)', letterSpacing: '0.1em', fontWeight: 700, background: '#eff6ff', padding: '2px 8px', borderRadius: '4px' }}>
                                                                            {logsInModule.length} TOPIC{logsInModule.length !== 1 ? 'S' : ''} LOGGED
                                                                        </span>
                                                                        {isOpen ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                                                                    </div>
                                                                        ...
                                                                        <div className="animate-fade-in" style={{ borderTop: '2px solid var(--mlab-blue)', paddingTop: '15px', marginTop: '-0.75rem' }}>
                                                                            {logsInModule.map(log => {
                                                                                const mappedLog = {
                                                                                    ...
                                                                                    dateLogged: log.coveredAt || log.dateLogged,
                                                                                    notes: log.systemNote || "Session details available in full report.",
                                                                                    presentLearnerIds: log.presentLearnerIds,
                                                                                    absentLearnerIds: log.absentLearnerIds,
                                                                                    acknowledgedBy: log.acknowledgedBy
                                                                                };

                                                                                return (
                                                                                    <CurriculumTraceabilityCard 
                                                                                        key={log.id} 
                                                                                        log={mappedLog} 
                                                                                        roster={enrolledLearners} 
                                                                                    />
                                                                                );
                                                                            })}
                                                                        </div>
...
//                 fetchedLogs.sort((a: any, b: any) => new Date(b.coveredAt || b.dateLogged).getTime() - new Date(a.coveredAt || a.dateLogged).getTime());
//                 setCurriculumLogs(fetchedLogs);
//             });
...
//                 await batch.commit();
//                 setSelectedTopics({});
//                 // 🚀 Automatically switch view to history after saving
//                 setCurriculumViewMode('history');
//                 showStatusPopup('success', 'Topics Logged & Published', `Session Report saved and ${selectedTopicIds.length} topics published to Learners.`);
...
//                                             {/* ─── VIEW 2: HISTORY / TRACEABILITY CARDS ─── */}
//                                             {curriculumViewMode === 'history' && (
...
//                                                             // Re-map the log so it matches the expected CurriculumLogData shape for the component
//                                                             const mappedLog = {
//                                                                 id: log.id,
//                                                                 title: log.topicTitle,
//                                                                 moduleCode: log.moduleCode,
