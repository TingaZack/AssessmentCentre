...
import {
    ...
    ChevronDown, ChevronUp, Sparkles, Link as LinkIcon, Plus, Trash2, Edit3, FileText, X, PenTool, TrendingUp
} from 'lucide-react';
                ...
                const generateFromGemini = async () => {
                    ...
                    try {
                        ...
                        if (data.success && data.html) {
                            ...
                            onShowStatus('success', 'AI Generation Complete', 'Gemini has successfully drafted your lesson plan using your teaching preferences. Please review and edit before saving.');
                        } else {
                        ...
                        let errorMessage = "Gemini AI is currently unavailable due to a network error. We have loaded a standard QCTO template for you.";
                        if (error.message && error.message.includes('429')) {
                            errorMessage = "Gemini AI Free Tier rate limit exceeded (Too Many Requests). We have loaded a standard QCTO template for you to use instead.";
                        }
    ...
    const handleEnhanceText = async () => {
        ...
        try {
            ...
            if (data.success && data.text) {
                ...
                onShowStatus('success', 'Text Enhanced', 'The highlighted text has been professionally rewritten by Gemini AI.');
            }
        } catch (error) {
            console.error("Enhance error:", error);
            onShowStatus('error', 'Enhancement Failed', 'Gemini AI is currently busy or unavailable. Please try again in a few moments.');
        } finally {
...
export const CohortDetailsPage: React.FC = () => {
    ...
    // 🚀 UNIFIED SAVE HANDLER (Create & Update)
                                                                    ...
                                                                    <table className="mlab-table" style={{ margin: 0, border: 'none' }}>
                                                                        ...
                                                                        <tbody>
                                                                            {group.items.map(item => {
                                                                                ...
                                                                                // 🚀 FIND ASSOCIATED REPORT FOR THIS SPECIFIC LOG 🚀
...
//     Briefcase, FolderOpen, UserMinus, AlertCircle, ChevronLeft,
//     CheckCircle, BookOpen, Building2, Award, Timer, Layers, LayoutList,
//     ChevronDown, ChevronUp, Sparkles, Link as LinkIcon, Plus, Trash2, Edit3, FileText, X, PenTool
// } from 'lucide-react';
// import { writeBatch, doc, collection, query, where, getDocs, increment, onSnapshot, updateDoc } from 'firebase/firestore';
// import { getFunctions, httpsCallable } from 'firebase/functions';
// import * as XLSX from 'xlsx';
...
//     const activeProgramme = useMemo(() => {
//         if (!cohort || !programmes.length) return null;
//         const templateId = (cohort as any).programmeId || (cohort as any).qualificationId;
//         return programmes.find(p => p.id === templateId || (p as any).saqaId === templateId) || null;
//     }, [cohort, programmes]);
...
// //     Briefcase, FolderOpen, UserMinus, AlertCircle, ChevronLeft,
// //     CheckCircle, BookOpen, Building2, Award, Timer, Layers, LayoutList,
// //     ChevronDown, ChevronUp, Sparkles, Link as LinkIcon, Plus, Trash2, Edit3, FileText, X
// // } from 'lucide-react';
// // import { writeBatch, doc, collection, query, where, getDocs, increment, onSnapshot, updateDoc } from 'firebase/firestore';
// // import { getFunctions, httpsCallable } from 'firebase/functions';
// // import * as XLSX from 'xlsx';
...
// //         ['clean']
// //     ],
// //     // Ensure the table module is active in the background
// //     table: true
// // };
...
// // // ─── AI LESSON PLAN MODAL (CONNECTED TO GEMINI) ───────────────────────────────
// // const AILessonPlanModal: React.FC<{
// //     isOpen: boolean;
// //     onClose: () => void;
// //     onSave: (planHtml: string, evidence: any[], isEdit: boolean, reportId?: string) => void;
...
// //     cohort: any;
// //     user: any;
// //     existingReport: any | null; // Pass null for new reports
// // }> = ({ isOpen, onClose, onSave, selectedTopics, curriculumItems, activeProgramme, cohort, user, existingReport }) => {
// //     const [isGenerating, setIsGenerating] = useState(true);
// //     const [isEnhancing, setIsEnhancing] = useState(false);
// //     const [planHtml, setPlanHtml] = useState('');
// //     const [evidenceItems, setEvidenceItems] = useState<{ url: string, description: string }[]>([{ url: '', description: '' }]);
...
// //     useEffect(() => {
// //         // If the modal is closed, reset the gatekeeper
// //         if (!isOpen) {
// //             hasGeneratedRef.current = false;
// //             return;
// //         }
...
// //         if (existingReport) {
// //             // 🚀 EDIT MODE: Instantly load existing data
// //             setPlanHtml(existingReport.reportHtml || '');
// //             setEvidenceItems(existingReport.evidenceLinks?.length ? existingReport.evidenceLinks : [{ url: '', description: '' }]);
// //             setIsGenerating(false);
// //             hasGeneratedRef.current = true;
// //         } else {
// //             // 🚀 CREATE MODE: Generate via REAL Gemini Cloud Function
// //             if (!hasGeneratedRef.current) {
// //                 hasGeneratedRef.current = true;
...
// //                     // 1. Prepare all variables outside the try block so the catch block can use them!
// //                     const selectedDefs = Object.keys(selectedTopics).map(id => curriculumItems.find(i => i.id === id)).filter(Boolean);
// //                     const moduleNames = Array.from(new Set(selectedDefs.map(d => d.moduleName))).join(', ');
// //                     const topicList = selectedDefs.map(d => `<li style="color: #000000;">${d.code ? `${d.code}: ` : ''}${d.title}</li>`).join('');
// //                     const savedTeachingStyle = user?.preferences?.teachingStyle || "Interactive lecture followed by pair-programming and practical lab exercises.";
// //                     const savedResources = user?.preferences?.resources || "Laptops, VS Code IDE, GitHub Repository, Slide Deck, Internet Access.";
...
// //                     } catch (error) {
// //                         console.error("AI Generation Failed:", error);
// //                         toast.info("AI Generation unavailable. Loaded standard template instead.", { duration: 4000 });

// //                         // 🚀 FALLBACK TEMPLATE: Generated locally if AI fails (e.g. rate limits)
// //                         const fallbackTemplate = `
// //                             <h3 style="color: #000000;">1. Programme & Module Information</h3>
// //                             <p style="color: #000000;"><strong>Programme:</strong> ${activeProgramme?.name || cohort?.name}</p>
// //                             <p style="color: #000000;"><strong>NQF Level:</strong> ${activeProgramme?.nqfLevel || 'N/A'}</p>
// //                             <p style="color: #000000;"><strong>Module(s):</strong> ${moduleNames}</p>
...
// //                             <h3 style="color: #000000;">5 & 10. Teaching Activities & Facilitation Approach</h3>
// //                             <p style="color: #000000;">${savedTeachingStyle}</p>

// //                             <h3 style="color: #000000;">6. Resources & Materials</h3>
// //                             <p style="color: #000000;">${savedResources}</p>

// //                             <h3 style="color: #000000;">7. Time Allocation</h3>
// //                             <ul style="color: #000000;">
// //                                 <li style="color: #000000;"><strong>Theory & Instruction:</strong> 30%</li>
// //                                 <li style="color: #000000;"><strong>Practical Application:</strong> 60%</li>
// //                                 <li style="color: #000000;"><strong>Assessment / Q&A:</strong> 10%</li>
// //                             </ul>

// //                             <h3 style="color: #000000;">8 & 9. Assessment Strategy & Evidence</h3>
// //                             <p style="color: #000000;">Formative observation during practical labs. Code commits and tasks uploaded to the Learner Portal.</p>

// //                             <h3 style="color: #000000;">11. Learner Support & Remediation</h3>
// //                             <p style="color: #000000;">One-on-one coaching provided during lab time for learners requiring additional assistance.</p>

// //                             <h3 style="color: #000000;">12. Evaluation & Reflection</h3>
// //                             <p style="color: #000000;"><em>(Facilitator to add brief notes here...)</em> Session objectives met. Learners engaged well with the practical material.</p>

// //                             <h3 style="color: #000000;">13. Alignment to Workplace</h3>
// //                             <p style="color: #000000;">Skills directly translate to Agile software development tasks and Jira ticket execution.</p>
...
// //             }
// //         }
// //     }, [isOpen, existingReport, selectedTopics, curriculumItems, activeProgramme, cohort, user, toast]);
...
// //         const range = editor.getSelection();
// //         if (!range || range.length === 0) {
// //             return toast.info("Please highlight the text you want the AI to enhance.");
// //         }
...
// //             if (data.success && data.text) {
// //                 // Delete old text and insert the new AI text
// //                 editor.deleteText(range.index, range.length);
// //                 editor.insertText(range.index, data.text);

// //                 // Update React state to match editor
// //                 setPlanHtml(editor.root.innerHTML);
...
// //                 <div className="lfm-body" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', background: '#f8fafc', padding: 0 }}>
// //                     {isGenerating ? (
// //                         <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '400px', color: 'var(--mlab-blue)' }}>
// //                             <Loader2 size={40} className="lfm-spin" style={{ marginBottom: '1rem' }} />
// //                             <h3 style={{ fontFamily: 'var(--font-heading)', textTransform: 'uppercase' }}>Gemini AI is drafting your report...</h3>
// //                             <p style={{ color: 'var(--mlab-grey)' }}>Pulling cohort data, topics, and your past preferences.</p>
// //                         </div>
// //                     ) : (
...
// //                                         disabled={isEnhancing}
// //                                         className="lfm-btn"
// //                                         style={{ background: '#fdf4ff', color: '#c026d3', border: '1px solid #f0abfc', borderRadius: '4px', padding: '6px 12px', fontSize: '0.75rem' }}
// //                                     >
// //                                         {isEnhancing ? <Loader2 size={14} className="lfm-spin" /> : <Sparkles size={14} />}
// //                                         Enhance Highlighted Text
// //                                     </button>
...
// //                                 {/* 🚀 FIX: Forced color: '#000000' so the text is fully visible 🚀 */}
// //                                 <div style={{ background: 'white', paddingBottom: 16, color: '#000000', border: '1px solid var(--mlab-border)', borderRadius: '8px', flex: 1, display: 'flex', flexDirection: 'column' }}>
// //                                     <ReactQuill
// //                                         ref={quillRef}
// //                                         theme="snow"
// //                                         value={planHtml}
...
// //                             <div style={{ flex: 1, padding: '1.5rem', background: 'white', overflowY: 'auto' }}>
// //                                 <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '1rem', color: 'var(--mlab-blue)', textTransform: 'uppercase', margin: '0 0 1rem' }}>
// //                                     <LinkIcon size={16} style={{ display: 'inline', verticalAlign: 'text-bottom', marginRight: '6px' }} />
// //                                     Session Evidence
// //                                 </h3>
...
// //                 await batch.commit();
// //                 setSelectedTopics({});
// //                 toast.success(`Session Report saved and ${selectedTopicIds.length} topics published to Learners.`);

// //             } catch (error) {
// //                 console.error("Failed to log curriculum:", error);
// //                 toast.error("Failed to publish topics. Please check your connection.");
// //             } finally {
// //                 setIsLogging(false);
// //             }
// //         }
...
// //     return (
// //         <div className="cdp-layout">
// //             {modalConfig.isOpen && createPortal(<StatusModal type={modalConfig.type} title={modalConfig.title} message={modalConfig.message} onClose={() => setModalConfig(p => ({ ...p, isOpen: false }))} />, document.body)}
// //             {learnerToPlace && createPortal(<WorkplacePlacementModal learner={learnerToPlace} onClose={() => setLearnerToPlace(null)} />, document.body)}
...
// //                             {/* 🚀 PAST SESSION REPORTS SECTION 🚀 */}
// //                             {sessionReports.length > 0 && (
// //                                 <div className="vp-card" style={{ marginBottom: '1.5rem', borderLeft: '4px solid var(--mlab-green)' }}>
// //                                     <div className="vp-card-header" style={{ padding: '12px 16px', background: '#f0fdf4' }}>
// //                                         <div className="vp-card-title-group">
// //                                             <FileText size={18} color="var(--mlab-green)" />
// //                                             <h3 style={{ margin: 0, fontFamily: 'var(--font-heading)', color: 'var(--mlab-green)', textTransform: 'uppercase' }}>
...
// //                                                     <tr key={report.id}>
// //                                                         <td style={{ fontWeight: 600, color: 'var(--mlab-blue)' }}>
// //                                                             {new Date(report.dateLogged).toLocaleDateString('en-ZA', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}
// //                                                         </td>
// //                                                         <td>{report.facilitatorName}</td>
// //                                                         <td><span className="mlab-badge" style={{ background: '#f1f5f9' }}>{report.topicsCovered?.length || 0} Topics</span></td>
// //                                                         <td style={{ textAlign: 'right' }}>
// //                                                             <button
...
// //                                                                         <thead style={{ background: 'var(--mlab-light-blue)' }}>
// //                                                                             <tr>
// //                                                                                 <th style={{ width: '50px', textAlign: 'center', borderRight: '1px solid var(--mlab-border)' }}>Log</th>
// //                                                                                 <th>Topic / Activity</th>
// //                                                                                 <th style={{ width: '200px' }}>Status / Date</th>
...
// //                                                                                         <td>
// //                                                                                             {isLogged ? (
// //                                                                                                 <span style={{ display: 'inline-block', background: '#dcfce7', color: '#166534', border: '1px solid #bbf7d0', padding: '4px 8px', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase' }}>Covered {new Date(logRecord.coveredAt).toLocaleDateString('en-ZA', { month: 'short', day: 'numeric' })}</span>
// //                                                                                             ) : isSelected ? (
// //                                                                                                 <input type="date" className="lfm-input" value={selectedTopics[item.id]} max={new Date().toISOString().split('T')[0]} onChange={(e) => handleIndividualDateChange(item.id, e.target.value)} style={{ padding: '4px 8px', fontSize: '0.75rem', width: '130px', height: '28px' }} />
// //                                                                                             ) : <span style={{ display: 'inline-block', background: '#f1f5f9', color: '#64748b', border: '1px solid #cbd5e1', padding: '4px 8px', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase' }}>Pending</span>}
// //                                                                                         </td>
// //                                                                                         <td>
// //                                                                                             {isLogged ? (
// //                                                                                                 <div className="cdp-progress-col" style={{ width: '100%' }}>
// //                                                                                                     <div className="cdp-progress-text"><span style={{ fontFamily: 'var(--font-heading)' }}>{logRecord?.acknowledgedBy?.length || 0} / {activeCount} ACKD</span><span style={{ fontWeight: 'bold', color: ackPct >= 80 ? 'var(--mlab-green)' : (ackPct >= 50 ? '#f59e0b' : '#ef4444') }}>{ackPct}%</span></div>
// //                                                                                                     <div className="cdp-progress-track" style={{ height: '4px', borderRadius: '0' }}><div className="cdp-progress-fill" style={{ width: `${ackPct}%`, background: ackPct >= 80 ? 'var(--mlab-green)' : (ackPct >= 50 ? '#f59e0b' : '#ef4444'), borderRadius: '0' }} /></div>
// //                                                                                                 </div>
// //                                                                                             ) : <span style={{ fontSize: '0.8rem', color: 'var(--mlab-grey-light)' }}>—</span>}
// //                                                                                         </td>
...
// // //     Users, Calendar, Clock, Loader2, RefreshCcw, DownloadCloud,
// // //     Briefcase, FolderOpen, UserMinus, AlertCircle, ChevronLeft,
// // //     CheckCircle, BookOpen, Building2, Award, Timer, Layers, LayoutList, ChevronDown, ChevronUp
// // // } from 'lucide-react';
// // // import { writeBatch, doc, collection, query, where, getDocs, increment, onSnapshot } from 'firebase/firestore';
// // // import * as XLSX from 'xlsx';
// // // import { db } from '../../lib/firebase';
// // // import { useStore } from '../../store/useStore';
// // // import { Sidebar } from '../../components/dashboard/Sidebar/Sidebar';
...
// // //                 if (!groups[modCode]) {
// // //                     groups[modCode] = { moduleName: mod.name || 'Unnamed Module', moduleType: type, items: [] };
// // //                 }
...
// // //                         id: uniqueId,
// // //                         code: sub.code || '',
// // //                         title: sub.title || sub.name || sub.description || 'Unnamed Item',
// // //                         moduleCode: modCode,
// // //                         moduleName: mod.name || '',
// // //                         moduleType: type,
// // //                         weight: sub.weight || sub.percentage || ''
// // //                     });
// // //                 });
// // //             });
// // //         };
...
// // //     const handleLogCurriculum = async () => {
// // //         const selectedTopicIds = Object.keys(selectedTopics);
// // //         if (selectedTopicIds.length === 0) return;
// // //         setIsLogging(true);
...
// // //                 for (const modCode in groupedCurriculum) {
// // //                     const match = groupedCurriculum[modCode].items.find(i => i.id === topicId);
// // //                     if (match) { itemDef = match; break; }
// // //                 }
// // //                 if (!itemDef) return;
...
// // //             await batch.commit();
// // //             setSelectedTopics({});
// // //             toast.success(`Successfully published ${selectedTopicIds.length} topics to the Learner Dashboards.`);
...
// // //         } catch (e: any) { setModalConfig({ isOpen: true, type: 'error', title: 'Sync Failed', message: e.message }); }
...
// // //                     <div className="lfm-tabs" style={{ marginBottom: '1.5rem' }}>
// // //                         <button
// // //                             className={`lfm-tab ${activeTab === 'learners' ? 'active' : ''}`}
// // //                             onClick={() => setActiveTab('learners')}
// // //                         >
// // //                             <Users size={16} /> Learner Roster
// // //                         </button>
// // //                         <button
// // //                             className={`lfm-tab ${activeTab === 'curriculum' ? 'active' : ''}`}
...
// // //                                                                 {isAdmin && (
// // //                                                                     <button
// // //                                                                         className={`mlab-btn mlab-btn--sm ${isPlaced ? 'mlab-btn--ghost' : 'mlab-btn--primary'}`}
...
// // //                                                                 {isAdmin && !isDropped && (
// // //                                                                     <button
// // //                                                                         className="mlab-btn mlab-btn--sm mlab-btn--error"
...
// // //                                 <div className="lfm-header">
// // //                                     <h2 className="lfm-header__title">
// // //                                         <BookOpen size={18} /> Curriculum Blueprint Tracker
// // //                                     </h2>
// // //                                 </div>
...
// // //                                         <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--mlab-grey)' }}>
// // //                                             <AlertCircle size={40} style={{ margin: '0 auto 1rem', opacity: 0.5 }} />
// // //                                             <p>The linked blueprint ({activeProgramme.name}) has no topics defined.</p>
// // //                                         </div>
// // //                                     ) : (
// // //                                         <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
// // //                                             {Object.keys(groupedCurriculum).map(modCode => {
...
// // //                                                             style={{ cursor: 'pointer', borderBottomColor: isComplete ? 'var(--mlab-green)' : 'var(--mlab-blue)', opacity: isOpen ? 1 : 0.85 }}
...
// // //                                                                                 <th style={{ width: '50px', color: 'grey', textAlign: 'center', borderRight: '1px solid var(--mlab-border)' }}>Log</th>
// // //                                                                                 <th style={{ color: 'grey' }}>Topic / Activity</th>
// // //                                                                                 <th style={{ width: '200px', color: 'grey' }}>Status / Date</th>
// // //                                                                                 <th style={{ width: '180px', color: 'grey' }}>Engagement</th>
// // //                                                                             </tr>
// // //                                                                         </thead>
// // //                                                                         <tbody>
...
// // //                                                                                 const acknowledgedCount = logRecord?.acknowledgedBy?.length || 0;
// // //                                                                                 const ackPct = activeCount > 0 ? Math.round((acknowledgedCount / activeCount) * 100) : 0;

// // //                                                                                 return (
// // //                                                                                     <tr key={item.id} style={{ background: isLogged ? '#f0fdf4' : (isSelected ? '#eff6ff' : 'white'), transition: 'background 0.2s' }}>
// // //                                                                                         <td style={{ textAlign: 'center', borderRight: '1px solid var(--mlab-border)' }}>
// // //                                                                                             {isLogged ? (
// // //                                                                                                 <CheckCircle size={18} color="var(--mlab-green)" style={{ margin: '0 auto' }} />
// // //                                                                                             ) : (
// // //                                                                                                 <div className="lfm-checkbox-row" style={{ justifyContent: 'center' }}>
// // //                                                                                                     <input
// // //                                                                                                         type="checkbox"
// // //                                                                                                         checked={isSelected}
// // //                                                                                                         onChange={() => toggleTopicSelection(item.id)}
// // //                                                                                                     />
// // //                                                                                                 </div>
// // //                                                                                             )}
// // //                                                                                         </td>
...
// // //                                                                                         <td>
// // //                                                                                             {isLogged ? (
// // //                                                                                                 <span style={{ display: 'inline-block', background: '#dcfce7', color: '#166534', border: '1px solid #bbf7d0', padding: '4px 8px', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
// // //                                                                                                     Covered {new Date(logRecord.coveredAt).toLocaleDateString('en-ZA', { month: 'short', day: 'numeric' })}
// // //                                                                                                 </span>
// // //                                                                                             ) : isSelected ? (
// // //                                                                                                 <input
// // //                                                                                                     type="date"
// // //                                                                                                     className="lfm-input"
// // //                                                                                                     value={selectedTopics[item.id]}
// // //                                                                                                     max={new Date().toISOString().split('T')[0]}
// // //                                                                                                     onChange={(e) => handleIndividualDateChange(item.id, e.target.value)}
// // //                                                                                                     style={{ padding: '4px 8px', fontSize: '0.75rem', width: '130px', height: '28px' }}
// // //                                                                                                 />
// // //                                                                                             ) : (
// // //                                                                                                 <span style={{ display: 'inline-block', background: '#f1f5f9', color: '#64748b', border: '1px solid #cbd5e1', padding: '4px 8px', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
// // //                                                                                                     Pending
// // //                                                                                                 </span>
// // //                                                                                             )}
// // //                                                                                         </td>
...
// // //                                                                                                         <span style={{ fontFamily: 'var(--font-heading)', textTransform: 'uppercase' }}>{acknowledgedCount} / {activeCount} ACKD</span>
// // //                                                                                                         <span style={{ fontWeight: 'bold', color: ackPct >= 80 ? 'var(--mlab-green)' : (ackPct >= 50 ? '#f59e0b' : '#ef4444') }}>{ackPct}%</span>
// // //                                                                                                     </div>
// // //                                                                                                     <div className="cdp-progress-track" style={{ height: '4px', borderRadius: '0' }}>
// // //                                                                                                         <div className="cdp-progress-fill" style={{ width: `${ackPct}%`, background: ackPct >= 80 ? 'var(--mlab-green)' : (ackPct >= 50 ? '#f59e0b' : '#ef4444'), borderRadius: '0' }} />
// // //                                                                                                     </div>
// // //                                                                                                 </div>
// // //                                                                                             ) : (
// // //                                                                                                 <span style={{ fontSize: '0.8rem', color: 'var(--mlab-grey-light)' }}>—</span>
...
// // //                             {/* 🚀 FLOATING ACTION BAR FOR BATCH LOGGING (STRICT LFM STYLING) 🚀 */}
// // //                             {selectedTopicCount > 0 && createPortal(
...
// // //                                     borderTop: '3px solid var(--mlab-green)',
// // //                                     padding: '1rem 2rem',
// // //                                     display: 'flex',
...
// // //                                             <CheckCircle size={24} color="var(--mlab-green)" />
// // //                                         </div>
// // //                                         <div style={{ display: 'flex', flexDirection: 'column' }}>
// // //                                             <span style={{ fontFamily: 'var(--font-heading)', fontSize: '1.1rem', fontWeight: 700, color: 'var(--mlab-white)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
...
// // //                                                 Learners will have 48 hours to acknowledge.
// // //                                             </span>
// // //                                         </div>
// // //                                     </div>

// // //                                     <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', flexWrap: 'wrap' }}>
// // //                                         <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 12px', border: '1px solid rgba(255,255,255,0.1)' }}>
// // //                                             <label style={{ fontFamily: 'var(--font-heading)', fontSize: '0.75rem', color: 'var(--mlab-green)', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700 }}>
// // //                                                 Batch Date:
// // //                                             </label>
// // //                                             <input
// // //                                                 type="date"
// // //                                                 className="lfm-input"
// // //                                                 value={globalCoveredDate}
// // //                                                 max={new Date().toISOString().split('T')[0]}
// // //                                                 onChange={handleGlobalDateChange}
// // //                                                 style={{ width: '130px', padding: '4px 8px', height: 'auto', fontSize: '0.8rem', background: 'var(--mlab-white)', color: 'var(--mlab-blue)' }}
// // //                                             />
// // //                                         </div>
// // //                                         <button
// // //                                             className="lfm-btn"
// // //                                             onClick={() => setSelectedTopics({})}
// // //                                             style={{ background: 'transparent', color: 'var(--mlab-white)', border: '2px solid rgba(255,255,255,0.3)' }}
// // //                                         >
// // //                                             Cancel
// // //                                         </button>
// // //                                         <button
// // //                                             className="lfm-btn"
// // //                                             onClick={handleLogCurriculum}
// // //                                             disabled={isLogging}
// // //                                             style={{ background: 'var(--mlab-green)', color: 'var(--mlab-blue)', border: '2px solid var(--mlab-green)' }}
// // //                                         >
// // //                                             {isLogging ? <Loader2 size={16} className="lfm-spin" /> : <Award size={16} />}
