...
import { doc, writeBatch, updateDoc, collection, where, query, getDocs } from 'firebase/firestore';
...
import { QmsComplianceDashboard } from './Compliance/QmsComplianceDashboard'; // 🚀 Added Dashboard import
...
type NavTabs = 'directory' | 'learners' | 'staff' | 'qualifications' | 'cohorts' |
    ...
    'assessments' | 'settings' | 'attendance' | 'ecosystem' | 'company-profile' | 'coaching' | 'surveys' | 'interview-studio' | 'content' | 'workplace-verification' | 'compliance';
    ...
    const checkAccess = (tab: NavTabs) => {
        ...
        if (adminRoles.includes(activeRole)) {
            ...
            const accessMap: Record<string, boolean> = {
                ...
                'compliance': privs.settings !== false, // 🚀 Bind Compliance access to settings/super
    ...
    return (
        <div className="admin-layout">
            ...
            <main className={`main-wrapper ${currentNav === 'workplace-verification' ? 'mentor-verify-override' : ''}`} style={{ padding: currentNav === 'workplace-verification' ? 0 : 16, paddingBottom: currentNav === 'workplace-verification' ? 0 : '5%' }}>
                ...
                {currentNav !== 'workplace-verification' && currentNav !== 'compliance' && (
                ...
                <div className="admin-content" style={{ opacity: isPending ? 0.5 : 1, transition: 'opacity 0.2s ease', height: currentNav === 'workplace-verification' ? '100%' : 'auto', padding: currentNav === 'compliance' ? '0px' : 'inherit' }}>
                    {!hasAccess ? (
                        ...
                        <>
                            {currentNav === 'compliance' && <QmsComplianceDashboard />}
                ...
                <CohortFormModal
                    ...
                    onSave={async (c) => {
                        try {
                            ...
                            // 1. Normalize New Selected Learner IDs
                            const cleanLearnerIds = (c.learnerIds || [])
                                .filter((id: string) => id && !id.startsWith("Unassigned_"))
                                .map((id: string) => {
                                    const match = store.learners.find(l => l.id === id || l.idNumber === id || l.learnerId === id);
                                    return match ? (match.idNumber || match.learnerId || match.id) : id;
                                });

                            const uniqueCleanIds = Array.from(new Set(cleanLearnerIds));
                            const cohortData = { ...c, learnerIds: uniqueCleanIds, id: cohortId, updatedAt: timestamp };
                            ...
                            // 2. Assign / Update Added Learners
                            uniqueCleanIds.forEach((lId) => {
                                ...
                                const match = store.learners.find(l => l.id === lId || l.idNumber === lId || l.learnerId === lId);
                                const targetLearnerDocId = match?.id || lId;
                                batch.set(doc(db, 'learners', targetLearnerDocId as string), { cohortId: cohortId, updatedAt: timestamp }, { merge: true });
                            });

                            // 3. Unassign Removed Learners Across 'enrollments' and 'learners'
                            if (selectedCohort) {
                                const rawDbIds = selectedCohort.learnerIds || [];

                                const pointerLearnerIds = store.learners
                                    .filter((l: any) => {
                                        const rawId = l.enrollment?.cohortId || l.cohortId || l.cohort || l.classId || '';
                                        return String(rawId).trim() === String(cohortId).trim();
                                    })
                                    .map((l: any) => l.idNumber || l.learnerId || l.id);

                                const allPreviousIds = Array.from(new Set([...rawDbIds, ...pointerLearnerIds]));

                                const normalizedPreviousIds = allPreviousIds.map((oldId: string) => {
                                    const match = store.learners.find(l => l.id === oldId || l.idNumber === oldId || l.learnerId === oldId);
                                    return match ? (match.idNumber || match.learnerId || match.id) : oldId;
                                });

                                const removedIds = Array.from(new Set(normalizedPreviousIds)).filter((oldId: string) => !uniqueCleanIds.includes(oldId));

                                for (const rId of removedIds) {
                                    const lMatch = store.learners.find(l => l.id === rId || l.idNumber === rId || l.learnerId === rId);
                                    const finalRid = lMatch ? (lMatch.idNumber || lMatch.learnerId || lMatch.id) : rId;
                                    const targetLearnerDocId = lMatch?.id || finalRid;

                                    if (targetLearnerDocId) {
                                        batch.set(doc(db, "learners", targetLearnerDocId), { cohortId: "", updatedAt: timestamp }, { merge: true });
                                    }

                                    const enrolQ1 = query(collection(db, "enrollments"), where("cohortId", "==", cohortId), where("learnerId", "==", finalRid));
                                    const enrolQ2 = query(collection(db, "enrollments"), where("cohortId", "==", cohortId), where("userId", "==", finalRid));

                                    const [snap1, snap2] = await Promise.all([getDocs(enrolQ1), getDocs(enrolQ2)]);
                                    const enrolDocsToUpdate = new Map<string, any>();

                                    snap1.forEach(d => enrolDocsToUpdate.set(d.id, d.ref));
                                    snap2.forEach(d => enrolDocsToUpdate.set(d.id, d.ref));

                                    if (enrolDocsToUpdate.size > 0) {
                                        enrolDocsToUpdate.forEach((ref) => {
                                            batch.update(ref, { cohortId: "", status: "unassigned", updatedAt: timestamp });
                                        });
                                    } else {
                                        batch.set(doc(db, "enrollments", `${cohortId}_${finalRid}`), { cohortId: "", status: "unassigned", updatedAt: timestamp }, { merge: true });
                                    }
                                }
                            }
