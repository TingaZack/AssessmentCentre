...
export const AdminProfileView: React.FC<ProfileProps> = ({ profile, user, onUpdate }) => {
    ...
    // 1. REAL-TIME LISTENER: Keep liveProfile in sync with Firestore
    useEffect(() => {
        if (!targetId) return;
    ...
    // 🚀 BACKFILL REPAIR METHOD: Strips lingering lockout flags on re-attempts
    const handleRunBackfill = async () => {
        if (!window.confirm("Are you sure you want to run the Reassessment Lock Repair Backfill? This will scan all learner submissions and safely clear stale termination/missed flags on re-attempts.")) {
            return;
        }

        setIsBackfilling(true);
        toast.info("Scanning learner submissions for stuck re-attempts...");
        ...
        try {
            ...
            for (const docSnap of snapshot.docs) {
                ...
                const isReattemptOrOverride = attemptNumber > 1 || data.hasOverride === true || data.overrideUnlock === true;

                const hasStaleFlags =
                    data.isTerminated !== undefined ||
                    data.invigilationBreached !== undefined ||
                    data.isMissed !== undefined ||
                    data.timeExpired !== undefined ||
                    data.terminatedAt !== undefined ||
                    data.missedAt !== undefined ||
                    data.terminationReason !== undefined;

                const isStuckMissedReattempt = data.status === 'missed' && isReattemptOrOverride;

                if (isReattemptOrOverride && (hasStaleFlags || isStuckMissedReattempt)) {
                    const targetRef = doc(db, 'learner_submissions', docSnap.id);
            ...
            toast.success(`Repair complete! Unlocked and fixed ${repairedCount} re-attempt record(s).`);
        } catch (err: any) {
                ...
                <div className="wm-page-header">
                    ...
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                        {/* <button
                            ...
                            style={{ background: '#f59e0b', color: 'white', border: 'none', display: 'flex', alignItems: 'center', gap: '6px' }}
                            title="Scan and clear stale termination flags on re-attempts"
                        >
                            {isBackfilling ? <Loader2 size={14} className="spin" /> : <Wrench size={14} />}
                            {isBackfilling ? 'Repairing...' : 'Repair Stuck Re-assessments'}
                        </button> */}
