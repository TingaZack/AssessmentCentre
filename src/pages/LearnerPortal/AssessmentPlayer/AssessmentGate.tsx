...
export const AssessmentGate: React.FC<AssessmentGateProps> = ({
    ...
    // 🚀 INTELLIGENT COMPLIANCE OVERRIDE
    // The parent component sometimes passes a false 'isFullyCompliant' because it checks 
    // the wrong ID format. We re-filter the array here using every known Learner Identifier.
    const actualPendingTopics = useMemo(() => {
        if (!pendingTopics || pendingTopics.length === 0) return [];

        const uid = learnerProfile?.id;
        const idNumber = learnerProfile?.idNumber;
        const subLearnerId = submission?.learnerId;
        const authUid = submission?.authUid || learnerProfile?.authUid;
        const email = String(learnerProfile?.email || '').toLowerCase().trim();

        return pendingTopics.filter(log => {
            const acks = log.acknowledgedBy || [];

            // If the learner's ID, ID Number, Auth ID, or Email is in the array, they are actually compliant.
            const isAcked =
                (uid && acks.includes(uid)) ||
                (idNumber && acks.includes(idNumber)) ||
                (subLearnerId && acks.includes(subLearnerId)) ||
                (authUid && acks.includes(authUid)) ||
                acks.some((a: string) => String(a).toLowerCase().trim() === email);

            return !isAcked; // Keep only if genuinely NOT acknowledged
        });
    }, [pendingTopics, learnerProfile, submission]);

    const isActuallyCompliant = isFullyCompliant || actualPendingTopics.length === 0;

    // Resolve coaching notes from all possible Firestore locations
    const coachingData = submission?.latestCoachingLog || (submission?.remediationNotes ? {
        notes: submission.remediationNotes,
        date: submission.remediatedAt || submission.lastStaffEditAt,
        facilitatorName: 'Facilitator'
    } : null) || (submission?.grading?.assessorOverallFeedback || submission?.grading?.facilitatorOverallFeedback ? {
        notes: submission?.grading?.assessorOverallFeedback || submission?.grading?.facilitatorOverallFeedback,
        date: submission?.grading?.gradedAt || submission?.grading?.facilitatorReviewedAt || submission?.lastStaffEditAt,
        facilitatorName: submission?.grading?.assessorName || submission?.grading?.facilitatorName || 'Facilitator'
    } : null);

    const isCoachingAckRequired = isRemediation && (!submission?.latestCoachingLog?.acknowledged || needsRemediationGate);
        ...
        <div className="ap-gate ap-animate" style={{ position: 'absolute', left: 0, right: 0, bottom: 0, top: 0 }}>
            ...
            <div className="ap-gate-body">
                <div className="ap-gate-left">
                    ...
                    {/* ALWAYS DISPLAY REMEDIATION / COACHING LOG FOR RE-ATTEMPTS */}
                    {(isRemediation || coachingData) && (
                        <div className="ap-coaching-log">
                            ...
                            <p className="ap-coaching-log__desc">
                                Review the guidance and notes recorded by your facilitator for Attempt #{submission.attemptNumber}.
                            </p>

                            {coachingData?.notes ? (
                                <div className="ap-coaching-log__quote">
                                    <span className="ap-coaching-log__quote-label">Facilitator Notes ({getSafeDate(coachingData.date)}):</span>
                                    <p className="ap-coaching-log__quote-text">"{coachingData.notes}" — {coachingData.facilitatorName || 'Facilitator'}</p>
                                </div>
                            ) : (
                                <div className="ap-coaching-log__quote">
                                    <p className="ap-coaching-log__quote-text" style={{ fontStyle: 'italic', color: 'var(--mlab-grey)' }}>
                                        "Re-assessment unlocked for Attempt #{submission.attemptNumber}. Please review your previous attempt feedback before proceeding."
                                    </p>
                                </div>
                            )}

                            {/* REQUEST PENDING / SUPPORT BUTTON */}
                            {submission?.coachingRequested ? (
                                <div className="ap-pending-tooltip-wrap" style={{ marginTop: '12px' }}>
                                    ...
                                    <div className="ap-pending-popover">
                                        ...
                                        <p>Your request for a 1-on-1 support meeting is being scheduled. Your facilitator will conduct the session and log meeting notes.</p>
                            ...
                            {/* ACKNOWLEDGEMENT CHECKBOX FOR RE-ATTEMPTS */}
                            {(!submission?.latestCoachingLog?.acknowledged || needsRemediationGate) ? (
                                <label className="ap-coaching-log__ack" style={{ marginTop: '12px', cursor: 'pointer' }}>
                                    <input
                                        type="checkbox"
                                        checked={coachingAckChecked}
                                        onChange={e => setCoachingAckChecked(e.target.checked)}
                                    />
                                    <span className="ap-coaching-log__ack-label">
                                        I acknowledge that I received and reviewed the coaching/feedback detailed above.
                                    </span>
                                </label>
                            ) : (
                                <div style={{ marginTop: '10px', fontSize: '0.75rem', color: '#166534', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                    <ShieldCheck size={14} /> Coaching Session Acknowledged / Unlocked by Educator
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    {isSummative && !passedFormative && !hasOverride && !isAppealUpheld ? (
                        ...
                        <div style={{ background: '#fffbeb', border: '1px solid #fde68a', padding: '1.5rem', borderRadius: '8px', marginTop: '1.5rem' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                {actualPendingTopics.map((log: any) => (
                                    <div key={log.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'white', padding: '8px', borderRadius: '4px', border: '1px solid #fde68a' }}>
                                        <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--mlab-blue)' }}>{log.topicTitle || log.title}</span>
                                        <button
                                            ...
                                            onClick={async () => {
                                                try {
                                                    ...
                                                    const targetId = learnerProfile?.idNumber || learnerProfile?.id || submission.learnerId;
                        ...
                        <div className="ap-declaration">
                            ...
                            <button
                                className={`ap-start-btn${(startDeclarationChecked && (!isCoachingAckRequired || coachingAckChecked)) ? ' ap-start-btn--ready' : ''}`}
                                onClick={onStart}
                                disabled={saving || isStarting || !startDeclarationChecked || (isCoachingAckRequired && !coachingAckChecked)}
                            >
                                {saving || isStarting ? <><div className="ap-spinner ap-spinner--sm" /> Preparing…</> : <><Play size={16} /> {isRemediation ? `Acknowledge & Resume Attempt #${submission.attemptNumber}` : 'I Agree, Begin Assessment'}</>}
                            </button>
                        </div>
                    )}
                </div>
