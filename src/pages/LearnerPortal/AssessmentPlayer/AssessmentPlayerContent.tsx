...
import { DynamicResourceLinks } from '../../../components/common/DynamicResourceLinks/DynamicResourceLinks';
import { UrlPreview } from '../../FacilitatorDashboard/SubmissionReview/SubmissionReview/SubmissionReviewPreviews';
...
export interface AssessmentPlayerContentProps {
    ...
    showSurveyModalIndicator?: boolean;
...
export const AssessmentPlayerContent: React.FC<AssessmentPlayerContentProps> = (props) => {
    ...
    // 🚀 RESTORED GET_STATUS_LABEL HELPER
    const getStatusLabel = (status: any, roleType: 'mentor' | 'facilitator' | 'assessor' | 'mod', isWP: boolean) => {
        if (!status) return 'Not Reviewed';
        const str = String(status).toUpperCase();
        if (str === 'HC' || str === '4') return 'Highly Competent (HC)';
        if (str === 'C' || str === '3') {
            if (roleType === 'mentor') return isWP ? 'Verified ✓' : 'Observed ✓';
            return 'Competent (C)';
        }
        if (str === 'DEV' || str === '2') return 'Developing (DEV)';
        if (str === 'NYC' || str === '1') {
            if (roleType === 'mentor') return isWP ? 'Not Verified ✗' : 'Not Observed ✗';
    ...
    // 🚀 DYNAMIC PASS MARK EXTRACTOR
    const requiredPassPercentage = assessment?.passPercentage || 80;
    const requiredPassMarks = Math.ceil((assessment?.totalMarks || grandTotalMax) * (requiredPassPercentage / 100));
            ...
            <div className="ap-player ap-animate">
                ...
                <div className="ap-player-topbar no-print">
                    ...
                    <div className="ap-player-topbar__right">
                        ...
                        {!isGloballyLocked && isPracticalModule && <div className="ap-timer ap-timer--untimed ap-hide-mobile"><Info size={13} /> {isSECAM ? 'SECAM Practical Assessment' : (assessment?.moduleType === 'workplace' ? 'Workplace Logbook' : 'Untimed Task')}</div>}
                ...
                <div className={`ap-player-body${isGloballyLocked && !isAwaitingSignoff ? ' is-locked' : ''}`}>
                    <nav className={`ap-sidebar no-print ${isMobileMenuOpen ? 'open' : ''}`}>
                        ...
                        <div className="ap-sidebar__meta-block">
                            ...
                            {!isWorkplaceModule && <div className="ap-sidebar__chip"><Award size={11} /> Pass: {requiredPassPercentage}%</div>}
                        </div>
                            ...
                            <div className={`ap-score-card${isModDone ? ' ap-score-card--verified' : ''}`}>
                                ...
                                <div className="ap-score-card__body">
                                    ...
                                    <div className="ap-score-card__fraction">
                                        ...
                                        <span className="ap-score-card__pass-note">Pass mark: {requiredPassMarks} ({requiredPassPercentage}%)</span>
                                    </div>
                                </div>
                                            ...
                                            <div className="ap-block-question__body">
                                                                                ...
                                                                                {activeTabId === 'text' && (
                                                                                    <div className={`ap-quill-wrapper${isUploadLocked || !canEditChecklist ? ' locked' : ''}`} onCopyCapture={preventCopyPasteAndDrop} onCutCapture={preventCopyPasteAndDrop} onPasteCapture={preventCopyPasteAndDrop} onDropCapture={preventCopyPasteAndDrop} onKeyDownCapture={preventCopyPasteAndDrop}>
                                                                                        {isGloballyLocked && !isAwaitingSignoff ? (
                                                                                            ...
                                                                                            <ReactQuill theme="snow" value={itemAns.text || ''} onChange={c => handleNestedAnswerChange(block.id, nestedKey, 'text', c)} readOnly={isUploadLocked || !canEditChecklist} modules={quillModules} formats={quillFormats} placeholder="Type your evidence here…" />
                                                                                        )}
                                                                                    </div>
                                                                                )}
                                                                                {activeTabId === 'url' && (
                                                                                    <div>
                                                                                        {itemAns.url && (isUploadLocked || !canEditChecklist) ? <UrlPreview url={itemAns.url} /> : <input type="url" className="ab-input" value={itemAns.url || ''} onChange={e => handleNestedAnswerChange(block.id, nestedKey, 'url', e.target.value)} disabled={isUploadLocked || !canEditChecklist} placeholder="https://…" />}
                                                                                    </div>
                                                                                )}
                                                                                {activeTabId === 'upload' && (
                                                                                    progress !== undefined ? <UploadProgress progress={progress} /> : itemAns.uploadUrl ? <LearnerFilePreview url={itemAns.uploadUrl} onRemove={!isUploadLocked && canEditChecklist ? () => handleNestedAnswerChange(block.id, nestedKey, 'uploadUrl', '') : undefined} disabled={isUploadLocked || !canEditChecklist} /> : (
                                                                                        <div className="ap-upload-empty">
                                                                                            {isUploadLocked || !canEditChecklist ? <span className="ap-upload-empty__hint">No file uploaded.</span> : <>
                                                                                                <p className="ap-upload-empty__hint">Select a file (Max {MAX_FILE_SIZE_MB}MB)</p>
                                                                                                <input type="file" onChange={e => handleFileValidationAndUpload(e, block.id, nestedKey)} disabled={isUploadLocked || !canEditChecklist} style={{ fontSize: '0.82rem' }} />
                                                                                            </>}
                                                                                        </div>
                                                                                    )
                                                                                )}
                                                                                ...
                                                                                {activeTabId === 'code' && (
                                                                                    <div style={ideWrapperStyle}>
                                                                                        <CodeSandboxPlayer
                                                                                            ...
                                                                                            learnerAns={mappedCodeData}
                                                                                            readOnly={isUploadLocked || !canEditChecklist}
                                                                                            onChange={async (val) => {
                                                                    ...
                                                                    <div className="ap-checklist__assessor-row">
                                                                        {res.status ? (
                                                                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                                                                <div><span className={`ap-checklist__status-chip ${res.status === 'C' || res.status === 'HC' ? 'ap-checklist__status-chip--c' : 'ap-checklist__status-chip--nyc'}`}>{getStatusLabel(res.status, savedFacRole === 'mentor' ? 'mentor' : 'facilitator', false)}</span></div>
                                                ...
                                                {block.type === 'code_sandbox' && (
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', width: '100%' }}>
                                                        {/* Dynamic External Links Section */}
                                                        <DynamicResourceLinks
                                                            links={answers[block.id]?.resourceLinks || []}
                                                            onChange={(updatedLinks) => {
                                                                const currentBlockAns = answers[block.id] || {};
