...
import { ArrowLeft, Info, ShieldAlert, Zap, Clock, Calendar, Send, X, BookOpen, AlertTriangle, ShieldCheck } from 'lucide-react';
...
// ─── SUPPORT / RE-ASSESSMENT SCHEDULING MODAL ───
const ScheduleSessionModal: React.FC<{
    ...
    onClose: () => void;
    toast: any;
}> = ({ submission, assessment, learnerProfile, user, staffList, cohortsList, onClose, toast }) => {
    const isMissedContext = submission?.status === 'missed';
    const isNYCContext = ['NYC', 'DEV', '1', '2'].includes(String(submission?.competency || '').toUpperCase());
    ...
    const defaultTopic = isMissedContext
        ? `Requesting re-schedule for missed assessment: ${submission?.moduleNumber || ''} - ${submission?.title || assessment?.title}`
        : `Remediation Coaching Request for ${submission?.moduleNumber || 'Module'}: ${submission?.title || assessment?.title}`;
    ...
    const activeCohort = cohortsList?.find(c => c.id === submission?.cohortId);
    const facilitator = staffList?.find(s => s.id === activeCohort?.facilitatorId);

    const resolvedStaffId = facilitator?.id || 'unassigned';
    const resolvedStaffEmail = facilitator?.email || 'codetribe@mlab.co.za';
    const resolvedStaffName = facilitator?.fullName || submission?.facilitatorName || 'Facilitator';

    const resolvedLearnerId = learnerProfile?.learnerId || learnerProfile?.id || user?.uid;
    const resolvedLearnerEmail = user?.email || 'noreply@mlab.co.za';
    const resolvedLearnerName = learnerProfile?.fullName || user?.fullName || 'Learner';

    const handleSubmit = async () => {
        if (!dateTime || !topic.trim()) {
            toast.error("Please select a preferred date/time and provide a reason.");
            return;
        }
        ...
        try {
            ...
            await scheduleCoachingSession({
                ...
                assessmentId: submission?.assessmentId || assessment?.id
            });
            ...
            toast.success("Support request submitted to your facilitator!");
            onClose();
        } catch (error: any) {
            ...
            <div className="lfm-modal animate-fade-in" onClick={e => e.stopPropagation()} style={{ maxWidth: '540px' }}>
                <div className="lfm-header">
                    <h2 className="lfm-header__title">
                        <Calendar size={16} /> Request Re-assessment Support
                    </h2>
                ...
                <div className="lfm-body">
                    <div className="lfm-flags-panel" style={{ marginTop: 0 }}>
                        <strong style={{ fontSize: '0.8rem', color: 'var(--mlab-blue)', textTransform: 'uppercase' }}>
                            {submission?.moduleNumber || 'Module'}: {submission?.title || assessment?.title}
                        </strong>
                        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: 'var(--mlab-grey)', lineHeight: 1.4 }}>
                            Request a 1-on-1 coaching session with your facilitator ({resolvedStaffName}) to review your feedback and unlock your next attempt.
                        </p>
                    </div>

                    <div className="lfm-fg">
                        <label>Session Category *</label>
                        <select className="lfm-input lfm-select" value={category} onChange={e => setCategory(e.target.value)}>
                    ...
                    <div className="lfm-fg">
                        <label>Reason / Areas Needing Guidance *</label>
                        <textarea className="lfm-input" rows={3} placeholder="Describe what areas you struggled with or need coaching on..." value={topic} onChange={e => setTopic(e.target.value)} style={{ resize: 'vertical' }} />
                    </div>
                </div>
                ...
                <div className="lfm-footer">
                    ...
                    <button type="button" className="lfm-btn lfm-btn--primary" onClick={handleSubmit} disabled={isSubmitting}>
                        {isSubmitting ? <><div className="ap-spinner ap-spinner--sm" /> Submitting…</> : <><Send size={13} /> Submit Support Request</>}
                    </button>
                </div>
            </div>
        </div>,
...
export const AssessmentPlayer: React.FC = () => {
    ...
    const { user, employers, staff, cohorts, fetchEmployers, fetchStaff, fetchCohorts } = useStore() as any;
    ...
    const isSummative = submission?.type?.toLowerCase().includes('summative') || assessment?.type?.toLowerCase().includes('summative') || false;
    const hasOverride = submission?.facilitatorOverride === true || submission?.hasOverride === true || submission?.overrideUnlock === true;

    const needsRemediationGate = isRemediation && submission?.latestCoachingLog && !submission?.latestCoachingLog?.acknowledged && !isAppealUpheld && !hasOverride;
    ...
    const pendingTopics = useMemo(() => {
        ...
        return moduleLogs.filter(log => {
            const isAcknowledged = possibleIds.some(id => log.acknowledgedBy?.includes(id));
            return !isAcknowledged;
        });
    }, [moduleLogs, submission, learnerProfile, user]);
    ...
    useEffect(() => {
        const checkSurveyStatus = async () => {
            ...
            if (!activeSurveyId || !submission?.id) {
            ...
            try {
                ...
                if (!snapSpecific.empty) {
                    ...
                    setIsSurveyCompleted(!snapSub.empty);
                }
            } catch (err: any) {
    ...
    }, [assessment?.surveyId, submission?.surveyId, submission?.id, assessmentId, user?.uid]);
    ...
    useEffect(() => {
        ...
        if (_isSummative) {
            ...
            const formQ = query(
                ...
                where('competency', '==', 'C'),
                where('status', 'in', ['graded', 'moderated', 'appealed'])
            );
    ...
    const handleStartAssessment = async () => {
        ...
        let t = new Date(getSecureNow()).toISOString();
        ...
        try {
            ...
            try {
                const res = await startFn({
                    ...
                    assessmentId: assessmentId,
                    learnerId: learnerProfile?.id || user?.uid || submission?.authUid || submission?.learnerId,
                    cohortId: submission.cohortId
                });
                const data = res.data as any;
                if (data?.startedAt) t = data.startedAt;
            } catch (cfErr: any) {
                if (cfErr?.code === 'functions/failed-precondition' || cfErr?.code === 'functions/permission-denied') {
                    throw cfErr;
                }
                console.warn("Cloud function fallback triggered:", cfErr.message);
            }
    ...
    const triggerSubmitConfirm = () => {
        ...
        if (assessment?.moduleType === 'workplace' && !isSECAM) {
    ...
    if (showGate) {
        return (
            <>
                <style>{`
                    .ap-pending-tooltip-wrap {
                        position: relative;
                        display: inline-flex;
                    }
                    .ap-pending-tooltip-wrap:hover .ap-pending-popover {
                        opacity: 1;
                        visibility: visible;
                    ...
                    .ap-pending-popover p {
                        margin: 0 0 6px 0;
                        color: #e2e8f0;
                    }
                    .ap-pending-popover small {
                        display: block;
                        color: #94a3b8;
                        font-size: 0.68rem;
                ...
                <AssessmentGate
                    ...
                    toast={toast}
                    onRequestSupport={() => setShowScheduleModal(true)}
                />
            </>
        );
    }
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: '#f8fafc' }}>
            <AssessmentPlayerContent
                ...
                showSurveyModalIndicator={isSubmitted && Boolean(activeSurveyId)}
...
//         //Collect all possible IDs the learner might be known by
//         const possibleIds = [
//             learnerProfile?.id,
//             submission.learnerId,
//             submission.authUid,
...
//         return moduleLogs.filter(log => {
//             // If the log is acknowledged by ANY of the learner's possible IDs, it's complete
//             const isAcknowledged = possibleIds.some(id => log.acknowledgedBy?.includes(id));
//             return !isAcknowledged;
//         });
//     }, [moduleLogs, submission, learnerProfile, user]);
...
//             try {
//                 // Check if THIS SPECIFIC assessment has a completed survey response
//                 const qSpecific = query(
//                     collection(db, 'survey_responses'),
//                     where('surveyId', '==', activeSurveyId),
//                     where('learnerId', '==', currentUid),
...
//                     setIsSurveyCompleted(true);
//                 } else {
//                     // Fallback to check via submissionId in case it was stored that way
//                     const qSub = query(
//                         collection(db, 'survey_responses'),
//                         where('surveyId', '==', activeSurveyId),
//                         where('learnerId', '==', currentUid),
...
//         // 🚀 Determine dynamic pass threshold (reads from assessment settings, defaults to 80%)
//         const passPct = assessment?.passPercentage || 80;
//         const passDecimal = passPct / 100;

//         const compStr = (submission?.competency || submission?.overallCompetency || submission?.outcome || '').toString().toLowerCase();

//         // Check if explicitly graded as competent (supports SECAM levels 3 & 4)
//         let isCompetent = compStr === 'c' || compStr === 'competent' || compStr === 'hc' || compStr === '4' || compStr === '3';

//         // Fallback to checking the mathematical score against the dynamic threshold
//         if (!isWorkplaceModule && !isCompetent && grandTotalAwarded !== undefined && (assessment?.totalMarks || grandTotalMax)) {
//             isCompetent = grandTotalAwarded >= (assessment?.totalMarks || grandTotalMax) * passDecimal;
//         }
...
//             try {
//                 // 🚀 Send fully loaded payload to prevent backend 400 Bad Request
//                 // Use the learnerProfile.id primarily so it matches the Dashboard's tracking
//                 const res = await startFn({
//                     submissionId: submission.id,
//                     assessmentId: assessmentId,
//                     learnerId: learnerProfile?.id || user?.uid || submission?.authUid || submission?.learnerId,
...
//                 if (data?.startedAt) t = data.startedAt;
//             } catch (cfErr: any) {
//                 // If the backend threw a specific Firebase HttpsError (like our failed-precondition for the gate), 
//                 // we want to bubble that up to the user. Otherwise, fall back to a direct client update.
//                 if (cfErr?.code === 'functions/failed-precondition' || cfErr?.code === 'functions/permission-denied') {
//                     throw cfErr;
//                 }
//                 console.warn("Cloud function fallback triggered:", cfErr.message);
...
//         // Only enforce formal QCTO Mentor Log approval for SETA-accredited modules
//         if (assessment?.moduleType === 'workplace' && !isSECAM) {
//             const workplaceBlocks = assessment?.blocks?.filter((b: any) => b.type === 'qcto_workplace') || [];
//             for (const block of workplaceBlocks) {
//                 for (const wa of block.workActivities || []) {
...
// //             // 🚀 UPDATED PREREQUISITE RULE: Graded OR Moderated is considered passed if Competency === 'C'
// //             const formQ = query(
// //                 collection(db, 'learner_submissions'),
// //                 where('authUid', '==', currentUid),
// //                 where('moduleNumber', '==', submission.moduleNumber),
