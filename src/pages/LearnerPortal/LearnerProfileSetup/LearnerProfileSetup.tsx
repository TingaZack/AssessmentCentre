...
import { SkillsSelector, type SkillItem } from '../../../components/common/SkillsSelector/SkillsSelector';
import type { SkillHistoryEntry } from '../LearnerDashboard/LearnerProfileView/SkillsProgressionTimeline';
...
export const LearnerProfileSetup: React.FC = () => {
    ...
    // Keep reference to initial skills and skills history for progression delta logging
    const [initialSkills, setInitialSkills] = useState<SkillItem[]>([]);
    const [initialSkillsHistory, setInitialSkillsHistory] = useState<SkillHistoryEntry[]>([]);
        ...
        const fetchExistingData = async () => {
            ...
            try {
                ...
                if (learnerData) {
                    ...
                    const loadedSkills = Array.isArray(learnerData.skills) ? learnerData.skills : [];
                    const loadedSkillsHistory = Array.isArray(learnerData.skillsHistory) ? learnerData.skillsHistory : [];

                    setInitialSkills(loadedSkills);
                    setInitialSkillsHistory(loadedSkillsHistory);
                    ...
                    setFormData({
                        ...
                        nokPhone: nok.phone || '',
                        skills: loadedSkills,
                        skillsHistory: loadedSkillsHistory
                    });
    ...
    const confirmMapCoordinates = () => {
        ...
        setIsMapModalOpen(true);
        setTimeout(() => setIsMapModalOpen(false), 300);
    };

    const handleChange = (field: keyof LearnerProfileData, value: any) => {
        setFormData(prev => {
    ...
    const handleNextToStep3 = () => {
        if (!formData.lat || !formData.lng || (formData.lat === 0 && formData.lng === 0)) {
            toast.warning("Please use the Google Maps search to find and confirm your exact location.");
            return;
        }
        if (!formData.statssaAreaCode) {
            toast.warning("Please select a STATS-SA Area Code to confirm your Municipality.");
            return;
        }
        if (!formData.streetAddress) {
            toast.warning("Please provide a street address.");
            return;
        }
        setStep(3);
    };
    ...
    const handleSubmit = async () => {
        ...
        if (!formData.popiaConsent) {
            toast.warning('You must accept the POPIA consent to continue.');
            return;
        }
        if (!formData.statssaAreaCode) {
            toast.warning('You must select a STATS-SA Area Code.');
            return;
        }
        ...
        try {
            ...
            savedSkills.forEach((skill: any) => {
                ...
                if (!prevSkill) {
                    generatedSkillsHistory.push({
                        id: `sh_setup_${Date.now()}_${Math.random().toString(36).substring(2, 6)}`,
                        skillId: String(key),
                        skillName: skill.name || skill.label,
                        category: skill.category || 'General Technical',
                        previousLevel: 'None',
                        newLevel: skill.level || 'Beginner',
                        updatedAt: nowIso
                    });
                } else if (prevSkill.level !== skill.level) {
                    generatedSkillsHistory.push({
                        id: `sh_setup_${Date.now()}_${Math.random().toString(36).substring(2, 6)}`,
                        skillId: String(key),
                        skillName: skill.name || skill.label,
                        category: skill.category || 'General Technical',
                        previousLevel: prevSkill.level || 'Beginner',
                        newLevel: skill.level || 'Intermediate',
                        updatedAt: nowIso
                    });
                }
            });
            ...
            Object.keys(finalData.demographics).forEach(key => {
                if (finalData.demographics[key] === undefined) finalData.demographics[key] = "";
            });
    ...
    if (fetchingInitial) return <Loader message='Syncing Record' />;
                ...
                <div className="lfm-overlay" onClick={() => setIsMapModalOpen(false)} style={{ zIndex: 99999 }}>
                    <div className="lfm-modal animate-fade-in" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '750px' }}>
                        <div className="lfm-header">
                            <h2 className="lfm-header__title"><MapPin size={16} /> Pinpoint Exact Residence Entrance</h2>
                            <button className="lfm-close-btn" type="button" onClick={() => setIsMapModalOpen(false)}><X size={20} /></button>
                        </div>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '2rem', padding: '1rem', background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                                <div style={{ position: 'relative' }}>
                                    <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: '#e2e8f0', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '3px solid white', boxShadow: '0 2px 5px rgba(0,0,0,0.1)' }}>
                                        {photoPreview ? <img src={photoPreview} alt="Profile" crossOrigin="anonymous" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <User size={40} color="#94a3b8" />}
                                    </div>
                    ...
                    {step === 2 && (
                        <div className="lp-form-body animate-fade-in">
                            ...
                            <div className="lp-grid mt-4">
                                ...
                                <FG label="City / Town (Auto-filled)">
                                    <input className="lp-input" readOnly value={formData.city || ''} style={{ backgroundColor: '#f1f5f9', color: '#64748b', cursor: 'not-allowed' }} placeholder="Use Maps search above..." />
                                </FG>
                                <div className="lp-fg">
                                    <label className="lp-fg-label">Province (Auto-filled)</label>
                                    <select className="lp-input lp-select" disabled value={formData.provinceCode || ""} style={{ backgroundColor: '#f1f5f9', color: '#64748b', cursor: 'not-allowed', opacity: 1 }}>
                                        <option value="">Use Maps search above...</option>
                                        {QCTO_PROVINCES.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}
                                    </select>
                                </div>
                                ...
                                <FG label="Local Municipality (Auto-filled) *">
                                    <input className="lp-input" readOnly value={formData.localMunicipality || ''} style={{ backgroundColor: '#f1f5f9', color: '#64748b', cursor: 'not-allowed' }} placeholder="Select STATS-SA Code above..." />
                                </FG>
                                <FG label="District / Metro (Auto-filled)">
                                    <input className="lp-input" readOnly value={formData.districtOrMetro || ''} style={{ backgroundColor: '#f1f5f9', color: '#64748b', cursor: 'not-allowed' }} placeholder="Select STATS-SA Code above..." />
                                </FG>
                            </div>
                            ...
                            <div style={{ marginTop: '1.25rem', padding: '0.85rem 1rem', background: '#fefce8', border: '1px solid #fef08a', borderRadius: '8px', color: '#713f12', fontSize: '0.83rem', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                ...
                                <span><strong>Proof of Address Requirement:</strong> You will be required to upload a valid Proof of Address document in Step 3 that reflects this exact residential address.</span>
                            </div>
                            ...
                            <h3 className="lp-section-title mt-8"><Code size={16} /> Technical Skills & Competencies</h3>
                            <div style={{ background: '#ffffff', border: '1px solid #e2e8f0', padding: '1.25rem', borderRadius: '8px' }}>
                                <p style={{ fontSize: '0.85rem', color: '#64748b', margin: '0 0 1rem 0' }}>
                                    Select your technical skills and adjust your proficiency levels. You can choose from predefined tech categories or type custom skills.
                                </p>
                                <SkillsSelector
                            ...
                            <div className="lp-actions">
                                ...
                                <button className="lp-btn-primary" onClick={handleNextToStep3}>Next <ChevronRight size={15} /></button>
                            </div>
                        </div>
                    )}
