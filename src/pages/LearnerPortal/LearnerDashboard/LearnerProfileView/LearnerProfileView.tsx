...
import { SkillsProgressionTimeline, type SkillHistoryEntry } from './SkillsProgressionTimeline';
...
export const LearnerProfileView: React.FC<ProfileProps> = ({ profile, user, onUpdate }) => {
    ...
    useEffect(() => {
        if (profile) {
            setLiveProfile((prev: any) => ({
                ...
                skillsHistory: profile.skillsHistory || prev?.skillsHistory || []
            }));
        }
    }, [profile]);
        ...
        const unsubscribe = onSnapshot(doc(db, 'users', targetId), (docSnap) => {
            if (docSnap.exists()) {
                ...
                setLiveProfile((currentProfile: any) => ({
                    ...currentProfile,
                    ...userData,
                    skills: userData.skills || currentProfile?.skills || [],
                    skillsHistory: userData.skillsHistory || currentProfile?.skillsHistory || [],
    ...
    const executeSave = async () => {
        ...
        try {
            ...
            savedSkills.forEach((skill: any) => {
                ...
                if (!prevSkill) {
                    ...
                    generatedSkillsHistory.push({
                        ...
                        skillName: skill.name || skill.label,
                        category: skill.category,
                        previousLevel: (prevSkill as any).level || 'Beginner',
            ...
            setModalConfig({ isOpen: true, type: 'success', title: 'Profile Updated', message: 'Your profile and skill progression milestones have been successfully updated.' });
        } catch (error) {
            ...
            <div className="lpv-layout">
                <div className="lpv-main-stack">
                    ...
                    <section className="lpv-panel">
                        ...
                        {/* ANIMATED SKILLS PROGRESSION TIMELINE */}
                        <SkillsProgressionTimeline
                            skills={displayData.skills || []}
                            skillsHistory={liveProfile.skillsHistory || []}
                        />
                    </section>
