...
export const LearnerProfileView: React.FC<ProfileProps> = ({ profile, user, onUpdate }) => {
    ...
    // 🚀 FIXED: Safely merge incoming profile prop without wiping existing skills
    useEffect(() => {
        if (profile) {
    ...
    // 🚀 FIXED: Real-time Firestore sync with safe fallback for skills
    useEffect(() => {
        if (!targetId) return;
        ...
        const unsubscribe = onSnapshot(doc(db, 'users', targetId), (docSnap) => {
            if (docSnap.exists()) {
                ...
                setLiveProfile((currentProfile: any) => ({
                    ...currentProfile,
                    ...userData,
                    skills: userData.skills || currentProfile?.skills || [], // 🚀 Preserve skills
    ...
    // Hydrate form when toggling edit mode
    useEffect(() => {
        if (!isEditing && liveProfile) {
            ...
            const initialLoadData = {
                ...
                skills: Array.isArray(liveProfile.skills) ? liveProfile.skills : [] // 🚀 Hydrate skills array
            };
    ...
    const executeSave = async () => {
        ...
        try {
            ...
            const savedSkills = Array.isArray(formData.skills) ? formData.skills : [];
            ...
            const updatedData = {
                ...
                skills: savedSkills, // 🚀 Save skills array explicitly
                demographics: {
            ...
            // 🚀 Write directly to Firestore `users` collection to prevent race conditions
            await updateDoc(doc(db, 'users', targetId), { skills: savedSkills });

            // Call parent update callback
            await onUpdate(profile.id || targetId, updatedData);

            // 🚀 Immediately sync local state so UI displays skills without waiting for re-render
            setLiveProfile((prev: any) => ({
                ...prev,
                skills: savedSkills
            }));
...
//                     nextOfKin: currentProfile.nextOfKin || userData.nextOfKin || {},
//                     uploadedDocuments: currentProfile.uploadedDocuments || userData.uploadedDocuments || [],
//                     skills: userData.skills || currentProfile.skills || [], // 🚀 Sync skills
//                     documentHistory: currentProfile.documentHistory || userData.documentHistory || [],
//                     addressHistory: currentProfile.addressHistory || userData.addressHistory || []
//                 }));
//             }
...
//                 nokPhone: liveProfile.nextOfKin?.phone || '',
//                 profilePhotoUrl: liveProfile.profilePhotoUrl || '',
//                 skills: liveProfile.skills || [] // 🚀 Hydrate skills
//             };
