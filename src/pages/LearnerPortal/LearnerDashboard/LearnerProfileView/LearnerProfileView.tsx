...
export const LearnerProfileView: React.FC<ProfileProps> = ({ profile, user, onUpdate }) => {
    ...
    // 🕵️‍♂️ DEBUG 1: Which ID are we actually looking for?
    const targetId = profile?.authUid || profile?.userId || profile?.uid || profile?.id;

    useEffect(() => {
        console.log("🎯 DEBUG 1 - TARGET ID:", targetId);
        console.log("🎯 DEBUG 1 - RAW PROFILE PROP:", profile);
    }, [targetId, profile]);
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(doc(db, 'users', targetId), (docSnap) => {
            if (docSnap.exists()) {
                ...
                // 🕵️‍♂️ DEBUG 2: What is inside the users document?
                console.log("📄 DEBUG 2 - FIRESTORE USER DATA:", userData);
                ...
                setLiveProfile((currentProfile: any) => {
                    ...
                    // 🕵️‍♂️ DEBUG 3: What does the final merged state look like?
                    console.log("🧠 DEBUG 3 - MERGED LIVE PROFILE:", merged);
                    return merged;
                });
            } else {
                console.log("⚠️ DEBUG 2 - FIRESTORE DOC DOES NOT EXIST FOR ID:", targetId);
            }
        });
    ...
    // 🕵️‍♂️ DEBUG 4: Render check
    useEffect(() => {
        console.log("🖼️ DEBUG 4 - SIGNATURE RENDER CHECK:", liveProfile?.signatureUrl);
        ...
        <div className="lpv-wrapper animate-fade-in">
            ...
            <div className="lpv-layout">
                <div className="lpv-main-stack">
                    ...
                    {/* SIGNATURE SECTION (READ-ONLY FOR ADMINS) */}
                    <section className="lpv-panel">
                        <div className="lpv-panel__header">
                            ...
                            {liveProfile?.signatureUrl ? (
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                    <img
                                        ...
                                        style={{ height: '60px', width: 'auto', maxWidth: '100%', objectFit: 'contain', filter: 'brightness(0)' }}
                                    />
                        ...
                        <p style={{ marginTop: '1rem', fontSize: '0.75rem', color: '#64748b', fontStyle: 'italic' }}>
                            Note: Admins cannot sign on behalf of learners. The learner must log into their portal to configure their signature.
                        </p>
                    </section>
                </div>
