...
import {
    ...
    Globe, MapPin, Target, Info, ClipboardList,
    HelpCircle, Send, ExternalLink, Tag
} from 'lucide-react';
...
import { AiMockInterviewStudio } from '../../../components/views/AiMockInterview/AiMockInterviewStudio';
import { CoachingScheduleView } from '../../../components/views/CoachingScheduleView/CoachingScheduleView';
import { LearnerContentHub } from '../LearnerContentHub/LearnerContentHub';
...
function ProfessionalismWidget({ pScore, pStreak }: { pScore: number; pStreak: number; }) {
    ...
    return (
        <div style={{ display: 'flex', alignItems: 'stretch', background: 'var(--mlab-white)', border: '1px solid var(--mlab-border)', borderRadius: '0', width: '100%', marginBottom: '2rem', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', padding: '1.5rem', borderRight: '1px solid var(--mlab-border)', minWidth: '180px' }}>
                                ...
                                return (
                                    <div key={modCode} style={{ background: 'white', borderRadius: '0', border: `1px solid ${viewMode === 'pending' ? '#fde68a' : '#bbf7d0'}`, overflow: 'hidden' }}>
                                        <div className="lfm-section-hdr" style={{ cursor: 'pointer', borderBottom: isOpen ? `1px solid ${viewMode === 'pending' ? '#fde68a' : '#bbf7d0'}` : 'none', background: headerBg, padding: '12px 16px' }} onClick={() => toggleModuleAccordion(modCode)}>
                                            ...
                                            <div style={{ flex: 1, display: 'center', alignItems: 'center', gap: '10px' }}><span style={{ color: headerColor, fontWeight: 700 }}>{modCode} <span style={{ fontWeight: 400, opacity: 0.8 }}>{moduleName}</span></span></div>
...
const LearnerDashboard: React.FC = () => {
    ...
    // READ ACTIVE TAB DIRECTLY FROM URL SEARCH PARAMS
    const urlTab = searchParams.get('tab');
    const validNavTabs = ['dashboard', 'profile', 'certificates', 'attendance', 'interviews', 'coaching', 'learning_units'];

    const [currentNav, setCurrentNav] = useState<
        'dashboard' | 'profile' | 'certificates' | 'attendance' | 'interviews' | 'coaching' | 'learning_units'
    >(() => {
        if (urlTab && validNavTabs.includes(urlTab)) return urlTab as any;
    ...
    // SYNC URL PARAMETERS IN REAL TIME WHEN TAB IS CHANGED IN COMPONENT OR VIA SIDEBAR
    useEffect(() => {
        if (urlTab && validNavTabs.includes(urlTab)) {
            if (urlTab !== currentNav) {
                setCurrentNav(urlTab as any);
            }
        }
    }, [urlTab]);

    // STABLE DUAL-DIRECTION NAV HANDLER TO PREVENT SEARCHPARAM CONFLATION
    const handleNavChange = (newNav: any) => {
                ...
                unsubscribeProfile = onSnapshot(profileDocRef, async (profileSnap: any) => {
                    const processProfileData = async () => {
                        ...
                        const enrolQ = query(
                            ...
                            where('learnerId', 'in', possibleIdentities.slice(0, 10)),
                        ...
                        try {
                            ...
                            enrolls = snapEnrol.docs.map(d => ({ id: d.id, ...d.data() }));
                        } catch (err: any) {
                            console.warn("Enrollments query bypassed:", err.message);
                        }
                        ...
                        let missed: any[] = [];
                        let pastPresents: any[] = [];

                        if (cohortIdsArray.length > 0) {
                            try {
                                const safeCohorts = Array.from(new Set(cohortIdsArray)).slice(0, 10);
                                const qAtt = query(collection(db, 'attendance'), where('cohortId', 'in', safeCohorts));
                                ...
                                console.warn("Attendance query bypassed:", err.message);
                            }
                        }
                        ...
                        const safeCohortIdsArray = Array.from(new Set(cohortIdsArray)).slice(0, 10);
                        ...
                        if (safeCohortIdsArray.length > 0) {
                            const logsQ = query(collection(db, 'attendance_logs'), where('cohortId', 'in', safeCohortIdsArray));
                            unsubscribeVirtualLogs = onSnapshot(logsQ, (logsSnap) => {
                                ...
                                console.warn("Virtual logs bypassed:", err.message);
                                combineVirtualSources();
                            });
                        } else {
                            combineVirtualSources();
                        ...
                        if (possibleScanIds.length > 0) {
                            const safeScans = possibleScanIds.slice(0, 10);
                            const recQ = query(collection(db, 'attendance_records'), where('learnerId', 'in', safeScans));
                            ...
                            unsubscribeVirtualRecords = onSnapshot(recQ, (recSnap) => {
                                ...
                                console.warn("Attendance records bypassed:", err.message);
                                combineVirtualSources();
                            });

                            const scansQ = query(collection(db, 'live_attendance_scans'), where('learnerId', 'in', safeScans));
                            unsubscribeScans = onSnapshot(scansQ, (snapScans) => {
                                setMyScans(snapScans.docs.map(d => ({ id: d.id, ...d.data() })));
                            }, (err) => console.warn("Live scans bypassed:", err.message));

                            // WORKPLACE LOGS DUAL-FETCH PIPELINE
                            const currentUid = store.user?.uid || auth.currentUser?.uid || '';
                            const fetchedWpLogsMap = new Map<string, any>();
                            let unsubAuthLogs: () => void = () => { };
                            let unsubLearnerLogs: () => void = () => { };

                            const updateWpLogsState = () => {
                                setMyWorkplaceLogs(Array.from(fetchedWpLogsMap.values()).sort((a: any, b: any) =>
                                    new Date(b.dateString).getTime() - new Date(a.dateString).getTime()
                                ));
                            };

                            if (currentUid) {
                                const qAuth = query(collection(db, 'workplace_logs'), where('authUid', '==', currentUid));
                                unsubAuthLogs = onSnapshot(qAuth, (snap) => {
                                    snap.docs.forEach(d => fetchedWpLogsMap.set(d.id, { id: d.id, ...d.data() }));
                                    updateWpLogsState();
                                }, (err) => console.warn("workplace_logs authUid bypassed:", err.message));
                            }

                            if (safeScans.length > 0) {
                                const qLearner = query(collection(db, 'workplace_logs'), where('learnerId', 'in', safeScans));
                                unsubLearnerLogs = onSnapshot(qLearner, (snap) => {
                                    snap.docs.forEach(d => fetchedWpLogsMap.set(d.id, { id: d.id, ...d.data() }));
                                    updateWpLogsState();
                                }, (err) => console.warn("workplace_logs learnerId bypassed:", err.message));
                            }

                            unsubscribeWorklogs = () => {
                                if (typeof unsubAuthLogs === 'function') unsubAuthLogs();
                                if (typeof unsubLearnerLogs === 'function') unsubLearnerLogs();
                            };
                        }
                    ...
                    console.warn("Profile listener bypassed:", err.message);
                    setIsLoading(false);
                });
    ...
    useEffect(() => {
        ...
        const possibleIds = [academicProfile.id, ...learnerEnrollments.map(e => e.id)].slice(0, 10);
        ...
        }, (err) => console.warn("Placements listener bypassed:", err.message));
    ...
    useEffect(() => {
        ...
        const unsubCheckins = onSnapshot(checkinsQ, (snap) => {
            setEventCheckins(snap.docs.map(d => ({ id: d.id, ...d.data() })));
        }, (err) => console.warn("Event checkins bypassed:", err.message));

        const eventsQ = query(collection(db, 'events'));
        const unsubEvents = onSnapshot(eventsQ, (snap) => {
            setAllEcosystemEvents(snap.docs.map(d => ({ id: d.id, ...d.data() })));
        }, (err) => console.warn("Events bypassed:", err.message));
    ...
    useEffect(() => {
        ...
        const unsubSurveys = onSnapshot(
            ...
            (err) => console.warn("Surveys listener bypassed:", err.message)
        );
        ...
        const fetchResponses = async () => {
            ...
            try {
                const qResponses = query(collection(db, 'survey_responses'), where('authUid', '==', currentUid));
                const snap = await getDocs(qResponses);
                setMySurveyResponses(snap.docs.map(d => ({ id: d.id, ...d.data() })));
            } catch (err: any) {
                console.warn("Learner survey responses (authUid) query bypassed:", err.message);

                try {
                    const qFallback = query(collection(db, 'survey_responses'), where('learnerId', '==', academicProfile.id));
                    const snapFb = await getDocs(qFallback);
                    setMySurveyResponses(snapFb.docs.map(d => ({ id: d.id, ...d.data() })));
                } catch (fallbackErr: any) {
                    console.warn("Learner survey responses (learnerId) query bypassed:", fallbackErr.message);
                }
            }
        };
    ...
    useEffect(() => {
        ...
        const cohortIds = Array.from(new Set(myCohorts.map(c => c.id))).slice(0, 10);
        ...
        }, (err) => console.warn("Assessments bypassed:", err.message));
    ...
    // ASSIGNED SURVEYS LOGIC
    const assignedSurveys = useMemo(() => {
        const myCohortIds = new Set<string>();
        learnerEnrollments.forEach(e => {
            if (e.cohortId) myCohortIds.add(e.cohortId);
        });
        if (academicProfile?.cohortId) myCohortIds.add(academicProfile.cohortId);
        ...
        return allSurveys.filter(survey => {
            ...
            const isCohortMatch = targetCohortIds.includes('ALL') ||
                targetCohortIds.includes('Unassigned') ||
                targetCohortIds.some((id: string) => myCohortIds.has(id)) ||
    ...
    }, [allSurveys, myCohorts, assessmentMap, learnerEnrollments, academicProfile]);
    ...
    useEffect(() => {
        ...
        const cohortIds = myCohorts.map(c => c.id).slice(0, 10);
        if (cohortIds.length === 0) return;
        ...
        }, (err) => console.warn("Curriculum logs bypassed:", err.message));
        return () => unsubscribe();
    ...
    // DERIVED SCORES & PAGE TITLES
    const latestInterviewScore = academicProfile?.employabilityScore ?? academicProfile?.latestInterviewScore ?? 0;
    const techScore = academicProfile?.technicalScore ?? 0;
    const behavioralScore = academicProfile?.behavioralScore ?? 0;

    const pageTitle = currentNav === 'dashboard' ? 'My Dashboard' : currentNav === 'certificates' ? 'My Certificates' : currentNav === 'attendance' ? 'Compliance Hub' : currentNav === 'interviews' ? 'AI Mock Interviews' : currentNav === 'coaching' ? 'Coaching & Support Schedule' : currentNav === 'learning_units' ? 'Learner Content Hub' : 'My Profile';
    const pageSub = currentNav === 'dashboard' ? `Welcome back, ${store.user?.fullName || 'Learner'}` : currentNav === 'certificates' ? 'Official certificates issued to your profile' : currentNav === 'attendance' ? 'Track your compliance matrix and hardware log status' : currentNav === 'interviews' ? 'Practice real-time technical & behavioral interviews with AI' : currentNav === 'coaching' ? 'Track your requested 1-on-1 sessions, scheduled meetings, and facilitator notes' : currentNav === 'learning_units' ? 'Explore assigned courses, view curriculum syllabi, and resume video lessons' : 'Manage your personal details and account';
        ...
        <>
            ...
            <div className="admin-layout learner-layout">
                ...
                <main className="main-wrapper">
                    ...
                    <div className="admin-content" ref={contentScrollRef}>
                            ...
                            <div className="ld-animate">
                                ...
                                {showKPIs && (
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.25rem', marginBottom: '2rem' }} className="animate-slide-down">
                                        ...
                                        <ModuleProgressCard
                                            type="Employability Score"
                                            data={{
                                                total: 100,
                                                logged: latestInterviewScore,
                                                subValue: "AI Mock Interview Practice",
                                                lines: [
                                                    { label: 'Technical Proficiency', value: techScore, total: 100, color: '#3b82f6', bg: '#e0f2fe' },
                                                    { label: 'STAR Communication', value: behavioralScore, total: 100, color: '#10b981', bg: '#dcfce7' }
                                                ]
                                            }}
                        ...
                        {/* 🚀 VIEW 2: LEARNER CONTENT HUB & PLAYER */}
                        {currentNav === 'learning_units' && (
                            <div className="animate-fade-in">
                                <LearnerContentHub />
                            </div>
                        )}

                        {/* ════ VIEW 3: ISOLATED ATTENDANCE HUB ════ */}
                        ...
                        {/* ════ VIEW 4: COACHING SCHEDULE ════ */}
                        {currentNav === 'coaching' && (
                            <div className="animate-fade-in">
                                <CoachingScheduleView />
                            </div>
                        )}

                        {/* ════ VIEW 5: ACCOUNT PROFILE EDITOR ════ */}
                        {currentNav === 'profile' && (
                            <LearnerProfileView
                        ...
                        {/* ════ VIEW 6: SECURED BLOCKCHAIN ACCREDITATIONS ════ */}
                        ...
                        {/* ════ VIEW 7: AI MOCK INTERVIEW STUDIO WORKSPACE ════ */}
                        {currentNav === 'interviews' && (
                            <AiMockInterviewStudio
                                learnerName={store.user?.fullName || 'Learner'}
                                learnerId={academicProfile?.id || store.user?.uid || ''}
                            />
                        )}
                    </div>
                </main>
