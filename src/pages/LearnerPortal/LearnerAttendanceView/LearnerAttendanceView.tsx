...
export const LearnerAttendanceView: React.FC<LearnerAttendanceViewProps> = ({
    ...
    const [attendanceMode, setAttendanceMode] = useState<'all' | 'physical' | 'virtual'>('all');
    ...
    // ─── COMBINED LEDGER (MERGING PHYSICAL, VIRTUAL, & ABSENCES) ───
    const combinedLedger = useMemo(() => {
        // 1. Map Physical Kiosk Scans
    ...
    // 🚀 FIXED: INCLUDES ALL ENROLLED COHORTS (EVEN IF NO ATTENDANCE SCANS EXIST YET)
    const uniqueLedgerCohorts = useMemo(() => {
        const seen = new Set<string>();
        const list: { id: string; name: string }[] = [];

        // 1. Add all active/enrolled cohorts passed in via props
        cohorts.forEach(c => {
        ...
        // 2. Add any additional historical/transferred cohorts found in the attendance logs
            ...
            {activeTab === 'campus' && (
                <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'white', padding: '1rem', borderRadius: '0', border: '1px solid var(--mlab-border)', flexWrap: 'wrap', gap: '1rem' }}>
                        ...
                        {/* FORMAT MODE TOGGLE SWITCH (ALL | PHYSICAL | VIRTUAL) */}
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: '#f1f5f9', padding: '3px', border: '1px solid var(--mlab-border)' }}>
                            <button
                                className={`ld-filter-chip ${attendanceMode === 'all' ? 'active' : ''}`}
                                style={{ borderRadius: 0, padding: '4px 10px', fontSize: '0.75rem', background: attendanceMode === 'all' ? MIDNIGHT : 'transparent', color: attendanceMode === 'all' ? 'white' : 'var(--mlab-grey)' }}
                            ...
                            <button
                                ...
                                style={{ borderRadius: 0, padding: '4px 10px', fontSize: '0.75rem', background: attendanceMode === 'physical' ? MIDNIGHT : 'transparent', color: attendanceMode === 'physical' ? 'white' : 'var(--mlab-grey)' }}
                            ...
                            <button
                                ...
                                style={{ borderRadius: 0, padding: '4px 10px', fontSize: '0.75rem', background: attendanceMode === 'virtual' ? MIDNIGHT : 'transparent', color: attendanceMode === 'virtual' ? 'white' : 'var(--mlab-grey)' }}
                        ...
                        <div className="ld-filter-chips">
                            ...
                            <button className={`ld-filter-chip ${filterStatus === 'present' ? 'active' : ''}`} style={{ borderRadius: 0 }} onClick={() => setFilterStatus('present')}>Present</button>
                        ...
                        <table className="mlab-table" style={{ margin: 0, border: 'none' }}>
                            <thead style={{ background: 'var(--mlab-light-blue)' }}>
                                {attendanceMode === 'virtual' ? (
                                    /* DYNAMIC VIRTUAL ZOOM TABLE HEADERS */
                                    <tr>
                                    ...
                                    /* DYNAMIC PHYSICAL KIOSK TABLE HEADERS */
                                    <tr>
                                        <th style={{ color: 'var(--mlab-grey)', borderBottom: '2px solid var(--mlab-border)', borderTop: 'none' }}>Session Date</th>
                                    ...
                                    finalFilteredDisplayLedger.map((record, idx) => {
                                        ...
                                        return (
                                            <tr key={idx} style={{ background: record.type === 'absent' || record.virtualStatus === 'Absent' ? '#fef2f2' : 'white' }}>
                                                ...
                                                {/* ENROLLED COHORT, VIRTUAL TOPIC TITLE, DESCRIPTION & VIDEO LINK */}
                                                <td>
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                                                            ...
                                                            {record.type === 'virtual' && (
                                                                <span style={{ fontSize: '0.62rem', background: '#f5f3ff', color: '#7c3aed', padding: '1px 6px', fontWeight: 800, border: '1px solid #ddd6fe', textTransform: 'uppercase' }}>
                                                                    Virtual Session
                                                                </span>
                                                            )}
                                                        </div>
                                                        ...
                                                        {/* CLICKABLE VIDEO RECORDING LINK - MATCHES BOOTCAMP COHORT VIEW */}
                                                        {videoLink && (
                                                ...
                                                {/* DYNAMIC ROW BODY CELL RENDERING */}
                                                {attendanceMode === 'virtual' ? (
                                                    /* 6-Column Dedicated Virtual Cells */
                                                    <>
                                                        ...
                                                        <td style={{ textAlign: 'center' }}>
                                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', flexWrap: 'wrap' }}>
                                                                <span style={{ fontSize: '0.75rem', fontWeight: 800, padding: '2px 8px', border: '1px solid var(--mlab-border)', background: '#f8fafc', color: record.virtualStatus === 'Present' ? '#16a34a' : record.virtualStatus === 'Partial' ? '#ea580c' : '#dc2626' }}>
                                                    ...
                                                    <td colSpan={4} style={{ textAlign: 'center', background: '#f8fafc', color: 'var(--mlab-midnight)' }}>
                                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', fontSize: '0.85rem', fontWeight: 600, flexWrap: 'wrap' }}>
                                                            ...
                                                            <span style={{ fontSize: '0.7rem', color: 'var(--mlab-grey)', background: 'white', padding: '2px 8px', border: '1px solid var(--mlab-border)' }}>
                                                                {record.expectedDuration > 0 ? Math.round((record.actualDuration / record.expectedDuration) * 100) : 0}% Compliance
                                                            </span>
                                                            {videoLink && (
                                                                <a
...
//                                                         {/* 🚀 CLICKABLE VIDEO RECORDING LINK - MATCHES BOOTCAMP COHORT VIEW EXACTLY */}
//                                                         {videoLink && (
//                                                             <div style={{ marginTop: '2px' }}>
//                                                                 <a
//                                                                     href={videoLink}
...
// //                 actualDuration: rec.actualDuration || 0,
// //                 expectedDuration: rec.expectedDuration || 0,
// //                 virtualStatus: rec.status // "Present", "Partial", "Absent"
// //             };
// //         });
...
// //     const finalFilteredDisplayLedger = useMemo(() => {
// //         return cohortFilteredLedger.filter(record => {
// //             // Mode filter (Physical vs Virtual)
// //             if (attendanceMode === 'physical' && record.type === 'virtual') return false;
// //             if (attendanceMode === 'virtual' && record.type !== 'virtual') return false;
...
// //                         {/* 🚀 NEW: FORMAT MODE TOGGLE SWITCH (ALL | PHYSICAL | VIRTUAL) */}
// //                         <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: '#f1f5f9', padding: '3px', border: '1px solid var(--mlab-border)' }}>
// //                             <button
// //                                 className={`ld-filter-chip ${attendanceMode === 'all' ? 'active' : ''}`}
// //                                 style={{ borderRadius: 0, padding: '4px 10px', fontSize: '0.75rem', background: attendanceMode === 'all' ? MIDNIGHT : 'transparent', color: attendanceMode === 'all' ? 'white' : 'var(--mlab-grey)' }}
...
// //                                         <th style={{ color: 'var(--mlab-grey)', borderBottom: '2px solid var(--mlab-border)', borderTop: 'none', textAlign: 'center' }}>Time Attended</th>
// //                                         <th style={{ color: 'var(--mlab-grey)', borderBottom: '2px solid var(--mlab-border)', borderTop: 'none', textAlign: 'center' }}>Session Duration</th>
// //                                         <th style={{ color: 'var(--mlab-grey)', borderBottom: '2px solid var(--mlab-border)', borderTop: 'none', textAlign: 'center' }}>Compliance Rate</th>
// //                                     </tr>
// //                                 ) : attendanceMode === 'physical' ? (
// //                                     /* 🚀 DYNAMIC PHYSICAL KIOSK TABLE HEADERS */
// //                                     <tr>
...
// //                                             ) : record.type === 'virtual' ? (
// //                                                 <td colSpan={4} style={{ textAlign: 'center', background: '#f8fafc', color: 'var(--mlab-midnight)' }}>
// //                                                     <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', fontSize: '0.85rem', fontWeight: 600 }}>
// //                                                         <Video size={14} color="var(--mlab-blue)" /> 
// //                                                         Zoom Session Attended: 
// //                                                         <span style={{ color: record.virtualStatus === 'Present' ? '#16a34a' : record.virtualStatus === 'Partial' ? '#ea580c' : '#dc2626' }}>
// //                                                             {record.actualDuration} / {record.expectedDuration} mins
...
// // // export interface LearnerAttendanceViewProps {
// // //     formattedScanHistory: any[];
// // //     virtualAttendance?: any[]; // 🚀 FIXED: Added optional virtualAttendance prop type
// // //     absenceDates: (AbsenceRecord | string)[];
// // //     attendancePercentage: string;
// // //     cohorts: any[];
// // //     workplaceLogs: any[];
...
// // //     // ─── INTERNAL UI STATE ───
// // //     const [activeTab, setActiveTab] = useState<'campus' | 'workplace'>('campus');
// // //     const [filterStatus, setFilterStatus] = useState<'all' | 'present' | 'absent' | 'virtual'>('all');
// // //     const [selectedCohortId, setSelectedCohortId] = useState<string>('all');
// // //     const [dateFilterMode, setDateFilterMode] = useState<'month' | 'week' | 'date'>('month');
// // //     const [dateSearch, setDateSearch] = useState('');
...
// // //             } else {
// // //                 const existing = mergedMap.get(key);
// // //                 if (existing.type === 'absent' && rec.type !== 'absent') {
// // //                     mergedMap.set(key, rec);
// // //                 }
// // //             }
// // //         });
...
// // //         return cohortFilteredLedger.filter(record => {
// // //             if (filterStatus !== 'all') {
// // //                 if (filterStatus === 'present' && !['present', 'virtual'].includes(record.type)) return false;
// // //                 if (filterStatus === 'absent' && record.type !== 'absent') return false;
// // //                 if (filterStatus === 'virtual' && record.type !== 'virtual') return false;
// // //             }
// // //             if (dateSearch) {
// // //                 if (dateFilterMode === 'month' && moment(record.dateObj).format('YYYY-MM') !== dateSearch) return false;
// // //                 if (dateFilterMode === 'week' && moment(record.dateObj).format('YYYY-[W]WW') !== dateSearch) return false;
...
// // //     const stats = useMemo(() => {
// // //         const presentCount = combinedLedger.filter(r => ['present', 'virtual'].includes(r.type)).length;
// // //         const absentCount = combinedLedger.filter(r => r.type === 'absent').length;
// // //         const total = presentCount + absentCount;
// // //         const ratio = total === 0 ? "100%" : Math.round((presentCount / total) * 100) + "%";
...
// // //             currentMonthEarnedStipend, baseStipendUsed: baseWageAmount
// // //         };
// // //     }, [combinedLedger, workplaceLogs, stipendAmount, directStipend, publicHolidays]);
...
// // //                         <div className="ld-filter-chips">
// // //                             <button className={`ld-filter-chip ${filterStatus === 'all' ? 'active' : ''}`} style={{ borderRadius: 0 }} onClick={() => setFilterStatus('all')}>All Statuses</button>
// // //                             <button className={`ld-filter-chip ${filterStatus === 'present' ? 'active' : ''}`} style={{ borderRadius: 0 }} onClick={() => setFilterStatus('present')}>Present / Virtual</button>
// // //                             <button className={`ld-filter-chip ${filterStatus === 'absent' ? 'active' : ''}`} style={{ borderRadius: 0 }} onClick={() => setFilterStatus('absent')}>Absent Only</button>
// // //                         </div>
// // //                     </div>
