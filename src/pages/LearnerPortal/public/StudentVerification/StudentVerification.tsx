...
import { ShieldCheck, ShieldAlert, Loader2, Calendar, MapPin, User, CheckCircle2, XCircle, AlertCircle } from 'lucide-react';
...
export const StudentVerification: React.FC = () => {
    ...
    useEffect(() => {
        const verifyStudent = async () => {
            ...
            try {
                // 1. Find the enrollment record using the verification code
                const q = query(
                ...
                // 2. Check if the enrollment is expired or dropped
                const isExpired = moment().isAfter(moment(enrollment.endDate)) || enrollment.status !== 'active';

                // 3. Fetch the Learner's public profile data (No sensitive info!)
                const learnerSnap = await getDoc(doc(db, 'learners', enrollment.learnerId));
                const learner = learnerSnap.exists() ? learnerSnap.data() : null;

                setStudentData({
                    name: learner?.fullName || 'Unknown Learner',
                    cohort: enrollment.cohortName || 'CodeTribe Academy',
                    campus: enrollment.campusName || enrollment.location || 'mLab Campus',
                    validThru: enrollment.endDate,
        ...
        <div className="lfm-overlay" style={{ position: 'fixed', inset: 0, zIndex: 99999 }}>
            <div className="lfm-modal" style={{ maxWidth: '480px', margin: 'auto' }}>
                ...
                <div className="lfm-body" style={{ padding: '2rem 1.5rem' }}>
                    ...
                    {/* STUDENT DEMOGRAPHICS PANEL (Matches mLab visual style) */}
                ...
                <div className="lfm-footer" style={{ justifyContent: 'center', padding: '1rem' }}>
                    <div style={{
                        ...
                        mLab Secure Blockchain Registry
                    </div>
                </div>
