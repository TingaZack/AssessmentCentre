...
export const RoleProtectedRoute: React.FC<Props> = ({
    ...
    const { user, setUser, loading } = useStore() as any;
    ...
    const rawUploadedDocs = (activeUser as any)?.uploadedDocuments;
    ...
    const d = (activeUser as any)?.demographics || {};

    const hasPopiaConsent =
        (activeUser as any)?.popiaConsent === true ||
        d.popiaConsent === true;

    const isLearnerFullyCompliant =
        activeUser?.role === 'learner' &&
        activeUser?.profileCompleted === true &&
        !!d.equityCode &&
        !!d.provinceCode &&
    ...
    const firstShownAtRaw = (activeUser as any)?.addressNoticeFirstShownAt;

    useEffect(() => {
        if (activeUser?.uid && activeUser.role === 'learner' && activeUser.profileCompleted === true && !isLearnerFullyCompliant && !firstShownAtRaw) {
    ...
    }, [activeUser?.uid, activeUser?.role, activeUser?.profileCompleted, isLearnerFullyCompliant, firstShownAtRaw, activeUser?.isTemporaryAuditor]);
    ...
    // 🚀 MAGIC LINK AUDITOR BYPASS:
    if (activeUser?.isTemporaryAuditor === true && (activeUser.role === 'qcto_auditor' || activeUser.role === 'seta_verifier')) {
        const hasAccess = allowedRoles.includes(activeUser.role);
        if (!hasAccess) {
            return (
    ...
    // 🔒 STANDARD USERS: Must have a valid Firebase Auth session
    if (!activeUser || !auth.currentUser) return <Navigate to="/login" state={{ from: location }} replace />;
    ...
    const requiresSignature = ["facilitator", "assistant_facilitator", "assessor", "moderator", "mentor", "learner"].includes(activeUser.role);
    if (requiresSignature && !activeUser.signatureUrl) {
        return (
            <SignatureSetupModal
                userUid={activeUser.uid}
                existingSignatureUrl={activeUser.signatureUrl}
                onComplete={() => window.location.reload()}
            />
        );
    }

    const hasStaffProvince = !!(activeUser as any).province;
    const isForeign = (activeUser as any).nationalityType === 'Foreign National';
    const hasPermit = isForeign ? hasDoc('permit') : true;

    const checkStaffCompliance = () => {
        if (activeUser.profileCompleted !== true) return false;
        ...
        switch (activeUser.role) {
            ...
            case 'assistant_admin':
                if ((activeUser as any).isSuperAdmin) return true;
                const hasSignature = hasDoc('signature') || !!(activeUser as any).signatureUrl;
    ...
    if (activeUser.role === "learner" && !hasPopiaConsent && location.pathname !== "/setup-profile") {
        return (
            <>
    ...
    if (activeUser.role === "learner" && !isLearnerFullyCompliant && location.pathname !== "/setup-profile") {
        if (!activeUser.profileCompleted) {
    ...
    if ((activeUser.role === "facilitator" || activeUser.role === "assistant_facilitator") && !isStaffFullyCompliant && location.pathname !== "/setup-facilitator") {
        return <Navigate to="/setup-facilitator" replace />;
    }

    if (activeUser.role === "assessor" && !isStaffFullyCompliant && location.pathname !== "/setup-assessor") return <Navigate to="/setup-assessor" replace />;
    if (activeUser.role === "moderator" && !isStaffFullyCompliant && location.pathname !== "/setup-moderator") return <Navigate to="/setup-moderator" replace />;
    if (activeUser.role === "mentor" && !isStaffFullyCompliant && location.pathname !== "/setup-mentor") return <Navigate to="/setup-mentor" replace />;

    if ((activeUser.role === "admin" || activeUser.role === "assistant_admin") && !isStaffFullyCompliant && !location.pathname.startsWith("/setup-admin") && !location.pathname.startsWith("/admin/profile")) {
        return <Navigate to="/setup-admin" replace />;
    }

    if (requireSuperAdmin && (activeUser as any).isSuperAdmin !== true) {
        return (
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, background: "var(--mlab-bg)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "1rem", zIndex: 9999 }}>
                ...
                <div style={{ background: "white", padding: "1.5rem", borderRadius: "8px", border: "1px solid #e2e8f0", textAlign: "center", maxWidth: "400px" }}>
                    <p style={{ margin: "0 0 0.5rem 0", color: "#0f172a" }}>Logged in as: <strong style={{ textTransform: "uppercase", color: "var(--mlab-grey)" }}>Standard {activeUser.role}</strong></p>
    ...
    const secondaryRoles = Array.isArray((activeUser as any)?.secondaryRoles) ? (activeUser as any).secondaryRoles : [];
    const hasAccess =
        allowedRoles.includes(activeUser.role) ||
        activeUser.role === "admin" ||
        activeUser.role === "assistant_admin" ||
...
//             // Check to prevent trying to write to auditor token sessions
//             if (!user.isTemporaryAuditor) {
//                 updateDoc(doc(db, 'users', user.uid), {
//                     addressNoticeFirstShownAt: serverTimestamp()
//                 }).catch(console.error);
//             }
//         }
//     }, [user?.uid, user?.role, user?.profileCompleted, isLearnerFullyCompliant, firstShownAtRaw, user?.isTemporaryAuditor]);
...
//     // MAGIC LINK AUDITOR BYPASS:
//     // If the user has a temporary Magic Link session, bypass ALL standard Firebase Auth & Compliance checks.
//     if (user?.isTemporaryAuditor === true && (user.role === 'qcto_auditor' || user.role === 'seta_verifier')) {
...
//     if (user.role === "learner" && !hasPopiaConsent && location.pathname !== "/setup-profile") {
//         return (
//             <>
//                 <MandatoryPolicyModal />
//                 {children}
...
// //     // 🚀 EXPLICIT POPIA & CODE OF CONDUCT CONSENT CHECK
// //     // Evaluates true ONLY if the user has explicitly accepted the updated policy modal
// //     const hasPopiaConsent =
// //         (user as any)?.popiaConsent === true ||
// //         d.popiaConsent === true;
...
// //     // 🚀 MANDATORY POPUP FOR LEARNERS WHO HAVE NOT CONSENTED TO THE NEW POLICY
// //     if (user.role === "learner" && !hasPopiaConsent && location.pathname !== "/setup-profile") {
// //         return (
// //             <>
// //                 <MandatoryPolicyModal />
