...
export const RoleProtectedRoute: React.FC<Props> = ({
    ...
    // Safe helpers for learner compliance evaluation
    ...
    // 🚀 STAMP START DATE FOR EXISTING (LEGACY) LEARNERS ONLY
    useEffect(() => {
    ...
    // Parse Firestore Timestamp or ISO String safely
    ...
    // --- LEARNER COMPLIANCE GATE ---
    if (user.role === "learner" && !isLearnerFullyCompliant && location.pathname !== "/setup-profile") {

        // 🚀 1. NEW LEARNERS (Initial Onboarding): Direct redirect without popup
        if (!user.profileCompleted) {
            return <Navigate to="/setup-profile" replace />;
        }

        // 🚀 2. EXISTING LEARNERS (Legacy Compliance Update): Show Grace Period Popup Intercept
    ...
    const hasAccess = allowedRoles.includes(user.role) || user.role === "admin" || user.role === "assistant_admin";
    if (!hasAccess) {
        return (
...
//     // 🚀 STAMP START DATE WITH FIREBASE SERVER TIMESTAMP (NOT LOCAL CLOCK)
//     useEffect(() => {
//         if (user?.uid && user.role === 'learner' && !isLearnerFullyCompliant && !firstShownAtRaw) {
//             updateDoc(doc(db, 'users', user.uid), {
//                 addressNoticeFirstShownAt: serverTimestamp()
//             }).catch(console.error);
//         }
//     }, [user?.uid, user?.role, isLearnerFullyCompliant, firstShownAtRaw]);
...
//     // 🚀 ANTI-TAMPER EVALUATION
//     // 1. Expired if current time - start time > 72 hours
//     // 2. Expired if local device clock is set BEFORE start time (user turned clock backward)
//     const isClockTampered = firstShownAtMs !== null && currentNow < firstShownAtMs;
//     const isGracePeriodExpired = firstShownAtMs !== null && (
//         (currentNow - firstShownAtMs > THREE_DAYS_MS) || isClockTampered
//     );
...
//     // --- LEARNER COMPLIANCE INTERCEPT POPUP ---
//     if (user.role === "learner" && !isLearnerFullyCompliant && location.pathname !== "/setup-profile") {

//         // Modal A: Grace Period Expired or Clock Tampered -> HARD BLOCK
//         if (isGracePeriodExpired) {
//             return (
//                 <div style={{ position: "fixed", inset: 0, background: "rgba(15, 23, 42, 0.85)", zIndex: 99999, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
//                     <StatusModal
...
//         // Modal B: Active Grace Period -> SOFT PROMPT WITH "DO IT LATER"
//         if (!dismissedNotice) {
//             return (
//                 <div style={{ position: "fixed", inset: 0, background: "rgba(15, 23, 42, 0.8)", zIndex: 99999, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
//                     <StatusModal
...
// //     // --- SIGNATURE GATE ---
// //     // 🚀 FIX: Added assistant_facilitator
// //     const requiresSignature = ["facilitator", "assistant_facilitator", "assessor", "moderator", "mentor", "learner"].includes(user.role);
// //     if (requiresSignature && !user.signatureUrl) {
// //         return (
// //             <SignatureSetupModal
...
// //         switch (user.role) {
// //             case 'facilitator':
// //             case 'assistant_facilitator': // 🚀 FIX: Added assistant_facilitator to the compliance check
// //                 return hasStaffProvince && hasDoc('id') && hasDoc('cv') && hasPermit;
...
// //     // 🚀 FIX: Include assistant_facilitator in the facilitator setup gate
// //     if ((user.role === "facilitator" || user.role === "assistant_facilitator") && !isStaffFullyCompliant && location.pathname !== "/setup-facilitator") {
// //         return <Navigate to="/setup-facilitator" replace />;
// //     }
