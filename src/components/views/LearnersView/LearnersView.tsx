...
export const LearnersView: React.FC<LearnersViewProps> = ({
    ...
    const handleSaveBulkResults = async (parsedLearners: any[]) => {
        try {
            ...
            parsedLearners.forEach(learner => {
                // 🚀 PREVENT 'UNASSIGNED' COHORT ID CRASH: Use empty string if no valid ID exists
                ...
                if (learner.isUpdate && learner.existingId) {
                    ...
                    batch.set(learnerRef, {
                        ...
                        learnerId: profileId,
                        authUid: profileId,
                        cohortId: targetCohortId || deleteField(), // Securely clear field if unassigned
                        status: "active",
                        authStatus: "pending",
                        isDraft: false,
                        isOffline: true,
    ...
    // 🚀 FLEXIBLE MANUAL PROMOTION ENGINE - SAFELY HANDLES UNASSIGNED
    const handleConfirmPromote = async () => {
        ...
        try {
            ...
            // DO NOT use the string "Unassigned" as this breaks Firebase triggers.
                ...
                <div className="mlab-action-bar">
                    ...
                    <div className="mlab-bulk-actions">
                        ...
                        {viewMode === 'bootcamp' && !showArchived && (
                            <button className="wm-btn wm-btn--primary" style={{ background: 'var(--mlab-green)', borderColor: 'var(--mlab-green-dark)' }} onClick={() => executeBulkAction('promote')}>
                                    ...
                                    <td style={{ textAlign: 'right' }}>
                                        <div className="mlab-icon-btn-group" style={{ justifyContent: 'flex-end' }}>
                                            ...
                                            {viewMode === 'bootcamp' && !learner.isArchived && (
                                                <button className="mlab-icon-btn mlab-icon-btn--green" onClick={() => setPromotingLearners([learner])} title="Promote to Active Enrollment">
                                                    <UserCheck size={14} />
                                                </button>
                                            )}
            ...
            {promotingLearners && promotingLearners.length > 0 && createPortal(
                <div className="wm-overlay animate-fade-in" onClick={() => setPromotingLearners(null)} style={{ zIndex: 99999 }}>
                    <div className="wm-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '500px' }}>
                        ...
                        <div className="wm-modal__footer">
                            <button className="wm-btn wm-btn--ghost" onClick={() => setPromotingLearners(null)} disabled={isPromoting}>Cancel</button>
                            <button className="wm-btn wm-btn--primary" style={{ background: 'var(--mlab-green)', borderColor: 'var(--mlab-green-dark)' }} onClick={handleConfirmPromote} disabled={isPromoting}>
...
//     // 🚀 FLEXIBLE MANUAL PROMOTION ENGINE
//     const handleConfirmPromote = async () => {
//         if (!promotingLearners || promotingLearners.length === 0) return;
...
//             // 🚀 Allows passing without a cohort (General Pool)
//             const targetCohortId = promoteCohortId || "Unassigned";

//             promotingLearners.forEach(learner => {
//                 const trueLearnerId = learner.idNumber || learner.learnerId || (learner.id?.includes('_') ? learner.id.substring(learner.id.indexOf('_') + 1) : learner.id);

//                 const learnerRef = doc(db, "learners", trueLearnerId);
//                 const newEnrollmentId = `${targetCohortId}_${trueLearnerId}`;
//                 const enrollmentRef = doc(db, "enrollments", newEnrollmentId);

//                 // 1. Update the learner profile to active (removes bootcamp flag)
//                 batch.set(learnerRef, {
//                     isBootcamp: false,
//                     cohortId: targetCohortId === "Unassigned" ? deleteField() : targetCohortId,
//                     enrollmentId: newEnrollmentId,
//                     authStatus: "pending",
//                     status: "active",
//                     updatedAt: timestamp
...
// //     // 🚀 NEW: Unified Status Modal State to eliminate native alerts
// //     const [statusModal, setStatusModal] = useState<{
// //         isOpen: boolean;
// //         type: 'info' | 'success' | 'error' | 'warning';
// //         title: string;
...
// //     // 🚀 Extracted Force Active execution logic for the Status Modal
// //     const executeForceActive = async (learner: DashboardLearner) => {
// //         setStatusModal(null);
// //         try {
// //             const batch = writeBatch(db);
...
// //     // 🚀 MANUAL PROMOTION ENGINE WITH FIXES
// //     const handleConfirmPromote = async () => {
...
// //                 // 3. 🚀 FIX: Mark old enrollment as transferred AND remove isBootcamp flag so they drop out of the Bootcamp UI
// //                 const oldEnrollmentId = learner.enrollmentId || (learner.id?.includes('_') ? learner.id : null);
// //                 if (oldEnrollmentId && oldEnrollmentId !== newEnrollmentId) {
// //                     batch.set(doc(db, "enrollments", oldEnrollmentId), {
// //                         status: "transferred",
...
// //                         <div className="wm-modal__footer">
// //                             <button className="wm-btn wm-btn--ghost" onClick={() => setPromotingLearners(null)} disabled={isPromoting}>Cancel</button>
// //                             <button className="wm-btn wm-btn--primary" style={{ background: 'var(--mlab-green)', borderColor: 'var(--mlab-green-dark)' }} onClick={handleConfirmPromote} disabled={isPromoting || !promoteCohortId}>
// //                                 {isPromoting ? <><Loader2 className="spin" size={16} /> Promoting...</> : <><UserCheck size={16} /> Confirm Promotion</>}
// //                             </button>
// //                         </div>
// //                     </div>
...
// // //             onBulkArchive?.(selected);
// // //         } else if (action === 'promote') {
// // //             // Trigger the promote modal for bulk selections
// // //             setPromotingLearners(selected);
// // //         } else if (action === 'discard') {
// // //             onBulkDiscard?.(selected);
// // //         }
...
// // //     // 🚀 MANUAL PROMOTION ENGINE
// // //     const handleConfirmPromote = async () => {
// // //         if (!promotingLearners || promotingLearners.length === 0 || !promoteCohortId) return;

// // //         // 🚀 DIAGNOSTIC LOGGING: Let's inspect the exact IDs available!
// // //         console.log("=== DIAGNOSTIC: PROMOTING LEARNERS IDs ===");
// // //         promotingLearners.forEach((learner, index) => {
// // //             console.log(`[Learner ${index + 1}] ${learner.fullName}`);
// // //             console.log(`  -> learner.idNumber:`, learner.idNumber);
// // //             console.log(`  -> learner.learnerId:`, learner.learnerId);
// // //             console.log(`  -> learner.id:`, learner.id);
// // //             console.log(`  -> learner.enrollmentId:`, learner.enrollmentId);
// // //             console.log("------------------------------------------");
// // //         });
...
// // //             promotingLearners.forEach(learner => {
// // //                 // 🚀 FIX: Safely extract the pure learner ID prioritizing idNumber (as the DB is keyed by it)
// // //                 const trueLearnerId = learner.idNumber || learner.learnerId || (learner.id?.includes('_') ? learner.id.substring(learner.id.indexOf('_') + 1) : learner.id);
...
// // //                     cohortId: promoteCohortId,
// // //                     enrollmentId: newEnrollmentId,
// // //                     authStatus: "pending", // 🚀 MANUAL MODE: Forces "Pending Setup" without triggering email
// // //                     status: "active",
// // //                     updatedAt: timestamp
// // //                 }, { merge: true });
...
// // //                 // 3. Mark the old bootcamp enrollment as transferred (preserves history)
// // //                 const oldEnrollmentId = learner.enrollmentId || (learner.id?.includes('_') ? learner.id : null);
// // //                 if (oldEnrollmentId && oldEnrollmentId !== newEnrollmentId) {
// // //                     batch.set(doc(db, "enrollments", oldEnrollmentId), {
// // //                         status: "transferred",
...
// // //             alert(`Successfully promoted ${promotingLearners.length} learner(s) to active enrollment! They are now ready for manual email invites.`);

// // //         } catch (error) {
// // //             console.error("Promotion failed", error);
// // //             alert("Failed to promote learners to active status.");
// // //         } finally {
// // //             setIsPromoting(false);
// // //         }
// // //     };
...
// // // //                 const enrollmentId = `${promoteCohortId}_${learner.id}`;
// // // //                 const enrollmentRef = doc(db, "enrollments", enrollmentId);

// // // //                 // 1. Update the learner profile to active
// // // //                 batch.update(learnerRef, {
// // // //                     isBootcamp: false,
// // // //                     cohortId: promoteCohortId,
// // // //                     enrollmentId: enrollmentId,
// // // //                     authStatus: "pending", // Sets them up to receive the email invite
// // // //                     status: "active",
// // // //                     updatedAt: timestamp
// // // //                 });
...
// // // //                             <button className="wm-modal__close" onClick={() => setPromotingLearners(null)} disabled={isPromoting}><X size={18} /></button>
// // // //                         </div>
// // // //                         <div className="wm-modal__body" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>

// // // //                             <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '0.85rem', color: '#475569' }}>
// // // //                                 This action will transfer the selected learners out of the Bootcamp ledger and place them into the active enrollment pipeline. They will be flagged as <strong>Pending Setup</strong> so you can send them an email invite to create their password.
// // // //                             </div>

// // // //                             <div className="mlab-form-group">
// // // //                                 <label style={{ fontWeight: 700, color: 'var(--mlab-midnight)', fontSize: '0.85rem', marginBottom: '8px', display: 'block' }}>
// // // //                                     Assign to Class / Cohort <span style={{ color: 'var(--mlab-red)' }}>*</span>
// // // //                                 </label>
// // // //                                 <select
