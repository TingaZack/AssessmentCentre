...
export const LearnersView: React.FC<LearnersViewProps> = ({
    ...
    const updateUrlParam = (key: string, value: string | boolean | number) => {
        ...
        // 🚀 ADD THESE 3 LINES: Stop the loop if the value hasn't actually changed
    ...
    const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked) setSelectedIds(new Set(paginatedLearners.map(l => l.id)));
                ...
                <table className="mlab-table">
                    <thead>
                        <tr>
                            <th style={{ width: '40px', textAlign: 'center' }}>
                                <input type="checkbox" onChange={handleSelectAll} checked={paginatedLearners.length > 0 && selectedIds.size === paginatedLearners.length} />
                            </th>
                            <th>Learner</th>
                    ...
                    <tbody>
                        {paginatedLearners.map(learner => {
                            ...
                            const rowClass = [learner.isArchived ? 'mlab-tr--archived' : '', isSelected ? 'mlab-tr--selected' : ''].filter(Boolean).join(' ');
                            const isDormant = !learner.enrollmentId;
                            ...
                            // 🚀 FIXED: Robust Multi-Enrollment Resolution System
                            ...
                            return (
                                <tr key={learner.id} className={rowClass}>
                                    <td style={{ textAlign: 'center' }}>
                                        <input type="checkbox" checked={isSelected} onChange={() => handleSelectOne(learner.id)} />
                                    </td>
                                    <td>
                                        <div className="mlab-cell-content">
                                            <div className="mlab-cell-header">
                                                ...
                                                {/* 🚀 FIXED: Dynamic Multi-Enrollment Indicator Hover State */}
                                                {isReturning && (
                                            ...
                                            <div className="mlab-cell-sub">{learner.idNumber} • {learner.trainingStartDate?.substring(0, 4) || "No Year"}</div>
                                    ...
                                    <td style={{ textAlign: 'right' }}>
                                        <div className="mlab-icon-btn-group" style={{ justifyContent: 'flex-end' }}>
                                            {viewMode === 'staging' && <button className="mlab-icon-btn mlab-icon-btn--green" onClick={() => { setApprovalMode('standard'); setApprovingLearners([learner]); }} title="Approve & Import"><ClipboardCheck size={14} /></button>}
...
//             fetchCohorts();
//         }
//     }, [programmes?.length, activeCohorts?.length, fetchProgrammes, fetchCohorts]);
...
// //     // 🚀 FIXED: Extracted fetchProgrammes, fetchCohorts, and storeCohorts to ensure data loads on mount
// //     const {
// //         user: currentUser, settings, programmes,
// //         fetchLearners, fetchProgrammes, fetchCohorts,
// //         cohorts: storeCohorts
...
// //     // 🚀 FIXED: Fallback to store cohorts if the parent prop hasn't passed them yet
// //     const activeCohorts = (cohorts && cohorts.length > 0) ? cohorts : (storeCohorts || []);

// //     // 🚀 FIXED: Instantly trigger background fetches if this tab was opened first
// //     useEffect(() => {
// //         if (!programmes?.length && fetchProgrammes) {
// //             fetchProgrammes();
// //         }
...
// //     // Isolated local input text layer prevents router/UI rendering lag when typing
// //     const [localSearch, setLocalSearch] = useState(urlSearchTerm);
...
// //     // Accelerated state modifier routine with automatic pagination reset parameters
// //     const updateUrlParam = (key: string, value: string | boolean | number) => {
// //         const nextParams = new URLSearchParams(searchParams);
// //         if (!value || value === 'all' || value === 'false') {
// //             nextParams.delete(key);
...
// //             nextParams.set(key, String(value));
// //         }
// //         // Force list context reset to page 1 upon modifying lookup parameters
// //         if (key !== 'page') {
// //             nextParams.delete('page');
// //         }
// //         setSearchParams(nextParams, { replace: true });
...
// //     // Fast O(1) Dictionary Caching Maps. Changes lookup loop complexity from O(N * M) down to immediate O(1) reads
// //     const cohortDict = useMemo(() => {
// //         const dict: Record<string, Cohort> = {};
// //         activeCohorts.forEach(c => { if (c.id) dict[c.id] = c; });
// //         return dict;
...
// //         for (let i = 0; i < learners.length; i++) {
// //             const num = learners[i].idNumber;
// //             if (num) counts[num] = (counts[num] || 0) + 1;
// //         }
// //         for (let i = 0; i < stagingLearners.length; i++) {
// //             const num = stagingLearners[i].idNumber;
// //             if (num) counts[num] = (counts[num] || 0) + 1;
// //         }
// //         return counts;
// //     }, [learners, stagingLearners]);

// //     // Optimized search filter routine using dictionary hashing indices
// //     const filteredLearners = useMemo(() => {
// //         let sourceData: DashboardLearner[] = [];
...
// //     // UI Pagination slicing pattern to prevent browser DOM paint complexity crashes
// //     const totalPages = Math.ceil(filteredLearners.length / ITEMS_PER_PAGE);
// //     const paginatedLearners = useMemo(() => {
// //         const startIdx = (currentPage - 1) * ITEMS_PER_PAGE;
// //         return filteredLearners.slice(startIdx, startIdx + ITEMS_PER_PAGE);
...
// //                             // 🚀 FIXED: Robust qualification resolution checking multiple IDs safely
// //                             const activeProgId = isDormant ? null : ((learner as any).programmeId || (learner as any).qualificationId || cohortObj?.programmeId || cohortObj?.qualificationId);
// //                             const progObj = activeProgId ? progDict[activeProgId] : null;
...
// //                                                 <span className="mlab-cell-name">{learner.fullName}</span>
// //                                                 {learner.isArchived && <span className="mlab-mini-badge mlab-mini-badge--archived">Archived</span>}
// //                                                 {isReturning && <span className="mlab-mini-badge mlab-mini-badge--multi" title="Enrolled in multiple classes"><History size={10} /> Multi-Course</span>}
// //                                             </div>
// //                                             <div className="mlab-cell-sub">{learner.idNumber} • {learner.trainingStartDate?.substring(0, 4) || "No Year"}</div>
// //                                             {viewMode === 'staging' && (learner as any).isExistingUser && (
// //                                                 <div style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', marginTop: '4px', padding: '2px 6px', background: '#e0f2fe', color: '#0369a1', borderRadius: '4px', fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase' }}>
