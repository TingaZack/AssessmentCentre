// src/pages/LearnersPage/views/LearnersView.tsx
...
import {
    ...
    ShieldCheck, X, AlertCircle, CheckCircle,
    Loader2, MapPin, Award, FileSpreadsheet, FileText, Layers, Filter, CheckSquare,
    UserCheck, MonitorOff, UserMinus, ChevronLeft, ChevronRight
} from 'lucide-react';
import { useNavigate, useSearchParams } from 'react-router-dom';
import { doc, deleteDoc, writeBatch, collection, deleteField } from 'firebase/firestore';
...
export const LearnersView: React.FC<LearnersViewProps> = ({
    ...
    const [promotingLearners, setPromotingLearners] = useState<DashboardLearner[] | null>(null);
    const [promoteCohortId, setPromoteCohortId] = useState<string>('');
    ...
    // 🚀 UNIFIED STATUS MODAL STATE FOR ALERTS & CONFIRMATIONS
    ...
    const handleSelectOne = (learner: DashboardLearner) => {
        ...
        if (viewMode === 'staging' && (!learner.idNumber || learner.idNumber.trim() === "" || isScientificNotation)) {
            setStatusModal({
                ...
                message: `This profile data is corrupt due to a missing ID or Excel float serialization (${learner.idNumber}). You must edit the field layout before approving.`,
    ...
    const executeBulkAction = (action: 'approve' | 'restore' | 'archive' | 'discard' | 'promote') => {
        ...
        if (action === 'approve') {
            ...
            setStatusModal({
                ...
                type: 'warning',
                title: 'Bulk Archive',
                message: `Are you sure you want to archive ${selected.length} selected enrollment(s)?`,
            ...
            setStatusModal({
                isOpen: true,
                type: 'error',
                title: 'Bulk Discard Drafts',
                message: `Are you sure you want to discard ${selected.length} selected staging draft(s)? This action cannot be undone.`,
    ...
    const executeForceActive = async (learner: DashboardLearner) => {
        setStatusModal(null);
        try {
            ...
            setStatusModal({
                ...
                message: `${learner.fullName} has been forced Active.`,
                onConfirm: () => setStatusModal(null)
            });
            ...
            setStatusModal({
                ...
                type: 'error',
                title: 'Update Failed',
                message: 'Failed to force active status.',
                onConfirm: () => setStatusModal(null)
            });
        }
    };

    const handleForceActive = (learner: DashboardLearner) => {
        setStatusModal({
            isOpen: true,
            type: 'warning',
            title: 'Force Active Status',
            message: `Force ${learner.fullName} to Active status? This overrides the email invite requirement.`,
            confirmText: 'Yes, Force Active',
            cancelText: 'Cancel',
            onCancel: () => setStatusModal(null),
            onConfirm: () => executeForceActive(learner)
        });
    };
    ...
    const handleSaveBulkResults = async (parsedLearners: any[]) => {
        try {
            ...
            parsedLearners.forEach(learner => {
                const targetCohortId = learner.cohortId || "";
                ...
                if (learner.isUpdate && learner.existingId) {
                    ...
                    batch.set(learnerRef, {
                        ...
                        learnerId: profileId,
                        authUid: profileId,
                        cohortId: targetCohortId || deleteField(),
                        status: "active",
                        authStatus: "pending",
                        isDraft: false,
                        isOffline: true,
                    ...
                    const enrollmentId = targetCohortId ? `${targetCohortId}_${profileId}` : `RPL_${profileId}`;
    ...
    const handleConfirmApprove = async () => {
        ...
        try {
            ...
            setStatusModal({
                ...
                message: 'Failed to approve staged learners. Please check the logs and try again.',
                onConfirm: () => setStatusModal(null)
            });
        } finally {
    ...
    const handleConfirmPromote = async () => {
        if (!promotingLearners || promotingLearners.length === 0) return;
        ...
        try {
            ...
            const targetCohortId = promoteCohortId || "";

            promotingLearners.forEach(learner => {
                const trueLearnerId = learner.idNumber || learner.learnerId || (learner.id?.includes('_') ? learner.id.substring(learner.id.indexOf('_') + 1) : learner.id);

                const learnerRef = doc(db, "learners", trueLearnerId);
                const newEnrollmentId = targetCohortId ? `${targetCohortId}_${trueLearnerId}` : `GENERAL_${trueLearnerId}`;
                ...
                if (oldEnrollmentId && oldEnrollmentId !== newEnrollmentId) {
                    batch.set(doc(db, "enrollments", oldEnrollmentId), {
                        status: "transferred",
                        isBootcamp: false,
                        transferredTo: targetCohortId || deleteField(),
                        updatedAt: timestamp
                    }, { merge: true });
                }
            });
            ...
            const promotedCount = promotingLearners.length;
            ...
            setStatusModal({
                ...
                message: `Successfully promoted ${promotedCount} learner(s) to active enrollment! They are now ready for manual email invites.`,
                onConfirm: () => setStatusModal(null)
            });
            ...
            setStatusModal({
                ...
                message: 'Failed to promote learners to active status. Check console for details.',
                onConfirm: () => setStatusModal(null)
            });
        } finally {
    ...
    const handleConfirmDiscard = async () => {
        ...
        try {
            ...
            setStatusModal({
                ...
                message: 'Failed to discard staged record. Check console for details.',
                onConfirm: () => setStatusModal(null)
            });
        } finally {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            setStatusModal({
                ...
                type: 'error',
                title: 'Delete Failed',
                message: 'Failed to permanently delete learner. Check console for details.',
                onConfirm: () => setStatusModal(null)
            });
        } finally {
            setIsDeleting(false);
    ...
    return (
        <div className="wm-root animate-fade-in mlab-learners">
            ...
            <div className="mlab-tab-bar" style={{ borderTop: 'none', background: 'white', marginTop: 16 }}>
                <button type="button" className={`mlab-tab ${viewMode === 'active' ? 'mlab-tab--active' : 'mlab-tab--inactive'}`} onClick={() => handleViewModeChange('active')}>
                    Enrollments (Active) <span className="mlab-tab__count">{activeCount}</span>
                </button>
                <button type="button" className={`mlab-tab ${viewMode === 'bootcamp' ? 'mlab-tab--active' : 'mlab-tab--inactive'}`} onClick={() => handleViewModeChange('bootcamp')}>
                    Bootcamp / Pre-selection <span className={`mlab-tab__count ${viewMode === 'bootcamp' ? 'mlab-tab__count--alt' : ''}`}>{bootcampCount}</span>
                </button>
                <button type="button" className={`mlab-tab ${viewMode === 'offline' ? 'mlab-tab--active' : 'mlab-tab--inactive'}`} onClick={() => handleViewModeChange('offline')}>
                    Offline / RPL <span className={`mlab-tab__count ${viewMode === 'offline' ? 'mlab-tab__count--alt' : ''}`}>{offlineCount}</span>
                </button>
                <button type="button" className={`mlab-tab ${viewMode === 'staging' ? 'mlab-tab--staging-active' : 'mlab-tab--inactive'}`} onClick={() => handleViewModeChange('staging')}>
            ...
            <div className="wm-toolbar" style={{ flexWrap: 'wrap', marginTop: '1rem', alignItems: 'center' }}>
                <div className="wm-search" style={{ flex: '1 1 200px' }}>
                    ...
                    />
                    {localSearch && (
                        <button type="button" className="wm-search__clear" onClick={() => updateUrlParam('q', '')}><X size={13} /></button>
                    )}
                </div>
            ...
            {selectedIds.size > 0 ? (
                <div className="mlab-action-bar">
                    ...
                    <div className="mlab-bulk-actions">
                        {viewMode === 'staging' && (
                            <>
                                <button type="button" className="wm-btn wm-btn--primary" style={{ background: 'var(--mlab-green)', borderColor: 'var(--mlab-green-dark)' }} onClick={() => executeBulkAction('approve')}><ClipboardCheck size={14} /> Approve</button>
                                <button type="button" className="wm-btn wm-btn--ghost" style={{ color: 'var(--mlab-red)', borderColor: 'var(--mlab-red)' }} onClick={() => executeBulkAction('discard')}><Trash2 size={14} /> Discard Drafts</button>
                            </>
                        )}
                        {viewMode === 'bootcamp' && !showArchived && (
                            <button type="button" className="wm-btn wm-btn--primary" style={{ background: 'var(--mlab-green)', borderColor: 'var(--mlab-green-dark)' }} onClick={() => executeBulkAction('promote')}>
                                <UserCheck size={14} /> Promote to Active
                            </button>
                        )}
                        {viewMode !== 'staging' && (
                            showArchived ? (
                                <button type="button" className="wm-btn wm-btn--ghost" style={{ color: 'var(--mlab-green)', borderColor: 'var(--mlab-green)' }} onClick={() => executeBulkAction('restore')}><RotateCcw size={14} /> Restore</button>
                            ) : (
                                <button type="button" className="wm-btn wm-btn--ghost" style={{ color: 'var(--mlab-amber)', borderColor: 'var(--mlab-amber)' }} onClick={() => executeBulkAction('archive')}><ArchiveIcon size={14} /> Archive</button>
                            )
                        )}
                    </div>
                </div>
                    ...
                    {viewMode !== 'active' && viewMode !== 'bootcamp' && (
                        <>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.35rem 0.5rem', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '6px' }}>
                                ...
                                <button type="button" className="wm-btn wm-btn--primary" onClick={onUpload} style={{ background: '#0ea5e9', borderColor: '#0ea5e9', padding: '0.35rem 0.75rem', fontSize: '0.75rem' }} title="Import New Learner Profiles"><Upload size={13} /> Import</button>
                                <button type="button" className="wm-btn wm-btn--ghost" onClick={handleExport} style={{ padding: '0.35rem 0.75rem', fontSize: '0.75rem', background: 'white' }}><Download size={13} /> Export</button>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.35rem 0.5rem', background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '6px' }}>
                                ...
                                <button type="button" className="wm-btn wm-btn--primary" onClick={() => setShowBulkResultsImporter(true)} style={{ background: '#16a34a', borderColor: '#16a34a', padding: '0.35rem 0.75rem', fontSize: '0.75rem' }} title="Import Competency Results"><Layers size={13} /> Bulk Import Results</button>
                            </div>
                        </>
                    )}
                    {viewMode === 'bootcamp' && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.35rem 0.5rem', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '6px' }}>
                            <button type="button" className="wm-btn wm-btn--primary" onClick={onUpload} style={{ background: '#0ea5e9', borderColor: '#0ea5e9', padding: '0.35rem 0.75rem', fontSize: '0.75rem' }} title="Import Bootcamp Applicants"><Upload size={13} /> Import Applicants</button>
                            <button type="button" className="wm-btn wm-btn--ghost" onClick={handleExport} style={{ padding: '0.35rem 0.75rem', fontSize: '0.75rem', background: 'white' }}><Download size={13} /> Export List</button>
                        </div>
                    )}
                    <button type="button" className="wm-btn wm-btn--primary" onClick={onAdd} style={{ padding: '0.45rem 1rem', marginLeft: 'auto' }}><Plus size={14} /> Add Single</button>
                </div>
            )}
                                ...
                                <tr key={learner.id} className={rowClass} style={isStagingBroken ? { background: '#fff1f2', borderLeft: '4px solid var(--mlab-red)' } : {}}>
                                    ...
                                    <td style={{ textAlign: 'right' }}>
                                        <div className="mlab-icon-btn-group" style={{ justifyContent: 'flex-end' }}>
                                            ...
                                            {viewMode === 'bootcamp' && !learner.isArchived && (
                                                <button type="button" className="mlab-icon-btn mlab-icon-btn--green" onClick={() => setPromotingLearners([learner])} title="Promote to Active Enrollment">
                                                    <UserCheck size={14} />
                                                </button>
                                            )}
                                            <button type="button" className="mlab-icon-btn mlab-icon-btn--blue" onClick={() => onEdit(learner)} title="Edit Details"><Edit size={14} /></button>
                                                ...
                                                <>
                                                    <button
                                                        ...
                                                        onClick={(e) => {
                                                            e.preventDefault();
                                                            e.stopPropagation();
                                                            navigate('/admin?tab=studio', { state: { learner } });
                                                        }}
                                                        title="Issue Certificate in Studio"
                                                    >
                                                        <Award size={14} />
                                                    </button>
                                                    <button type="button" className="mlab-icon-btn mlab-icon-btn--blue" onClick={() => navigate(`/sor/${learner.id}`)} title="View Results"><Eye size={14} /></button>
                                                    <button type="button" className="mlab-icon-btn mlab-icon-btn--blue" style={{ color: copiedId === learner.idNumber ? 'var(--mlab-emerald)' : '' }} onClick={() => handleCopyLink(learner.id, learner.idNumber)} title="Copy Public Verifier Link">{copiedId === learner.idNumber ? <ClipboardCheck size={14} /> : <Share2 size={14} />}</button>
                                                    {viewMode === 'active' && isPendingSetup && <button type="button" className="mlab-icon-btn" style={{ color: '#d97706', background: '#fef3c7', border: '1px solid #fde68a' }} onClick={() => handleForceActive(learner)} title="Force Active Status"><CheckSquare size={14} /></button>}
                                                    {viewMode === 'active' && <button type="button" className={`mlab-icon-btn ${learner.profileCompleted ? '' : 'mlab-icon-btn--green'}`} style={learner.profileCompleted ? { background: '#f8fafc', color: '#cbd5e1', cursor: 'not-allowed', border: '1px solid #e2e8f0' } : {}} onClick={() => !learner.profileCompleted && onInvite(learner)} disabled={learner.profileCompleted} title={learner.profileCompleted ? 'Learner registered' : 'Send Invite'}>{learner.profileCompleted ? <UserCheck size={14} /> : <Mail size={14} />}</button>}
                                                    <button type="button" className="mlab-icon-btn mlab-icon-btn--amber" onClick={() => onArchive(learner)} title="Archive Record"><ArchiveIcon size={14} /></button>
                                                </>
                                            )}
                                            {(viewMode === 'offline' || viewMode === 'bootcamp' || learner.isArchived) && <button type="button" className="mlab-icon-btn mlab-icon-btn--red" onClick={() => setDeletingLearner(learner)} title="Delete Permanently"><Trash2 size={14} /></button>}
                                            {viewMode !== 'staging' && learner.isArchived && <button type="button" className="mlab-icon-btn mlab-icon-btn--emerald" onClick={() => onRestore(learner)} title="Restore"><RotateCcw size={14} /></button>}
                                            {viewMode === 'staging' && <button type="button" className="mlab-icon-btn mlab-icon-btn--red" onClick={() => setDiscardingLearner(learner)} title="Discard Draft"><X size={14} /></button>}
                                        </div>
                                    </td>
                                </tr>
                            );
            ...
            {totalPages > 1 && (
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', padding: '1.5rem', borderTop: '1px solid var(--mlab-border)', background: 'white' }}>
                    <button type="button" className="mlab-btn mlab-btn--ghost mlab-btn--sm" onClick={() => updateUrlParam('page', currentPage - 1)} disabled={currentPage === 1}>
                    ...
                    <button type="button" className="mlab-btn mlab-btn--ghost mlab-btn--sm" onClick={() => updateUrlParam('page', currentPage + 1)} disabled={currentPage === totalPages}>
                ...
                <div className="wm-overlay animate-fade-in" onClick={() => setApprovingLearners(null)} style={{ zIndex: 99999 }}>
                    <div className="wm-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '600px' }}>
                        <div className="wm-modal__header" style={{ borderBottom: '1px solid var(--mlab-border)', paddingBottom: '1rem' }}>
                            ...
                            <button type="button" className="wm-modal__close" onClick={() => setApprovingLearners(null)} disabled={isApproving}><X size={18} /></button>
                        </div>
                        ...
                        <div className="wm-modal__footer">
                            <button type="button" className="wm-btn wm-btn--ghost" onClick={() => setApprovingLearners(null)} disabled={isApproving}>Cancel</button>
                            <button type="button" className="wm-btn wm-btn--primary" onClick={handleConfirmApprove} disabled={isApproving}>
            ...
            {deletingLearner && createPortal(
                <div className="mlab-modal-overlay" style={{ zIndex: 99999 }}>
                    <div className="mlab-modal mlab-modal--sm">
                        <div className="mlab-modal__header">
                            ...
                            <button type="button" className="mlab-modal__close" onClick={() => setDeletingLearner(null)}><X size={20} /></button>
                        </div>
                        ...
                        <div className="mlab-modal__footer">
                            <button type="button" className="wm-btn wm-btn--ghost" onClick={() => setDeletingLearner(null)}>Cancel</button>
                            <button type="button" className="mlab-btn mlab-btn--red" disabled={!deleteReason.trim() || isDeleting} onClick={handleConfirmDelete}>{isDeleting ? <Loader2 className="spin" size={16} /> : "Confirm Delete"}</button>
                        </div>
                    </div>
                </div>,
                document.body
            ...
            {promotingLearners && promotingLearners.length > 0 && createPortal(
                <div className="wm-overlay animate-fade-in" onClick={() => setPromotingLearners(null)} style={{ zIndex: 99999 }}>
                    <div className="wm-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '500px' }}>
                        <div className="wm-modal__header" style={{ borderBottom: '1px solid var(--mlab-border)', paddingBottom: '1rem' }}>
                            <div className="wm-modal__header-icon" style={{ background: '#dcfce7', color: '#16a34a' }}><UserCheck size={20} /></div>
                            <div>
                                <h2 className="wm-modal__title">Promote {promotingLearners.length} Applicant{promotingLearners.length > 1 ? 's' : ''}</h2>
                                <p className="wm-modal__subtitle">Move selected bootcamp learners into an active class.</p>
                            </div>
                            <button type="button" className="wm-modal__close" onClick={() => setPromotingLearners(null)} disabled={isPromoting}><X size={18} /></button>
                        </div>
                        <div className="wm-modal__body" style={{ display: 'flex', flexDirection: 'column', gap: '16px', padding: '1.5rem' }}>
                            ...
                            <div style={{ background: '#e0f2fe', padding: '12px', borderRadius: '0', border: '1px solid #bae6fd', fontSize: '0.85rem', color: '#0369a1', fontWeight: 500 }}>
                                These learners will be officially enrolled into the Active pipeline. <strong>Their historical data and attendance will remain permanently visible in the Bootcamp roster for your records.</strong> In the main system, they will be marked as <strong>Pending Setup</strong> so you can manually send them email invitations when ready.
                            </div>
                            ...
                            <div className="mlab-form-group">
                                <label style={{ fontWeight: 700, color: 'var(--mlab-midnight)', fontSize: '0.85rem', marginBottom: '8px', display: 'block', textTransform: 'uppercase' }}>
                                    Assign to Class / Cohort (Optional)
                                </label>
                                <select
                                ...
                                >
                                    ...
                                    {activeCohorts.filter(c => !c.isArchived).map(c => (
                        ...
                        <div className="wm-modal__footer">
                            <button type="button" className="wm-btn wm-btn--ghost" onClick={() => setPromotingLearners(null)} disabled={isPromoting}>Cancel</button>
                            <button type="button" className="wm-btn wm-btn--primary" style={{ background: 'var(--mlab-green)', borderColor: 'var(--mlab-green-dark)' }} onClick={handleConfirmPromote} disabled={isPromoting}>
