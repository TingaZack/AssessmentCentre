...
import {
    ...
    FolderOpen, Clock, CheckCircle2, AlertCircle, XCircle,
...
export const QCTOCohortView: React.FC<{ cohort: any }> = ({ cohort }) => {
    ...
    const { user, learners, programmes, settings, employers } = useStore();

    // 🚀 SYNC TABS SAFELY USING 'VIEW' PARAMETER
    const urlView = searchParams.get('view');
    const activeTab = (urlView === 'curriculum' || urlView === 'calendar' || urlView === 'tracker' || urlView === 'surveys')
        ? urlView
        : 'learners';

    const setActiveTab = (view: 'learners' | 'curriculum' | 'calendar' | 'tracker' | 'surveys') => {
    ...
    // 🚀 NEW: UNIVERSAL ATTENDANCE ENGINE THAT LISTENS TO ALL 3 STRUCTURES
    useEffect(() => {
        if (!cohort?.id) return;
    ...
    const dailyRegisters = useMemo(() => {
        ...
        rawAttendance.forEach(record => {
            if (record.presentLearners || record.absentLearners) {
                // Legacy Aggregated Record
                unifiedMap.set(record.id, {
                    id: record.id,
                    date: record.date || 'Unknown',
                    expectedDuration: record.expectedDuration || 0,
                    presentLearners: record.presentLearners || [],
                    absentLearners: record.absentLearners || [],
                    partialLearners: record.partialLearners || []
                });
            } else if (record.learnerId && record.date) {
                // Ecosystem Sync (Individual Records)
                const dateKey = `eco_${record.date}`;
                if (!unifiedMap.has(dateKey)) {
                    unifiedMap.set(dateKey, {
                        id: dateKey,
                        date: record.date,
                        expectedDuration: record.hoursCredited ? record.hoursCredited * 60 : 480, // Default 8 hours
                        presentLearners: [],
                        absentLearners: [],
                        partialLearners: []
                    });
        ...
        // 2. Process Zoom Data (attendance_logs + attendance_records)
        rawAttendanceLogs.forEach(log => {
            const dateStr = log.sessionDate ? log.sessionDate.split('T')[0] : log.createdAt?.split('T')[0] || 'Unknown';
            const recordsForLog = rawAttendanceRecords.filter(r => r.sessionId === log.id || r.date === dateStr);
            ...
            recordsForLog.forEach(r => {
                ...
                if (status.includes('present')) present.push(r.learnerId);
                else if (status.includes('absent')) absent.push(r.learnerId);
                else if (status.includes('partial')) partial.push(r.learnerId);
            });

            unifiedMap.set(log.id, {
                id: log.id,
                date: dateStr,
                expectedDuration: log.expectedDuration || 0,
                presentLearners: present,
                absentLearners: absent,
                partialLearners: partial,
                sessionTitle: log.sessionTitle,
                sessionZoomLink: log.sessionZoomLink
            });
        });
        ...
        return unifiedArray;
    }, [rawAttendance, rawAttendanceLogs, rawAttendanceRecords]);
    ...
    const rosterAttendanceMap = useMemo(() => {
        ...
        // Initialize maps holding by both Internal ID and ID Number for maximum matching flexibility
        enrolledLearners.forEach(l => {
            if (l.id) map.set(l.id, { attended: 0, total: totalSessions, pct: 0 });
        ...
        dailyRegisters.forEach(reg => {
            ...
            present.forEach((identifier: string) => {
                if (map.has(identifier)) {
                    map.get(identifier)!.attended += 1;
                }
            });
        });

        // Consolidate map back down to a single best-score per learner
        const finalMap = new Map<string, { attended: number; total: number; pct: number }>();
        enrolledLearners.forEach(l => {
            const byId = map.get(l.id)?.attended || 0;
            const byIdNum = l.idNumber ? (map.get(l.idNumber)?.attended || 0) : 0;
            const bestAtt = Math.max(byId, byIdNum);

            finalMap.set(l.idNumber || l.id, {
                attended: bestAtt,
                total: totalSessions,
                pct: totalSessions > 0 ? Math.round((bestAtt / totalSessions) * 100) : 0
            });
        });
    ...
    const attendanceHealth = useMemo(() => {
        ...
        enrolledLearners.forEach(l => {
            const stats = rosterAttendanceMap.get(l.idNumber || l.id);
    ...
    const filteredLearners = useMemo(() => {
        return enrolledLearners.filter(learner => {
            ...
            const stats = rosterAttendanceMap.get(learner.idNumber || learner.id) || { attended: 0, total: dailyRegisters.length, pct: 0 };
    ...
    const handleQCTOExport = async () => {
        ...
        } catch { toast.error('Export failed. Check console for details.'); } finally { setIsExporting(false); }
    };
    ...
    return (
        <div className="cdp-layout">
            ...
            {/* ADD THE ZOOM DROP ZONE MODAL HERE */}
            ...
            <main className="cdp-main">
                ...
                <div className="cdp-content" style={{ padding: '2rem' }}>
                    ...
                    <div className="lfm-tabs" style={{ marginBottom: '2rem' }}>
                        ...
                        <button className={`lfm-tab ${activeTab === 'tracker' ? 'active' : ''}`} onClick={() => setActiveTab('tracker')}>
                                                    ...
                                                    {filteredDailyRegisters.map((reg) => {
                                                        ...
                                                        return (
                                                            <tr
                                                                ...
                                                                onMouseOver={e => e.currentTarget.style.backgroundColor = 'var(--mlab-bg)'}
