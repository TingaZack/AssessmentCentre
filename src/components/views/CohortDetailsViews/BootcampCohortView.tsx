...
import {
    ...
    Search, X, Info, BarChart2, UserMinus, Edit2, Loader2, Video, History, ChevronDown, ChevronUp, MapPin, Filter, ChevronRight, Globe,
    Timer, PenTool, BookOpen, Award, ShieldCheck, User, Maximize, Minimize,
    FileText, Briefcase, AlertTriangle, ClipboardList, Send, MinusCircle, HelpCircle, Star, Eye, Link as LinkIcon, Trash2
} from 'lucide-react';
...
export const BootcampCohortView: React.FC<{ cohort: any }> = ({ cohort }) => {
    ...
    // 🚀 SURVEY TAB SUB-NAVIGATION STATE
    ...
    // ACTIVE NON-DROPPED LEARNER COUNT
    const activeCount = useMemo(() => {
        return cohortAnalytics?.systemActiveCount || enrolledLearners.filter(l => l.status !== 'dropped').length;
    }, [cohortAnalytics, enrolledLearners]);

    // 🚀 STRICT COHORT SURVEY FILTER: ONLY INCLUDE SURVEYS ASSIGNED DIRECTLY TO THIS COHORT
    // OR LINKED VIA ASSESSMENTS/SUBMISSIONS IN THIS COHORT
    const cohortTargetedSurveys = useMemo(() => {
        ...
        return surveyTemplates.filter(s => {
            ...
            // 1. Directly assigned to this specific cohort ID
            const isDirectlyAssigned = cIds.includes(cohort.id) || s.cohortId === cohort.id;

            // 2. Linked to an assessment belonging to this cohort
            const isAssessmentLinked = (s.assessmentId && cohortAssessmentIds.has(s.assessmentId)) ||
                cohortSubmissionSurveyIds.has(s.id);

            // 3. Has existing feedback responses from this cohort
            const hasResponsesInCohort = cohortResponseSurveyIds.has(s.id);

            return isDirectlyAssigned || isAssessmentLinked || hasResponsesInCohort;
        });
                ...
                <div className="cdp-content">
                                ...
                                <div className="mlab-table-wrap">
                                    <table className="mlab-table" style={{ margin: 0 }}>
                                        ...
                                        <tbody>
                                            {paginatedLearners.length === 0 ? (
                                                <tr>
                                                    <td colSpan={10} style={{ padding: '3rem', textAlign: 'center', color: 'var(--mlab-grey)' }}>
                    ...
                    {activeTab === 'surveys' && (
                        <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            ...
                            <div style={{ display: 'flex', gap: '8px', borderBottom: '2px solid var(--mlab-border)', paddingBottom: '0px' }}>
                                <button
                                    ...
                                    style={{
                                        ...
                                        cursor: 'pointer',
                                        background: surveySubTab === 'templates' ? 'var(--mlab-blue)' : 'transparent',
                                        color: surveySubTab === 'templates' ? 'var(--mlab-white)' : 'var(--mlab-grey)',
                                        border: 'none',
                                        borderBottom: surveySubTab === 'templates' ? '3px solid var(--mlab-green)' : '3px solid transparent',
                                        display: 'flex',
                                        alignItems: 'center',
                                        gap: '6px'
                                    }}
                                >
                                    <HelpCircle size={14} /> Assigned Surveys ({cohortTargetedSurveys.length})
                                </button>
                                <button
                                    ...
                                    style={{
                                        ...
                                        letterSpacing: '0.05em',
                                        cursor: 'pointer',
                                        background: surveySubTab === 'responses' ? 'var(--mlab-blue)' : 'transparent',
                                        color: surveySubTab === 'responses' ? 'var(--mlab-white)' : 'var(--mlab-grey)',
                                        border: 'none',
                                        borderBottom: surveySubTab === 'responses' ? '3px solid var(--mlab-green)' : '3px solid transparent',
                                        display: 'flex',
                                        alignItems: 'center',
                                        gap: '6px'
                                    }}
                                >
                                    <ClipboardList size={14} /> Survey Submissions ({filteredSurveyResponses.length})
                                </button>
                            </div>

                            {/* SUB-TAB 1: ASSIGNED SURVEYS TABLE */}
                            {surveySubTab === 'templates' && (
                                <div className="animate-fade-in" style={{ border: '2px solid var(--mlab-blue)', background: 'var(--mlab-white)' }}>
                                    <div className="lfm-header">
                                        <h2 className="lfm-header__title">
                                            <HelpCircle size={18} /> Assigned Survey Forms ({cohortTargetedSurveys.length})
                                        </h2>
                                    </div>
                                    <div className="lfm-body" style={{ padding: 0 }}>
                                        <div className="mlab-table-wrap">
                                                    ...
                                                    {cohortTargetedSurveys.length === 0 ? (
                                                        <tr>
                                                            <td colSpan={5} style={{ padding: '3rem', textAlign: 'center', color: 'var(--mlab-grey)' }}>
                                                                No survey forms assigned strictly to this cohort or its assessments.
                                                            </td>
                                                        </tr>
                                                    ) : (
                                                        cohortTargetedSurveys.map(survey => {
                                                            ...
                                                            return (
                                                                <tr key={survey.id}>
                                                                    <td>
                                                                        <div style={{ fontWeight: 'bold', color: 'var(--mlab-blue)' }}>{survey.title}</div>
                                                                    </td>
                                                                    <td>
                                                                        <div style={{ fontSize: '0.8rem', color: 'var(--mlab-grey)', maxWidth: '300px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                                                            {survey.description || 'No description'}
                                                                        </div>
                                                                    </td>
                                                                    <td style={{ fontWeight: 600 }}>{survey.questions?.length || 0} Questions</td>
                                                                    <td>
                                                                        <span style={{ fontWeight: 700, color: subCount > 0 ? 'var(--mlab-green-dark)' : 'var(--mlab-grey)' }}>
                            ...
                            {/* SUB-TAB 2: SURVEY SUBMISSIONS TABLE */}
                            {surveySubTab === 'responses' && (
                                <div className="animate-fade-in" style={{ border: '2px solid var(--mlab-blue)', background: 'var(--mlab-white)' }}>
                                    <div className="lfm-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                                        <h2 className="lfm-header__title">
                                            <ClipboardList size={18} /> Cohort Survey Submissions ({filteredSurveyResponses.length})
                                        </h2>
