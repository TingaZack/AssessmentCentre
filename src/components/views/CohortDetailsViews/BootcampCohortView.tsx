...
//     FileText, Briefcase, AlertTriangle, ClipboardList, ExternalLink, Send, MinusCircle, Sparkles, Navigation, Building2, Cpu, Copy, Check
// } from 'lucide-react';
...
//     '3': { name: 'Northern Cape', lat: -28.7282, lng: 24.7499 },
//     '4': { name: 'Free State', lat: -29.1181, lng: 26.2243 },
//     '5': { name: 'KwaZulu-Natal', lat: -29.8587, lng: 31.0218 },
//     '6': { name: 'North West', lat: -25.8560, lng: 25.6442 },
...
// const SA_TOWN_COORDS: Record<string, [number, number]> = {
//     'boksburg': [-26.2126, 28.2618],
...
//     'east london': [-33.0153, 27.8935],
//     'bloemfontein': [-29.1181, 26.2243],
//     'polokwane': [-23.9045, 29.4688],
//     'mbombela': [-25.4753, 30.9853],
//     'nelspruit': [-25.4753, 30.9853],
//     'mahikeng': [-25.8560, 25.6442],
//     'rustenburg': [-25.6544, 27.2559],
//     'pietermaritzburg': [-29.6006, 30.3794]
// };

// // 🚀 STATS-SA & MUNICIPALITY LOOKUP DICTIONARY
// const MUNICIPAL_LOOKUP: Record<string, { municipality: string; district: string; prov: string; lat: number; lng: number }> = {
//     'katlehong': { municipality: 'City of Ekurhuleni', district: 'Ekurhuleni Metro', prov: 'Gauteng', lat: -26.3167, lng: 28.1667 },
//     'boksburg': { municipality: 'City of Ekurhuleni', district: 'Ekurhuleni Metro', prov: 'Gauteng', lat: -26.2126, lng: 28.2618 },
//     'germiston': { municipality: 'City of Ekurhuleni', district: 'Ekurhuleni Metro', prov: 'Gauteng', lat: -26.2257, lng: 28.1708 },
//     'soweto': { municipality: 'City of Johannesburg', district: 'Johannesburg Metro', prov: 'Gauteng', lat: -26.2485, lng: 27.8540 },
//     'johannesburg': { municipality: 'City of Johannesburg', district: 'Johannesburg Metro', prov: 'Gauteng', lat: -26.2041, lng: 28.0473 },
//     'alexandra': { municipality: 'City of Johannesburg', district: 'Johannesburg Metro', prov: 'Gauteng', lat: -26.1033, lng: 28.0933 },
//     'pretoria': { municipality: 'City of Tshwane', district: 'Tshwane Metro', prov: 'Gauteng', lat: -25.7479, lng: 28.2293 },
//     'mamelodi': { municipality: 'City of Tshwane', district: 'Tshwane Metro', prov: 'Gauteng', lat: -25.7083, lng: 28.3750 },
//     'centurion': { municipality: 'City of Tshwane', district: 'Tshwane Metro', prov: 'Gauteng', lat: -25.8603, lng: 28.1894 },
//     'umlazi': { municipality: 'eThekwini', district: 'eThekwini Metro', prov: 'KwaZulu-Natal', lat: -29.9667, lng: 30.8833 },
//     'durban': { municipality: 'eThekwini', district: 'eThekwini Metro', prov: 'KwaZulu-Natal', lat: -29.8587, lng: 31.0218 },
//     'kimberley': { municipality: 'Sol Plaatje', district: 'Frances Baard', prov: 'Northern Cape', lat: -28.7282, lng: 24.7499 },
//     'polokwane': { municipality: 'Polokwane', district: 'Capricorn', prov: 'Limpopo', lat: -23.9045, lng: 29.4688 },
//     'cape town': { municipality: 'City of Cape Town', district: 'Cape Town Metro', prov: 'Western Cape', lat: -33.9249, lng: 18.4241 },
//     'gqeberha': { municipality: 'Nelson Mandela Bay', district: 'Nelson Mandela Bay Metro', prov: 'Eastern Cape', lat: -33.9608, lng: 25.6022 },
//     'east london': { municipality: 'Buffalo City', district: 'Buffalo City Metro', prov: 'Eastern Cape', lat: -33.0153, lng: 27.8935 },
//     'bloemfontein': { municipality: 'Mangaung', district: 'Mangaung Metro', prov: 'Free State', lat: -29.1181, lng: 26.2243 },
//     'rustenburg': { municipality: 'Rustenburg', district: 'Bojanala Platinum', prov: 'North West', lat: -25.6544, lng: 27.2559 },
//     'mbombela': { municipality: 'City of Mbombela', district: 'Ehlanzeni', prov: 'Mpumalanga', lat: -25.4753, lng: 30.9853 }
// };

// const resolveMunicipalityFromLearner = (learner: DashboardLearner): { municipality: string; district: string; province: string; lat: number; lng: number } => {
//     const demos = learner.demographics || (learner as any);
//     const statsSa = String(learner.statsSaAreaCode || demos.statsSaAreaCode || '').toLowerCase();
//     const city = String(demos.learnerHomeAddress2 || demos.city || demos.town || '').toLowerCase();
//     const provStr = String(demos.provinceCode || demos.province || '').toLowerCase();

//     const searchBlob = `${statsSa} ${city} ${provStr}`;

//     for (const [key, data] of Object.entries(MUNICIPAL_LOOKUP)) {
//         if (searchBlob.includes(key)) {
//             return { municipality: data.municipality, district: data.district, province: data.prov, lat: data.lat, lng: data.lng };
//         }
//     }

//     if (provStr.includes('gauteng') || provStr === '7') return { municipality: 'Gauteng Municipal Region', district: 'Gauteng', province: 'Gauteng', lat: -26.2041, lng: 28.0473 };
//     if (provStr.includes('western cape') || provStr === '1') return { municipality: 'City of Cape Town', district: 'Cape Town Metro', province: 'Western Cape', lat: -33.9249, lng: 18.4241 };
//     if (provStr.includes('kwazulu') || provStr === '5') return { municipality: 'eThekwini', district: 'eThekwini Metro', province: 'KwaZulu-Natal', lat: -29.8587, lng: 31.0218 };
//     if (provStr.includes('free state') || provStr === '4') return { municipality: 'Mangaung', district: 'Mangaung Metro', province: 'Free State', lat: -29.1181, lng: 26.2243 };
//     if (provStr.includes('northern cape') || provStr === '3') return { municipality: 'Sol Plaatje', district: 'Frances Baard', province: 'Northern Cape', lat: -28.7282, lng: 24.7499 };

//     return { municipality: 'Unspecified Municipality', district: 'Unspecified District', province: 'Unspecified', lat: -28.4793, lng: 24.6727 };
// };

// const sanitizeSACoords = (rawLat: any, rawLng: any): [number, number] | null => {
...
//     if (lat >= -35.5 && lat <= -21.0 && lng >= 15.0 && lng <= 33.5) {
//         return [lat, lng];
//     }
...
// // 🚀 SEARCHABLE MUNICIPALITY DROPDOWN COMPONENT
// const SearchableMunicipalityDropdown: React.FC<{
//     selected: string | null;
//     onSelect: (muni: string | null) => void;
//     municipalities: { name: string; count: number; province: string }[];
// }> = ({ selected, onSelect, municipalities }) => {
//     const [isOpen, setIsOpen] = useState(false);
//     const [search, setSearch] = useState('');
//     const ref = useRef<HTMLDivElement>(null);
...
//     const filtered = municipalities.filter(m => m.name.toLowerCase().includes(search.toLowerCase()) || m.province.toLowerCase().includes(search.toLowerCase()));
...
//                     color: 'var(--mlab-white)', fontFamily: 'var(--font-heading)', fontWeight: 700,
//                     fontSize: '0.75rem', textTransform: 'uppercase', cursor: 'pointer', borderRadius: 0, outline: 'none',
//                     display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'space-between', minWidth: '230px'
//                 }}
//             >
//                 <span style={{ display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
//                     <Building2 size={14} color="#38bdf8" />
//                     {selected ? `Muni: ${selected}` : 'All Municipalities'}
//                 </span>
//                 <ChevronDown size={14} style={{ flexShrink: 0 }} />
//             </button>

//             {isOpen && (
//                 <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, background: 'var(--mlab-white)', border: '1px solid var(--mlab-border)', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)', zIndex: 1002, minWidth: '250px' }}>
//                     <div style={{ padding: '8px', borderBottom: '1px solid var(--mlab-border)', display: 'flex', alignItems: 'center', gap: '8px', background: 'var(--mlab-bg)' }}>
//                         <Search size={14} color="var(--mlab-grey)" />
//                         <input
//                             type="text"
...
//                             style={{ padding: '8px 12px', cursor: 'pointer', background: !selected ? 'var(--mlab-light-blue)' : 'var(--mlab-white)', color: 'var(--mlab-blue)', fontWeight: !selected ? 700 : 500, fontSize: '0.8rem', display: 'flex', justifyContent: 'space-between' }}
//                         >
//                             <span>🏛️ All Municipalities</span>
//                         </div>
...
//                                     background: selected === m.name ? 'var(--mlab-light-blue)' : 'var(--mlab-white)',
//                                     color: selected === m.name ? 'var(--mlab-blue)' : 'var(--mlab-midnight)',
//                                     fontWeight: selected === m.name ? 700 : 500, fontSize: '0.8rem',
...
//                                     <span>{m.name}</span>
//                                     <span style={{ fontSize: '0.65rem', color: 'var(--mlab-grey)' }}>{m.province}</span>
//                                 </div>
//                                 <span style={{ fontSize: '0.7rem', fontWeight: 700, background: 'var(--mlab-bg)', padding: '2px 6px', border: '1px solid var(--mlab-border)' }}>
//                                     {m.count}
//                                 </span>
//                             </div>
//                         ))}
...
//         return (
//             <span
//                 title="No email address found on profile"
//                 style={{
...
//         return (
//             <span
//                 title={`Successfully delivered to ${emailStatus.recipient || targetEmail}`}
//                 style={{
//                     display: 'inline-flex', alignItems: 'center', gap: '4px',
//                     padding: '2px 8px', fontSize: '0.65rem', fontWeight: 700,
...
//         return (
//             <span
//                 title={`Delivery Error: ${emailStatus.error}`}
//                 style={{
//                     display: 'inline-flex', alignItems: 'center', gap: '4px',
//                     padding: '2px 8px', fontSize: '0.65rem', fontWeight: 700,
//                     background: '#fef2f2', color: '#dc2626', border: '1px solid #fca5a5',
...
//     return (
//         <span
//             title="Exit survey email has not been sent yet"
//             style={{
//                 display: 'inline-flex', alignItems: 'center', gap: '4px',
//                 padding: '2px 8px', fontSize: '0.65rem', fontWeight: 700,
...
//     // GEO AI STATE
//     const [isGeoAiLoading, setIsGeoAiLoading] = useState(false);
//     const [geoAiResult, setGeoAiResult] = useState<any | null>(null);
...
//     const municipalityFilter = searchParams.get('municipality') || 'all';
...
//     // 🚀 GEO AI ANALYSIS CALLABLE HANDLER
//     const runGeoAiAnalysis = async (targetLocName?: string) => {
//         const locToAnalyze = targetLocName || municipalityFilter !== 'all' ? municipalityFilter : (selectedMapLocation || "Cohort General Cluster");

//         setIsGeoAiLoading(true);
//         setShowGeoAiModal(true);
//         setGeoAiResult(null);

//         try {
//             const resolveCallable = httpsCallable<any, { success: boolean, data: any }>(
//                 functions,
//                 "resolveLearnerMunicipalityAI"
//             );

//             const matchedLearner = enrolledLearners.find(l => {
//                 const res = resolveMunicipalityFromLearner(l);
//                 return res.municipality.toLowerCase() === locToAnalyze.toLowerCase() ||
//                     getLocationString(l).toLowerCase().includes(locToAnalyze.toLowerCase());
//             }) || enrolledLearners[0];

//             const response = await resolveCallable({
//                 statsSaAreaCode: matchedLearner?.statsSaAreaCode || (matchedLearner as any)?.demographics?.statsSaAreaCode,
//                 streetAddress: (matchedLearner as any)?.demographics?.learnerHomeAddress1 || matchedLearner?.address,
//                 city: locToAnalyze !== "Cohort General Cluster" ? locToAnalyze : (matchedLearner?.city || "Gauteng"),
//                 province: matchedLearner?.demographics?.province || "Gauteng",
//                 postalCode: (matchedLearner as any)?.demographics?.postalCode
//             });

//             if (response.data?.success) {
//                 setGeoAiResult(response.data.data);
//             } else {
//                 toast.error("Failed to generate Geo AI resolution.");
//             }
//         } catch (err: any) {
//             console.error("Geo AI execution failed:", err);
//             toast.error("Geo AI Cloud Service unavailable or unconfigured.");
//         } finally {
//             setIsGeoAiLoading(false);
//         }
//     };
...
//     useEffect(() => {
//         setCurrentPage(1);
//     }, [urlSearchTerm, statusFilter, attendanceFilter, activationFilter, locationFilter, municipalityFilter, customMinPct, customMaxPct, exitEmailFilter]);
...
//     // 🚀 DYNAMIC LIST OF MUNICIPALITIES DERIVED FROM ENROLLED LEARNERS
//     const dynamicMunicipalitiesList = useMemo(() => {
//         const muniMap = new Map<string, { count: number; province: string }>();

//         enrolledLearners.forEach(l => {
//             const res = resolveMunicipalityFromLearner(l);
//             if (res.municipality && res.municipality !== 'Unspecified Municipality') {
//                 if (!muniMap.has(res.municipality)) {
//                     muniMap.set(res.municipality, { count: 0, province: res.province });
//                 }
//                 muniMap.get(res.municipality)!.count++;
//             }
//         });

//         return Array.from(muniMap.entries()).map(([name, data]) => ({
//             name,
//             count: data.count,
...
//                 if (validCoords) provData.get(provInfo.name)!.latLngs.push(validCoords);
//             } else if (provStr) {
//                 if (!provData.has(provStr)) provData.set(provStr, { latLngs: [] });
//                 if (validCoords) provData.get(provStr)!.latLngs.push(validCoords);
//             }

//             if (town) {
//                 if (!cityData.has(town)) cityData.set(town, { latLngs: [] });
//                 if (validCoords) cityData.get(town)!.latLngs.push(validCoords);
//             }
//         });
...
//                 lookup.set(name, [avgLat, avgLng]);
//             } else {
//                 const townKey = name.toLowerCase();
//                 if (SA_TOWN_COORDS[townKey]) {
//                     lookup.set(name, SA_TOWN_COORDS[townKey]);
//                 }
//             }
//         });
...
//             const matchedMuni = dynamicMunicipalitiesList.find(m => m.name.toLowerCase() === municipalityFilter.toLowerCase());
//             if (matchedMuni) {
//                 const sampleLearner = enrolledLearners.find(l => resolveMunicipalityFromLearner(l).municipality.toLowerCase() === municipalityFilter.toLowerCase());
//                 if (sampleLearner) {
//                     const res = resolveMunicipalityFromLearner(sampleLearner);
//                     return [res.lat, res.lng] as [number, number];
//                 }
//             }
//         }
//         if (!selectedMapLocation) return null;
//         return targetCoordsLookup.get(selectedMapLocation) || SA_TOWN_COORDS[selectedMapLocation.toLowerCase()] || null;
//     }, [selectedMapLocation, municipalityFilter, targetCoordsLookup, dynamicMunicipalitiesList, enrolledLearners]);
...
//             // 🚀 MUNICIPALITY FILTER CRITERIA
//             if (municipalityFilter !== 'all') {
//                 const res = resolveMunicipalityFromLearner(learner);
//                 if (res.municipality.toLowerCase() !== municipalityFilter.toLowerCase()) {
//                     return false;
//                 }
//             }
...
//         });
//     }, [enrolledLearners, urlSearchTerm, statusFilter, activationFilter, exitEmailFilter, municipalityFilter, customMinPct, customMaxPct, attendanceFilter, cohortAnalytics]);

//     const filteredLearners = useMemo(() => {
//         if (!cohortAnalytics) return [];

//         return learnersForMap.filter(learner => {
...
//                 const fullLocStr = getLocationString(learner).toLowerCase();
//                 if (fullLocStr.includes(filterLower) || (provName && filterLower.includes(provName.toLowerCase()))) return true;
...
//         });
//     }, [learnersForMap, locationFilter, cohortAnalytics]);

//     const locationStats = useMemo(() => {
//         const stats = new Map<string, { lat: number, lng: number, count: number, name: string, latLngs: [number, number][] }>();

//         learnersForMap.forEach(l => {
//             const demos = l.demographics || (l as any);
//             const provStr = String(demos.provinceCode || demos.province || '').trim();
//             const provInfo = getProvInfo(provStr) || { name: 'Unknown', lat: -28.4793, lng: 24.6727 };
//             const provName = provInfo.name;
//             const town = String(demos.learnerHomeAddress2 || demos.city || demos.town || '').trim();

//             const groupName = town || provName || 'Not specified';
//             const townKey = groupName.toLowerCase();

//             const defaultGPS = SA_TOWN_COORDS[townKey] || [provInfo.lat, provInfo.lng];
//             const validGPS = sanitizeSACoords(demos.lat, demos.lng);
...
//             const muniRes = resolveMunicipalityFromLearner(l);
...
//                 "Local Municipality": muniRes.municipality,
//                 "District / Metro": muniRes.district,
//                 "Activation Status": isActivated ? 'Started' : 'Never Attended',
//                 "Attendance Score": `${attended}/${processedAttendanceLogs.length} (${pct}%)`,
...
//         toast.success('Roster exported successfully with municipal & tracking metrics.');
//     };
...
//     // 🚀 MAP RENDERER WITH MUNICIPALITY DROPDOWN & LOCATION DROPDOWN
...
//                     {/* GEO AI ACTION BUTTON */}
//                     <button
//                         onClick={() => runGeoAiAnalysis()}
//                         style={{
...
//                         }}
//                     >
//                         <Sparkles size={14} /> Geo AI LED Report
//                     </button>

//                     {/* 🚀 MUNICIPALITY SEARCH DROPDOWN */}
//                     <SearchableMunicipalityDropdown
//                         selected={municipalityFilter === 'all' ? null : municipalityFilter}
//                         onSelect={(muni) => updateUrlParams({ municipality: muni, tab: 'learners' })}
//                         municipalities={dynamicMunicipalitiesList}
//                     />
...
//                         const isSelected = !!selectedMapLocation && (
//                             loc.name.toLowerCase() === selectedMapLocation.toLowerCase() ||
//                             selectedMapLocation.toLowerCase().includes(loc.name.toLowerCase())
//                         );
...
//                                 fillColor={isSelected ? "var(--mlab-blue)" : "var(--mlab-green)"}
//                                 color={isSelected ? "var(--mlab-midnight)" : "var(--mlab-green-dark)"}
//                                 weight={isSelected ? 3 : 1}
//                                 opacity={0.9}
//                                 fillOpacity={isSelected ? 0.85 : 0.6}
//                                 eventHandlers={{
//                                     click: () => {
//                                         const newLocation = isSelected ? 'all' : loc.name;
//                                         updateUrlParams({ location: newLocation, tab: 'learners' });
//                                     }
//                                 }}
//                             >
//                                 <LeafletTooltip>
//                                     <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.8rem', textAlign: 'center', padding: '4px' }}>
//                                         <strong>{loc.name}</strong><br />
//                                         {loc.count} Learner{loc.count !== 1 ? 's' : ''}<br />
//                                         <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '6px' }}>
//                                             <span style={{ fontSize: '0.7rem', color: isSelected ? 'var(--mlab-red)' : 'var(--mlab-blue)', fontWeight: 700 }}>
//                                                 {isSelected ? 'Click to clear filter' : 'Click to filter roster'}
//                                             </span>
//                                             <button
//                                                 onClick={(e) => {
//                                                     e.stopPropagation();
//                                                     runGeoAiAnalysis(loc.name);
//                                                 }}
//                                                 style={{
//                                                     background: 'var(--mlab-midnight)', color: 'var(--mlab-white)',
...
//                                                 }}
//                                             >
//                                                 <Sparkles size={10} color="#38bdf8" /> AI Analyze Region
//                                             </button>
//                                         </div>
//                                     </div>
//                                 </LeafletTooltip>
...
//             {/* ZOOM UPLOAD POPUP MODAL */}
//             <ZoomAttendanceDropZone
//                 cohort={cohort}
//                 enrolledLearners={enrolledLearners}
//                 isOpen={showZoomUploadModal}
//                 onClose={() => setShowZoomUploadModal(false)}
//             />

//             {/* TARGETED SESSION AUDIENCE MODAL */}
...
//             {/* GEO AI INTELLIGENCE & LED REPORT MODAL */}
//             {showGeoAiModal && createPortal(
//                 <div className="wm-overlay animate-fade-in" onClick={() => setShowGeoAiModal(false)} style={{ zIndex: 999999 }}>
//                     <div className="wm-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '680px', borderRadius: 0, border: '2px solid #0ea5e9' }}>

//                         {/* HEADER */}
//                         <div className="wm-modal__header" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', padding: '1.25rem 1.5rem' }}>
//                             <div className="wm-modal__header-icon" style={{ background: '#0284c7', color: 'white', borderRadius: 0 }}>
//                                 <Sparkles size={20} />
//                             </div>
//                             <div>
//                                 <h2 className="wm-modal__title" style={{ color: 'white' }}>
//                                     Geo AI LED Talent & Transit Intelligence
//                                 </h2>
//                                 <p className="wm-modal__subtitle" style={{ color: '#94a3b8' }}>
//                                     Target Location: <strong>{municipalityFilter !== 'all' ? municipalityFilter : (selectedMapLocation || 'Cohort Cluster')}</strong>
//                                 </p>
//                             </div>
//                             <button className="wm-modal__close" onClick={() => setShowGeoAiModal(false)}><X size={18} color="white" /></button>
//                         </div>

//                         {/* BODY */}
//                         <div className="wm-modal__body" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem', background: '#f8fafc' }}>
//                             {isGeoAiLoading ? (
//                                 <div style={{ textAlign: 'center', padding: '3rem 0' }}>
//                                     <Loader2 className="spin" size={40} color="#0ea5e9" style={{ margin: '0 auto 1rem' }} />
//                                     <h4 style={{ margin: '0 0 6px', color: '#0f172a', fontFamily: 'var(--font-heading)', textTransform: 'uppercase' }}>Decoding Stats-SA Boundaries & Transport Corridors...</h4>
//                                     <p style={{ margin: 0, fontSize: '0.85rem', color: '#64748b' }}>Consulting OpenRouter & Hugging Face AI Router</p>
//                                 </div>
//                             ) : geoAiResult ? (
//                                 <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>

//                                     {/* MUNICIPAL & DISTRICT HEADER CARD */}
//                                     <div style={{ background: 'white', border: '1px solid #cbd5e1', padding: '1rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
//                                         <div>
//                                             <span style={{ fontSize: '0.65rem', fontFamily: 'var(--font-heading)', fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>Local Municipality</span>
//                                             <div style={{ fontSize: '0.95rem', fontWeight: 800, color: '#0369a1', marginTop: '2px', display: 'flex', alignItems: 'center', gap: '6px' }}>
//                                                 <Building2 size={16} /> {geoAiResult.localMunicipality} {geoAiResult.municipalityCode ? `(${geoAiResult.municipalityCode})` : ''}
//                                             </div>
//                                         </div>
//                                         <div>
//                                             <span style={{ fontSize: '0.65rem', fontFamily: 'var(--font-heading)', fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>District / Metro & Province</span>
//                                             <div style={{ fontSize: '0.95rem', fontWeight: 700, color: '#1e293b', marginTop: '2px' }}>
//                                                 {geoAiResult.districtOrMetro}, {geoAiResult.province}
//                                             </div>
//                                         </div>
//                                     </div>

//                                     {/* ECONOMIC HUBS & TRANSIT CORRIDORS */}
//                                     <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
//                                         <div style={{ background: 'white', border: '1px solid #cbd5e1', padding: '1rem' }}>
//                                             <span style={{ fontSize: '0.7rem', fontFamily: 'var(--font-heading)', fontWeight: 800, color: '#0f172a', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
//                                                 <Building2 size={14} color="#0ea5e9" /> Key Employment Hubs
//                                             </span>
//                                             <ul style={{ margin: 0, paddingLeft: '1.2rem', fontSize: '0.8rem', color: '#334155' }}>
//                                                 {geoAiResult.economicHubs?.map((hub: string, idx: number) => (
//                                                     <li key={idx} style={{ marginBottom: '4px' }}>{hub}</li>
//                                                 ))}
//                                             </ul>
//                                         </div>
...
//                                         <div style={{ background: 'white', border: '1px solid #cbd5e1', padding: '1rem' }}>
//                                             <span style={{ fontSize: '0.7rem', fontFamily: 'var(--font-heading)', fontWeight: 800, color: '#0f172a', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
//                                                 <Navigation size={14} color="#8b5cf6" /> Transit Trails to Work
//                                             </span>
//                                             <ul style={{ margin: 0, paddingLeft: '1.2rem', fontSize: '0.8rem', color: '#334155' }}>
//                                                 {geoAiResult.transitTrails?.map((trail: string, idx: number) => (
//                                                     <li key={idx} style={{ marginBottom: '4px' }}>{trail}</li>
//                                                 ))}
//                                             </ul>
//                                         </div>
//                                     </div>

//                                     {/* GENERATED LED EXECUTIVE PITCH */}
//                                     <div style={{ background: '#f0f9ff', border: '1px solid #bae6fd', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '8px' }}>
//                                         <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
//                                             <span style={{ fontSize: '0.7rem', fontFamily: 'var(--font-heading)', fontWeight: 800, color: '#0369a1', textTransform: 'uppercase' }}>
//                                                 Government & Municipal Pitch Statement
//                                             </span>
//                                             <button
//                                                 onClick={() => {
//                                                     navigator.clipboard.writeText(geoAiResult.placementInsight || '');
//                                                     setCopiedPitch(true);
//                                                     setTimeout(() => setCopiedPitch(false), 2000);
//                                                 }}
//                                                 style={{ background: 'white', border: '1px solid #bae6fd', color: '#0369a1', padding: '4px 8px', fontSize: '0.7rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}
//                                             >
//                                                 {copiedPitch ? <Check size={12} /> : <Copy size={12} />}
//                                                 {copiedPitch ? 'Copied!' : 'Copy Pitch'}
//                                             </button>
//                                         </div>
//                                         <p style={{ margin: 0, fontSize: '0.85rem', color: '#0c4a6e', lineHeight: 1.5, fontWeight: 500 }}>
//                                             "{geoAiResult.placementInsight}"
//                                         </p>
//                                     </div>
...
//                         {/* FOOTER */}
//                         <div className="wm-modal__footer" style={{ borderTop: '1px solid #cbd5e1', padding: '1rem 1.5rem', background: 'white' }}>
//                             <button className="wm-btn wm-btn--ghost" style={{ borderRadius: 0 }} onClick={() => setShowGeoAiModal(false)}>Close</button>
//                             <button
//                                 className="wm-btn wm-btn--primary"
//                                 style={{ background: '#0ea5e9', borderColor: '#0ea5e9', borderRadius: 0 }}
//                                 onClick={() => runGeoAiAnalysis()}
//                                 disabled={isGeoAiLoading}
//                             >
...
//                                     <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#e0f2fe', padding: '4px 10px', border: '1px solid #bae6fd', fontSize: '0.8rem', fontWeight: 700, color: '#0369a1' }}>
//                                         <Building2 size={14} /> Muni: <strong>{municipalityFilter}</strong>
//                                         <X size={14} style={{ cursor: 'pointer', marginLeft: '6px' }} onClick={() => updateUrlParams({ municipality: 'all' })} />
//                                     </div>
//                                 )}
//                             </div>
...
//                                             value={exitEmailFilter}
//                                             onChange={(e) => updateUrlParams({ exitEmail: e.target.value })}
...
//                                     {/* 🚀 MUNICIPALITY DROPDOWN CONTROL */}
//                                     <div className="lfm-fg" style={{ gap: '4px' }}>
//                                         <label>Municipality:</label>
//                                         <select
//                                             className="lfm-input lfm-select"
//                                             value={municipalityFilter}
//                                             onChange={(e) => updateUrlParams({ municipality: e.target.value })}
//                                             style={{ minWidth: '170px', borderRadius: 0, fontWeight: municipalityFilter !== 'all' ? 700 : 400 }}
//                                         >
//                                             <option value="all">All Municipalities</option>
//                                             {dynamicMunicipalitiesList.map(m => (
...
//                                                 <th style={{ color: 'var(--mlab-grey)', borderBottom: '1px solid var(--mlab-border)', borderTop: 'none', borderRadius: 0 }}>Location & Municipality</th>
...
//                                             {paginatedLearners.length === 0 ? (
//                                                 <tr>
//                                                     <td colSpan={9} style={{ padding: '3rem', textAlign: 'center', color: 'var(--mlab-grey)' }}>
...
//                                                     const muniRes = resolveMunicipalityFromLearner(learner);
...
//                                                                     <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.8rem', fontWeight: 700, color: 'var(--mlab-blue)' }}>
//                                                                         <Building2 size={12} /> {muniRes.municipality}
//                                                                     </span>
...
//                                                             </td>
//                                                             <td>
//                                                                 {renderWithdrawalEmailBadge(learner)}
//                                                             </td>
//                                                             <td>
...
//                                                                         {stats && stats.exempt > 0 && (
//                                                                             <span style={{ fontSize: '0.7rem', color: '#8b5cf6', marginLeft: '4px' }}>
//                                                                                 ({stats.exempt} exempt)
//                                                                             </span>
//                                                                         )}
//                                                                     </span>
...
//                                                                 {log.sessionTitle || `Session ${log.sessionNumber || 1}`}
//                                                             </span>
...
//                                                 borderRadius: 0
//                                             }}
//                                             onClick={() => setShowZoomUploadModal(true)}
//                                         >
//                                             <UploadCloud size={14} /> Upload Zoom Attendance
//                                         </button>
...
//                                                             <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
//                                                                 Exempt (N/A)
//                                                                 <span title="Learners who were not invited to this targeted session." style={{ cursor: 'help', display: 'flex' }}>
//                                                                     <Info size={14} color="var(--mlab-grey)" />
//                                                                 </span>
//                                                             </div>
//                                                         </th>
...
//                                                                                 <Clock size={10} style={{ display: 'inline', marginBottom: '-1px' }} /> SESSION {log.sessionNumber || 1} OF {log.totalDaySessions}
//                                                                             </span>
//                                                                         )}

//                                                                         {log.sessionScope === 'targeted' && (
...
//                                                                                 <UserCheck size={10} /> TARGETED ({log.invitedLearnerIds?.length || 0} INVITED)
//                                                                             </span>
//                                                                         )}
...
//                                                             </td>
//                                                             <td>
//                                                                 <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '2px 6px', background: '#f1f5f9', color: '#64748b', fontSize: '0.8rem', fontWeight: 600, borderRadius: '0', border: '1px solid #cbd5e1' }}>
//                                                                     <MinusCircle size={12} /> {log.totalExempt || 0}
//                                                                 </span>
//                                                             </td>
//                                                             <td style={{ textAlign: 'right' }}>
...
//                                                                     <button
//                                                                         className="lfm-btn lfm-btn--ghost"
//                                                                         style={{ padding: '6px 10px', fontSize: '0.7rem', borderRadius: 0, color: 'var(--mlab-blue)' }}
//                                                                         onClick={() => setSelectedSessionForAudience(log)}
//                                                                         title="Manage Invited Audience"
//                                                                     >
...
//                                                                             navigate(`/facilitator/attendance/${cohort.id}?date=${cleanDate}${log.id ? `&sessionId=${log.id}` : ''}`);
//                                                                         }}
//                                                                     >
...
//                             {/* SURVEY SUMMARY KPI CARDS */}
//                             <div className="mc-cards-wrapper">
//                                 <div className="mc">
//                                     <div className="mc-hdr">
//                                         <div>
//                                             <div className="mc-label">Form Compliance</div>
//                                             <div className="mc-title">Exit Survey Link</div>
//                                         </div>
//                                         <ClipboardList size={20} color="var(--mlab-blue)" />
//                                     </div>
//                                     <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto', background: 'var(--mlab-bg)', border: '1px solid var(--mlab-border)', padding: '12px' }}>
//                                         <span style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--mlab-blue)' }}>Zoho Exit Form</span>
//                                         <a
//                                             href="https://survey.zohopublic.eu/zs/0HByGj"
//                                             target="_blank"
//                                             rel="noopener noreferrer"
//                                             style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '0.75rem', color: 'var(--mlab-green-dark)', background: 'var(--mlab-green-bg)', padding: '4px 8px', border: '1px solid var(--mlab-green)', fontWeight: 700, textDecoration: 'none' }}
//                                         >
...
//                                     <div className="mc-hdr">
//                                         <div>
//                                             <div className="mc-label">Response Metrics</div>
//                                             <div className="mc-title">Exit Form Delivered</div>
//                                         </div>
//                                         <Send size={20} color="var(--mlab-blue)" />
//                                     </div>
//                                     <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto', background: 'var(--mlab-bg)', border: '1px solid var(--mlab-border)', padding: '12px' }}>
//                                         <span style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--mlab-blue)' }}>
//                                             {enrolledLearners.filter(l => l.status === 'dropped' && l.withdrawalEmailStatus?.sent === true).length} / {enrolledLearners.filter(l => l.status === 'dropped').length}
//                                         </span>
//                                         <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--mlab-grey)' }}>Withdrawn Participants</span>
//                                     </div>
//                                 </div>
...
//                                     <div className="mc-hdr">
//                                         <div>
//                                             <div className="mc-label">Active Surveys</div>
//                                             <div className="mc-title">Feedback Pipeline</div>
//                                         </div>
//                                         <CheckCircle2 size={20} color="var(--mlab-blue)" />
//                                     </div>
//                                     <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto', background: 'var(--mlab-bg)', border: '1px solid var(--mlab-border)', padding: '12px' }}>
//                                         <span style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--mlab-midnight)' }}>Exit Survey Trigger</span>
//                                         <span style={{ fontSize: '0.7rem', fontWeight: 800, background: '#f0fdf4', color: '#15803d', border: '1px solid #bbf7d0', padding: '2px 6px', textTransform: 'uppercase' }}>Automated</span>
//                                     </div>
//                                 </div>
//                             </div>

//                             {/* SURVEY TRACKER TABLE */}
//                             <div style={{ border: '1px solid var(--mlab-border)', borderRadius: 0, backgroundColor: 'var(--mlab-white)' }}>
//                                 <div className="lfm-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
//                                     <h2 className="lfm-header__title">
//                                         <ClipboardList size={18} /> Cohort Survey Response Roster
//                                     </h2>
//                                 </div>
...
//                                                 <tr>
//                                                     <th style={{ color: 'var(--mlab-grey)', borderBottom: '1px solid var(--mlab-border)', borderTop: 'none', borderRadius: 0 }}>Learner</th>
//                                                     <th style={{ color: 'var(--mlab-grey)', borderBottom: '1px solid var(--mlab-border)', borderTop: 'none', borderRadius: 0 }}>Cohort Status</th>
//                                                     <th style={{ color: 'var(--mlab-grey)', borderBottom: '1px solid var(--mlab-border)', borderTop: 'none', borderRadius: 0 }}>Exit Form Survey Link</th>
//                                                     <th style={{ color: 'var(--mlab-grey)', borderBottom: '1px solid var(--mlab-border)', borderTop: 'none', borderRadius: 0 }}>Sent Timestamp</th>
//                                                     <th style={{ textAlign: 'right', color: 'var(--mlab-grey)', borderBottom: '1px solid var(--mlab-border)', borderTop: 'none', borderRadius: 0 }}>Actions</th>
//                                                 </tr>
//                                             </thead>
//                                             <tbody>
...
//                                                             No learners enrolled in this cohort.
//                                                         </td>
//                                                     </tr>
//                                                 ) : (
//                                                     enrolledLearners.map((learner: any, idx: number) => {
//                                                         const isDropped = learner.status === 'dropped';
//                                                         const emailStatus = learner.withdrawalEmailStatus;

//                                                         return (
//                                                             <tr key={learner.idNumber || learner.id || idx} className="animate-fade-in">
//                                                                 <td>
//                                                                     <div className="cdp-learner-cell">
//                                                                         <div className="cdp-learner-avatar" style={{ borderRadius: 0, background: 'var(--mlab-light-blue)', color: 'var(--mlab-blue)', border: '1px solid var(--mlab-border)' }}>{learner.fullName?.charAt(0)}</div>
//                                                                         <div className="cdp-learner-cell__info">
//                                                                             <span className="cdp-learner-cell__name" style={{ fontFamily: 'var(--font-heading)', fontSize: '0.9rem', color: 'var(--mlab-blue)' }}>{learner.fullName}</span>
//                                                                             <span className="cdp-learner-cell__id" style={{ fontFamily: 'var(--font-body)', fontSize: '0.75rem', color: 'var(--mlab-grey)' }}>{learner.email || 'No Email'}</span>
//                                                                         </div>
//                                                                     </div>
//                                                                 </td>
//                                                                 <td>
//                                                                     {isDropped ? (
//                                                                         <span style={{ display: 'inline-block', padding: '4px 8px', fontFamily: 'var(--font-heading)', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', background: '#fee2e2', color: '#991b1b', border: '1px solid #fca5a5', borderRadius: 0 }}>Withdrawn</span>
//                                                                     ) : (
//                                                                         <span style={{ display: 'inline-block', padding: '4px 8px', fontFamily: 'var(--font-heading)', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', background: 'var(--mlab-green-bg)', color: 'var(--mlab-green-dark)', border: '1px solid var(--mlab-green)', borderRadius: 0 }}>Active</span>
//                                                                     )}
//                                                                 </td>
...
//                                                                 <td>
//                                                                     <span style={{ fontSize: '0.8rem', color: 'var(--mlab-grey)', fontWeight: 600 }}>
//                                                                         {emailStatus?.sentAt ? new Date(emailStatus.sentAt.toDate ? emailStatus.sentAt.toDate() : emailStatus.sentAt).toLocaleString('en-ZA') : '—'}
//                                                                     </span>
//                                                                 </td>
//                                                                 <td style={{ textAlign: 'right' }}>
//                                                                     {isDropped && (
...
//                                                                             onClick={() => setLearnerToDrop(learner)}
//                                                                         >
//                                                                             <FileText size={12} /> View Exit Details
//                                                                         </button>
//                                                                     )}
//                                                                 </td>
//                                                             </tr>
...
// //     // 🚀 NEW STATE: GEO AI ANALYSIS MODAL
// //     const [isGeoAiLoading, setIsGeoAiLoading] = useState(false);
// //     const [geoAiResult, setGeoAiResult] = useState<any | null>(null);
// //     const [showGeoAiModal, setShowGeoAiModal] = useState(false);
// //     const [copiedPitch, setCopiedPitch] = useState(false);
...
// //     // 🚀 DYNAMIC CALLABLE HANDLER: RUN GEO AI ANALYSIS
// //     const runGeoAiAnalysis = async (targetLocName?: string) => {
// //         const locToAnalyze = targetLocName || selectedMapLocation || "Cohort General Cluster";
...
// //             // Find sample learner matching location
// //             const matchedLearner = enrolledLearners.find(l =>
// //                 getLocationString(l).toLowerCase().includes(locToAnalyze.toLowerCase())
// //             ) || enrolledLearners[0];
...
// //     // 🚀 INTERACTIVE MAP RENDERER WITH GEO AI TOOLTIP & HEADER TRIGGER
// //     const renderMapComponent = (isFullscreen: boolean) => (
// //         <div style={{ height: '100%', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--mlab-white)' }}>
...
// //                 <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
// //                     {/* 🚀 GEO AI ANALYSIS HEADER ACTION BUTTON */}
// //                     <button
// //                         onClick={() => runGeoAiAnalysis()}
// //                         style={{
// //                             background: 'linear-gradient(135deg, #0ea5e9 0%, #0284c7 100%)',
...
// //                                 </h2>
// //                                 <p className="wm-modal__subtitle" style={{ color: '#94a3b8' }}>
// //                                     Target Location: <strong>{selectedMapLocation || 'Cohort Cluster'}</strong>
// //                                 </p>
// //                             </div>
// //                             <button className="wm-modal__close" onClick={() => setShowGeoAiModal(false)}><X size={18} color="white" /></button>
// //                         </div>
...
import {
    ...
    Search, X, Info, BarChart2, UserMinus, Edit2, Loader2, Video, Layers, History, ChevronDown, ChevronUp, MapPin, Filter, ChevronRight, Globe,
    Timer, PenTool, BookOpen, Award, ShieldCheck, User, Maximize, Minimize,
    FileText, Briefcase, AlertTriangle, ClipboardList, ExternalLink, Send, MinusCircle
} from 'lucide-react';
import * as XLSX from 'xlsx';
import { collection, query, where, onSnapshot, doc, updateDoc, writeBatch, increment, getDoc } from 'firebase/firestore';
import { MapContainer, TileLayer, CircleMarker, Tooltip as LeafletTooltip, ZoomControl, useMap } from 'react-leaflet';
...
// 🚀 ACCURATE PROVINCE DEFAULT COORDINATES
const PROVINCE_MAP: Record<string, { name: string, lat: number, lng: number }> = {
    '1': { name: 'Western Cape', lat: -33.9249, lng: 18.4241 },
    '2': { name: 'Eastern Cape', lat: -33.9608, lng: 25.6022 },
    '3': { name: 'Northern Cape', lat: -28.7282, lng: 24.7499 },
...
// 🚀 SA MAJOR TOWNS & CITIES GPS LOOKUP (Fixes Boksburg, Kimberley, etc.)
const SA_TOWN_COORDS: Record<string, [number, number]> = {
    'boksburg': [-26.2126, 28.2618],
    'kimberley': [-28.7282, 24.7499],
    'durban': [-29.8587, 31.0218],
...
// 🚀 COORDINATE SANITIZER & AUTO-CORRECT ENGINE FOR SOUTH AFRICA
const sanitizeSACoords = (rawLat: any, rawLng: any): [number, number] | null => {
    let lat = typeof rawLat === 'number' ? rawLat : parseFloat(rawLat);
    let lng = typeof rawLng === 'number' ? rawLng : parseFloat(rawLng);
    ...
    // Detect Swapped Lat/Lng (e.g., Lat is 28.26, Lng is -26.21)
    if (lat > 10 && lng < 0) {
        const temp = lat;
        lat = lng;
        lng = temp;
    ...
    // Auto-correct positive SA latitudes (e.g., 26.2041 -> -26.2041)
    if (lat > 20 && lat < 36) {
        lat = -lat;
    }

    // Strict South African Geographic Bounding Box Check:
    // Lat: -35.5 to -21.0 | Lng: 15.0 to 33.5
    if (lat >= -35.5 && lat <= -21.0 && lng >= 15.0 && lng <= 33.5) {
        return [lat, lng];
    }
...
export const BootcampCohortView: React.FC<{ cohort: any }> = ({ cohort }) => {
    ...
    // 🚀 TARGET COORDINATES LOOKUP WITH SANITIZED GPS & ACCURATE TOWN/PROVINCE FALLBACK
    const targetCoordsLookup = useMemo(() => {
        const lookup = new Map<string, [number, number]>();
        const provData = new Map<string, { latLngs: [number, number][] }>();
        const cityData = new Map<string, { latLngs: [number, number][] }>();
    ...
    // 🚀 FLEXIBLE LOCATION MATCHER FOR BOTH PROVINCE & CITY CLICKED MARKERS
    const filteredLearners = useMemo(() => {
        if (!cohortAnalytics) return [];
    ...
    // 🚀 MAP MARKER AGGREGATOR WITH SANITIZED COORDINATES & SA TOWN FALLBACKS
    const locationStats = useMemo(() => {
        const stats = new Map<string, { lat: number, lng: number, count: number, name: string, latLngs: [number, number][] }>();
        ...
        learnersForMap.forEach(l => {
            ...
            // 1. Precise SA town coordinates, else Province center
            const defaultGPS = SA_TOWN_COORDS[townKey] || [provInfo.lat, provInfo.lng];

            // 2. Validate/Sanitize raw Firestore GPS values
            const validGPS = sanitizeSACoords(demos.lat, demos.lng);
    ...
    // INTERACTIVE MAP RENDERER WITH CLICK-TO-FILTER & HIGHLIGHTING
    const renderMapComponent = (isFullscreen: boolean) => (
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--mlab-white)' }}>
