...
const ManageSessionModal: React.FC<{
    ...
    const handleSaveUpdates = async (markComplete: boolean) => {
        ...
        try {
            ...
            // 🚀 NEW: AUTO-UNLOCK THE LINKED ASSESSMENT!
            if (markComplete && session.assessmentId) {
                try {
                    ...
                    if (!subSnap.empty) {
                        ...
                        // Only run the unlock if the workbook actually needs it (e.g. NYC / Failed)
                        if (['graded', 'moderated', 'returned'].includes(subData.status) && ['NYC', 'DEV', '1', '2'].includes(String(subData.competency || '').toUpperCase())) {
                            ...
                            // 2. Reset the submission and inject the notes automatically
                            ...
                            onClose();
                            return; // Exit early so we don't trigger the generic success toast below
                        }
                    }
                } catch (unlockErr) {
...
export const CoachingScheduleView: React.FC = () => {
    const { user, learners, fetchLearners } = useStore() as any;
