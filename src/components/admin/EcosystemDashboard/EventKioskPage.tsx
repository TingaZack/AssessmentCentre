...
// ─── TIME PILL ───────────────────────────────────────────────────────────────
function TimePill({ label, time, color, now }: { label: string; time: string | number | undefined; color: string; now: number }) {
    if (!time) return null;
    const isRecent = now - new Date(time).getTime() < 15000;
                                ...
                                {recentScans.length === 0 ? (
                                    ...
                                    recentScans.map((scan: any, idx: number) => (
                                        ...
                                        >
                                            ...
                                            <TimePill label="CHECK-IN" time={scan.timestamp} color="#3b82f6" now={currentTimeMs} />
                                        </div>
                                    ))
                                )}
                            </div>
