...
import type { EcosystemEvent, EcosystemTarget } from '../../../types/ecosystem.types';
...
const TargetAnalyticsModal = ({ target, events, guests, checkins, onClose }: any) => {
    ...
    const masterFieldDictionary = useMemo(() => {
        ...
        globalDemos.forEach((f: string) => {
            ...
            if (!dict[squashedKey]) {
                dict[squashedKey] = f.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
            }
        });
        ...
        <div className="lfm-overlay" onClick={onClose} style={{ zIndex: 9999 }}>
            <div className="lfm-modal animate-fade-in" onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: '1100px', height: '85vh', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
                    <div style={{ width: '280px', background: 'var(--mlab-light-blue)', borderRight: '2px solid var(--mlab-border)', padding: '1.5rem', overflowY: 'auto' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px', marginBottom: '1rem' }}>
                            <button type="button" onClick={() => setSelectedFields(availableFields)} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', background: 'white', border: '1px solid var(--mlab-border)', padding: '4px 8px', fontSize: '0.65rem', fontWeight: 600, color: 'var(--mlab-blue)', borderRadius: '4px', cursor: 'pointer' }}>
                                <CheckSquare size={12} /> Select All
                            </button>
                            <button type="button" onClick={() => setSelectedFields([])} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', background: 'white', border: '1px solid var(--mlab-border)', padding: '4px 8px', fontSize: '0.65rem', fontWeight: 600, color: 'var(--mlab-grey)', borderRadius: '4px', cursor: 'pointer' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                            {availableFields.map(field => {
                                ...
                                return (
                                    <label key={field} className="lfm-checkbox-row" style={{ background: isChecked ? 'var(--mlab-white)' : 'transparent', padding: '0.6rem', border: `1px solid ${isChecked ? 'var(--mlab-blue)' : 'transparent'}`, transition: 'all 0.15s', borderRadius: '4px' }}>
                                        <input type="checkbox" checked={isChecked} onChange={() => toggleField(field)} />
                                        <span style={{ fontWeight: isChecked ? 700 : 600, color: isChecked ? 'var(--mlab-blue)' : 'var(--mlab-grey)', textTransform: 'uppercase', fontSize: '0.7rem', letterSpacing: '0.05em' }}>{field}</span>
                                    </label>
                                );
                            })}
                        </div>
                ...
                <div className="lfm-footer" style={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                    <button className="lfm-btn lfm-btn--ghost" onClick={handleExportReport} style={{ borderColor: 'var(--mlab-green)', color: 'var(--mlab-green-dark)' }} disabled={selectedFields.length === 0}>
                        <Download size={14} /> Export Checked Data
                    </button>
                    <button className="lfm-btn lfm-btn--ghost" onClick={onClose}>Close Report</button>
                </div>
            </div>
        </div>
    );
...
export const EcosystemDashboard: React.FC = () => {
    const toast = useToast();
    const { programmes, user, fetchProgrammes, events, guests, checkins, targets, ecosystemLoading, fetchEcosystemData, saveEcosystemEvent, saveKpiTarget, archiveKpiTarget, deleteKpiTarget } = useStore();
    ...
    const [kpiForm, setKpiForm] = useState<EcosystemTarget | null>(null);
    const [kpiEventSearch, setKpiEventSearch] = useState('');

    // Connect real-time state listeners through central store bindings
    useEffect(() => {
        if (fetchEcosystemData) fetchEcosystemData();
        if (programmes.length === 0 && fetchProgrammes) fetchProgrammes();
    }, [fetchEcosystemData, fetchProgrammes, programmes.length]);

    const handleSaveEvent = async (eventData: Partial<EcosystemEvent>) => {
        try {
            await saveEcosystemEvent(eventData, selectedEvent?.id);
            toast.success(selectedEvent?.id ? "Event updated successfully!" : "Event created successfully!");
            setShowEventModal(false);
            setSelectedEvent(null);
        } catch (error) {
            console.error("Failed to save event:", error);
            toast.error("Failed to persist event infrastructure parameters.");
        }
    };
    ...
    const openKpiModal = (targetData?: EcosystemTarget) => {
        setKpiEventSearch('');
        if (targetData) {
    ...
    const handleSaveKpi = async () => {
        if (!kpiForm) return;
        try {
            await saveKpiTarget(kpiForm);
            toast.success("Impact Target saved successfully.");
            setShowKpiModal(false);
        } catch (error) {
            console.error(error);
            toast.error("Failed to save target payload configuration mapping.");
        }
    };
    ...
    const executeArchiveKpi = async () => {
        ...
        try {
            await archiveKpiTarget(targetToArchive);
            toast.success("Impact Target moved into operational archives.");
        } catch (error) {
            console.error(error);
            toast.error("Network synchronization abort: unable to archive structural reference data.");
        } finally {
    ...
    const executeDeleteKpi = async () => {
        ...
        try {
            await deleteKpiTarget(targetToDelete);
            toast.success("Impact Target wiped from database records completely.");
        } catch (error) {
            console.error(error);
            toast.error("DANGER: Cascade write failure while deleting target node.");
        } finally {
    ...
    const calculatedTargets = useMemo(() => {
        return targets
            ...
            .map(target => {
                ...
                events.forEach(e => {
                    const checkinsCount = e.currentCheckIns || 0;
                    ...
                    if (countsTowardsTarget) kpiWindowImpact += checkinsCount;
                });
    ...
    // ─── OVERALL STRUCTURAL METRICS CARD GENERATOR (FALLBACK VIEW WHEN TARGET REFRESH IS EMPTY) ───
    const structuralOverviewStats = useMemo(() => {
        if (calculatedTargets.length > 0) return null; // Only evaluate if no active target panels exist

        let accumulatedGlobalExpected = 0;
        let accumulatedGlobalAcheived = 0;

        events.forEach(e => {
            accumulatedGlobalExpected += e.maxCapacity || 0;
            accumulatedGlobalAcheived += e.currentCheckIns || 0;
        });

        return {
            totalCapacityPool: accumulatedGlobalExpected,
            totalCheckedInPool: accumulatedGlobalAcheived,
            aggregateEfficiencyRate: accumulatedGlobalExpected > 0 ? Math.round((accumulatedGlobalAcheived / accumulatedGlobalExpected) * 100) : 0
        };
    }, [events, calculatedTargets]);
    ...
    if (ecosystemLoading) {
        return <div className="att-loader-wrap"><Loader message="Loading Ecosystem Data Directory…" /></div>;
    }
    ...
    return (
        <div className="att-root animate-fade-in">
            ...
            {showKpiModal && kpiForm && createPortal(
                <div className="lfm-overlay" onClick={() => setShowKpiModal(false)} style={{ zIndex: 99 }}>
            ...
            {/* 🚀 HORIZONTAL KPI TARGET SCROLL / STRUCTURAL OVERVIEW SUMMARY 🚀 */}
            <div style={{ display: 'flex', gap: '1rem', overflowX: 'auto', paddingBottom: '1rem', marginBottom: '0.5rem', scrollbarWidth: 'none' }}>
                ...
                {/* ── FALLBACK AD-HOC STRUCTURAL SUMMARY CARD ── */}
                {structuralOverviewStats && (
                    <div style={{ minWidth: '360px', flex: '1 0 auto', background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)', padding: '1.5rem', color: 'white', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderLeft: '4px solid var(--mlab-blue)', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--mlab-blue)', fontWeight: 700, fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>
                                <Activity size={12} /> Aggregate Ecosystem Yield
                            </div>
                            <h2 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 800, fontFamily: 'var(--font-heading)' }}>
                                {structuralOverviewStats.totalCheckedInPool.toLocaleString()} <span style={{ fontSize: '0.9rem', color: '#64748b', fontWeight: 600 }}>/ {structuralOverviewStats.totalCapacityPool.toLocaleString()} Seats Filled</span>
                            </h2>
                            <p style={{ margin: '4px 0 0', fontSize: '0.75rem', color: '#94a3b8' }}>Ecosystem utilization stats evaluated across all historic active events.</p>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                            <div style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--mlab-blue)', fontFamily: 'var(--font-heading)', lineHeight: 1 }}>{structuralOverviewStats.aggregateEfficiencyRate}%</div>
                            <div style={{ fontSize: '0.65rem', color: '#64748b', textTransform: 'uppercase', fontWeight: 700, marginTop: '2px' }}>Fill Efficiency</div>
                        </div>
                    </div>
                )}
