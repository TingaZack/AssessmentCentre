...
import {
    ...
    Settings, CheckCircle, Globe, Database, Copy // 🚀 Added Copy icon
} from 'lucide-react';
...
export const EcosystemDashboard: React.FC = () => {
    ...
    const handleDuplicateEvent = (eventToCopy: EcosystemEvent) => {
        // We spread the old event to keep its data, but explicitly remove the ID and system stats.
        const { id, currentCheckIns, createdAt, updatedAt, status, createdBy, ...clonedData } = eventToCopy as any;

        // Append "(Copy)" so the admin immediately knows they are working on the duplicated version
        const newEventDraft = {
            ...clonedData,
