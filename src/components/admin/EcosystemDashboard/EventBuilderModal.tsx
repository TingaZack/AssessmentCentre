...
// import {
//     X, Save, Loader2, AlertCircle, Calendar,
//     Settings, ListPlus, Plus, Trash2, Globe, MapPin, Lock, Tag, Wifi
// } from "lucide-react";
// import Autocomplete from "react-google-autocomplete";
// import { useToast } from "../../../components/common/Toast/Toast";
// import { useStore } from "../../../store/useStore";
...
//     const [wifiSsid, setWifiSsid] = useState((event?.settings)?.wifiSsid || "");
//     const [wifiPassword, setWifiPassword] = useState((event?.settings)?.wifiPassword || "");

//     // ─── CORE EVENT STATE ───
//     const [eventName, setEventName] = useState(event?.eventName || "");
//     const [date, setDate] = useState(event?.date ? event.date.split("T")[0] : new Date().toISOString().split("T")[0]);
//     const [maxCapacity, setMaxCapacity] = useState<number>(event?.maxCapacity || 50);
//     const [requireIdPassport, setRequireIdPassport] = useState<GuestIdRequirement>(event?.settings?.requireIdPassport || "optional");
...
//         // Extract a shorter venue name if available
//         const venueName = place.name || formatted.split(',')[0];
//         setLocation(venueName);
//     };
...
//         const eventPayload: Partial<EcosystemEvent> = {
//             eventName,
//             location, // Saved as the short venue name
...
//                 .map(f => ({
//                     ...f,
//                     // Clean up dropdown options: trim spaces and remove empties
//                     options: f.type === 'dropdown' && f.options
//                         ? f.options.map(opt => opt.trim()).filter(opt => opt !== "")
//                         : []
//                 }))
...
//                                                 {/* 🚀 DYNAMIC DROPDOWN OPTIONS INPUT 🚀 */}
//                                                 {field.type === 'dropdown' && (
//                                                     <div className="lfm-fg lfm-fg--full" style={{ marginTop: '0.5rem' }}>
//                                                         <label>Dropdown Options (Comma Separated) *</label>
//                                                         <input
