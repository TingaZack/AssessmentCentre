...
import {
    ...
    Settings, ListPlus, Plus, Trash2, Globe, Lock, Tag, Wifi, Target, Users, BookOpen
} from "lucide-react";
...
interface EventBuilderModalProps {
    ...
    availableTargets?: any[]; // Passed down from dashboard
    availableCohorts?: any[]; // Passed down to allow cohort daily logbook reconciliation
    onClose: () => void;
...
export const EventBuilderModal: React.FC<EventBuilderModalProps> = ({
    ...
    // ─── LOGBOOK & CURRICULUM RECONCILIATION STATE ───
    const [syncToDailyLogbook, setSyncToDailyLogbook] = useState<boolean>((event?.settings as any)?.syncToDailyLogbook || false);
    const [linkedCohortIds, setLinkedCohortIds] = useState<string[]>((event as any)?.linkedCohortIds || []);
    ...
    const allLockedFields = React.useMemo(() => {
        ...
        const globalDemos = ((settings as any)?.globalDemographicFields || ["gender", "race", "disability"])
            .map((f: string) => f.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()));
    ...
    const toggleCohort = (cohortId: string) => {
        if (linkedCohortIds.includes(cohortId)) {
            setLinkedCohortIds(prev => prev.filter(id => id !== cohortId));
        } else {
            setLinkedCohortIds(prev => [...prev, cohortId]);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (syncToDailyLogbook && linkedCohortIds.length === 0) {
            setErrorMessage("Please select at least one target cohort to reconcile daily logbook attendance.");
            return;
        }
        ...
        if (event) {
            const originalPayload: Partial<EcosystemEvent> = {
                ...
                linkedCohortIds: (event as any).linkedCohortIds || [],
                ...
                settings: {
                    ...
                    syncToDailyLogbook: (event.settings as any)?.syncToDailyLogbook || false,
                    allowedProgrammes: []
                } as any,
                    ...
                    <div className="lfm-body" style={{ padding: "1.5rem" }}>
                        ...
                        {isEventEnded && (
                            <div className="lfm-error-banner" style={{ background: '#fffbeb', border: '2px solid #f59e0b', borderLeft: '5px solid #f59e0b', color: '#b45309', marginBottom: '1rem' }}>
                                ...
                                <span><strong>DATA INTEGRITY ENGAGED:</strong> This ecosystem event has concluded. Core records are cryptographically frozen to avoid altering demographic metrics. <strong>Note: Facilitator assignments, Cohort Sync, and Custom Questions can still be modified.</strong></span>
                            </div>
                        )}
                        ...
                        <div style={{ display: 'flex', gap: '2rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                            ...
                            <div style={{ flex: '1 1 450px', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                ...
                                {/* ─── CURRICULUM RECONCILIATION & DAILY LOGBOOK SYNC ─── */}
                                <div>
                                    <div className="lfm-section-hdr"><BookOpen size={13} /> Curriculum & Daily Register Sync</div>
                                    <div style={{ background: 'white', padding: '1.25rem', borderRadius: '8px', border: '1px solid var(--mlab-border)' }}>
                                        <label className="lfm-checkbox-row" style={{ marginBottom: '0.75rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                            <input
                                                type="checkbox"
                                                checked={syncToDailyLogbook}
                                                onChange={(e) => setSyncToDailyLogbook(e.target.checked)}
                                            />
                                            <span style={{ fontWeight: 600, fontSize: '0.85rem', color: 'var(--mlab-midnight)' }}>
                                                Credit Attendees in Daily Compliance Register
                                            </span>
                                        </label>
                                        <p style={{ fontSize: '0.75rem', color: 'var(--mlab-grey)', marginBottom: '14px', lineHeight: 1.4 }}>
                                            When checked, learners attending this curriculum-aligned workshop will be credited with <strong style={{ color: 'var(--mlab-blue)' }}>"Present - Workshop"</strong> in their cohort daily logbooks for date <strong style={{ color: '#0f172a' }}>{startDate}</strong>, resolving missing scan absences.
                                        </p>

                                        {syncToDailyLogbook && (
                                            <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '1px solid #e2e8f0' }}>
                                                <label style={{ fontSize: '0.8rem', fontWeight: 600, color: '#334155', marginBottom: '6px', display: 'block' }}>
                                                    Target Cohort(s) for Logbook Reconciliation *
                                                </label>
                                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                                    {availableCohorts?.map(cohort => {
                                                        const isSelected = linkedCohortIds.includes(cohort.id);
                                                        return (
                                                            <button
                                                                key={cohort.id}
                                                                type="button"
                                                                ...
                                                                style={{
                                                                    background: isSelected ? 'var(--mlab-green)' : '#f8fafc',
                                                                    color: isSelected ? 'white' : 'var(--mlab-grey)',
                                                                    border: `1px solid ${isSelected ? 'var(--mlab-green)' : '#cbd5e1'}`,
                                                                    padding: '5px 12px',
                                                                    borderRadius: '16px',
                                                                    fontSize: '0.75rem',
                                                                    fontWeight: 600,
                                                    ...
                                                    {(!availableCohorts || availableCohorts.length === 0) && (
                                                        <span style={{ fontSize: '0.75rem', color: '#94a3b8', fontStyle: 'italic' }}>
                                                            No active cohorts found in system.
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
