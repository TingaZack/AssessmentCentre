...
const emptyLearner: DashboardLearner = {
    id: "", learnerId: "", enrollmentId: "",
    fullName: "", firstName: "", lastName: "", idNumber: "", dateOfBirth: "", email: "", phone: "", mobile: "",
    cohortId: "", campusId: "", trainingStartDate: "", trainingEndDate: "", isArchived: false,
    authStatus: "pending", status: "active", isDraft: false, isOffline: false, qualification: { ...emptyQualification },
...
export const LearnerFormModal: React.FC<LearnerFormModalProps> = ({
    ...
    const { fetchCohorts, settings } = useStore();
    ...
    const [formData, setFormData] = useState<DashboardLearner>(() => {
        ...
        return { ...emptyLearner, cohortId: currentCohortId || "" };
    });
    ...
    const handleAddressSelected = (place: any) => {
        ...
        if (match) {
            setFormData(prev => ({
                ...prev, demographics: { ...prev.demographics, statssaAreaCode: match.statssa_area_code }
            }));
            setAreaSearch(`${match.statssa_area_code} (${match.town} - ${match.district_municipality} - ${match.area})`);
        }
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                setStatusModal({ type: "error", title: "Import Failed", message: "Could not parse the file. Please ensure it is a valid .csv or .xlsx file.", onClose: () => setStatusModal(null) });
            }
        };
            ...
            rows.forEach((cols) => {
                ...
                if (isModuleSection) {
                    ...
                    if (col1 && col1.toLowerCase() !== "module name") {
                        ...
                        const rawStatus = String(cols[8] || cols[7] || "").trim() || "Not Started";
    ...
    const handleQuickCohortCreate = async (cohortData: Omit<Cohort, "id" | "createdAt" | "staffHistory" | "isArchived">, reasons?: { facilitator?: string; assessor?: string; moderator?: string }) => {
        try {
            ...
            setStatusModal({ type: "error", title: "Creation Failed", message: err.message || "Could not save the new class to the database.", onClose: () => setStatusModal(null) });
        }
    };
                    ...
                    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", overflow: "hidden", flex: 1 }}>
                        <div className="lfm-body">
                            ...
                            <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", padding: "1rem", borderRadius: "8px", marginBottom: "1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "1rem" }}>
                                ...
                                <button type="button" className="lfm-btn" onClick={handleSyncCohortResults} disabled={isSyncing || !formData.cohortId} style={{ background: "#16a34a", color: "white", border: "none", padding: "0.55rem 1rem", whiteSpace: "nowrap" }}>
