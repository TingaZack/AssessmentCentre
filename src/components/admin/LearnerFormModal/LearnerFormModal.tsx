        ...
        try {
            ...
            rows.forEach((cols) => {
                ...
                if (!isModuleSection) {
                    ...
                    else if (col0Lower.includes("issue date")) issueDateStr = parseLocalToSA(cols[1]); // 🚀 NO UTC SHIFT
                ...
                if (isModuleSection) {
                    ...
                    if (col1 && col1.toLowerCase() !== "module name") {
                        ...
                        const mod = {
                            ...
                            status: status,
                            // 🚀 STRICT SYNC: Date Assessed equals Statement Issue Date
            ...
            setFormData((prev) => ({
                ...
                qualification: {
                    ...
                    // 🚀 STRICT SYNC: Qualification Date Assessed equals Statement Issue Date
                    dateAssessed: issueDateStr,
                },
                demographics: {
                    ...(prev.demographics || {}),
            ...
            setStatusModal({
                ...
                message: `Successfully populated details for ${fullName || "Learner"}. Review the fields below and click "Save Learner".`,
...
//         field: keyof LearnerDemographics,
...
//     // ─── SMART CSV IMPORTER FOR OFFLINE LEARNERS ───
//     const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
//         const file = e.target.files?.[0];
//         if (!file) return;
...
//             let importedCohortName = "";
//             let importedSdpCode = GLOBAL_SDP_CODE;
...
//             rows.forEach((cols) => {
//                 if (!cols || cols.length === 0 || (cols.length === 1 && cols[0] === ""))
//                     return;
...
//                 if (!isModuleSection) {
//                     if (
//                         col0Lower === "qualification title" ||
//                         col0Lower.includes("qualification title")
//                     )
//                         qualName = cols[1]?.replace(/,/g, "").trim();
//                     else if (col0Lower === "nqf level" || col0Lower.includes("nqf level"))
//                         nqfLevel = parseInt(cols[1]?.replace(/,/g, "").trim()) || 0;
//                     else if (
//                         col0Lower === "saqa qual id" ||
//                         col0Lower.includes("saqa qual id")
//                     )
//                         saqaId = cols[1]?.replace(/,/g, "").trim();
//                     else if (col0Lower === "credits" || col0Lower.includes("credits"))
//                         totalCredits = parseInt(cols[1]?.replace(/,/g, "").trim()) || 0;
//                     else if (
//                         col0Lower.includes("name") &&
//                         (col0Lower.includes("learner") || col0Lower.includes("leaner"))
//                     )
//                         fullName = cols[1]?.replace(/,/g, "").trim();
//                     else if (col0Lower === "id number" || col0Lower.includes("id number"))
//                         idNumber = cols[1]?.replace(/,/g, "").trim();
//                     else if (
//                         col0Lower === "email address" ||
//                         col0Lower.includes("email address")
//                     )
//                         emailAddress = cols[1]?.replace(/,/g, "").trim();
//                     else if (
//                         col0Lower === "phone number" ||
//                         col0Lower.includes("phone number")
//                     )
//                         phoneNumber = cols[1]?.replace(/,/g, "").trim();
//                     else if (col0Lower.includes("start date"))
//                         startDate = safeParseDate(cols[1]);
//                     else if (col0Lower === "cohort" || col0Lower.includes("cohort"))
//                         importedCohortName = cols[1]?.replace(/,/g, "").trim();
//                     else if (
//                         col0Lower === "sdp code" ||
//                         col0Lower.includes("sdp code") ||
//                         col0Lower === "provider code"
//                     ) {
//                         importedSdpCode =
//                             cols[1]?.replace(/,/g, "").trim() || GLOBAL_SDP_CODE;
//                     }

//                     if (
//                         (col0Lower === "modules" || col0Lower === "") &&
//                         col1.toLowerCase() === "module name"
//                     ) {
//                         isModuleSection = true;
//                         return;
//                     }
...
//                 if (isModuleSection) {
//                     if (col0Lower.includes("knowledge")) {
//                         currentSection = "K";
//                     } else if (col0Lower.includes("practical")) {
//                         currentSection = "P";
//                     } else if (
//                         col0Lower.includes("work experience") ||
//                         col0Lower.includes("workplace")
//                     ) {
//                         currentSection = "W";
//                     }

//                     if (col1 && col1.toLowerCase() !== "module name") {
//                         const modName = col1.replace(/\n|\r/g, " ").trim();
//                         const modCode = cols[2]?.trim() || "";
...
//                             dateSignedOff: new Date().toISOString().split("T")[0],
//                             isTemplateLocked: false,
//                         };

//                         let targetSection = currentSection;
//                         if (modCode.toUpperCase().startsWith("KM")) targetSection = "K";
//                         else if (modCode.toUpperCase().startsWith("PM"))
//                             targetSection = "P";
//                         else if (
//                             modCode.toUpperCase().startsWith("WM") ||
//                             modCode.toUpperCase().startsWith("WE")
//                         )
//                             targetSection = "W";

//                         if (targetSection === "K") knowledgeModules.push(mod);
//                         else if (targetSection === "P") practicalModules.push(mod);
//                         else if (targetSection === "W") workExperienceModules.push(mod);
//                         else knowledgeModules.push(mod);
//                     }
//                 }
//             });
...
//                         importedCohortName.toLowerCase().trim(),
//                 )
//                 : null;
...
//                 email: emailAddress || prev.email,
//                 mobile: phoneNumber || prev.mobile,
//                 trainingStartDate: startDate || prev.trainingStartDate,
//                 cohortId: matchedCohort ? matchedCohort.id : prev.cohortId,
//                 isOffline: true,
//                 qualification: {
//                     name: qualName || prev.qualification.name,
...
//                     totalNotionalHours: (totalCredits || 0) * 10,
//                     dateAssessed:
//                         prev.qualification.dateAssessed ||
...
//             if (importedCohortName && !matchedCohort) {
//                 setStatusModal({
//                     type: "warning",
//                     title: "Cohort Not Found",
//                     message: `Imported data for ${fullName}, but the class "${importedCohortName}" does not exist in your system. Please select an existing class from the dropdown or click "+ New Class" to create it.`,
//                     onClose: () => setStatusModal(null),
//                 });
//             } else {
//                 setStatusModal({
//                     type: "success",
//                     title: "Import Successful",
//                     message: `Successfully imported details for ${fullName || "Learner"}.${matchedCohort ? ` Automatically assigned to "${matchedCohort.name}".` : ""} Review the details and click Save.`,
//                     onClose: () => setStatusModal(null),
//                 });
//             }
...
//                 title: "Import Failed",
//                 message:
//                     "Could not parse the CSV file. Please ensure it matches the standard layout.",
//                 onClose: () => setStatusModal(null),
//             });
//         }
//     };
...
//         cohortData: Omit<
//             Cohort,
//             "id" | "createdAt" | "staffHistory" | "isArchived"
//         >,
//         reasons?: { facilitator?: string; assessor?: string; moderator?: string },
//     ) => {
//         try {
...
//                 type: "success",
//                 title: "Class Created",
//                 message: `New class "${cohortData.name}" created and assigned.`,
//                 onClose: () => setStatusModal(null),
//             });
//         } catch (err: any) {
//             throw new Error(
//                 err.message || "Could not save the new class to the database.",
//             );
//         }
//     };
...
//                 message: "Please assign the learner to a valid cohort first.",
//                 onClose: () => setStatusModal(null),
//             });
//             return;
//         }
...
//         const templateId =
//             (cohort as any).programmeId ||
//             (cohort as any).qualificationId ||
//             selectedTemplateId;
//         const template = programmes.find((p) => p.id === templateId);
...
//                 message: `Cohort "${cohort.name}" does not have a linked Programme Template.`,
//                 onClose: () => setStatusModal(null),
//             });
//             return;
//         }
...
//                 const q = query(
//                     subRef,
//                     where("learnerId", "==", formData.learnerId || formData.id),
...
//                         (s) => s.moduleNumber === m.code || s.moduleNumber === m.name,
//                     );
//                     let status = "Not Started";
//                     let dateAssessed = "";

//                     if (sub) {
//                         dateAssessed = sub.assignedAt
//                             ? new Date(sub.assignedAt).toISOString().split("T")[0]
//                             : "";
//                         if (sub.status === "graded") {
//                             status =
//                                 sub.marks >= sub.totalMarks / 2
//                                     ? "Competent"
//                                     : "Not Yet Competent";
//                         } else {
//                             status = "Pending Grading";
...
//                 knowledgeModules: lockSystemModules(template.knowledgeModules),
//                 practicalModules: lockSystemModules(template.practicalModules),
//                 workExperienceModules: lockSystemModules(
//                     template.workExperienceModules,
//                 ),
//             }));
...
//                 type: "success",
//                 title: "Sync Complete",
//                 message: `Pulled ${submissions.length} authentic assessment result(s).`,
//                 onClose: () => setStatusModal(null),
//             });
//         } catch (error) {
//             setStatusModal({
//                 type: "error",
//                 title: "Sync Failed",
//                 message: "Could not retrieve database results.",
//                 onClose: () => setStatusModal(null),
//             });
//         } finally {
//             setIsSyncing(false);
...
//             message: `Load the "${template.name}" blueprint?`,
...
//                     },
//                     knowledgeModules: [
//                         ...(prev.knowledgeModules || []),
//                         ...lockModules(template.knowledgeModules),
//                     ],
//                     practicalModules: [
//                         ...(prev.practicalModules || []),
//                         ...lockModules(template.practicalModules),
//                     ],
//                     workExperienceModules: [
//                         ...(prev.workExperienceModules || []),
//                         ...lockModules(template.workExperienceModules),
//                     ],
//                 }));
...
//         try {
//             // 1. Establish the "Master Date" for the certificate
//             const finalIssueDate = formData.issueDate || new Date().toISOString().split("T")[0];

//             // 2. Properly format names
//             let fName = formData.firstName;
//             let lName = formData.lastName;
//             if (formData.fullName && (!fName || !lName)) {
//                 const parts = formData.fullName.trim().split(" ");
...
//             // 3. 🚀 CRITICAL: Generate correct MLAB SoR ID
//             // If they don't have one, OR if they have the old random "SOR-XXXX" format, replace it!
//             let finalVerificationCode = formData.verificationCode;
//             if (!finalVerificationCode || finalVerificationCode.startsWith("SOR-")) {
//                 const sdpCode = formData.demographics?.sdpCode || GLOBAL_SDP_CODE;
//                 finalVerificationCode = generateSorId(formData.fullName || "Unknown", finalIssueDate, sdpCode);
...
//             // 4. 🚀 CRITICAL: Force Date Assessed to equal the Issue Date
//             const updatedQual = { ...formData.qualification };
//             if (!updatedQual.dateAssessed) {
//                 updatedQual.dateAssessed = finalIssueDate;
//             }

//             // 5. Build final save object
//             const savedLearner: any = {
//                 ...formData,
//                 firstName: fName,
//                 lastName: lName,
...
//             // Clean up undefined fields before Firebase throws an error
//             Object.keys(savedLearner).forEach((key) => {
//                 if (savedLearner[key] === undefined) {
//                     delete savedLearner[key];
//                 }
//             });
...
//             // 7. Handle relational Cohort mappings if needed
//             if (formData.cohortId && formData.cohortId !== "Unassigned") {
//                 const cohort = cohorts.find((c) => c.id === formData.cohortId);
//                 const pId = cohort?.programmeId || cohort?.qualificationId;

//                 if (learner?.id && learner.cohortId !== formData.cohortId && pId) {
//                     await enrollLearnerInCohort(
...
//                 knowledgeModules: [
//                     ...(prev.knowledgeModules || []),
//                     { ...base, dateAssessed: "", status: "Not Started" } as any,
//                 ],
//             }));
...
//                 workExperienceModules: [
//                     ...(prev.workExperienceModules || []),
//                     { ...base, dateSignedOff: "", status: "Not Started" } as any,
//                 ],
//             }));
//     };
...
//                         ...updated[index],
//                         credits: value,
//                         notionalHours: (Number(value) || 0) * 10,
//                     }
...
//             setFormData((prev) => ({
//                 ...prev,
//                 knowledgeModules: patch(prev.knowledgeModules),
//             }));
//         else if (type === "practical")
//             setFormData((prev) => ({
//                 ...prev,
//                 practicalModules: patch(prev.practicalModules),
//             }));
//         else
//             setFormData((prev) => ({
//                 ...prev,
//                 workExperienceModules: patch(prev.workExperienceModules),
//             }));
//     };
...
//             setFormData((prev) => ({
//                 ...prev,
//                 knowledgeModules: prev.knowledgeModules.filter((_, i) => i !== index),
//             }));
//         else if (type === "practical")
//             setFormData((prev) => ({
//                 ...prev,
//                 practicalModules: prev.practicalModules.filter((_, i) => i !== index),
//             }));
//         else
//             setFormData((prev) => ({
//                 ...prev,
//                 workExperienceModules: prev.workExperienceModules.filter(
//                     (_, i) => i !== index,
//                 ),
//             }));
//     };

//     const currentModuleCount = (tab: ModuleCategory) =>
//         (formData[`${tab}Modules`] as any[])?.length || 0;
...
//                                             onChange={(e) =>
//                                                 updateField("trainingStartDate", e.target.value)
//                                             }
//                                         />
//                                     </div>
...
//                                             type="text"
//                                             required
//                                             value={formData.demographics?.sdpCode || ""}
//                                             onChange={(e) =>
//                                                 updateDemographics("sdpCode", e.target.value)
//                                             }
//                                             style={{
...
//                                     </h4>
//                                     <p
//                                         style={{ margin: 0, fontSize: "0.8rem", color: "#15803d" }}
//                                     >
//                                         Pull this learner's actual graded assessments directly from
//                                         the system database.{" "}
//                                         <strong>
//                                             Results imported this way will be fully secured and
//                                             read-only.
//                                         </strong>
//                                     </p>
//                                 </div>
...
//                                     type="button"
//                                     className="lfm-btn"
//                                     onClick={handleSyncCohortResults}
//                                     disabled={isSyncing || formData.cohortId === "Unassigned"}
//                                     style={{
//                                         background: "#16a34a",
//                                         color: "white",
//                                         border: "none",
...
//                                 >
//                                     {isSyncing ? (
//                                         <Loader2 size={16} className="lfm-spin" />
//                                     ) : (
//                                         "Sync Database Results"
//                                     )}
//                                 </button>
//                             </div>
...
//                                     {/* Right: Upload CSV Statement of Results */}
//                                     <div style={{ paddingLeft: "0.5rem" }}>
//                                         <h4
//                                             style={{
//                                                 margin: "0 0 0.5rem 0",
...
//                                             }}
//                                         >
//                                             Upload an offline Statement of Results CSV to instantly
//                                             populate modules and achievements.
//                                         </p>
//                                         <input
//                                             type="file"
//                                             accept=".csv"
...
//                                             onChange={(e) =>
//                                                 updateQualification("name", e.target.value)
//                                             }
//                                         />
//                                     </div>
//                                     <div className="lfm-fg">
...
//                                             onChange={(e) =>
//                                                 updateQualification("saqaId", e.target.value)
//                                             }
//                                         />
//                                     </div>
//                                     <div className="lfm-fg">
...
//                                 <div className="lfm-section-hdr">
//                                     <Layers size={13} />
//                                     Statement of Results / Module Scores
//                                 </div>
//                                 <div className="lfm-tabs">
//                                     {(Object.keys(TAB_META) as ModuleCategory[]).map((tab) => (
//                                         <button
//                                             key={tab}
//                                             type="button"
//                                             className={`lfm-tab ${activeTab === tab ? "active" : ""}`}
...
//                                             {currentModuleCount(tab) > 0 && (
//                                                 <span
//                                                     className={`lfm-tab__badge ${activeTab === tab ? "active" : ""}`}
//                                                 >
//                                                     {currentModuleCount(tab)}
//                                                 </span>
//                                             )}
//                                         </button>
//                                     ))}
...
//                                         modules={
//                                             activeTab === "knowledge"
//                                                 ? formData.knowledgeModules
//                                                 : activeTab === "practical"
//                                                     ? formData.practicalModules
...
//                                         onChange={(e) =>
//                                             updateField("eisaAdmission", e.target.checked)
//                                         }
//                                     />
//                                     Learner has gained admission to the EISA
//                                 </label>
...
//                                             className="lfm-input"
//                                             type="date"
//                                             value={formData.nextEisaDate || ""}
//                                             onChange={(e) =>
//                                                 updateField("nextEisaDate", e.target.value)
//                                             }
//                                         />
//                                     </div>
//                                 )}
...
//                                         onChange={(e) =>
//                                             updateField("isArchived", e.target.checked)
//                                         }
//                                     />{" "}
...
//                             <button
//                                 type="button"
//                                 className="lfm-demographics-toggle"
...
//                                             ["citizenResidentStatusCode", "Citizen Resident Status"],
//                                             ["socioeconomicStatusCode", "Socioeconomic Status"],
//                                             ["disabilityStatusCode", "Disability Status"],
//                                             ["disabilityRating", "Disability Rating"],
//                                             ["immigrantStatus", "Immigrant Status"],
//                                             ["learnerMiddleName", "Middle Name"],
//                                             ["learnerTitle", "Title"],
//                                         ].map(([field, label]) => (
//                                             <div key={field} className="lfm-fg">
//                                                 <label>{label}</label>
...
//                                                     className="lfm-input"
//                                                     type="text"
//                                                     value={(formData.demographics as any)?.[field] || ""}
//                                                     onChange={(e) =>
//                                                         updateDemographics(
//                                                             field as keyof LearnerDemographics,
//                                                             e.target.value,
//                                                         )
//                                                     }
//                                                 />
...
//                                         ))}
//                                         {[
//                                             ["learnerHomeAddress1", "Home Address 1"],
//                                             ["learnerPostalAddress1", "Postal Address 1"],
//                                         ].map(([field, label]) => (
//                                             <div key={field} className="lfm-fg lfm-fg--full">
//                                                 <label>{label}</label>
//                                                 <input
...
//                                         ))}
//                                         {[
//                                             ["learnerHomeAddress2", "Home Address 2"],
//                                             ["learnerHomeAddress3", "Home Address 3"],
//                                             ["learnerPostalAddress2", "Postal Address 2"],
//                                             ["learnerPostalAddress3", "Postal Address 3"],
//                                             ["learnerHomeAddressPostalCode", "Home Postal Code"],
//                                             ["learnerPostalAddressPostCode", "Postal Code"],
//                                             ["provinceCode", "Province Code"],
//                                             ["statsaaAreaCode", "STATSAA Area Code"],
//                                             ["assessmentCentreCode", "Assessment Centre Code"],
//                                         ].map(([field, label]) => (
//                                             <div key={field} className="lfm-fg">
//                                                 <label>{label}</label>
//                                                 <input
...
//                                 {isSaving ? (
//                                     <>
//                                         <Loader2 size={13} className="lfm-spin" /> Saving…
//                                     </>
//                                 ) : (
//                                     <>
//                                         <Save size={13} /> Save Learner
//                                     </>
//                                 )}
//                             </button>
//                         </div>
