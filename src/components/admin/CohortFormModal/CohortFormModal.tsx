...
//     useEffect(() => {
//         if (cohort) {
//             // 1. Get IDs stored directly in the cohort document (which might be slightly delayed)
//             const dbIds = cohort.learnerIds || [];

//             // 2. Self-Healing: Extract IDs from any learner profile pointing to this cohort
//             // This catches newly imported learners where the cohort array hasn't refreshed locally yet!
//             const pointerIds = learners
//                 .filter(l => getResolvedCohortId(l) === String(cohort.id).trim())
//                 .map(l => l.learnerId || l.idNumber || l.id);
...
//                                 {/* : Hide Assessors/Moderators for Bootcamps */}
//                                 {ROLE_META.filter(r => formData.type === 'standard' || (r.key === 'facilitator' || r.key === 'supportFacilitator')).map(({ key, label }) => {
//                                     const roleLookup = key === 'supportFacilitator' ? 'facilitator' : key;
//                                     const options = staff.filter(s => s.role === roleLookup);
//                                     const currentId = formData[`${key}Id` as keyof typeof formData] as string;
...
//                                         </div>
//                                     ) : filteredLearners.map(learner => {
//                                         // 🚀 FIXED: Cross-checks against all possible identities correctly
//                                         const isSelected = formData.learnerIds.includes(learner.id) ||
//                                             (learner.idNumber && formData.learnerIds.includes(learner.idNumber)) ||
//                                             (learner.learnerId && formData.learnerIds.includes(learner.learnerId));
...
// //     const toggleLearner = (learner: any) => {
// //         const id = learner.id;
// //         const idNum = learner.idNumber;

// //         setFormData(prev => {
// //             const isCurrentlySelected = prev.learnerIds.includes(id) || (idNum && prev.learnerIds.includes(idNum));

// //             if (isCurrentlySelected) {
// //                 return { ...prev, learnerIds: prev.learnerIds.filter(lid => lid !== id && lid !== idNum) };
// //             } else {
// //                 return { ...prev, learnerIds: [...prev.learnerIds, idNum || id] };
// //             }
// //         });
// //     };
