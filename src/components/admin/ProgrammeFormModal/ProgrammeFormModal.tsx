...
export const ProgrammeFormModal: React.FC<ProgrammeFormModalProps> = ({
    ...
    // 🚀 HYBRID PARSER: Handles both CSV/TSV table formats and raw QCTO/SAQA document dumps
    const processRawTextData = (textToParse: string) => {
        ...
        const isCsvOrTsv = (firstLine.includes(',') || firstLine.includes('\t')) && (
            firstLine.includes('type') || firstLine.includes('category') || firstLine.includes('module') || firstLine.includes('code')
        );
        ...
        if (isCsvOrTsv) {
            ...
            lines.forEach((line) => {
                ...
                if (rowType === 'type' || rowType === 'category' || rowType === 'module code') return; // Skip headers

                const cat = (cols[1] || 'workplace').toLowerCase();
                const mCode = (cols[2] || '').trim();
                const mName = (cols[3] || '').trim();
                const nqf = parseInt(cols[4]) || formData.nqfLevel || 4;
                const creditsNum = parseInt(cols[5]) || 0;
                const tCode = (cols[6] || '').trim();
                const tName = (cols[7] || '').trim();
                const weight = parseInt(cols[8]) || 0;
                const cCode = (cols[9] || '').trim();
                const cDesc = (cols[10] || '').trim();

                if (!mCode) return;

                const pushModuleToCategory = (modObj: any) => {
                    if (cat.includes('knowledge')) kMs.push(modObj);
                    else if (cat.includes('practical')) pMs.push(modObj);
                    else wMs.push(modObj);
                };

                if (rowType === 'module') {
                    const mod = {
                        code: mCode,
                        name: mName || `${mCode} Module`,
                        nqfLevel: nqf,
                        credits: creditsNum,
                        notionalHours: creditsNum * 10,
                        topics: []
                    };
                    moduleMap.set(mCode, mod);
                    pushModuleToCategory(mod);
                } else if (rowType === 'topic' && tCode) {
                    let parentMod = moduleMap.get(mCode);
                    if (!parentMod) {
                        parentMod = {
                            code: mCode,
                            name: mName || `${mCode} Module`,
                        ...
                        moduleMap.set(mCode, parentMod);
                        pushModuleToCategory(parentMod);
                    }

                    const topic = {
                        code: tCode,
                        title: tName || `${tCode} Topic`,
                        weight: weight,
                        criteria: []
                    };
                    parentMod.topics.push(topic);
                    topicMap.set(`${mCode}_${tCode}`, topic);
                } else if (rowType === 'criteria') {
                    const parentTopic = topicMap.get(`${mCode}_${tCode}`);
                    if (parentTopic) {
                        parentTopic.criteria.push({
                            code: cCode,
                            description: cDesc
                        });
                    }
            ...
            // ─── 2. RAW QCTO FORM 2 / SAQA PDF DUMP PARSER PIPELINE ───
            ...
            const saveCurrentModule = () => {
                if (currentModule) {
                    ...
                    if (!currentModule.topics) currentModule.topics = [];
            ...
            for (let i = 0; i < rawLines.length; i++) {
                ...
                let modMatch = line.match(/^((?:US-\d+|251201-\d{3}-\d{2}-(?:KM|PM|WM)-\d{2}|[A-Z0-9\-]+))\s*(KM|PM|WM)?\b/i);
                const isTopicCode = line.match(/^(SO\d+|CT-[A-Z0-9\.\-]+|AC\d+|TASK-\d+)/i);

                const isExplicitModule = modMatch && !isTopicCode && (
                    line.toUpperCase().includes('MODULE') ||
                    line.match(/^(US-\d+|251201-\d+)/i) ||
                    modMatch[2] !== undefined ||
                    line.match(/\b(KM|PM|WM)\b/i)
                );
                ...
                if (isExplicitModule) {
                    ...
                    let type = (modMatch[2] || '').toUpperCase();

                    if (!type) {
                        if (line.match(/\bKM\b/i) || code.includes('-KM-')) type = 'KM';
                        else if (line.match(/\bPM\b/i) || code.includes('-PM-')) type = 'PM';
                        else if (line.match(/\bWM\b/i) || code.includes('-WM-')) type = 'WM';
                        else type = 'WM';
                    }

                    let name = line.replace(modMatch[0], '').replace(/^(KM|PM|WM)\s*/i, '').trim();

                    currentModule = {
                        code: code,
                        type: type,
                        name: name || `${code} Module`,
                        nqfLevel: formData.nqfLevel || 4,
                        credits: formData.credits || 0,
                        notionalHours: (formData.credits || 0) * 10,
                        topics: []
                    };
                    continue;
                }
                ...
                if (!currentModule) {
                    currentModule = {
                        code: formData.curriculumCode || formData.saqaId || 'US-114600',
                        type: 'WM',
                        name: formData.name || 'Occupational Programme Module',
                        nqfLevel: formData.nqfLevel || 4,
                        credits: formData.credits || 0,
                        notionalHours: (formData.credits || 0) * 10,
                        topics: []
                ...
                const topicMatch = line.match(/^([A-Z0-9\-]+-(?:KT|PS|WE)\d{2,4}|SO\d+|CT-[A-Z0-9\.\-]+|TASK-\d+|TOPIC-\d+)\s*(.*)/i);
                ...
                if (nqfMatch && currentModule) currentModule.nqfLevel = parseInt(nqfMatch[1], 10);
                if (credMatch && currentModule) {
                    currentModule.credits = parseInt(credMatch[1], 10);
                    currentModule.notionalHours = currentModule.credits * 10;
                }

                if (currentTopic) {
                    const critMatch = line.match(/^(?:•\s*|-|\*\s*)?(AC\d+(?:\.\d+)?|(?:KT|PS|WE)\s*\d{2,4})?\s*(.*)/i);

                    if (critMatch && (critMatch[1] || line.startsWith('•') || line.startsWith('-') || line.match(/^AC\d+/i))) {
                        ...
                        if (cCode || cDesc) {
                            currentTopic.criteria.push({
                                code: cCode || `AC${currentTopic.criteria.length + 1}`,
                                description: cDesc
                            });
                        }
                    } else if (line.trim() && !nqfMatch && !credMatch) {
        ...
        if (wMs.length > 0) setActiveTab('workExperience');
        else if (pMs.length > 0) setActiveTab('practical');
        else if (kMs.length > 0) setActiveTab('knowledge');

        setStatusModal({
            type: 'success',
            title: 'Parsing Complete',
            message: `Extracted ${kMs.length} Knowledge, ${pMs.length} Practical, and ${wMs.length} Workplace Modules successfully!`,
                            ...
                            <div>
                                ...
                                <div className="pfm-details-grid">
                                    <div className="pfm-fg pfm-fg--full">
                                        ...
                                        <input className="pfm-input" type="text" name="name" required value={formData.name} onChange={handleChange} placeholder="e.g. MICT SETA Internship - New Venture Creation" />
                                    </div>
                                    ...
                                    <div className="pfm-fg">
                                        <label>SAQA ID *</label>
                                        <input className="pfm-input" type="text" name="saqaId" required value={formData.saqaId} onChange={handleChange} placeholder="e.g. 114600" />
                                    </div>
                                    <div className="pfm-fg">
                                        <label>Curriculum Code</label>
                                        <input className="pfm-input" type="text" name="curriculumCode" value={(formData as any).curriculumCode || ''} onChange={handleChange} placeholder="e.g. US-114600" />
                                    </div>
                            ...
                            <div>
                                ...
                                {showTextParser && activeTab !== 'evidence' && (
                                    <div className="pfm-parser-panel">
                                        <p className="pfm-parser-panel__hint">Paste CSV tables, tab-separated values, or raw text directly from the QCTO Form 2 / SAQA PDF here.</p>
                                        <textarea className="pfm-parser-textarea" placeholder="Paste CSV or raw text here…" value={rawText} onChange={e => setRawText(e.target.value)} disabled={isProcessingText} />
                                ...
                                {activeTab === 'evidence' ? (
                                    ...
                                    <div className="pfm-module-list">
                                        {currentModules.length === 0 ? (
                                            <div className="pfm-empty-state"><Layers size={32} style={{ opacity: 0.3, marginBottom: '0.6rem' }} /><div>No modules yet. Use 'Import File' or 'Paste QCTO Text' above.</div></div>
