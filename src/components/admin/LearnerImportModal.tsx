...
export const LearnerImportModal: React.FC<LearnerImportModalProps> = ({
    ...
    // 🚀 HELPER: Force South African Date Format (DD-MM-YYYY)
                    ...
                    for (let i = 14; i < rawData.length; i++) {
                        ...
                        if (modName && modName.toLowerCase() !== "module name") {
                            ...
                            const baseMod = {
                                ...
                                // 🚀 SYNC: Every module's Date Assessed = Statement Issue Date
                                dateAssessed: issueDateStr,
...
//         // 🚀 CHANGED: Using skipEmptyLines: false and no headers
//         // This allows us to read the Excel file like a grid (rows and columns)
//         Papa.parse(file, {
//             skipEmptyLines: false,
//             complete: async (results) => {
//                 const rawData = results.data as string[][];
...
//                 try {
//                     // 1. PLUCK SPECIFIC CELLS (Based on your exact CSV layout)
//                     // Note: Arrays are 0-indexed, so Row 8 in Excel is rawData[7]
//                     const fullName = (rawData[7]?.[1] || "Unknown Learner").trim();
//                     const idNumber = (rawData[8]?.[1] || "").trim();

//                     const issueDateRaw = rawData[12]?.[1];
//                     const issueDateStr = issueDateRaw ? issueDateRaw.trim() : new Date().toISOString().split("T")[0];
//                     const providerCode = "SDP070824115131";
...
//                         dateOfBirth: "",
//                         cohortId: selectedCohortId || "Unassigned",
//                         trainingStartDate: (rawData[5]?.[1] || "").trim(),
//                         createdAt: new Date().toISOString(),
//                         createdBy: "admin-import",
...
//                             totalNotionalHours: (parseInt(rawData[4]?.[1] || "0")) * 10,
//                             nqfLevel: parseInt(rawData[1]?.[1] || "0"),
//                             // Date Assessed is forced to equal Issue Date!
//                             dateAssessed: issueDateStr,
//                         },
//                         knowledgeModules: [],
//                         practicalModules: [],
//                         workExperienceModules: [],
//                         eisaAdmission: true, // Assuming true for graduates being imported
...
//                         const modType = (row[0] || "").toLowerCase();
//                         const modName = (row[1] || "").trim();

//                         // Stop if we hit empty rows at the bottom
//                         if (!modName && !modType) continue;

//                         // Remember the last seen module type 
//                         // (Because "Knowledge Modules" is only in column A once, then blank underneath)
//                         let currentType = modType;
//                         if (!currentType && i > 14) {
//                             // Look backwards to find the category if it's blank
//                             for (let j = i; j >= 14; j--) {
//                                 if (rawData[j] && rawData[j][0]) {
//                                     currentType = rawData[j][0].toLowerCase();
//                                     break;
//                                 }
//                             }
//                         }

//                         if (modName && modName !== "Module Name") { // Skip the header row if accidentally caught
//                             const baseMod = {
//                                 name: modName,
//                                 credits: parseInt(row[4] || "0"),
//                                 notionalHours: parseInt(row[4] || "0") * 10,
//                                 nqfLevel: parseInt(row[3] || "0"),
//                             };

//                             const result = (row[8] || "").trim(); // Achievement Column

//                             if (currentType.includes("knowledge")) {
//                                 newLearner.knowledgeModules?.push({ ...baseMod, dateAssessed: issueDateStr, status: result } as KnowledgeModule);
//                             } else if (currentType.includes("practical") || currentType.includes("skills")) {
//                                 newLearner.practicalModules?.push({ ...baseMod, dateAssessed: issueDateStr, status: result } as PracticalModule);
//                             } else if (currentType.includes("work") || currentType.includes("experience")) {
//                                 newLearner.workExperienceModules?.push({ ...baseMod, dateSignedOff: issueDateStr, status: result } as WorkExperienceModule);
//                             }
//                         }
//                     }
...
//                     learnersMap.set(idNumber, newLearner);
//                     setStats({ total: 1, skipped: 0 });
//                     addToLog(`Processed 1 Single-Student Template.`);
