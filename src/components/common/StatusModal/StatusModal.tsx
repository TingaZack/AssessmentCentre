...
export const StatusModal: React.FC<StatusModalProps> = ({
    type, title, message, onClose, onCancel, confirmText = 'Okay', cancelText = 'Cancel'
}) => {
    const { Icon, accentColor, bgColor, borderColor, labelColor } = STATUS_CONFIG[type];

    return (
        /* 🚀 HIGH Z-INDEX (999999) ENSURES DIALOG RENDERS ABOVE ALL MODALS */
        <div className="stm-overlay" style={{ zIndex: 999999 }}>
            <div
                className="stm-modal"
            ...
            >
                ...
                {/* Footer */}
                <div
                    className={`stm-footer ${onCancel ? 'stm-footer--split' : ''}`}
                    style={{
                        display: 'flex',
                        alignItems: 'center',
                ...
                >
                    ...
                    <button
                        type="button"
                        className="stm-btn stm-btn--confirm"
                        onClick={onClose}
                        style={{ flex: 1, cursor: 'pointer' }}
                    >
                        {confirmText}
