...
import {
    ...
    Video, Sparkles, CheckCircle, Activity, Timer, Clock, Award
} from 'lucide-react';
...
export type ModuleCardType =
    ...
    | 'Grading by Type' | 'Grading by Module' | 'Assigned Cohorts' | 'Total Pending Action'
    | 'Facilitator Pace' | 'Assessor Pace' | 'QCTO Compliance'
    | 'Queue Aging & SLA' | 'Moderation Health' | 'First-Time Pass Rate' | 'Estimated Earnings' | 'EISA Clearance';
...
export const ModuleProgressCard: React.FC<ModuleProgressCardProps> = ({
    ...
    const numericLogged = typeof data.logged === 'number' ? data.logged : 0;
    const targetPct = data.total > 0 ? Math.round((numericLogged / data.total) * 100) : (typeof data.logged === 'string' ? 100 : 0);
    const remaining = Math.max(0, data.total - numericLogged);
    ...
    const isStarted = typeof data.logged === 'string' || data.logged > 0;
    const isComplete = typeof data.logged !== 'string' && data.total > 0 && data.logged >= data.total;

    const config: Record<string, any> = {
        // 🚀 NEW ADVANCED ANALYTICS
        'Queue Aging & SLA': {
            color: '#ef4444', bg: '#fef2f2', title: 'Queue Aging & SLA', unit: 'tasks', label: 'Bottleneck KPI', bar1: 'Within SLA', bar2: 'Breached',
            description: 'Monitors the age of submissions currently sitting in your pending queue. Items over 5 days are considered SLA breaches.',
            icon: <AlertTriangle size={19} color="#ef4444" />
        },
        'Moderation Health': {
            color: '#8b5cf6', bg: '#f3e8ff', title: 'Moderation Health', unit: 'assessments', label: 'IQA Audit KPI', bar1: 'Accepted', bar2: 'Returned',
            description: 'Tracks the ratio of your graded assessments that were accepted by the Moderator versus those returned for urgent fixes.',
            icon: <ShieldCheck size={19} color="#8b5cf6" />
        },
        'First-Time Pass Rate': {
            color: '#10b981', bg: '#dcfce7', title: 'First-Time Pass', unit: 'learners', label: 'Competency KPI', bar1: 'Passed 1st Try', bar2: 'Needed Reassessment',
            description: 'Percentage of learners who achieved Competency on their absolute first submission attempt.',
            icon: <Award size={19} color="#10b981" />
        },
        'Estimated Earnings': {
            color: '#eab308', bg: '#fef9c3', title: 'Est. Earnings', unit: '', label: 'Financial KPI', bar1: 'Accrued', bar2: 'Unbilled',
            description: 'Estimated payout value of all graded assessments based on standard Formative/Summative billing units.',
            icon: <Wallet size={19} color="#eab308" />
        },
        'EISA Clearance': {
            color: '#0ea5e9', bg: '#e0f2fe', title: 'EISA Cleared', unit: 'learners', label: 'Admission KPI', bar1: 'Cleared', bar2: 'Pending Requirements',
            description: 'Learners who have achieved 100% completion across all Formative, Practical, and Workplace modules and are ready for the final EISA.',
            icon: <GraduationCap size={19} color="#0ea5e9" />
        },

        // PREVIOUS CONFIGS
        'QCTO Compliance': {
            color: '#16a34a', bg: '#dcfce7', title: 'QCTO Blueprint Compliance', unit: 'modules', label: 'Strict Baseline', bar1: 'Fully Graded', bar2: 'Outstanding',
            description: 'Strict absolute progress against the full QCTO programme template. Includes all unstarted, busy, and pending tasks across all active learners to verify full curriculum coverage.',
            icon: <GraduationCap size={19} color="#16a34a" />
        },
        'Assigned Cohorts': {
            color: '#3b82f6', bg: '#eff6ff', title: 'Assigned Cohorts', unit: 'cohorts', label: 'Capacity KPI', bar1: 'Active', bar2: 'Historical',
            description: 'Total number of active cohorts directly assigned to your assessor profile.',
            icon: <Layers size={19} color="#3b82f6" />
        },
        'Total Pending Action': {
            color: '#f59e0b', bg: '#fff7ed', title: 'Pending Action', unit: 'tasks', label: 'Queue KPI', bar1: 'Pending', bar2: 'Processed',
            description: 'Total number of submissions currently sitting in your queue waiting for grading.',
            icon: <Clock size={19} color="#f59e0b" />
        },
        'Facilitator Pace': {
            color: '#0ea5e9', bg: '#e0f2fe', title: 'Avg Pre-Mark', unit: '', label: 'Incoming KPI', bar1: 'Avg Time', bar2: 'Target',
            description: 'Average time spent by facilitators reviewing submissions before they hit your desk.',
            icon: <Activity size={19} color="#0ea5e9" />
        },
        'Assessor Pace': {
            color: '#ef4444', bg: '#fef2f2', title: 'Avg Marking Pace', unit: '', label: 'Performance KPI', bar1: 'Avg Time', bar2: 'Target',
            description: 'Average time you spend officially grading and reviewing an individual submission.',
            icon: <Timer size={19} color="#ef4444" />
        },
        'Grading by Type': {
            color: '#10b981', bg: '#dcfce7', title: 'Grading by Assessment Type', unit: 'processed', label: 'Marking KPI', bar1: 'Completed', bar2: 'Pending',
            description: 'Breakdown of your marking volume across Formative, Summative, and Other assessment types.',
            icon: <CheckCircle size={19} color="#10b981" />
        },
        'Grading by Module': {
            color: '#8b5cf6', bg: '#f3e8ff', title: 'Grading by Module Domain', unit: 'processed', label: 'Curriculum KPI', bar1: 'Completed', bar2: 'Pending',
            description: 'Breakdown of your marking volume across Knowledge, Practical, and Workplace modules.',
            icon: <BookOpen size={19} color="#8b5cf6" />
        },

        // Default Fallbacks
        'Knowledge': { color: '#94c73d', bg: '#f0f7e1', title: 'Knowledge Modules', unit: 'topics', label: 'Module Type', bar1: 'Covered', bar2: 'Remaining', icon: <BookOpen size={19} color="#94c73d" /> },
        'Practical': { color: '#073f4e', bg: '#e8f0f3', title: 'Practical Modules', unit: 'topics', label: 'Module Type', bar1: 'Covered', bar2: 'Remaining', icon: <Activity size={19} color="#073f4e" /> }
    };
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                                ...
                                <span style={{ fontFamily: 'var(--font-heading)', fontSize: '0.75rem', fontWeight: 700, color: 'var(--mlab-blue)' }}>{data.logged} {typeof data.logged === 'number' ? unit : ''}</span>
                            </div>
                        ...
                        {data.total > 0 && typeof data.logged === 'number' && (
                            <div>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
            ...
            >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexShrink: 0 }}>
                    <div style={{ position: 'relative', width: '60px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {renderRingSVG(42, 10)}
                        <span style={{ position: 'absolute', fontSize: typeof data.logged === 'string' ? '0.7rem' : '0.85rem', fontWeight: 800, color: 'var(--mlab-midnight)' }}>
                            {typeof data.logged === 'string' ? '' : `${targetPct}%`}
                        </span>
                    </div>
                </div>
            </div>
    ...
    return (
        ...
        >
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px', borderBottom: '1px solid var(--mlab-border)', paddingBottom: '0.75rem' }}>
                ...
                <div style={{ fontFamily: 'var(--font-heading)', fontSize: '1.4rem', fontWeight: 800, color: 'var(--mlab-blue)', flexShrink: 0 }}>
                    {typeof data.logged === 'string' ? '' : `${targetPct}%`}
                </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', padding: '0.5rem 0' }}>
                ...
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1px' }}>
                    <div style={{ fontFamily: 'var(--font-heading)', fontSize: typeof data.logged === 'string' ? '1rem' : '1.3rem', fontWeight: 800, lineHeight: 1, color: 'var(--mlab-midnight)' }}>{data.logged}</div>
                    {data.total > 0 && typeof data.logged === 'number' && (
...
//     MapPin, GraduationCap, AlertTriangle, ShieldCheck,
//     Landmark, Wallet, Percent, User, Briefcase, CalendarCheck,
//     Video, Sparkles, CheckCircle, Activity, Timer, Clock
// } from 'lucide-react';
...
//     | 'Gender Diversity' | 'Performance Risk' | 'Employability Score' | 'AI Mock Interviews'
//     | 'Grading by Type' | 'Grading by Module' | 'Assigned Cohorts' | 'Total Pending Action'
//     | 'Facilitator Pace' | 'Assessor Pace' | 'QCTO Compliance'; // 🚀 NEW TYPE ADDED
...
//     const config: Record<string, any> = {
//         // 🚀 NEW QCTO COMPLIANCE CONFIG
//         'QCTO Compliance': {
//             color: '#16a34a', bg: '#dcfce7', title: 'QCTO Blueprint Compliance', unit: 'modules', label: 'Strict Baseline', bar1: 'Fully Graded', bar2: 'Outstanding (All Stages)',
//             description: 'Strict absolute progress against the full QCTO programme template. Includes all unstarted, busy, and pending tasks across all active learners to verify full curriculum coverage.',
//             icon: <GraduationCap size={19} color="#16a34a" />
//         },
//         'Assigned Cohorts': {
...
//         },
//         'Assessor Pace': {
//             color: '#ef4444', bg: '#fef2f2', title: 'Marking Pace', unit: '', label: 'Performance KPI', bar1: 'Avg Time', bar2: 'Target',
//             description: 'Average time you spend officially grading and reviewing an individual submission.',
//             icon: <Timer size={19} color="#ef4444" />
//         },
//         'Grading by Type': {
