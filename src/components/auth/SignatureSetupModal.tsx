...
export const SignatureSetupModal: React.FC<Props> = ({
    userUid,
    ...
    // Only fetch employers if we are authenticated and dealing with a mentor
    useEffect(() => {
        if (!isGuestMode && user?.role === 'mentor' && employers.length === 0) {
            fetchEmployers();
        }
    }, [isGuestMode, user?.role, employers.length, fetchEmployers]);

    // Fallback to Guest data if in Guest Mode
    const mentorEmployer = useMemo(() => employers.find((e: any) => e.id === user?.employerId), [employers, user?.employerId]);
    const displayCompanyName = isGuestMode ? guestCompany : (mentorEmployer?.name || user?.companyName || 'Unassigned Workplace');
    const displayRole = isGuestMode ? 'mentor' : user?.role?.toLowerCase();
    ...
    const penSettings = useMemo(() => {
        switch (displayRole) {
            case 'facilitator':
            case 'mentor':
                return { color: '#0ea5e9', rgb: [14, 165, 233], label: displayRole === 'mentor' ? 'Workplace Mentor (Blue Pen)' : 'Facilitator (Blue Pen)' };
            case 'assessor':
    ...
    const saveSignature = async () => {
        ...
        if (inputMode === 'draw') {
            ...
            // 🚀 FIXED: using getCanvas() to avoid trim-canvas dependency crash
        ...
        // 🚀 GUEST MODE BYPASS: Hand it back to the parent and exit!
        if (isGuestMode && onGuestSave) {
            onGuestSave(finalDataUrl);
            onComplete();
            return;
        }

        // 🚀 STANDARD AUTHENTICATED UPLOAD FLOW
        setLoading(true);
            ...
            <div className="lfm-modal" style={{ maxWidth: '600px' }}>
                ...
                <div className="lfm-body" style={{ paddingBottom: '2rem' }}>
                    ...
                    <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.85rem', color: 'var(--mlab-blue)', marginTop: 0, marginBottom: '0.5rem' }}>
                        In accordance with QCTO requirements, your role as {isGuestMode ? 'an Authorized Mentor' : `a ${displayRole}`} requires signatures to be digitally stamped in <strong>{penSettings.color}</strong> ink.
                    </p>
